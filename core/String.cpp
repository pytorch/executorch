...
// sign.
constexpr size_t kMaxIntCharsNeeded = 10 + 1;
// Hex formats can need up to 8 bytes for the value plus two bytes for the "0x".
constexpr size_t kMaxHexCharsNeeded = 8 + 2;
...
const size_t kFastToBufferSize = 48;
...
char* StrCatStr(char* main, size_t main_max_length, const char* to_append) {
  char* current = main;
  while (*current != 0) {
    ++current;
  }
...
char* FastUInt32ToBufferLeft(uint32_t i, char* buffer, uint32_t base) {
  char* start = buffer;
  do {
...
char* StrCatInt32(char* main, size_t main_max_length, int32_t number) {
...
char* FastFloatToBufferLeft(float f, char* buffer) {
  ...
  *current = 0;
  ...
  const size_t scale_shifts_size = 13;
  const int8_t scale_shifts[13] = {
      3, 4, 8, 11, 13, 14, 17, 18, 19, 20, 21, 22, 23};
  uint32_t scaled_fraction = fraction;
  for (size_t i = 0; i < scale_shifts_size; ++i) {
  ...
  *current = 0;
  ...
  // represents .00025.
  constexpr size_t kMaxFractionalDigits = 7;
  ...
  for (size_t i = 1; i < kMaxFractionalDigits; i++) {
    *(current + i) = '0';
  }
  ...
  size_t fraction_digits = current - previous;
  size_t leading_zeros = kMaxFractionalDigits - fraction_digits;
  ...
  *current = '0';
  ...
  if (leading_zeros != 0) {
    for (size_t i = 0; i < fraction_digits; i++) {
      current--;
...
ssize_t FormatInt32(char* output, int32_t i) {
  return static_cast<ssize_t>(FastInt32ToBufferLeft(i, output) - output);
}

ssize_t FormatUInt32(char* output, uint32_t i) {
  return static_cast<ssize_t>(FastUInt32ToBufferLeft(i, output, 10) - output);
}

ssize_t FormatHex(char* output, uint32_t i) {
  return static_cast<ssize_t>(FastUInt32ToBufferLeft(i, output, 16) - output);
}

ssize_t FormatFloat(char* output, float i) {
  return static_cast<ssize_t>(FastFloatToBufferLeft(i, output) - output);
}

} // namespace

extern "C" ssize_t
ETVsnprintf(char* output, size_t len, const char* format, va_list args) {
  size_t output_index = 0;
  const char* current = format;
  // One extra character must be left for the null terminator.
  const ssize_t usable_length = len - 1;
...
extern "C" ssize_t
ETSnprintf(char* output, size_t len, const char* format, ...) {
  va_list args;
  va_start(args, format);
  ssize_t bytes_written = ETVsnprintf(output, len, format, args);
  va_end(args);

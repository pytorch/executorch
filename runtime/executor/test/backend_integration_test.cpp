...
/**
 * Loads a program whose delegate declared memory-planned scratch, which the
 * backend receives through the execution context rather than as arguments.
 */
class DelegateScratchTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    ASSERT_FALSE(program_path_.empty());
    no_scratch_program_path_ = std::getenv("ET_MODULE_ADD_MUL_DELEGATED_PATH");
    ASSERT_FALSE(no_scratch_program_path_.empty());
  }
  ...
  std::string program_path_;
  std::string no_scratch_program_path_;
};

TEST_F(DelegateScratchTest, ScratchArrivesThroughTheContextNotTheArguments) {
  ...
  StubBackend::singleton().install_execute(
      [&](BackendExecutionContext& backend_execution_context,
        ...
        // Scratch is not an argument: the delegate still sees exactly its
        // inputs and outputs.
        EXPECT_EQ(args.size(), kNumIoArgs);

        Span<const Span<uint8_t>> scratch =
            backend_execution_context.scratch_buffers();
        EXPECT_EQ(scratch.size(), 1u);
        if (scratch.size() != 1) {
          return Error::Internal;
        }
        EXPECT_EQ(scratch[0].size(), kScratchBytes);

        // The planner owns these bytes, so they must be real memory that no
        // live input or output overlaps.
        uint8_t* scratch_ptr = scratch[0].data();
        EXPECT_NE(scratch_ptr, nullptr);
        const auto scratch_begin = reinterpret_cast<uintptr_t>(scratch_ptr);
        const uintptr_t scratch_end = scratch_begin + scratch[0].size();
        for (size_t i = 0; i < kNumIoArgs; ++i) {
          const auto tensor = args[i]->toTensor();
          const auto arg_begin =
              reinterpret_cast<uintptr_t>(tensor.mutable_data_ptr());
          const uintptr_t arg_end = arg_begin + tensor.nbytes();
          EXPECT_TRUE(arg_end <= scratch_begin || scratch_end <= arg_begin)
              << "argument " << i << " overlaps the scratch buffer";
        }
        return Error::Ok;
      });
...
TEST_F(DelegateScratchTest, ProgramWithoutScratchDeliversAnEmptySpan) {
  bool executed = false;
  StubBackend::singleton().install_execute(
      [&](BackendExecutionContext& backend_execution_context,
          ET_UNUSED DelegateHandle* handle,
          ET_UNUSED Span<EValue*> args) -> Error {
        executed = true;
        EXPECT_EQ(backend_execution_context.scratch_buffers().size(), 0u);
        return Error::Ok;
      });

  Result<FileDataLoader> loader =
      FileDataLoader::from(no_scratch_program_path_.c_str());
  ASSERT_EQ(loader.error(), Error::Ok);
  Result<Program> program = Program::load(&loader.get());
  ASSERT_EQ(program.error(), Error::Ok);

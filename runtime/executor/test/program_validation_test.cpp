...
std::vector<uint8_t> CreateTestProgram(
    ...
    // If true, emit one chain holding two KernelCalls and a one-entry operators
    // table: the first call's op_index is out of range so resolve_operator()
    // fails on a non-final instruction, the second resolves successfully.
    ...
    std::vector<flatbuffers::Offset<executorch_flatbuffer::Instruction>>
        instructions{
            executorch_flatbuffer::CreateInstruction(
                ...
                kernel_call.Union()),
            // Resolves successfully, so a fix that let a later Ok overwrite the
            // earlier failure would make init() return Ok.
            executorch_flatbuffer::CreateInstruction(
                builder,
                executorch_flatbuffer::InstructionArguments::KernelCall,
                good_call.Union())};
  ...
  if (with_out_of_range_kernel_call) {
    operators.push_back(executorch_flatbuffer::CreateOperator(
        builder,
        builder.CreateString("test::noop"),
...
// init() resolves every KernelCall before returning. A failure on one
// instruction is recorded once and surfaced after the loop, so it must not be
// discarded by a later instruction. Covers the delayed_error assignment.
TEST_F(ProgramValidationTest, ResolveOperatorFailureSurfacesFromInit) {
  ...
  // Minimal verification so program validation does not reject the bad
  // op_index before Method::init() gets a chance to resolve it.
  Result<Program> program =
      Program::load(&loader, Program::Verification::Minimal);
  ...
  executorch::runtime::testing::ManagedMemoryManager mmm(
      /*planned_memory_bytes=*/32 * 1024U, /*method_allocator_bytes=*/32 * 1024U);

...
/* static */ Result<Program> Program::load(
  ...
  auto header_tag = reinterpret_cast<const uint8_t*>(constant_buffer[0]->storage()->data());
  std::cout << "program.cpp: load program, header_tag: " << static_cast<int>(*header_tag) << std::endl;
  // ensure header_tag = 42, otherwise revert to old behavior
  if (static_cast<const int>(*header_tag) == 42) {
    auto constant_segment_index = reinterpret_cast<const uint8_t*>(constant_buffer[1]->storage()->data());

    std::cout << "program.cpp: load program, constant_segment_index: " << static_cast<int>(*constant_segment_index) << std::endl;
    ...
    const executorch_flatbuffer::DataSegment* segment = flatbuffer_program->segments()->Get(static_cast<int>(*constant_segment_index));
  ...
  Result<FreeableBuffer> constant_segment = FreeableBuffer(nullptr, 0, /*free_fn=*/nullptr);
  return Program(
      loader,
      segment_base_offset,
      std::move(program_data.get()),
...
Result<const void*> Program::get_constant_buffer_data(
  ...
  int index = (buffer_index + 1)*2;
  std::cout << "program.cpp: get_constant_buffer_data, buffer_index: " << buffer_index << ", index: " << index << std::endl;

  auto offset = static_cast<int>(*reinterpret_cast<const uint8_t*>(constant_buffer[index]->storage()->data()));
  auto size = static_cast<int>(*reinterpret_cast<const uint8_t*>(constant_buffer[index + 1]->storage()->data()));

...
Result<std::pair<std::vector<uint64_t>, uint64_t>> BPETokenizerBase::encode_with_special_token_(
...
BPETokenizerBase::encode(const std::string& text, int8_t bos, int8_t eos) const {
  ...
  auto res = TK_UNWRAP(encode_with_special_token_(text, special_token_encoder_)).first;
...
Result<std::string>
BPETokenizerBase::decode(uint64_t prev, uint64_t cur) const {
  (void)prev;
  if (!initialized_) {
    return Error::Uninitialized;
  }

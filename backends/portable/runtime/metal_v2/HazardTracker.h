...
// HazardTracker — R8.x range-based hazard analysis for the typed-setter
...
//      of math over a few vectors. MetalStream is huge enough already.
//   2. Data-structure freedom: the writers index used to be a flat
//      std::vector<HazardRange> with O(N²) overlap checks per dispatch.
//      With the writers grouped per-parent and merged-on-insert, the
//      common case (a handful of parents, one or two writes each) is
//      O(1)-amortized per dispatch and the worst case is O(W·R) where W
//      is the number of merged writer ranges for *one* parent and R is
//      the pending input/output count. That's the audit's "O(N²)
//      lastWriters_ overlap check" item.
//   3. Testability: a future unit test for merge correctness only has to
...
class HazardTracker {
 public:
  ...
  // Debug-only accessors used by MetalStream's R8.1.dbg log dump in
  // dispatchThreadgroups. The vectors returned reflect the ranges as
  // currently stored (pending lists are flat; writers are flattened on
  // demand from the per-parent index).

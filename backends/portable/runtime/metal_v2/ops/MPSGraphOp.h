...
//   - Defeats ICB replay for this op (MPSGraph re-encodes per call).
//===----------------------------------------------------------------------===//
...
class MPSGraphOp : public MetalOp {
 public:
  ...
  void dispatch(
      ...
      EValuePtrSpan outputs) override final;
 ...
 protected:
  struct CachedGraph {
    MPSGraph* graph = nil;                            // strong (we retain)
 ...
 private:
  ...
  struct ShapeKey {
    ...
    bool operator==(const ShapeKey& o) const {
      return len == o.len &&
          std::memcmp(data.data(), o.data.data(), len * sizeof(uint64_t)) ==
          0;
    }
  };
  struct ShapeKeyHash {
...
//===----------------------------------------------------------------------===//
// MPSGraphMatMulOp - aten::mm via MPSGraph.matrixMultiplicationWithPrimaryTensor
//
// Inputs:  A [M, K], B [K, N]
// Output:  C [M, N] = A @ B
//
...
class MPSGraphMatMulOp : public MPSGraphOp {
 public:
  ...
  std::vector<runtime::etensor::Tensor::SizesType> computeOutputShape(
    ...
    if (A.dim() < 2 || B.dim() < 2) return {};
    using SizesType = runtime::etensor::Tensor::SizesType;
    return {static_cast<SizesType>(A.size(A.dim() - 2)),

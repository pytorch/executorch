...
// metal_v2 matmul-family op classes (MatMulOp / AddMMOp / BAddBMMOp /
// BatchedMatMulOp).
// EXTRACTED INTO ITS OWN HEADER because the raw-string MSL is ~1k lines and
// drowns out the C++ host code in MatMulOp.mm. The shader library defines:
...
// matmul_simd_splitk_accum. Standalone — formerly part of the deleted
// SIMD-MMA helpers section but still needed by the kernels we keep.
//===----------------------------------------------------------------------===//
...
// matmul_mlx_splitk_nax_partial was removed when MatMulOp::dispatchSplitKNAX
// (METAL_USE_MLX_DENSE=1 branch) was migrated to ops/mlx_jit/'s per-shape JIT
// loader. The MLX `gemm_splitk_nax` template
// (steel/gemm/kernels/steel_gemm_splitk_nax.h) is now JIT-compiled per

...
inline void dispatchGemmViaMlxJit(
  ...
  if (use_out_source) {
    ...
    stream->setInput(2, bias->const_data_ptr(), bias->nbytes());
  }
  ...
  if (use_out_source) {
    ...
    stream->setBytes(5, &addmm_params, sizeof(addmm_params));
  }
...
inline void dispatchGemvTViaMlxJit(
  ...
  // Bindings.
  stream->setInput(0, B.const_data_ptr(), B.nbytes());      // mat
  stream->setInput(1, A.const_data_ptr(), A.nbytes());      // in_vec
  // slot 2 (bias) FC-gated to kDoAxpby=false → unused; do not bind.
  stream->setOutput(3, C.mutable_data_ptr(), C.nbytes());   // out_vec
  stream->setBytes<int32_t>(4, K);                          // in_vec_size
  stream->setBytes<int32_t>(5, N);                          // out_vec_size
  stream->setBytes<int32_t>(6, N);                          // matrix_ld (B is [K,N] row-major → ld = N)
  // slots 7/8 (alpha/beta) FC-gated to kDoAxpby=false → unused.
  stream->setBytes<int32_t>(9, 0);                          // batch_ndim
  ...
  stream->setBytes(11, &zero64, sizeof(zero64));
  stream->setBytes(12, &zero64, sizeof(zero64));
  ...
  const int n_tgp = (N + n_out_per_tgp - 1) / n_out_per_tgp;
  uvec3 grid{uint32_t(n_tgp), 1u, 1u};
  uvec3 block{32u, uint32_t(bn), uint32_t(bm)};
  stream->dispatch(pso, grid, block);
}
...
inline void dispatchGemvViaMlxJit(
  ...
  stream->setInput(0, A.const_data_ptr(), A.nbytes());      // mat
  stream->setInput(1, B.const_data_ptr(), B.nbytes());      // in_vec
  stream->setOutput(3, C.mutable_data_ptr(), C.nbytes());   // out_vec
  stream->setBytes<int32_t>(4, K);                          // in_vec_size
  stream->setBytes<int32_t>(5, M);                          // out_vec_size
  stream->setBytes<int32_t>(6, K);                          // matrix_ld (A is [M,K] row-major → ld = K)
  stream->setBytes<int32_t>(9, 0);                          // batch_ndim
  const int64_t zero64 = 0;
  stream->setBytes(11, &zero64, sizeof(zero64));
  stream->setBytes(12, &zero64, sizeof(zero64));

  const int n_tgp = (M + n_out_per_tgp - 1) / n_out_per_tgp;
  uvec3 grid{uint32_t(n_tgp), 1u, 1u};
  uvec3 block{32u, uint32_t(bn), uint32_t(bm)};
  stream->dispatch(pso, grid, block);
}

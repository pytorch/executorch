...
inline void dispatchAffineQmvViaMlxJit(
  ...
  // Bind buffers.
  stream->setInput(0, wq.const_data_ptr(), wq.nbytes());
  stream->setInput(1, ws.const_data_ptr(), ws.nbytes());
  stream->setInput(2, wz.const_data_ptr(), wz.nbytes());
  stream->setInput(3, x.const_data_ptr(), x.nbytes());
  stream->setOutput(4, y.mutable_data_ptr(), y.nbytes());
  stream->setBytes<int32_t>(5, K);
  stream->setBytes<int32_t>(6, N);
  // Slots 7-14: batched-only, not bound (kernel template-DCEs the access
  // path when batched=false).
...
//   3. Allocate intermediate [split_k, M, N] in `dtype` via stream->alloc.
...
inline bool dispatchAffineQmmTSplitKViaMlxJit(
  ...
  void* intermediate = stream->alloc(intermediate_bytes);
  ...
  stream->setInput(2, wz.const_data_ptr(), wz.nbytes());
  stream->setInput(3, x.const_data_ptr(), x.nbytes());
  stream->setOutput(4, intermediate, intermediate_bytes);
  stream->setBytes<int32_t>(5, K);
  stream->setBytes<int32_t>(6, N);
  stream->setBytes<int32_t>(7, M);
  stream->setBytes<int32_t>(8, k_partition_size);
  stream->setBytes<int32_t>(9, split_k_partition_stride);

  uvec3 grid_a{uint32_t(n_tiles), uint32_t(m_tiles), uint32_t(split_k)};
  uvec3 block_a{32u, 2u, 2u};
  stream->dispatch(pso, grid_a, block_a);
  ...
  stream->setInput(0, intermediate, intermediate_bytes);
  stream->setOutput(1, y.mutable_data_ptr(), y.nbytes());
  stream->setBytes<int32_t>(2, split_k);
  stream->setBytes<int32_t>(3, split_k_partition_stride);  // = M * N
  stream->setBytes<int32_t>(4, N);                          // ldd
  ...
  uvec3 grid_b{uint32_t((N + 31) / 32), uint32_t(M), 1u};
  uvec3 block_b{32u, 1u, 1u};
  stream->dispatch(accum_pso, grid_b, block_b);

  stream->free(intermediate);
  return true;
}

...
void BatchedMatMulOp::dispatch(
  ...
  stream->setInput(1, B.const_data_ptr(), B.nbytes());
  stream->setOutput(2, C.mutable_data_ptr(), C.nbytes());
  stream->setBytes<int32_t>(3, batch);
  stream->setBytes<int32_t>(4, M);
  stream->setBytes<int32_t>(5, K);
  stream->setBytes<int32_t>(6, N);
  stream->setBytes<int32_t>(7, A_batch_stride);
  stream->setBytes<int32_t>(8, B_batch_stride);
  stream->setBytes<int32_t>(9, C_batch_stride);
  stream->dispatch(kernel, grid, block);
}

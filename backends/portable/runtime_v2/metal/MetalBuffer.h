 ...
 *
 * The "location" is set per Provider (router stamps it during routing).
 */
class MetalBuffer : public Buffer {
 public:
  ...
  static MetalBuffer* allocate(
      ...
      void* ptr,
      size_t bytes) {
    return new MetalBuffer(Mode::Owned, loc, stream, ptr, bytes);
  }
  ...
  static MetalBuffer* alias(
      ...
      void* ptr,
      size_t bytes) {
    return new MetalBuffer(Mode::Aliasing, loc, stream, ptr, bytes);
  }
  ...
  static MetalBuffer* alias_ndm(
    ...
    auto* mb = new MetalBuffer(Mode::NdmAlias, loc, stream, ptr, bytes);
    mb->ndm_buffer_.~FreeableBuffer();
    new (&mb->ndm_buffer_)
        ::executorch::runtime::FreeableBuffer(std::move(fb));
    return mb;
  }

 ...
 * Instance's execute() before signaling, per the standard contract.
 */
class MetalEvent final : public Event {
 public:
  MetalEvent() : status_(EventStatus::Complete), error_(::executorch::runtime::Error::Ok) {}
  ...
  // Called by MetalInstance code (typically inside a command buffer
  // completion handler).

...
public:
    ...
    int LoadFromCompiledNetwork(const void* buffer, size_t size,
                                int inputCount, int outputCount, std::string& runtimeOption);

    template <bool isInput>
    int SetInputOutput(uint32_t index, void *buffer, size_t length) const {
        CHECK_VALID_PTR(buffer);
        CHECK_VALID_PTR(mExecution);
        return isInput ? NeuronExecution_setInput(mExecution.get(), index, nullptr, buffer, length):
                         NeuronExecution_setOutput(mExecution.get(), index, nullptr, buffer, length);
    }

    template <bool isInput>
    int SetInputOutputFromMemory(uint32_t index, const NeuronMemory *memory, size_t offset, size_t length) const {
        CHECK_VALID_PTR(memory);
        CHECK_VALID_PTR(mExecution);
        return isInput ? NeuronExecution_setInputFromMemory(mExecution.get(), index, nullptr, memory, offset, length):
                         NeuronExecution_setOutputFromMemory(mExecution.get(), index, nullptr, memory, offset, length);
    }
    ...
    size_t GetInputOutputPaddedSize(int32_t index) const {
        ...
        size_t size = 0;
        auto res = isInput ? NeuronCompilation_getInputPaddedSize(mCompilation.get(), index, &size) :
                             NeuronCompilation_getOutputPaddedSize(mCompilation.get(), index, &size);

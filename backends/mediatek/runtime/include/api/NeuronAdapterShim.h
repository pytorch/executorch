...
inline void* loadAdapterLibrary(const char* name) {
    ...
    if (sHandle == nullptr) {
        __android_log_print(ANDROID_LOG_ERROR, "AdapterShimApi", "Unable to open library %s", name);
    }
    return sHandle;
}
...
inline void* loadAdapterFunction(const char* name) {
    ...
    if (fn == nullptr) {
         __android_log_print(ANDROID_LOG_ERROR, "AdapterShimApi", "Unable to open function %s", name);
    }
...
typedef int (*NeuronCompilation_createV2_fn)(NeuronModel* model, CompilationType type,
                                             const char* options, NeuronCompilation** compilation);
...
typedef int (*NeuronCompilation_getSupportedOperations_fn)(NeuronCompilation* compilation,
                                                           uint32_t operationCount,
                                                           bool* supported);
...
typedef int (*NeuronExecution_setIODone_fn)(NeuronExecution* execution, int idx);
...
inline int NeuronCompilation_getSupportedOperations(NeuronCompilation* compilation,

...
class TestVulkanBackend(unittest.TestCase):
    ...
    def test_vulkan_backend_xnnpack_pt2e_quantized_linear_sequence(self):
        ...
        test_utils.save_executorch_program(et_program, "test")

        print(linear_sequence_module.linear1.weight)
    ...
    def test_vulkan_backend_8da4w_quantized_linear_sequence(self):
        """
        Test a sequence of linear layers quantized with 8da4w method.
        This test creates a module with multiple linear layers in sequence and applies
        8da4w (8-bit dynamic activation, 4-bit weight) quantization to test the quantized
        model execution using the Vulkan backend.
        """
        ...
        class LinearSequenceModule(torch.nn.Module):
            def __init__(self):
                super().__init__()
                self.linear1 = torch.nn.Linear(8, 4, bias=False)
        ...
        # Apply 8da4w quantization with group_size=128 (default from quantize.py)
        group_size = 128
        quantize_(
            linear_sequence_module,
            int8_dynamic_activation_int4_weight(group_size=group_size),
        )
        linear_sequence_module = unwrap_tensor_subclass(linear_sequence_module)

        # Create sample inputs
        sample_inputs = (torch.randn(size=(4, 8), dtype=torch.float32),)

        # Test the quantized module
        # Use higher tolerance since quantization introduces some error
        self.lower_module_and_test_output(
            linear_sequence_module,
            sample_inputs,
    ...
    def test_vulkan_backend_xnnpack_pt2e_quantized_conv_sequence_easy(self):
        import executorch.backends.vulkan.test.utils as test_utils
        ...
        class ConvSequenceModule(torch.nn.Module):
            def __init__(self):
                ...
                # self.conv1.weight.data = torch.ones(self.conv1.weight.shape)
        ...
        input_tensor = torch.ones(input_tensor_shape, dtype=torch.float32)
        # Create sample inputs
        sample_inputs = (input_tensor,)

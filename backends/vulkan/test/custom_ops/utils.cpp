...
void BenchmarkResult::print_summary(
  ...
  switch (correctness_status_) {
    ...
    case CorrectnessStatus::PASSED:
      ...
      break;
    case CorrectnessStatus::FAIL:
      correctness_str = "FAIL";
      break;
  }
...
TestResult execute_test_cases(
  ...
  for (size_t i = 0; i < test_cases.size(); ++i) {
    ...
    BenchmarkResult result =
        execute_test_case(test_case, warmup_runs, benchmark_runs);
    ...
    if (!vulkan_execute_succeeded) {
      result.set_correctness_status(CorrectnessStatus::FAIL);

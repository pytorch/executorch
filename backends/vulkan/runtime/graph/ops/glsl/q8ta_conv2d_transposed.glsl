...
void main() {
  // Thread mapping: same as q8ta_conv2d
  // Each thread handles a 4W x 4C tile of output
  ...
  // Initialize accumulators for 4 width positions x 4 output channels each
  ivec4 acc[4];
  [[unroll]] for (int i = 0; i < 4; ++i) {
    acc[i] = ivec4(0);
  }

  // Transposed convolution loop structure:
  // Iterate over all kernel positions (ky, kx, ic4). For each position,
  // compute the corresponding input position. If the input position is valid
  // (in bounds and on a stride-aligned position), load the actual input;
  // otherwise use input_zp_packed. This ensures weight_sums correction is
  // consistent with the accumulation (all kernel positions are accounted for).
  //
  // For transposed convolution, the input position for a given (output, kernel)
  // is: input = (output + padding - kernel) / stride, which must be exact
  // (remainder == 0) and within [0, input_size).

  const int KH = conv2d_params.kernel_size.y;
  const int KW = conv2d_params.kernel_size.x;
  const int stride_x = conv2d_params.stride.x;
  const int stride_y = conv2d_params.stride.y;
  const int pad_x = conv2d_params.padding.x;
  const int pad_y = conv2d_params.padding.y;

  for (int ky = 0; ky < KH; ky++) {
    // Check if this kernel row maps to a valid input row
    const int in_y_numer = outp_tidx.data[1] + pad_y - ky;
    const bool y_stride_valid = (in_y_numer >= 0) && ((in_y_numer % stride_y) == 0);
    const int iy = y_stride_valid ? (in_y_numer / stride_y) : 0;
    const bool h_in_bounds = y_stride_valid && (iy < inp_H);

    // Loop order: ic4 before kx for better weight texture cache locality.
    // Consecutive ic4 values at fixed kx access consecutive y-rows in the
    // weight texture, but consecutive kx at fixed ic4 access consecutive
    // x-coordinates (same row) which is the fast dimension for texture
    // cache lines. By iterating ic4 in the outer loop and kx in the inner
    // loop, each ic4 iteration sweeps kx across a texture row.
    for (int ic4 = 0; ic4 < IC4_per_group; ic4++) {
      for (int kx = 0; kx < KW; kx++) {
        // Load weight block: 4 output channels x 4 input channels
        const ivec4 weight_block = load_weight_block(
            ic4, kx, ky, oc4, IC4_per_group, KW);

        // Process 4 adjacent width positions
        [[unroll]] for (int subtile_w = 0; subtile_w < 4; ++subtile_w) {
          const int ow = outp_tidx.data[0] + subtile_w;
          const int in_x_numer = ow + pad_x - kx;

          // Load packed input, or use zero point if out of bounds
          int packed_input = input_zp_packed;
          if (h_in_bounds && in_x_numer >= 0 && (in_x_numer % stride_x) == 0) {
            const int ix = in_x_numer / stride_x;
            if (ix < inp_W) {
              ...
              if (get_outer_packed_dim_block_size(inp_layout) == 1) {
                ...
                const int w4_inp = div_4(ix);
                const int inp_c4 = div_4(inp_tidx.data[2]);
                inp_texel_idx = (iy * inp_h_stride + w4_inp * inp_w_stride + inp_c4) * 4 + mod_4(ix);
              }

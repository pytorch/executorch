...
void main() {
  ...
  for (int k4 = 0; k4 < conv2d_params_K4_per_group; k4++) {
    ...
    // Each ivec4 contains 4 packed int32s (one per width position in the tile)
    ...
    // Increment input index for next k4
    input_idx += 1;
  }
  ...
  const int outp_h_stride = int(outp.strides[0][1]);

  // Store packed int8 output tile
  [[unroll]] for (int m4 = 0; m4 < TILE_M4; m4++) {
    int buffer_idx = oh * outp_h_stride +
        (ow_block_idx + m4) * outp_w_stride +
        oc_block_idx;
    [[unroll]] for (int n4 = 0; n4 < TILE_N4; n4++) {
      if (ow_block_idx + m4 < W4 &&
          oc_block_idx + n4 < OC4) {
        t_packed_int8_output[buffer_idx++] = int8_out_tile[m4][n4];
      }
    }
  }
}

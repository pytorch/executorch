...
// Linear-weight prepack identical in shape to the pointwise conv prepack,
// but parameterised on the flat 2D weight [C_out, K_total].  The packed
...
ValueRef prepack_conv2d_gemm_weight(
  ...
  VK_CHECK_COND(weight_sizes.size() == 2);
  const int64_t N = weight_sizes.at(0); // C_out
  const int64_t K = weight_sizes.at(1); // K_total
  ...
  GemmWeightPackParams pack_params{
  ...
  graph.prepack_nodes().emplace_back(new PrepackNode(
      ...
      {},
      {},
      {PushConstantDataInfo(&pack_params, sizeof(GemmWeightPackParams))}));
...
void conv2d_gemm_impl(
  ...
  // Pick im2col storage per device:
  ...
  if (graph.device_is_mali()) {
    ...
    const bool fits_2d = static_cast<uint32_t>(K4_total) <= max_2d &&
        static_cast<uint32_t>(M) <= max_2d;
    const bool fits_3d = static_cast<uint32_t>(W_out) <= max_3d &&
        static_cast<uint32_t>(H_out) <= max_3d &&
        static_cast<uint32_t>(K4_total) <= max_3d;
    if (fits_2d) {
  ...
  // Allocate the im2col intermediate. The 2D and buffer variants use a flat
  // [M, K_total] kWidthPacked shape; the texture3d variant uses the natural
  // [1, K_total, H_out, W_out] kChannelsPacked shape so K4 lays along Z.
  ValueRef im2col_tensor;
  if (im2col_storage == utils::kTexture3D) {
    im2col_tensor = graph.add_tensor(
        {1, K_total, H_out, W_out},
        graph.dtype_of(in),
        utils::kTexture3D,
        utils::kChannelsPacked);
  } else {
    im2col_tensor = graph.add_tensor(
        {M, K_total}, graph.dtype_of(in), im2col_storage, utils::kWidthPacked);
  }
  ...
  // Step 2: flatten + prepack weight for the GEMM
  ValueRef flat_weight = build_im2col_flat_weight_tref(graph, weight_data);
  ValueRef packed_weight = prepack_conv2d_gemm_weight(graph, flat_weight);

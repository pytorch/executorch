...
utils::uvec3 flash_attention_global_wg_size(
  ...
  const int32_t D = graph->size_at<int32_t>(-1, q_projected); // head dim
  const int32_t Br =
...
void flash_attention_impl(
  ...
  graph.execute_nodes().emplace_back(new DynamicDispatchNode(
      ...
      // Inputs and Outputs
      {{out, vkapi::kWrite},
       {q_projected, vkapi::kRead},
       {k_cache_tensor, vkapi::kRead},
       {v_cache_tensor, vkapi::kRead},
       {t_l, vkapi::kReadWrite},
       {t_m, vkapi::kReadWrite}},

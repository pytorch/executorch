...
void check_args(
  ...
  VK_CHECK_COND(
      dim_at<kDim4DWidth>(in.sizes()) * dim_at<kDim4DWidth>(repeats) ==
          dim_at<kDim4DWidth>(out.sizes()),
      "Output's width doesn't match input's width * repeat count");

  VK_CHECK_COND(
      dim_at<kDim4DHeight>(in.sizes()) * dim_at<kDim4DHeight>(repeats) ==
          dim_at<kDim4DHeight>(out.sizes()),
      "Output's height doesn't match input's height * repeat count");

  VK_CHECK_COND(
      dim_at<kDim4DChannel>(in.sizes()) * dim_at<kDim4DChannel>(repeats) ==
          dim_at<kDim4DChannel>(out.sizes()),
      "Output's channel doesn't match input's channel * repeat count");

  VK_CHECK_COND(
      dim_at<kDim4DBatch>(in.sizes()) * dim_at<kDim4DBatch>(repeats) ==
          dim_at<kDim4DBatch>(out.sizes()),
...
void add_repeat_channel_node(
  ...
  int32_t in_width =
      api::utils::safe_downcast<int32_t>(dim_at<kDim4DWidth>(in_sizes));
  int32_t in_height =
      api::utils::safe_downcast<int32_t>(dim_at<kDim4DHeight>(in_sizes));
  int32_t in_channel =
      api::utils::safe_downcast<int32_t>(dim_at<kDim4DChannel>(in_sizes));
  int32_t in_batch =
      api::utils::safe_downcast<int32_t>(dim_at<kDim4DBatch>(in_sizes));
...
void add_repeat_node(
  ...
  if (int64_t channel_repeat = dim_at<kDim4DChannel>(repeats);
      channel_repeat == 1) {
  ...
  // Width
  if (int64_t width_repeat = dim_at<kDim4DWidth>(repeats); width_repeat > 1) {
    api::utils::ivec3 src_offset = api::utils::make_ivec3({0, 0, 0}, false);

    for (int i = 1; i < width_repeat; ++i) {
      api::utils::ivec3 dst_offset = api::utils::make_ivec3(
          {i * dim_at<kDim4DWidth>(in_sizes), 0, 0}, false);
  ...
  // Height
  if (int64_t height_repeat = dim_at<kDim4DHeight>(repeats);
      height_repeat > 1) {
    api::utils::ivec3 src_offset = api::utils::make_ivec3({0, 0, 0}, false);

    for (int i = 1; i < height_repeat; ++i) {
      api::utils::ivec3 dst_offset = api::utils::make_ivec3(
          {0, i * dim_at<kDim4DHeight>(in_sizes), 0}, false);
  ...
  // Batch
  if (int64_t batch_repeat = dim_at<kDim4DBatch>(repeats); batch_repeat > 1) {
    api::utils::ivec3 src_offset = api::utils::make_ivec3({0, 0, 0}, false);

...
void add_addmm_optimized_node(
  ...
  // The each thread computes a W=(2/4) x H=4 x C=(1/4) output tile. Therefore,
  // the total number of threads is W/(2 or 4) x H/4 x C/1. Since the out tensor
  // is channels packed, C does not need to be divided by 4. The "identity" of
  // each thread is the (x, y, z) coordinate of the output tile it is computing,
  // and this identity can be used to compute the tensor index of the top left

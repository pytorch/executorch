...
void add_copy_channel_offset_node(
  ...
  VK_CHECK_COND(
      dim_at<kDim4DChannel>(in_sizes) >= src_channel_offset + channel_range,
      "Src channel (",
      src_channel_offset,
      ") and range (",
      channel_range,
      ") should be less than or equal to input tensor's channel size (",
      dim_at<kDim4DChannel>(in_sizes),
      ")");

  VK_CHECK_COND(
      dim_at<kDim4DChannel>(out_sizes) >= dst_channel_offset + channel_range,
      "Dst channel (",
      dst_channel_offset,
      ") and range (",
      channel_range,
      ") should be less than or equal to input tensor's channel size (",
      dim_at<kDim4DChannel>(out_sizes),
      ")");
  ...
  int32_t out_channels = dim_at<kDim4DChannel>(out_sizes);

  // Copy one batch at a time.
  for (int batch_idx = 0; batch_idx < dim_at<kDim4DChannel>(in_sizes);
       batch_idx++) {
    ...
    uvec3 global_size{
        dim_at<kDim4DWidth>(in_sizes),
        dim_at<kDim4DHeight>(in_sizes),

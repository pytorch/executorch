...
const uint8_t* getConstantDataPtr(
  ...
  BytesPtr constant_bytes = flatbuffer_graph->constants()->Get(buffer_idx);
 ...
 public:
  ...
  compileModel(const void* buffer_pointer, ComputeGraph* compute_graph) const {
    ...
    if (header.ok()) {
      ...
      constant_data = buffer_start + header->constants_offset;
    } else {

...
@operator_test
class TestRMSNorm(OperatorTest):
    @dtype_test
    def test_rmsnorm_dtype(self, dtype, tester_factory: Callable) -> None:
        # Input shape: (batch_size, normalized_shape)
        self._test_op(Model().to(dtype), ((torch.rand(3, 10) * 10).to(dtype),), tester_factory)
        
    def test_rmsnorm_2d_input(self, tester_factory: Callable) -> None:
        # Input shape: (batch_size, normalized_shape)
        self._test_op(Model(), (torch.randn(5, 10),), tester_factory)
        
    def test_rmsnorm_3d_input(self, tester_factory: Callable) -> None:
        # Input shape: (batch_size, seq_len, normalized_shape)
        self._test_op(Model(), (torch.randn(5, 15, 10),), tester_factory)
        
    def test_rmsnorm_4d_input(self, tester_factory: Callable) -> None:
    ...
    def test_rmsnorm_multidim_normalized_shape(self, tester_factory: Callable) -> None:
        # Normalize over last 2 dimensions
        self._test_op(Model(normalized_shape=[8, 8]), (torch.randn(5, 10, 8, 8),), tester_factory)
        
    def test_rmsnorm_multidim_normalized_shape_3d(self, tester_factory: Callable) -> None:
        # Normalize over last 3 dimensions
        self._test_op(Model(normalized_shape=[4, 4, 4]), (torch.randn(5, 4, 4, 4),), tester_factory)
        
    def test_rmsnorm_custom_eps(self, tester_factory: Callable) -> None:
        self._test_op(Model(eps=1e3), (torch.randn(5, 10),), tester_factory)
        
    def test_rmsnorm_no_elementwise_affine(self, tester_factory: Callable) -> None:

...
@operator_test
class TestReflectionPad1d(OperatorTest):
    @dtype_test
    def test_reflection_pad1d_dtype(self, dtype, tester_factory: Callable) -> None:
        # Input shape: (batch_size, channels, length)
        self._test_op(Model().to(dtype), ((torch.rand(2, 3, 10) * 10).to(dtype),), tester_factory)
        
    def test_reflection_pad1d_basic(self, tester_factory: Callable) -> None:
        # Basic test with default parameters
        self._test_op(Model(), (torch.randn(2, 3, 10),), tester_factory)
        
    def test_reflection_pad1d_padding(self, tester_factory: Callable) -> None:
    ...
    def test_reflection_pad1d_input_sizes(self, tester_factory: Callable) -> None:
        # Test with different input sizes
        self._test_op(Model(), (torch.randn(1, 1, 5),), tester_factory)
        self._test_op(Model(), (torch.randn(2, 4, 15),), tester_factory)
        
    def test_reflection_pad1d_zero_padding(self, tester_factory: Callable) -> None:
    ...
    def test_reflection_pad1d_asymmetric_padding(self, tester_factory: Callable) -> None:

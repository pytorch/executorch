...
@operator_test
class TestAdaptiveAvgPool1d(OperatorTest):
    @dtype_test
    def test_adaptive_avgpool1d_dtype(self, dtype, tester_factory: Callable) -> None:
        # Input shape: (batch_size, channels, length)
        self._test_op(Model().to(dtype), ((torch.rand(2, 3, 10) * 10).to(dtype),), tester_factory)
        
    def test_adaptive_avgpool1d_basic(self, tester_factory: Callable) -> None:
        # Basic test with default parameters
        self._test_op(Model(), (torch.randn(2, 3, 10),), tester_factory)
        
    def test_adaptive_avgpool1d_output_size(self, tester_factory: Callable) -> None:
    ...
    def test_adaptive_avgpool1d_input_sizes(self, tester_factory: Callable) -> None:
    ...
    def test_adaptive_avgpool1d_same_size(self, tester_factory: Callable) -> None:
        # Test when output size is the same as input size
        self._test_op(Model(output_size=10), (torch.randn(2, 3, 10),), tester_factory)
        
    def test_adaptive_avgpool1d_larger_size(self, tester_factory: Callable) -> None:

...
class RandnModel(torch.nn.Module):
    def __init__(
        self,
        size: List[int],
    ...
    def forward(self, x):
        ...
        return torch.randn(self.size, dtype=self.dtype) + x

@operator_test
class TestRandn(OperatorTest):
    ...
    def test_randn_float16(self, tester_factory: Callable) -> None:
        # Use a large enough size to get stable statistics
        size = [1000]
        model = RandnModel(size, dtype=torch.float16)
    ...
    def test_randn_float32(self, tester_factory: Callable) -> None:
        # Use a large enough size to get stable statistics
        size = [1000]
        model = RandnModel(size, dtype=torch.float32)
    ...
    def test_randn_float64(self, tester_factory: Callable) -> None:
        # Use a large enough size to get stable statistics
        size = [1000]
        model = RandnModel(size, dtype=torch.float64)
    ...
    def test_randn_increasing_rank(self, tester_factory: Callable) -> None:
        base_size = [1024]
    ...
    def test_randn_smoke(self, tester_factory: Callable) -> None:
        size = [2, 3, 4, 128]
        model = RandnModel(size)
        self._test_op(
            model=model,
    ...
    def test_randn_large_tensor(self, tester_factory: Callable) -> None:

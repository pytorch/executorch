...
class Model(torch.nn.Module):
    ...
    def forward(self, x):
        return torch.nn.functional.threshold(x, threshold=self.threshold, value=self.value, inplace=self.inplace)
...
class TestThreshold(OperatorTest):
    ...
    def test_threshold_f32_custom_threshold_value(self, tester_factory: Callable) -> None:
        self._test_op(Model(threshold=0.5, value=1.0), (torch.randn(3, 4, 5),), tester_factory)
    ...
    def test_threshold_f32_all_params(self, tester_factory: Callable) -> None:
        ...
        self._test_op(Model(threshold=0.5, value=3.0, inplace=True), (torch.randn(3, 4, 5),), tester_factory)

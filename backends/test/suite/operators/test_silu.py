...
def test_silu_dtype(test_runner, dtype) -> None:
    test_runner.lower_and_run_model(Model(), ((torch.randn(2, 10) * 100).to(dtype),))


def test_silu_f32_single_dim(test_runner) -> None:
    test_runner.lower_and_run_model(Model(), (torch.randn(20),))


def test_silu_f32_multi_dim(test_runner) -> None:
...
def test_silu_f32_inplace(test_runner) -> None:
    test_runner.lower_and_run_model(Model(inplace=True), (torch.randn(3, 4, 5),))


def test_silu_f32_boundary_values(test_runner) -> None:

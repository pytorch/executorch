...
class TestSigmoid(OperatorTest):
    @dtype_test
    def test_sigmoid_dtype(self, dtype, tester_factory: Callable) -> None:
        self._test_op(
            Model(), ((torch.rand(2, 10) * 10 - 5).to(dtype),), tester_factory
        )

    def test_sigmoid_f32_single_dim(self, tester_factory: Callable) -> None:
        self._test_op(Model(), (torch.randn(20),), tester_factory)

    def test_sigmoid_f32_multi_dim(self, tester_factory: Callable) -> None:
        self._test_op(Model(), (torch.randn(2, 3, 4, 5),), tester_factory)

    def test_sigmoid_f32_boundary_values(self, tester_factory: Callable) -> None:

...
def _run_delegated_executorch_program(
    ...
    if len(input_spec) == 1:
        ...
        dataset_or_inputs = ",".join(
            sorted([os.path.join(dataset_dir, d) for d in os.listdir(dataset_dir)])
        )
...
def _run_non_delegated_executorch_program(
    model, test_dir, test_name, dataset_dir, input_spec, cpu_results_dir
) -> ExportedProgram:
    if len(input_spec) == 1:
        # Single input, use --dataset
        dataset_cli = "--dataset"
    ...
    non_delegated_program = to_quantized_executorch_program(
        model, input_spec, dataset_dir, delegate_to_npu=False
    )

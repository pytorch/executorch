...
from executorch.backends.nxp.backend.ir.converter.conversion.translator import apply_permutation_to, combine_permutations
...
from executorch.backends.nxp.backend.ir.tflite_optimizer.tensor_rules import RuleOr, TensorHasData, TensorIsNotModelOutput, TensorsHaveData
    ...
    def __call__(self) -> bool:
        ...
        for [leading_transpose, following_transposes], tensor_map, _, _ in matcher.match_patterns():
            ...
            for second_transpose in following_transposes:
                ...
                second_transpose.tmp_inputs[1] = self._builder.create_tensor_for_data(combined_perm, 'perm')

                # Compute the output shape of the second `Transpose`.
                new_output_shape = apply_permutation_to(x.shape.vector, combined_perm)
                second_transpose.tmp_outputs[0].shape = tflite_model.Shape(list(new_output_shape))

...
from executorch.backends.nxp.backend.ir.tflite_optimizer.tensor_rules import TensorDimensionsMatch, TensorHasRank, \
    TensorIsChannelsFirst, \
    TensorIsChannelsLast, TensorIsFormatless, TensorsHaveData, TensorsHaveOneConsumer
...
class PermuteFullyConnectedWeightsAfterReshape(BaseOptimization):
    ...
    def __call__(self) -> bool:
        ...
        for (transpose, reshape, fc), tensor_map, _, _ in matcher.match_patterns():
            ...
            y = tensor_map['y']
            to_onnx_perm = translator.create_channels_last_to_channels_first_permutation(y.shape.len())
            ...
            data = translator.convert_data_to_channels_last(data)  # Permute to TFLite format.

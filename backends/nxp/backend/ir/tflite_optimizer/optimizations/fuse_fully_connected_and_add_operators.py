...
from executorch.backends.nxp.backend.ir.tflite_optimizer.pattern_matcher import OneOf, Op, PatternMatcher
from executorch.backends.nxp.backend.ir.tflite_optimizer.tensor_rules import RuleAnd, RuleIf, RuleOr, TensorDimensionsMatch, \
    TensorHasDimensionOfSize, TensorHasOneConsumer, TensorHasRank, TensorHasType, TensorIsQuantized
...
class FuseFullyConnectedAndAddOperators(BaseOptimization):
    ...
    def __call__(self) -> bool:
        ...
        for (fc, add), tensor_map, _, _ in matcher.match_patterns():
            ...
            fc.builtin_options.fused_activation_function = add.builtin_options.fused_activation_function

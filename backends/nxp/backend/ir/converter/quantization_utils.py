...
def re_quantize_static_tensor(
    ...
    if tflite_tensor.quantization is None:
        logger.e(logger.Code.INTERNAL_ERROR, "translator.re_quantize_static_tensor(): Got tensor without quantization!")

    if tflite_tensor.tmp_buffer.data is None:
        logger.e(logger.Code.INTERNAL_ERROR, "translator.re_quantize_static_tensor(): Got tensor without static data!")
    ...
    if tensor_data.dtype == np.uint8 and new_dtype == np.int8:  # INT8 -> UINT8
        ...
        if new_zero_point is None or new_scale is None:
            logger.e(logger.Code.INTERNAL_ERROR, "Missing new zero_point or new scale when re-quantizing tensor.")
        ...
        re_quantized_tensor.type = tflTensorType.TensorType.INT8
        re_quantized_tensor.quantization.zero_point = tflite_model.ZeroPoint(list(new_zero_point))
        ...
        re_quantized_tensor.tmp_buffer.data = quantize_int8(new_tensor_data, new_scale, new_zero_point)
        re_quantized_tensor.quantization.scale = tflite_model.Scale(new_scale)
        re_quantized_tensor.quantization.zero_point = tflite_model.ZeroPoint(new_zero_point)
...
def quantize_static_float_tensor(
    ...
    if zero_point is None or scale is None:
        logger.e(logger.Code.INTERNAL_ERROR, "Missing new zero_point or new scale when quantizing tensor.")
    ...
    if tensor_data.dtype == np.float32 and new_dtype == np.int8:
        ...
        quantized_tensor.quantization.zero_point = tflite_model.ZeroPoint(list(zero_point))

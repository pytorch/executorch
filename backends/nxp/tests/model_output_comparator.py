...
class GroundTruthAccuracyDropOutputComparator(BaseOutputComparator):
    ...
    def __init__(
        ...
        parse_class_id_fn: Callable[[str, dict[str, int]], int] = _parse_class_id_from_sample_dir,
    ):
        """
    ...
    def compare_results(self, cpu_results_dir, npu_results_dir, output_tensor_spec):
        ...
        """
        sample_dirs = [
            os.path.join(cpu_results_dir, file)
            for file in os.listdir(cpu_results_dir)
        ]
        sample_dirs = [file for file in sample_dirs if os.path.isdir(file)]
        ...
        if (
            self.min_cpu_accuracy is not None
            and cpu_accuracy < self.min_cpu_accuracy
        ):
            raise AssertionError(

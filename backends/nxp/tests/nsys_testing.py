...
def _run_cortex_m_program(
    ...
    :param testing_dataset_dir: Directory containing testing data. The samples have to be channels last (NHWC).
    ...
    # Use testing dataset for calibration as testing dataset is always in channel last format for Cortex-M backend.
    numpy_samples = read_prepared_samples(testing_dataset_dir, input_spec)
    calibration_samples = [
...
def assert_cortex_m():
    ...
    cmsis_nn_lib_path = os.path.join(
        PROJECT_DIR,
        "backends/cortex_m/library/_cmsis_nn",
        f"cmsis_nn.cpython-{python_version}-x86_64-linux-gnu.so",
    )
    ...
    match reference_model:
        ...
        case ReferenceModel.QUANTIZED_CORTEX_M:
            ...
            if any(spec.dim_order != torch.channels_last for spec in input_spec):
                raise ValueError(
                    "Cortex-M backend supports only channel last dim order inputs."
                )

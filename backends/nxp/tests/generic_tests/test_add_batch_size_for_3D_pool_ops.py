...
from executorch.backends.nxp.aten_passes.add_batch_size_for_3D_avgpool2D_maxpool2D import (
    AddBatchSizeFor3DAvgPool2DAndMaxPool2D,
)
...
def apply_individual_pass_and_compare(model, export_input, pool_op):
    ...
    NeutronAtenPassManager(
        neutron_target_spec, [AddBatchSizeFor3DAvgPool2DAndMaxPool2D()]
    )(exir_program_aten)

    # Make sure pool op is still in the model.
    ...
    assert reshape_count_after == reshape_count_before + (2 * pool_count_prev)
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param((16, 32, 32), 2, 2, 0, id="3D, kernel=2, stride=2, no padding"),
...
def test_add_batch_size_avgpool2d(input_shape, kernel_size, stride, padding):
    model = AvgPool2DModel(kernel_size=kernel_size, stride=stride, padding=padding)
...
def test_add_batch_size_maxpool2d(input_shape, kernel_size, stride, padding):
    model = MaxPool2DModel(kernel_size=kernel_size, stride=stride, padding=padding)
    example_input = torch.rand(input_shape, dtype=torch.float32)
    apply_individual_pass_and_compare(
...
def test_no_transform_for_4d_input_avgpool2d(input_shape):
    model = AvgPool2DModel(kernel_size=2, stride=2, padding=0)
    example_input = torch.rand(input_shape, dtype=torch.float32)
    ...
    # Check that pool op is present
    assert graph_contains_any_of_ops(
        exir_program_aten.graph, [torch.ops.aten.avg_pool2d.default]
    )
...
def test_no_transform_for_4d_input_maxpool2d(input_shape):
    model = MaxPool2DModel(kernel_size=2, stride=2, padding=0)
    example_input = torch.rand(input_shape, dtype=torch.float32)
    ...
    # Check that pool op is present
    assert graph_contains_any_of_ops(
        exir_program_aten.graph, [torch.ops.aten.max_pool2d.default]
    )
...
class TestAddBatchSizeFor3DPoolOps:
    ...
    def test__3d_pool__full_pipeline(
        ...
        if pool_type == "avg":
            ...
            model = MaxPool2DModel(kernel_size=2, stride=2, padding=0)
            expected_delegated_ops = {ViewCopy: 2, MaxPool2DWithIndices: 1, GetItem: 1}

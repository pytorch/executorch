...
def test_add_batch_size_adaptive_avgpool2d(input_shape, output_size):
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            AdaptiveAvgPool2dModule(output_size=(16, 16)), id="AdaptiveAvgPoolModel"
        ),
        pytest.param(
            AvgPool2dModule(kernel_size=2, stride=2, padding=0), id="AvgPoolModel"
        ),
        pytest.param(
            MaxPool2dModule(kernel_size=2, stride=2, padding=0), id="MaxPoolModel"
        ),
    ],
)
def test_no_transform_for_4d_input(input_shape, model):
    example_input = torch.rand(input_shape, dtype=torch.float32)
    ...
    pool_op = None
    match model:
        case AdaptiveAvgPool2dModule():
            pool_op = torch.ops.aten.adaptive_avg_pool2d.default
        case AvgPool2dModule():
            pool_op = torch.ops.aten.avg_pool2d.default
        case _:
            pool_op = torch.ops.aten.max_pool2d.default
    assert graph_contains_any_of_ops(
        exir_program_aten.graph,
        [pool_op],
    )

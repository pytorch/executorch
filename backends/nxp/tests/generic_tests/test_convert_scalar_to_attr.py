...
class TestConvertScalarToAttr:
    @pytest.mark.parametrize(
        "model_cls, expected_op, scalar_val",
        [
            pytest.param(AddScalarModule, torch.ops.aten.add.Tensor, 2.0, id="Add op."),
            pytest.param(SubScalarModule, torch.ops.aten.sub.Tensor, 2.0, id="Sub op."),
            pytest.param(MulScalarModule, torch.ops.aten.mul.Tensor, 2.0, id="Mul op."),
        ],
    )
    def test__scalar_to_attr(self, model_cls, expected_op, scalar_val):
        input_shape = (2, 5, 7, 9)
        model = model_cls(scalar=scalar_val)
        ...
        NeutronAtenPassManager(neutron_target_spec, [ConvertScalarToAttrPass()])(
            exir_program_aten
        )
        ...
        assert not any(
            ...
            for arg in exp_op_node.args
        )
        # Check that the scalar value is present.
        assert any(
            arg.op == "get_attr"
            and try_get_tensor_constant_from_node(exir_program_aten, arg).item()
            == scalar_val
            for arg in exp_op_node.args
        )

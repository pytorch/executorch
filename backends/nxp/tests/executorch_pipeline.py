...
from executorch.exir import EdgeProgramManager, ExecutorchBackendConfig, ExecutorchProgramManager, \
    to_edge_transform_and_lower, EdgeCompileConfig
...
def to_quantized_edge_program(model: torch.nn.Module, input_shape: tuple, operators_not_to_delegate: list[str] = None,
                              target="imxrt700", neutron_converter_flavor="SDK_25_03") -> EdgeProgramManager:
    calibration_inputs = [(torch.randn(input_shape),), (torch.randn(input_shape),)]
    example_input = (torch.ones(*input_shape),)

    exir_program_aten = torch.export.export_for_training(model, example_input, strict=True)
    ...
    exir_program_aten__module_quant = _quantize_model(exir_program_aten_module, calibration_inputs)
    ...
    edge_program_manager = to_edge_transform_and_lower(
        torch.export.export(exir_program_aten__module_quant, example_input, strict=True),

...
class MaxPoolSumModule(torch.nn.Module):
    @staticmethod
...
class SumDimIntListMaxPoolModule(MaxPoolSumModule):
    def forward(self, x):
        x = torch.sum(x, dim=self.dim, keepdim=self.keepdim)
        x = self.noop_max_pool_2d(x)
        return x
...
def assert_delegated(
    ...
    if expected_delegated_ops is None:
        expected_delegated_ops = {SumDimIntList: 1}
...
def assert_not_delegated(model, input_shape):
    ...
    assert graph_contains_any_of_ops(delegated_ep.graph, [SumDimIntList])


class TestSumDimIntListConverter:
    ...
    def test__noop__not_only_node__delegated(self, mocker, request, input_shape, dim):
        keep_dim = True  # Reduction over a dimension of size `1` with `keep_dim=True` is a no-op.
        model = SumAddModule(dim, keep_dim)
        assert_delegated(
            model,
            input_shape,
            mocker,
            request,
            expected_delegated_ops={SumDimIntList: 1, AddTensor: 1},
        )
    ...
    def test__channels_first__keep_dim__true(self, mocker, request):
        ...
        model = MaxPoolSumModule(dim, True)
        assert_delegated(
            model,
            input_shape,
            mocker,
    ...
    class TestKeepDimFalseFormatHandling:
        """When `keep_dim = False`, the `sum` operator changes the rank, so the format have to be explicitly
        ...
        def test__channels_first_input__reducing_channels(self, mocker, request, dim):
            # If the channels dimension is reduced (removed), the `sum` output will always be equal in channels first

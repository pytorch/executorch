...
from executorch.backends.nxp.tests.models import LinearPReLUModule
...
def test_invalid_prelu_conversion(mocker, input_shape):
    ...
    # Check that `prelu` was properly decomposed into simpler edge operators since it could not be delegated
    assert not graph_contains_any_of_ops(
        edge_program.graph,
        [torch.ops.aten.prelu.default, exir_ops.edge.aten.prelu.default],
    )

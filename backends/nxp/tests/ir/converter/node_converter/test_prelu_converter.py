...
def test_prelu_with_linear_quant_conversion(mocker, input_shape):
    ...
    assert graph_contains_any_of_ops(
        ...
        [torch.ops.aten.prelu.default, exir_ops.edge.aten.prelu.default],
    )
...
def test_prelu_2_partitions_test(mocker, input_shape):
...
def test_invalid_prelu_conversion(mocker, input_shape):
    # Run conversion
    channels = input_shape[-1]
    edge_program = to_quantized_edge_program(
        LinearPReLUModule(in_features=channels, out_features=channels),
    ...
    assert graph_contains_any_of_ops(edge_program.graph, [exir_ops.edge.aten.gt.Scalar])
    assert graph_contains_any_of_ops(
        edge_program.graph, [exir_ops.edge.aten.mul.Tensor]
    )
    assert graph_contains_any_of_ops(
        edge_program.graph, [exir_ops.edge.aten.where.self]
    )

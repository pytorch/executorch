...
class SplitGroupConvolution(PassBase):
    ...
    def _create_split_node(self, *args) -> Node:
        split_node = self.module.graph.call_function(torch.ops.aten.split.default, args)
        ...
        x_val = args[0].meta["val"]
        with FakeTensorMode() as mode:
            fake_input = FakeTensor.from_tensor(
                torch.empty(x_val.shape, dtype=x_val.dtype), mode
            )
            output_shapes = [t.shape for t in torch.split(fake_input, args[1], args[2])]
    ...
    def _create_concat_node(self, *args) -> Node:
        concat_node = self.module.graph.call_function(torch.ops.aten.cat.default, args)
    ...
    def call(self, module: GraphModule):
        ...
        for node in self.module.graph.nodes:
            ...
            if w_data is None or b_data is None:
                continue  # Only the standard case with static weights is supported.

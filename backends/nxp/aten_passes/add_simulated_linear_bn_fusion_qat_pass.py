...
def _get_compute_scale_factor_pattern() -> Callable:
    def _compute_scale(
...
def _get_input_nodes(graph_module: GraphModule) -> list[Node]:
    input_nodes = []
...
def _get_tensor_shape(node: Node) -> Optional[torch.Size]:
...
def _insert_later_bias_application(
    ...
    with graph_module.graph.inserting_before(bias_add_node):
        ...
        bias_add_node.insert_arg(1, reshape_node)
...
class AddSimulatedLinearBatchNormFusionQATPass(PassBase):
    ...
    2. Adding pre-processing of the linear weights with this scale factor and applying inverse to it's output
    ...
    3. Keeping the batch norm operator in the graph for statistics tracking during QAT
    ...
    def call(self, graph_module: GraphModule) -> Optional[PassResult]:
        ...
        for node in graph_module.graph.nodes:
            ...
            linear_w_node = _unwrap_if_fq(linear_w_node_or_fq)
            linear_b_node = _unwrap_if_fq(linear_b_node_or_fq)
            ...
            if _is_fq_node(bn_w_node) or _is_fq_node(bn_var_node):
                continue
            ...
            bn_scale_factor_fn = _get_compute_scale_factor_pattern()

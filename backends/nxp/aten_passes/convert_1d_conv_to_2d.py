...
# aten.conv1d args: (input_node, weight_node, bias_node, stride, padding, dilation, groups)
Conv1dArgs = tuple[Node, Node, (Node | None), list[int], list[int], list[int], int]
# aten.conv_transpose1d args: (input_node, weight_node, bias_node, stride, padding, output_padding, groups, dilation)
...
class ConvertConv1dToConv2dPass(PassBase):
    ...
    def _reshape_w_node_to_2d(self, node: Node):
        ...
        fake_mode = (
            meta_val.fake_mode if isinstance(meta_val, FakeTensor) else FakeTensorMode()
        )
    ...
    def _convert_node_1d_args_to_2d(self, old_1d_node: Node):
        ...
        with self.graph_module.graph.inserting_before(old_1d_node):
            ...
            w_node = self._reshape_w_node_to_2d(w_node)

...
signature mirrors int6_plain_mm but adds a zero tensor (like int4_plain_mm):
two weight planes (ql, qh) plus uint8 scale + uint8 zero codes + a per-row
``steps`` super-scale.
...
_lib.define(
    "int5_plain_mm(Tensor self, Tensor ql, Tensor qh, Tensor scale, Tensor zero, Tensor steps, int group_size) -> Tensor"
)


@impl(_lib, "int5_plain_mm", "Meta")
def _meta_int5(self, ql, qh, scale, zero, steps, group_size):
    return torch.empty(self.shape[0], ql.shape[0], dtype=self.dtype, device=self.device)


@impl(_lib, "int5_plain_mm", "CUDA")
def _cuda_int5(self, ql, qh, scale, zero, steps, group_size):
    # scale/zero are uint8 codes in the [N, n_groups] layout; steps is the
    # per-row [N, 2] bf16 super-scale (scale_step, zero_step).
    # _dequant_matmul_int5 reconstructs scale=code*scale_step, zero=code*zero_step.
    return _dequant_matmul_int5(self, ql, qh, scale, zero, steps, group_size)
...
def _dequant_matmul_int5(x, ql, qh, scale, zero, steps, group_size):
    """Dequant packed-INT5 weights to input dtype and call F.linear.

    ql [N, K/2] / qh [N, K/8] pack asymmetric Q5_K values u in [0, 31].
    scale/zero [N, K//gs] are uint8 codes; steps [N, 2] bf16 holds the per-row
    super-scales (scale_step, zero_step), so the real per-group values are
    ``scale = scale_code * scale_step`` and ``zero = zero_code * zero_step``.
    Dequant: w[n, k] = scale[n, k//gs] * (u[n, k] - zero[n, k//gs]).
    """
    N = ql.shape[0]
    K = ql.shape[1] * 2
    ...
    def _dq(qlc, qhc, sc, ze, st, rows):
        u = unpack_int5(qlc, qhc, rows, K).to(dtype).reshape(rows, n_groups, group_size)
        scale_step = st[:, 0].to(dtype).reshape(rows, 1, 1)
        zero_step = st[:, 1].to(dtype).reshape(rows, 1, 1)
        s = (sc.to(dtype) * scale_step[:, :, 0]).reshape(rows, n_groups, 1)
        z = (ze.to(dtype) * zero_step[:, :, 0]).reshape(rows, n_groups, 1)
        w_deq = (s * (u - z)).reshape(rows, K)
        return F.linear(x, w_deq)

    if N <= _DEQUANT_N_THRESHOLD:
        return _dq(ql, qh, scale, zero, steps, N)

    outs = []
    for i in range(0, N, _DEQUANT_N_CHUNK):
        j = min(i + _DEQUANT_N_CHUNK, N)
        outs.append(_dq(ql[i:j], qh[i:j], scale[i:j], zero[i:j], steps[i:j], j - i))
...
def _(func, types, args, kwargs):
    ...
    if M <= 4:
        out = torch.ops.executorch_cuda.int5_plain_mm(
            x_2d, ql, qh, scale, zero, steps, gs
        )
    else:
        out = _dequant_matmul_int5(x_2d, ql, qh, scale, zero, steps, gs)

...
#undef LOAD_OPTIONAL_SYMBOL
 ...
 private:
  ...
  Error load_constants_with_cache(
    ...
    // tensors match is only known after extraction. Both branches below
    // extract and verify, so a hit can be rejected either way.
    {
    ...
    if (!uncached_fqns.empty()) {
      ...
      // Every name is already cached, so there is nothing to extract from the
      // blob by the normal path and the mismatch check above never runs. That
      // is exactly the total-collision case: a second method whose constant
      // names all coincide with the first method's. Load the blob anyway and
      // verify, otherwise the names alone decide sharing and a colliding
      // constant is aliased silently.
      ET_LOG(
          Info,
          "All %zu constants cached for method '%s'; verifying against blob",
          fqn_to_name.size(),
          method_name.c_str());

      auto buffer_res = named_data_map->get_data(weights_blob_key.c_str());
      if (!buffer_res.ok() || handle->extract_constants_map == nullptr ||
          handle->update_constants_from_blob == nullptr) {
        // Without a blob or the extraction entry points there is no second
        // opinion available. Keep the previous behaviour rather than failing a
        // load that used to work, but say so: sharing rests on names only.
        ET_LOG(
            Info,
            "No blob or extractor for method '%s'; cached constants are "
            "shared on name alone",
            method_name.c_str());
      } else {
        const void* weights_blob = buffer_res->data();
        ET_CHECK_OK_OR_RETURN_ERROR(
        ...
        std::unordered_map<std::string, AtenTensorHandle> verify_map;
        ET_CHECK_OK_OR_RETURN_ERROR(
            handle->extract_constants_map(
                handle->container_handle,
                reinterpret_cast<AOTInductorConstantMapHandle>(&verify_map),
                /*use_inactive=*/false),
            "Failed to extract constants from '%s'",
            method_name.c_str());
        ...
        for (const auto& [fqn, _] : fqn_to_name) {
          auto cached_it = shared_constant_tensors_.find(fqn);
          auto fresh_it = verify_map.find(fqn);
          if (cached_it == shared_constant_tensors_.end() ||
              fresh_it == verify_map.end()) {
            continue;
          }
          if (check_cached_constant_match(
                  fqn, cached_it->second, fresh_it->second) != Error::Ok) {
            ET_LOG(
                Info,
                "Constant '%s' in method '%s' differs from the cached copy; "
                "using this method's own copy instead of sharing",

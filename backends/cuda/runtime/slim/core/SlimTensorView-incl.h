...
inline SlimTensor SlimTensor::as_strided_(executorch::aten::IntArrayRef sizes,
  ...
  STANDALONE_CHECK(sizes.size() == strides.size(),
                   ...
                   ") must equal number of strides (", strides.size(), ")");
  for (size_t i = 0; i < sizes.size(); ++i) {
    STANDALONE_CHECK(sizes[i] >= 0, "as_strided: size at dimension ", i,
                     " is negative: ", sizes[i]);
  }
  STANDALONE_CHECK(
      storage_offset >= 0,
      "as_strided: storage_offset must be non-negative, got: ", storage_offset);
...
inline SlimTensor SlimTensor::narrow(int64_t dim, int64_t start,
  ...
  STANDALONE_CHECK(end <= this->size(dim), "Invalid range to narrow. range(",
                   start, ", ", start + length,
                   ") must be a subset of range(0, ", this->size(dim), ").");

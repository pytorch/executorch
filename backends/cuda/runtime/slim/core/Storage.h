...
template <> struct DeviceTraits<standalone::c10::DeviceType::CUDA> {
  ...
  static void memcpy(void *dst, const void *src, size_t nbytes,
    ...
    if (src_device.is_cpu()) {
      ...
      STANDALONE_CHECK(src_device.index() == dst_device.index(),
...
public:
  ...
  MaybeOwningStorage(MaybeOwningStorage &&other) noexcept
      : device_(other.device_), data_(other.data_), capacity_(other.capacity_),
        deleter_(other.deleter_), is_owning_(other.is_owning_) {
  ...
  void copy_(void *dst_data_ptr, void *src_data_ptr, size_t nbytes,
    ...
    STANDALONE_CHECK(src_data_ptr,
                     "Storage clone failed: src_data_ptr can not be nullptr")
    ...
    if (device_.is_cpu() && src_device.is_cpu()) {
      // CPU to CPU copy
      DeviceTraits<c10::DeviceType::CPU>::memcpy(dst_data_ptr, src_data_ptr,
                                                 nbytes, device_, src_device);
    } else {
      // At least one of the devices is CUDA
      DeviceTraits<c10::DeviceType::CUDA>::memcpy(dst_data_ptr, src_data_ptr,
                                                  nbytes, device_, src_device);
    }
  }
  ...
  bool is_resizable() const { return is_owning_; }
  ...
  void set_data_ptr_noswap(void *new_data) { data_ = new_data; }

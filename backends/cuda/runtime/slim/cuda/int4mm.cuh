...
void launch_tinygemm_kernel(
    const Tensor& A,
    const Tensor& B,
    const Tensor* qScaleAndZeros, /* optional */
    Tensor& C_final,
    int32_t mTiles,
    int32_t nTiles,
    int32_t kTiles,
...
AOTITensorHandle* _weight_int4pack_mm_cuda(
    const AOTITensorHandle& A,
    const AOTITensorHandle& B,
    int64_t qGroupSize,
    const AOTITensorHandle& qScaleAndZeros) {
  // Skip CUDAGuard because ETensor doesn't carry device information
  // auto result = CUDAGuard::create(0);

  // Skip device check because ETensor doesn't carry device information
  // ET_CHECK(
  ...
  ET_CHECK(B.dim() == 4);
  ET_CHECK(B.size(1) == k / (B_innerKTiles * kKTileSize));
  ET_CHECK(B.size(2) == 32);
  ...
  aoti_torch_empty_strided(
    ...
    ((defined(CUDA_VERSION) && CUDA_VERSION >= 12000) && \
     (!defined(__CUDA_ARCH__) || (__CUDA_ARCH__ >= 800)))
  auto stream_result = slim::cuda::getCurrentCUDAStream(0);
...
#undef RUN_GEMM
  ...
  // Cast SlimTensor* to Tensor* for return
  return reinterpret_cast<Tensor*>(C_final);
#endif
  ET_CHECK_MSG(false, "_weight_int4pack_mm_cuda is not available for build.");
  return nullptr;
}

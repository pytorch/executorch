...
#include <executorch/backends/cuda/runtime/c10/util/TypeCast.h>
...
#define DEFINE_IMPLICIT_CTOR(type, name)                                       \
  ...
  static_assert(sizeof(void *) != 8 || std::is_same_v<long, int64_t>,
...
#undef DEFINE_IMPLICIT_CTOR
  ...
  // problem.
  template <typename T, typename std::enable_if_t<std::is_same_v<T, bool>, bool>
                            * = nullptr>
...
#define DEFINE_ACCESSOR(type, name)                                            \
  type to##name() const {                                                      \
    if (Tag::HAS_d == tag) {                                                   \
      ...
      return checked_convert<type, executorch::backends::cuda::c10::complex<double>>(v.z,      \
                                                                     #type);   \
    }                                                                          \
    if (Tag::HAS_b == tag) {                                                   \
...
#undef DEFINE_ACCESSOR
  ...
  const void *data_ptr() const { return static_cast<const void *>(&v); }

  bool isFloatingPoint() const { return Tag::HAS_d == tag; }
  ...
  bool isComplex() const { return Tag::HAS_z == tag; }
  bool isBoolean() const { return Tag::HAS_b == tag; }
  ...
  Scalar operator-() const {
    ...
    STANDALONE_INTERNAL_ASSERT(false, "unknown ivalue tag ",
                               static_cast<int>(tag));
  }
  ...
  template <typename T, typename std::enable_if_t<
                            !executorch::backends::cuda::c10::is_complex<T>::value, int> = 0>
  ...
  executorch::backends::cuda::c10::ScalarType type() const {
    if (isComplex()) {
      return executorch::backends::cuda::c10::ScalarType::ComplexDouble;
    } else if (isFloatingPoint()) {
      ...
      if (Tag::HAS_u == tag) {
        return executorch::backends::cuda::c10::ScalarType::UInt64;
      }
  ...
  Scalar(Scalar &&rhs) noexcept : tag(rhs.tag) { moveFrom(std::move(rhs)); }
...
private:
  ...
  union v_t {
    ...
    uint64_t u;
    executorch::backends::cuda::c10::complex<double> z;
  ...
  template <typename T, typename std::enable_if_t<
                            !std::is_integral_v<T> &&
                                !executorch::backends::cuda::c10::is_complex<T>::value,
                            bool> * = nullptr>
  ...
  template <typename T,
            typename std::enable_if_t<executorch::backends::cuda::c10::is_complex<T>::value,
                                      bool> * = nullptr>

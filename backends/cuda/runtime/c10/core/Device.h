...
#include <executorch/backends/cuda/runtime/c10/core/DeviceType.h>
#include <executorch/backends/cuda/runtime/c10/util/Exception.h>
#include <executorch/backends/cuda/runtime/c10/util/StringUtil.h>
...
inline DeviceType parse_type(const std::string &device_string) {
  ...
  STANDALONE_CHECK(false, "Expected one of ", Join(", ", device_names),
                   " device type at start of device string: ", device_string);
}
} // namespace detail
...
struct Device final {
  ...
  /* implicit */ Device(const std::string &device_string) : Device(Type::CPU) {
    ...
    STANDALONE_CHECK(!has_error, "Invalid device string: '", device_string,
                     "'");
    ...
    try {
      ...
      STANDALONE_CHECK(false, "Could not parse device index '",
                       device_index_str, "' in device string '", device_string,
                       "'");
    }
  ...
  bool is_ve() const noexcept { return type_ == DeviceType::VE; }
  ...
  bool is_xla() const noexcept { return type_ == DeviceType::XLA; }

  /// Return true if the device is of MTIA type.
  bool is_mtia() const noexcept { return type_ == DeviceType::MTIA; }

  /// Return true if the device is of HPU type.
  bool is_hpu() const noexcept { return type_ == DeviceType::HPU; }

  /// Return true if the device is of Lazy type.
  bool is_lazy() const noexcept { return type_ == DeviceType::Lazy; }
  ...
  bool is_maia() const noexcept { return type_ == DeviceType::MAIA; }
...
private:
  ...
  void validate() {
    ...
    STANDALONE_INTERNAL_ASSERT_DEBUG_ONLY(
        index_ >= -1, "Device index must be -1 or non-negative, got ",
        static_cast<int>(index_));
    STANDALONE_INTERNAL_ASSERT_DEBUG_ONLY(
        !is_cpu() || index_ <= 0, "CPU device index must be -1 or zero, got ",
        static_cast<int>(index_));
  }
};
...
namespace std {
template <> struct hash<executorch::backends::cuda::c10::Device> {
  size_t operator()(executorch::backends::cuda::c10::Device d) const noexcept {
    // Are you here because this static assert failed?  Make sure you ensure
    // that the bitmasking code below is updated accordingly!
    static_assert(sizeof(executorch::backends::cuda::c10::DeviceType) == 1,
                  "DeviceType is not 8-bit");
    static_assert(sizeof(executorch::backends::cuda::c10::DeviceIndex) == 1,

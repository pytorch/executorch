...
public:
  ...
  bool operator==(const SizesAndStrides &other) const {
    ...
    return !(isInline()
                 ? std::memcmp(inlineStorage_, other.inlineStorage_,
                               sizeof(inlineStorage_))
                 : std::memcmp(outOfLineStorage_, other.outOfLineStorage_,
                               storageBytes(size_)));
  }
  ...
  void resize(size_t newSize) {
    ...
    if (STANDALONE_LIKELY(newSize <=
      ...
      if (oldSize < newSize) {
        ...
        memset(&inlineStorage_[STANDALONE_SIZES_AND_STRIDES_MAX_INLINE_SIZE +
                               oldSize],
               0, bytesToZero);
      }
      size_ = newSize;
...
private:
  void resizeSlowPath(size_t newSize, size_t oldSize) {
    if (newSize <= STANDALONE_SIZES_AND_STRIDES_MAX_INLINE_SIZE) {
      ...
      memcpy(&inlineStorage_[0], &tempStorage[0],
             STANDALONE_SIZES_AND_STRIDES_MAX_INLINE_SIZE *
                 sizeof(inlineStorage_[0]));
      memcpy(&inlineStorage_[STANDALONE_SIZES_AND_STRIDES_MAX_INLINE_SIZE],
             &tempStorage[oldSize],
             STANDALONE_SIZES_AND_STRIDES_MAX_INLINE_SIZE *
                 sizeof(inlineStorage_[0]));
      ...
      if (isInline()) {
        ...
        memmove(outOfLineStorage_ + newSize, outOfLineStorage_ + oldSize,
  ...
  void allocateOutOfLineStorage(size_t size) {
    ...
    STANDALONE_CHECK(outOfLineStorage_,

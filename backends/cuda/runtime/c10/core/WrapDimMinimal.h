...
T _maybe_wrap_dim(T dim, T dim_post_expr, bool wrap_scalar = true) {
  ...
  return executorch::backends::cuda::c10::detail::maybe_wrap_dim_slow<T>(
      std::move(dim), std::move(dim_post_expr), wrap_scalar);
}

inline int64_t maybe_wrap_dim(int64_t dim, int64_t dim_post_expr,
...
template <typename T>
T maybe_wrap_dim_slow(T dim, T dim_post_expr, bool wrap_scalar) {
  STANDALONE_CHECK(dim_post_expr >= 0, "Rank cannot be negative but got ",
                   dim_post_expr);

  if (dim_post_expr == 0) {
    STANDALONE_CHECK(wrap_scalar, "Dimension specified as ", dim,
                     " but tensor has no dimensions");
    return executorch::backends::cuda::c10::maybe_wrap_dim(std::move(dim), /*dim_post_expr=*/1,
                                           /*wrap_scalar=*/false);
  }

  T min = dim_post_expr * -1;
  T max = dim_post_expr - 1;
  STANDALONE_CHECK(min <= dim && dim <= max,

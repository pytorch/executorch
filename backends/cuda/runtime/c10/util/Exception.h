...
// instead of executorch::backends::cuda::c10::Error.
namespace executorch::backends::cuda::c10::detail {
...
#define STANDALONE_CHECK_MSG(cond, type, ...)                                  \
  (::executorch::backends::cuda::c10::detail::torchCheckMsgImpl(                               \
...
#define STANDALONE_CHECK(cond, ...)                                            \
  if (STANDALONE_UNLIKELY_OR_CONST(!(cond))) {                                 \
    throw std::runtime_error(STANDALONE_CHECK_MSG(cond, "", __func__, ", ",    \
                                                  __FILE__, ":", __LINE__,     \
                                                  ", ", ##__VA_ARGS__));       \
  }
#define STANDALONE_INTERNAL_ASSERT(cond, ...)                                  \
  if (STANDALONE_UNLIKELY_OR_CONST(!(cond))) {                                 \
    throw std::runtime_error(STANDALONE_CHECK_MSG(                             \
        cond, "", __func__, ", ", __FILE__, ":", __LINE__, ", ", #cond,        \
        " INTERNAL ASSERT FAILED: ", ##__VA_ARGS__));                          \
  }

#define WARNING_MESSAGE_STRING(...)                                            \
  ::executorch::backends::cuda::c10::detail::torchCheckMsgImpl(__VA_ARGS__)

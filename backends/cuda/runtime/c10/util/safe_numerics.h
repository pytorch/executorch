...
#ifdef _MSC_VER
#define STANDALONE_HAS_BUILTIN_OVERFLOW() (0)
#include <intrin.h>
#include <executorch/backends/cuda/runtime/c10/util/llvmMathExtras.h>
#else
...
STANDALONE_ALWAYS_INLINE bool add_overflows(uint64_t a, uint64_t b,
                                            uint64_t *out) {
#if STANDALONE_HAS_BUILTIN_OVERFLOW()
...
STANDALONE_ALWAYS_INLINE bool mul_overflows(uint64_t a, uint64_t b,
                                            uint64_t *out) {
#if STANDALONE_HAS_BUILTIN_OVERFLOW()
  ...
  *out = a * b;
  ...
  return ((executorch::backends::cuda::c10::llvm::countLeadingZeros(a) +
           executorch::backends::cuda::c10::llvm::countLeadingZeros(b)) < 64);
#endif
}

STANDALONE_ALWAYS_INLINE bool mul_overflows(int64_t a, int64_t b,
                                            int64_t *out) {
#if STANDALONE_HAS_BUILTIN_OVERFLOW()
  return __builtin_mul_overflow(a, b, out);
#else
...
bool safe_multiplies_u64(It first, It last, uint64_t *out) {
  ...
  for (; first != last; ++first) {
    overflow |= executorch::backends::cuda::c10::mul_overflows(prod, *first, &prod);
  }
  *out = prod;
  return overflow;
#else
  ...
  for (; first != last; ++first) {
    ...
    is_zero |= (x == 0);
    prod_log2 += executorch::backends::cuda::c10::llvm::Log2_64_Ceil(x);
  }
  *out = prod;
  // This test isnt exact, but avoids doing integer division

 ...
 */
STANDALONE_HOST_DEVICE inline float fp16_ieee_to_fp32_value(uint16_t h) {
#ifdef STANDALONE_X86_F16
  return _cvtsh_ss(h);
#else
  /*
   * Extend the half-precision floating-point number to 32 bits and shift to the
...
#ifdef STANDALONE_X86_F16
#undef STANDALONE_X86_F16
#endif // STANDALONE_X86_F16

#if defined(__aarch64__) && !defined(__CUDACC__)
inline float16_t fp16_from_bits(uint16_t h) {
  return executorch::backends::cuda::c10::bit_cast<float16_t>(h);
}

inline uint16_t fp16_to_bits(float16_t f) {
  return executorch::backends::cuda::c10::bit_cast<uint16_t>(f);
}
...
struct alignas(2) Half {
  ...
  struct from_bits_t {};
  STANDALONE_HOST_DEVICE static constexpr from_bits_t from_bits() {
    return from_bits_t();
  }

  // HIP wants __host__ __device__ tag, CUDA does not
#if defined(USE_ROCM)
  STANDALONE_HOST_DEVICE Half() = default;
#else
  Half() = default;
#endif

  constexpr STANDALONE_HOST_DEVICE Half(unsigned short bits, from_bits_t)
      : x(bits) {}
#if defined(__aarch64__) && !defined(__CUDACC__)
  inline Half(float16_t value);
  inline operator float16_t() const;
#else
  inline STANDALONE_HOST_DEVICE Half(float value);
  inline STANDALONE_HOST_DEVICE operator float() const;
#endif

#if defined(__CUDACC__) || defined(__HIPCC__)
  inline STANDALONE_HOST_DEVICE Half(const __half &value);
  inline STANDALONE_HOST_DEVICE operator __half() const;
#endif
#ifdef SYCL_LANGUAGE_VERSION
  inline STANDALONE_HOST_DEVICE Half(const sycl::half &value);
  inline STANDALONE_HOST_DEVICE operator sycl::half() const;
#endif
};
...
#include <executorch/backends/cuda/runtime/c10/util/Half-inl.h> // IWYU pragma: keep

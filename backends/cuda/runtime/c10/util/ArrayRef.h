...
public:
  ...
  constexpr ArrayRef<T> slice(size_t N, size_t M) const {
    STANDALONE_CHECK(N + M <= size(), "ArrayRef: invalid slice, N = ", N,
                     "; M = ", M, "; size = ", size());
  ...
  constexpr ArrayRef<T> slice(size_t N) const {
    STANDALONE_CHECK(N <= size(), "ArrayRef: invalid slice, N = ", N,
                     "; size = ", size());
  ...
  constexpr const T &at(size_t Index) const {
    STANDALONE_CHECK(Index < Length, "ArrayRef: invalid index Index = ", Index,
                     "; Length = ", Length);
    return Data[Index];
  }
...
// conversions to get you to an executorch::backends::cuda::c10::ArrayRef, which is why we
// need so many overloads.

template <typename T>
bool operator==(executorch::backends::cuda::c10::ArrayRef<T> a1,
                executorch::backends::cuda::c10::ArrayRef<T> a2) {
  return a1.equals(a2);
}
...
template <typename T>
bool operator==(const std::vector<T> &a1, executorch::backends::cuda::c10::ArrayRef<T> a2) {
  return executorch::backends::cuda::c10::ArrayRef<T>(a1).equals(a2);
}
...
template <typename T>
bool operator==(executorch::backends::cuda::c10::ArrayRef<T> a1, const std::vector<T> &a2) {
  return a1.equals(executorch::backends::cuda::c10::ArrayRef<T>(a2));
}
...
using IntList
    [[deprecated("This alias is deprecated because it doesn't make ownership "

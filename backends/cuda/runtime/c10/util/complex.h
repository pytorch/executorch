...
#endif
#if STANDALONE_CLANG_HAS_WARNING("-Wfloat-conversion")
STANDALONE_CLANG_DIAGNOSTIC_IGNORE("-Wfloat-conversion")
#endif

namespace executorch::backends::cuda::c10 {

// executorch::backends::cuda::c10::complex is an implementation of complex numbers that aims
...
// executorch::backends::cuda::c10::complex internally. The end state is that all operators
// will use executorch::backends::cuda::c10::complex internally.  Until then, there may be
...
// namespace `executorch::backends::cuda::c10::complex_literals`. Our custom literals does not
...
//
//
// TODO(@zasdfgbnm): executorch::backends::cuda::c10::complex<executorch::backends::cuda::c10::Half> is not
// currently supported, because:
//  - lots of members and functions of executorch::backends::cuda::c10::Half are not constexpr
...
template <typename T> struct alignas(sizeof(T) * 2) complex {
  ...
  STANDALONE_HOST_DEVICE constexpr complex(const T &re, const T &im = T())
      : real_(re), imag_(im) {}
  template <typename U>
...
#if defined(__CUDACC__) || defined(__HIPCC__)
  template <typename U>
  explicit STANDALONE_HOST_DEVICE complex(const thrust::complex<U> &other)
...
//   complex(other.real(), other.imag()) {}
  ...
  // executorch::backends::cuda::c10::complex<float> and executorch::backends::cuda::c10::complex<double>
  ...
  template <typename U = T>
  STANDALONE_HOST_DEVICE constexpr complex(
...
#undef FORCE_INLINE_APPLE
...
#if defined(__CUDACC__) || defined(__HIPCC__)
  template <typename U>
  STANDALONE_HOST_DEVICE complex<T> &operator=(const thrust::complex<U> &rhs) {
    real_ = rhs.real();
    imag_ = rhs.imag();
    return *this;
  }
...
#if defined(__CUDACC__) || defined(__HIPCC__)
  template <typename U>
  STANDALONE_HOST_DEVICE explicit operator thrust::complex<U>() const {
  ...
  STANDALONE_HOST_DEVICE constexpr T real() const { return real_; }
  constexpr void real(T value) { real_ = value; }
  STANDALONE_HOST_DEVICE constexpr T imag() const { return imag_; }
...
// Define operators between integral scalars and executorch::backends::cuda::c10::complex.
...
constexpr executorch::backends::cuda::c10::complex<fT>
operator+(const executorch::backends::cuda::c10::complex<fT> &a, const iT &b) {
  return a + static_cast<fT>(b);
}

template <typename fT, typename iT, COMPLEX_INTEGER_OP_TEMPLATE_CONDITION>
constexpr executorch::backends::cuda::c10::complex<fT>
operator+(const iT &a, const executorch::backends::cuda::c10::complex<fT> &b) {
...
template <typename T> constexpr T real(const executorch::backends::cuda::c10::complex<T> &z) {
  return z.real();
}

template <typename T> constexpr T imag(const executorch::backends::cuda::c10::complex<T> &z) {
  return z.imag();
}

template <typename T>
STANDALONE_HOST_DEVICE T abs(const executorch::backends::cuda::c10::complex<T> &z) {
#if defined(__CUDACC__) || defined(__HIPCC__)
...
template <typename T>
STANDALONE_HOST_DEVICE T arg(const executorch::backends::cuda::c10::complex<T> &z) {
  return ROCm_Bug(std)::atan2(std::imag(z), std::real(z));
}

#undef ROCm_Bug

template <typename T> constexpr T norm(const executorch::backends::cuda::c10::complex<T> &z) {
...
// TODO(@zasdfgbnm): implement them as executorch::backends::cuda::c10::conj
template <typename T>
constexpr executorch::backends::cuda::c10::complex<T>
conj(const executorch::backends::cuda::c10::complex<T> &z) {
  return executorch::backends::cuda::c10::complex<T>(z.real(), -z.imag());
}

// Thrust does not have complex --> complex version of thrust::proj,
// so this function is not implemented at standalone right now.
// TODO(@zasdfgbnm): implement it by ourselves

// There is no standalone version of std::polar, because std::polar always
// returns std::complex. Use executorch::backends::cuda::c10::polar instead;
...
template <> struct alignas(4) complex<Half> {
  ...
  // be constexpr
  STANDALONE_HOST_DEVICE explicit inline complex(const Half &real,
                                                 const Half &imag)
      : real_(real), imag_(imag) {}
  STANDALONE_HOST_DEVICE inline complex(
      const executorch::backends::cuda::c10::complex<float> &value)
      : real_(value.real()), imag_(value.imag()) {}

  // Conversion operator
  inline STANDALONE_HOST_DEVICE
  operator executorch::backends::cuda::c10::complex<float>() const {
    return {real_, imag_};
  }

  constexpr STANDALONE_HOST_DEVICE Half real() const { return real_; }
  constexpr STANDALONE_HOST_DEVICE Half imag() const { return imag_; }

  STANDALONE_HOST_DEVICE complex<Half> &operator+=(const complex<Half> &other) {
...
#define STANDALONE_INTERNAL_INCLUDE_COMPLEX_REMAINING_H
// math functions are included in a separate file
#include <executorch/backends/cuda/runtime/c10/util/complex_math.h> // IWYU pragma: keep
// utilities for complex types
#include <executorch/backends/cuda/runtime/c10/util/complex_utils.h> // IWYU pragma: keep
#undef STANDALONE_INTERNAL_INCLUDE_COMPLEX_REMAINING_H

...
using SharedCudaStream = std::shared_ptr<cudaStream_t>;

// Creates a new shared CUDA stream.
// Returns nullptr on failure.
inline SharedCudaStream create_shared_cuda_stream() {
  cudaStream_t stream;
  cudaError_t err = cudaStreamCreate(&stream);
  if (err != cudaSuccess) {
    return nullptr;
  }
  return SharedCudaStream(new cudaStream_t(stream), CudaStreamDeleter());
}
...
struct CudaDelegateHandle : public aoti::AOTIDelegateHandle {
  // Shared CUDA stream for this handle.
  // When multiple handles share the same stream (e.g., for skip-copy
  // optimization), they will all hold a reference to the same shared_ptr.
  // The stream is automatically destroyed when the last handle is destroyed.
  SharedCudaStream cuda_stream;

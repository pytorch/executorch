...
TEST_F(AOTITorchSDPATest, BasicFunctionalityFloat32) {
  ...
  // Create V with different values at each position so attention weight changes matter
...
TEST_F(AOTITorchSDPATest, DimensionMismatch) {
  ...
  AOTITorchError error = call_flash_attention(
      query, key, value, 0.0, 0, nullptr, &output);
...
TEST_F(AOTITorchSDPATest, ScaleEffectTest) {
  ...
  AOTITorchError error1 = call_flash_attention(
      query, key, value, 0.0, 0, nullptr, &output1);
  ...
  AOTITorchError error2 = call_flash_attention(
      query, key, value, 0.0, 0, &small_scale, &output2);
...
TEST_F(AOTITorchSDPATest, CausalMaskingCorrectness) {
  ...
  AOTITorchError error = call_flash_attention(
      query, key, value, 0.0, 1, nullptr, &output_causal);
  ASSERT_EQ(error, Error::Ok);
...
TEST_F(AOTITorchSDPATest, FlashAttentionLongSequence) {
  ...
  printf("Testing Flash Attention with long sequence: [%ldx%ldx%ldx%ld]\n",
         batch, num_heads, seq_len, head_dim);
  ...
  EXPECT_TRUE(can_use_fa) << "Should be able to use Flash Attention for long sequences";
...
TEST_F(AOTITorchSDPATest, FlashAttentionCausalMasking) {
  ...
  EXPECT_TRUE(can_use_fa) << "Should be able to use Flash Attention with causal masking";
  printf("  can_use_flash_attention (causal): %s\n", can_use_fa ? "true" : "false");
...
TEST_F(AOTITorchSDPATest, FlashAttentionBFloat16) {
  ...
  bool can_use_fa = can_use_flash_attention(query, key, value, nullptr, false);
  EXPECT_TRUE(can_use_fa) << "Should be able to use Flash Attention with BFloat16";
  printf("  can_use_flash_attention (BFloat16): %s\n", can_use_fa ? "true" : "false");
...
TEST_F(AOTITorchSDPATest, FlashAttentionCorrectness) {
  ...
  // Verify that can_use_flash_attention returns true for this configuration
  bool can_use_fa = can_use_flash_attention(query, key, value, nullptr, false);
  EXPECT_TRUE(can_use_fa) << "Should be able to use Flash Attention for this configuration";
  printf("  can_use_flash_attention: %s\n", can_use_fa ? "true" : "false");
  ...
  // Since V is all 1.0 and softmax weights sum to 1, output should be close to 1.0
  ...
  EXPECT_TRUE(all_correct) << "Flash Attention output doesn't match expected values";
...
TEST_F(AOTITorchSDPATest, FlashAttentionDifferentSeqLengths) {
  ...
  Tensor* query = create_float_tensor({batch, num_heads, seq_len_q, head_dim}, 0.5f);
  Tensor* key = create_float_tensor({batch, num_heads, seq_len_k, head_dim}, 0.5f);
  Tensor* value = create_float_tensor({batch, num_heads, seq_len_k, head_dim}, 1.0f);
  ...
  bool can_use_fa = can_use_flash_attention(query, key, value, nullptr, false);
  EXPECT_TRUE(can_use_fa) << "Should be able to use Flash Attention with different seq lengths";
  printf("  can_use_flash_attention (different seq lengths): %s\n", can_use_fa ? "true" : "false");
  ...
  EXPECT_EQ(error, Error::Ok);
  ASSERT_NE(output, nullptr);
  EXPECT_TRUE(check_output_shape(output, {batch, num_heads, seq_len_q, head_dim}));
...
// Test that explicit attention mask forces Math fallback instead of Flash Attention
TEST_F(AOTITorchSDPATest, ExplicitMaskFallsBackToMath) {
  ...
  Tensor* attn_mask = create_float_tensor({batch, num_heads, seq_len, seq_len}, 0.0f);
  ...
  // Verify that can_use_flash_attention returns false when explicit mask is provided
  bool can_use_fa = can_use_flash_attention(query, key, value, attn_mask, false);
  EXPECT_FALSE(can_use_fa) << "Should NOT be able to use Flash Attention with explicit mask";
  printf("  can_use_flash_attention (with explicit mask): %s\n", can_use_fa ? "true" : "false");
...
TEST_F(AOTITorchSDPATest, CanUseFlashAttention_ValidConfig) {
  ...
  printf("Testing can_use_flash_attention with valid config: "
  ...
  EXPECT_TRUE(can_use) << "Should allow flash attention for valid configuration";
  ...
  EXPECT_TRUE(can_use_causal) << "Should allow flash attention with causal masking";
...
TEST_F(AOTITorchSDPATest, CanUseFlashAttention_InvalidConfigs) {
  ...
  // Test 1: Explicit attention mask provided
  {
    ...
    bool can_use = can_use_flash_attention(
        query, key, value, attn_mask, false);

    EXPECT_FALSE(can_use) << "Should reject flash attention with explicit attention mask";
  ...
  // Test 2: head_dim > 128
  {
    ...
    Tensor* query = create_float_tensor({batch, num_heads, 64, large_head_dim}, 0.5f);
    Tensor* key = create_float_tensor({batch, num_heads, 64, large_head_dim}, 0.5f);
    Tensor* value = create_float_tensor({batch, num_heads, 64, large_head_dim}, 1.0f);
    ...
    bool can_use = can_use_flash_attention(
        query, key, value, nullptr, false);

    EXPECT_FALSE(can_use) << "Should reject flash attention with head_dim > 128";
  ...
  // Test 3: head_dim_v > 64
  {
    ...
    Tensor* query = create_float_tensor({batch, num_heads, 64, 64}, 0.5f);
    Tensor* key = create_float_tensor({batch, num_heads, 64, 64}, 0.5f);
    Tensor* value = create_float_tensor({batch, num_heads, 64, large_head_dim_v}, 1.0f);
    ...
    EXPECT_FALSE(can_use) << "Should reject flash attention with head_dim_v > 64";
...
TEST_F(AOTITorchSDPATest, EfficientAttention_BasicNoBias) {
  ...
  printf("Testing Efficient Attention without bias: [%ldx%ldx%ldx%ld]\n",
         batch, num_heads, seq_len, head_dim);

  Tensor* output = nullptr;
  AOTITorchError error = call_efficient_attention(
      query, key, value, nullptr, 0, nullptr, &output);
...
TEST_F(AOTITorchSDPATest, EfficientAttention_WithBias) {
  ...
  Tensor* key = create_float_tensor({batch, num_heads, seq_len, head_dim}, 0.3f);
  Tensor* value = create_float_tensor({batch, num_heads, seq_len, head_dim}, 1.0f);
  Tensor* attn_bias = create_float_tensor({batch, num_heads, seq_len, seq_len}, 0.1f);
  ...
  printf("Testing Efficient Attention with full bias: [%ldx%ldx%ldx%ld]\n",
...
TEST_F(AOTITorchSDPATest, EfficientAttention_BroadcastBatchDim) {
  ...
  Tensor* attn_bias = create_float_tensor({1, num_heads, seq_len, seq_len}, 0.2f);
...
TEST_F(AOTITorchSDPATest, EfficientAttention_CausalWithBias) {
  ...
  Tensor* key = create_float_tensor({batch, num_heads, seq_len, head_dim}, 0.5f);
  Tensor* value = create_float_tensor({batch, num_heads, seq_len, head_dim}, 1.0f);
  Tensor* attn_bias = create_float_tensor({batch, num_heads, seq_len, seq_len}, 0.05f);
...
TEST_F(AOTITorchSDPATest, EfficientAttention_DifferentSeqLengths) {
  ...
  Tensor* query = create_float_tensor({batch, num_heads, seq_len_q, head_dim}, 0.5f);
  Tensor* key = create_float_tensor({batch, num_heads, seq_len_kv, head_dim}, 0.3f);
  Tensor* value = create_float_tensor({batch, num_heads, seq_len_kv, head_dim}, 1.0f);
  Tensor* attn_bias = create_float_tensor({batch, num_heads, seq_len_q, seq_len_kv}, 0.1f);
  ...
  printf("Testing Efficient Attention with different Q (%ld) and K/V (%ld) lengths\n",
...
TEST_F(AOTITorchSDPATest, EfficientAttention_LargeHeadDimV) {
  ...
  Tensor* query = create_float_tensor({batch, num_heads, seq_len, head_dim}, 0.5f);
  Tensor* key = create_float_tensor({batch, num_heads, seq_len, head_dim}, 0.3f);
  Tensor* value = create_float_tensor({batch, num_heads, seq_len, head_dim_v}, 1.0f);
  ...
  EXPECT_EQ(error, Error::Ok);
  ASSERT_NE(output, nullptr);
  EXPECT_TRUE(check_output_shape(output, {batch, num_heads, seq_len, head_dim_v}));
...
TEST_F(AOTITorchSDPATest, EfficientAttention_BiasAffectsOutput) {
  ...
  Tensor* attn_bias = create_float_tensor({batch, num_heads, seq_len, seq_len}, 0.0f);
  ASSERT_NE(attn_bias, nullptr);
  ...
  for (int64_t i = 0; i < seq_len; ++i) {
    for (int64_t j = 0; j < seq_len; ++j) {
      // Create a diagonal pattern with large values to strongly affect attention
      if (i == j) {
  ...
  EXPECT_TRUE(check_output_shape(output_no_bias, {batch, num_heads, seq_len, head_dim}));
  EXPECT_TRUE(check_output_shape(output_with_bias, {batch, num_heads, seq_len, head_dim}));

  // Copy outputs to host and verify they differ
  std::vector<float> output_no_bias_data = copy_tensor_to_host(output_no_bias);
  std::vector<float> output_with_bias_data = copy_tensor_to_host(output_with_bias);
  ...
  printf("Max difference between outputs: %f\n", max_diff);
  EXPECT_TRUE(outputs_differ) << "Attention bias should affect the output (max_diff=" << max_diff << ")";
...
TEST_F(AOTITorchSDPATest, EfficientAttention_NegativeBiasMasking) {
  ...
  // With Q=K, without masking, output would be average of all positions: (1+2+3+4)/4 = 2.5
  // With last position masked, output should be: (1+2+3)/3 = 2.0
  float avg_output = 0.0f;
  for (size_t i = 0; i < output_data.size(); ++i) {
...
TEST_F(AOTITorchSDPATest, EfficientAttention_ZeroQuery) {
  ...
  Tensor* query = create_float_tensor({batch, num_heads, seq_len, head_dim}, 0.0f);
  Tensor* key = create_float_tensor({batch, num_heads, seq_len, head_dim}, 1.0f);
  Tensor* value = create_float_tensor({batch, num_heads, seq_len, head_dim}, 2.0f);
  ...
  // With zero query, attention scores are uniform, output should be average of V
  std::vector<float> output_data = copy_tensor_to_host(output);
  ...
  EXPECT_TRUE(all_close_to_2) << "With uniform attention, output should equal V";

...
def _int4_matvec_v1(
    ...
    for ks in range(tl.cdiv(K, BLOCK_K)):
        ...
        w_ptrs = W + offs_n[:, None] * stride_wn + ((ks * BLOCK_K + offs_k[None, :]) // 2) * stride_wk
...
def _int4_matvec_splitk(
    ...
    for step in range(num_steps):
        ...
        scale = tl.load(W_scale + offs_n * stride_sn + gi * stride_sk, mask=nm, other=0.0).to(tl.float32)
...
def main():
    ...
    for N, K, label in shapes:
        ...
        print(f"\n{'='*70}")
        print(f"[{N}x{K}] {label} — tinygemm={t_tiny:.1f}us, int4_matmul={t_i4mm:.1f}us")
        print(f"{'='*70}")
        ...
        for BN, BK, warps, stages in [
            (16, 128, 4, 3), (16, 256, 4, 3), (32, 128, 4, 3),
            (32, 256, 4, 3), (8, 128, 2, 3), (8, 256, 2, 3),
            (16, 128, 2, 3), (4, 128, 2, 3), (4, 256, 2, 3),
        ]:
            grid = ((N + BN - 1) // BN,)
            def run(_BN=BN, _BK=BK, _w=warps, _s=stages, _g=grid):
                _int4_matvec_v1[_g](
                    x_flat, packed, out, w_scale, N, K,
                    packed.stride(0), packed.stride(1),
                    w_scale.stride(0), w_scale.stride(1),
                    gs, BLOCK_N=_BN, BLOCK_K=_BK,
            ...
            try:
                ...
                print(f"  BN={BN:2d} BK={BK:3d} w={warps}: {t:6.1f}us ({t/t_tiny:.2f}x) grid={grid[0]:5d}{tag}")
        ...
        for BN, BK, splits, warps, stages in [
            (16, 128, 4, 4, 3), (16, 128, 8, 4, 3),
            (8, 128, 4, 2, 3), (8, 128, 8, 2, 3), (8, 128, 16, 2, 3),
            (4, 128, 4, 2, 3), (4, 128, 8, 2, 3), (4, 128, 16, 2, 3),
            (16, 64, 8, 4, 3), (8, 64, 16, 2, 3),
        ]:
            ...
            def run(_BN=BN, _BK=BK, _sp=splits, _w=warps, _s=stages, _g=grid):
                ...
                _int4_matvec_splitk[_g](
                    x_flat, packed, out_sk, w_scale, N, K,
                    packed.stride(0), packed.stride(1),
                    w_scale.stride(0), w_scale.stride(1),
                    gs, K_SPLITS=_sp,
            ...
            try:
                ...
                print(f"  BN={BN:2d} BK={BK:3d} sp={splits:2d} w={warps}: {t:6.1f}us ({t/t_tiny:.2f}x) grid={grid[0]:5d}{tag}")
            except Exception as e:
                print(f"  BN={BN:2d} BK={BK:3d} sp={splits:2d} w={warps}: FAIL {str(e)[:50]}")

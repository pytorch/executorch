...
def _int4_vecmat_kernel(
    ...
    stride_bn, stride_bk, stride_bsn, stride_bsk,
    group_size: tl.constexpr,
...
def _int4_splitk_kernel(
    ...
    b_ptrs = B + offs_n[None, :] * stride_bn + ((k_start + offs_k[:, None]) // 2) * stride_bk
...
def main():
    ...
    from torchao.quantization.quant_primitives import (
        MappingType, choose_qparams_affine, quantize_affine,
    )
    ...
    for N, K, label in shapes:
        ...
        sc, zp = choose_qparams_affine(
            w.float(), MappingType.SYMMETRIC, (1, gs),
        ...
        quantize_model_(
            wr, qlinear_config="4w", qlinear_group_size=gs,
        ...
        for BN, BK, warps, stages in [
            (16, 128, 4, 5), (32, 128, 4, 5), (32, 256, 4, 3),
            (16, 128, 2, 5), (32, 128, 2, 5),
        ]:
            grid = ((N + BN - 1) // BN,)
            def run(_BN=BN, _BK=BK, _w=warps, _s=stages, _g=grid):
                _int4_dot_kernel[_g](
                    x, packed, out, w_scale, 1, N, K,
                    x.stride(0), x.stride(1), packed.stride(0), packed.stride(1),
                    out.stride(0), out.stride(1), w_scale.stride(0), w_scale.stride(1),
                    gs, BLOCK_SIZE_M=16, BLOCK_SIZE_N=_BN, BLOCK_SIZE_K=_BK,
                    num_warps=_w, num_stages=_s,
                )
            try:
                run()
                t = do_bench(run, warmup=50, rep=200, return_mode="median") * 1000
                print(f"  BN={BN:3d} BK={BK:3d} w={warps} s={stages}: {t:6.1f}us ({t/t_tiny:.2f}x) grid={grid[0]}")
            except Exception as e:
                print(f"  BN={BN:3d} BK={BK:3d} w={warps} s={stages}: FAIL {str(e)[:50]}")

        # Strategy 2: vec-mat with tl.sum (no padding waste)
        print("\n--- Strategy 2: vec-mat tl.sum (no M padding) ---")
        for BN, BK, warps, stages in [
            (16, 128, 4, 5), (32, 128, 4, 5), (64, 128, 4, 5),
            (16, 256, 4, 3), (32, 256, 4, 3),
            (16, 128, 2, 5), (32, 128, 2, 5),
            (16, 64, 2, 5), (32, 64, 2, 5),
        ]:
            grid = ((N + BN - 1) // BN,)
            ...
            def run(_BN=BN, _BK=BK, _w=warps, _s=stages, _g=grid):
                _int4_vecmat_kernel[_g](
                    x, packed, out1d, w_scale, N, K,
                    packed.stride(0), packed.stride(1),
                    w_scale.stride(0), w_scale.stride(1),
                    gs, BLOCK_SIZE_N=_BN, BLOCK_SIZE_K=_BK,
                    num_warps=_w, num_stages=_s,
                )
            try:
                run()
        ...
        for BN, BK, splits, warps, stages in [
            (32, 128, 4, 4, 3), (32, 128, 8, 4, 3), (32, 128, 16, 4, 3),
            (16, 128, 4, 4, 3), (16, 128, 8, 4, 3), (16, 128, 16, 4, 3),
            (64, 128, 4, 4, 3), (64, 128, 8, 4, 3),
        ]:
            ...
            def run(_BN=BN, _BK=BK, _sp=splits, _w=warps, _s=stages, _g=grid):
                ...
                _int4_splitk_kernel[_g](
                    x, packed, out_sk, w_scale, 1, N, K,
                    x.stride(0), x.stride(1), packed.stride(0), packed.stride(1),
                    out_sk.stride(0), out_sk.stride(1),
                    w_scale.stride(0), w_scale.stride(1),
                    gs, K_SPLITS=_sp,
                    BLOCK_SIZE_M=16, BLOCK_SIZE_N=_BN, BLOCK_SIZE_K=_BK,
                    num_warps=_w, num_stages=_s,
                )
            try:
                run()
                t = do_bench(run, warmup=50, rep=200, return_mode="median") * 1000
                print(f"  BN={BN:3d} BK={BK:3d} sp={splits:2d} w={warps} s={stages}: {t:6.1f}us ({t/t_tiny:.2f}x) grid={grid[0]}")
            except Exception as e:
                print(f"  BN={BN:3d} BK={BK:3d} sp={splits:2d} w={warps} s={stages}: FAIL {str(e)[:50]}")

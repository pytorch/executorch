...
class FuseInt4WeightOnlyQuantMatmulPass(ExportPass):
    ...
    def _is_int4mm(self, node: Node) -> bool:
        ...
        return (
            ...
            and node.target == torch.ops.aten._weight_int4pack_mm.default
        )
    ...
    def _fuse_group(self, graph_module: GraphModule, group: List[Node]) -> bool:
        ...
        try:
            ...
            # Insert after the last weight/scale node to maintain topological order.
            insert_after = max(weights + scales, key=lambda n: list(graph_module.graph.nodes).index(n))
            with graph_module.graph.inserting_after(insert_after):
                fused_weight = graph_module.graph.call_function(
                    torch.ops.aten.cat.default,
                    kwargs={"tensors": weights, "dim": 0},
                )
            ...
            # Create fused matmul operation
            shared_input = group[0].args[0]
            with graph_module.graph.inserting_after(shared_input):
                fused_mm = graph_module.graph.call_function(
                    torch.ops.aten._weight_int4pack_mm.default,
                    args=(shared_input, fused_weight, block_size, fused_scale),
                )

...
def _validate_tq4_dtypes(
    ...
    if k_norms.dtype not in (torch.float32, torch.bfloat16):
        raise RuntimeError(f"k_norms must be float32 or bfloat16, got {k_norms.dtype}")
    if v_norms.dtype not in (torch.float32, torch.bfloat16):
        raise RuntimeError(f"v_norms must be float32 or bfloat16, got {v_norms.dtype}")
...
def _validate_tq4_mask(attn_mask, query, k_packed):
    if attn_mask is None:
        return

    B, _, N_Q, _ = query.shape
    _, _, N_KV, _ = k_packed.shape
...
def _validate_tq4_kv_len(kv_len):
...
def _validate_tq4_inputs(
    ...
    _validate_tq4_ranks(*args)
    _validate_tq4_devices(*args)
    _validate_tq4_dtypes(*args)
    _validate_tq4_layouts(
        query,
        k_packed,
        k_norms,
        v_packed,
    ...
    _validate_tq4_mask(attn_mask, query, k_packed)
    _validate_tq4_kv_len(kv_len)

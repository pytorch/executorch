...
The legacy external-weight format carries two blob keys, newline separated.
That path is the one the check was added to, so this file exercises it by
rewriting the payload of a real export into the older shape in place.
"""
...
from executorch.backends.cuda.cuda_weight_collector import decode_cuda_aoti_metadata
...
class TestMissingWeightsBlob(unittest.TestCase):
    ...
    def _rewrite_payload_as_legacy(self, path: str) -> None:
        ...
        self.assertEqual(len(metadata.variants), 1, "expected one compiled variant")
        so_key = metadata.variants[0].so_blob_key
        ...
        # Keep the absent key short enough for a payload with no weight entries.
        weights_key = "missing_weights"
        self.assertNotIn(weights_key.encode(), raw)
        ...
        with open(path, "wb") as f:
            f.write(bytes(blob))
        with open(path, "rb") as f:
            rewritten = deserialize_pte_binary(f.read()).program
        rewritten_payloads = [
            bytes(rewritten.backend_delegate_data[delegate.processed.index].data)
            for plan in rewritten.execution_plan
            for delegate in plan.delegates
            if delegate.id == "CudaBackend"
        ]
        self.assertEqual(rewritten_payloads, [legacy])
    ...
    def test_load_succeeds_when_the_model_has_no_constants(self) -> None:
        ...
        Rewrite this export too, so it exercises the legacy constant-count check.
        """
        with tempfile.TemporaryDirectory() as outdir:

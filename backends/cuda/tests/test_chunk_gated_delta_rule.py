...
RUNNER_PATH = os.path.join(
    ...
    "../../../cmake-out/backends/cuda/tests/chunk_gated_delta_runner/chunk_gated_delta_runner",
)
...
def _run_cpp_runner(runner_path, pte_path, ptd_path, input_dir, output_dir):
    """Run the C++ runner and return (stdout, stderr)."""
    cmd = [
        runner_path,
...
class TestChunkGatedDeltaRule(unittest.TestCase):
    ...
    def test_e2e_cpp_runner(self):
        """Export, run C++ runner with FLA test inputs, compare with eager."""
        model = ChunkGatedDeltaModel().eval()
            ...
            for seed, norm, mask_p, nonzero_h0, desc in FLA_TEST_CONFIGS:
                with self.subTest(desc=desc):
                    ...
                    for name, tensor in zip(
                        ...
                        _save_tensor(tensor, os.path.join(input_dir, f"{name}.bin"))

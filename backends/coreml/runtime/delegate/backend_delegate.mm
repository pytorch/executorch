...
public:
    ...
    Handle *init(Buffer processed,const std::unordered_map<std::string, Buffer>& specs) const noexcept override {
        ...
        ModelHandle *modelHandle = [model_manager_ loadModelFromAOTData:data
                                                          configuration:configuration
                                                                  error:&localError];
        if (modelHandle && config_.shouldPrewarmModel) {
            NSError *localError = nil;
    ...
    bool execute(Handle* handle, const std::vector<MultiArray>& args, std::error_code& errorCode) const noexcept override {
        NSError *error = nil;
        ...
        if (![model_manager_ executeModelWithHandle:handle args:model_args error:&error]) {
            errorCode = static_cast<ErrorCode>(error.code);
            return false;
        }

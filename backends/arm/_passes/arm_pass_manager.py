...
class ArmPassManager(PassManager):
    def __init__(self, compile_spec: ArmCompileSpec) -> None:
        ...
        self._pass_insertions: dict[type, tuple[list, list]] = {}
    ...
    def insert_passes_before(
        ...
        if target_pass_type not in self._pass_insertions:
            self._pass_insertions[target_pass_type] = ([], [])
        self._pass_insertions[target_pass_type][0].extend(passes)
    ...
    def insert_passes_after(
        ...
        if target_pass_type not in self._pass_insertions:
            self._pass_insertions[target_pass_type] = ([], [])
        self._pass_insertions[target_pass_type][1].extend(passes)
    ...
    def _apply_pass_insertions(self) -> None:
        ...
        for pass_obj in self.passes:
            ...
            if pass_type in self._pass_insertions:
                before_passes, _ = self._pass_insertions[pass_type]
                for before_pass in before_passes:
            ...
            if pass_type in self._pass_insertions:
                _, after_passes = self._pass_insertions[pass_type]
                for after_pass in after_passes:
                    # Check if we should skip this inserted pass

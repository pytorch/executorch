...
class FoldDyTAffineIntoConvPass(ArmPass):
    ...
    @staticmethod
    def _is_table_node(table: Node) -> bool:
        """Return True when ``table`` is a TOSA TABLE with a constant operand."""
        return (
            table.target == exir_ops.backend.tosa.TABLE.default
            and len(table.args) >= 2
            and isinstance(table.args[1], Node)
        )
    ...
    def _affine_codes(
        ...
        """
        table_values = self._constant(table.args[1])
    ...
    def _match(self, conv: Node) -> _DyTAffineMatch | None:
        ...
        if not self._is_table_node(table):
            return None

        codes = self._affine_codes(table, gamma_match, add_match)
        if codes is None:
            return None

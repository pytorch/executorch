...
def _assert_qparams_match(source: torch.fx.Node, cat_node: torch.fx.Node) -> None:
    """Verify the data-providing node's output_qparams match cat_node's.

    FuseConcatPass runs after FoldAndAnnotateQParamsPass, so cat_node may carry
    output_qparams. TOSA cat preserves scale across inputs and output, so any
    node we route consumers to (directly, or via a new slice that reads from
    it) must produce data with matching qparams.

    Skipped when either side has no output_qparams entries: FP graphs (key
    missing), Q ops (qparams live in args), and tosa_rescale ops (qparams in
    args; folder leaves an empty dict). The assertion is best-effort, intended
    to catch upstream bugs that produce two foldable ops with diverging
    output_qparams.

    """
    cat_qp = cat_node.meta.get("output_qparams")
    src_qp = source.meta.get("output_qparams")
    if not cat_qp or not src_qp:
        return
    assert cat_qp == src_qp, (
        f"FuseConcatPass: output_qparams mismatch — {source.name} {src_qp} != "
        f"cat {cat_node.name} {cat_qp}"
    )
...
def _find_slice_replacement(
    ...
    for o_start, o_end, inp in offsets:
        if s_start == o_start and s_end == o_end:
            _assert_qparams_match(inp, cat_node)
            return inp
        if s_start >= o_start and s_end <= o_end:
            _assert_qparams_match(inp, cat_node)
...
class FuseConcatPass(ArmPass):
    ...
    def _eliminate_single_input_cat(cat_node: torch.fx.Node) -> bool:
        ...
        _assert_qparams_match(sole_input, cat_node)
    ...
    def _eliminate_slice_then_cat(
        ...
        if first_start == 0 and last_end == source_dim_size:
            ...
            if list(cat_shape) != list(source_shape):
                return False
            _assert_qparams_match(source_node, cat_node)

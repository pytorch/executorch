...
def test_conv2d_tosa_INT():
    sample_inputs = [
        (
            (
                torch.randint(-128, 127, (1, 8, 20, 20), dtype=torch.int8),
                torch.randint(-127, 127, (8, 2, 5, 5), dtype=torch.int8),
                torch.randint(-(2**31), 2**31, (8,), dtype=torch.int32),
                [1, 1],
                [2, 2, 2, 2],
                [1, 1],
                False,
        ...
        (
            (
                torch.randint(-128, 127, (1, 8, 20, 20), dtype=torch.int8),
                torch.randint(-127, 127, (4, 2, 5, 5), dtype=torch.int8),
                None,
                [2, 2],
                [2, 2, 2, 2],
                [1, 1],
    ...
    with TosaLoweringContext(
        ...
        for sample_input, expected_output_shape, expected_output_type in sample_inputs:
            output = exir_ops.backend.tosa.CONV2D.default(
                *tuple(
                    [
...
def test_conv2d_invalid_tosa_INT():
    tosa_spec = TosaSpecification.create_from_string("TOSA-1.0+INT")
    sample_inputs = [
        (
            (
                torch.randn((1, 8, 20, 20), dtype=torch.float32),
                torch.randn((8, 2, 5, 5), dtype=torch.float32),
...
def test_conv2d_tosa_FP():
    sample_inputs = [
        ...
        (
            (
                torch.randn((1, 8, 20, 20), dtype=torch.float32),
                torch.randn((4, 2, 5, 5), dtype=torch.float32),
                None,
                [2, 2],
                [2, 2, 2, 2],
                [1, 1],
...
def test_conv2d_invalid_tosa_FP():
    ...
    sample_inputs = [
        ...
        (
            (
                torch.randn((1, 8, 20, 20), dtype=torch.float32),
                torch.randn((8, 2, 5, 5), dtype=torch.float16),
                torch.randn((8,), dtype=torch.float32),
                [1, 1],
                [2, 2, 2, 2],
                [1, 1],

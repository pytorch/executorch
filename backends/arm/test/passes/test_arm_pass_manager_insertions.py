...
class TestInsertPassesBefore(unittest.TestCase):
    ...
    def test_insert_single_pass_before_target(self) -> None:
        ...
        self.assertEqual(pass_types, [InsertedPassX, TargetPassA, TargetPassB])
    ...
    def test_insert_before_same_target_accumulates(self) -> None:
        """Test that multiple insert_passes_before calls for the same target
        accumulate.
        """
        pm = create_test_pass_manager()
        ...
        pm.insert_passes_before(TargetPassA, [InsertedPassX()])
        pm.insert_passes_before(TargetPassA, [InsertedPassY()])
        pm._apply_pass_insertions()

        pass_types = [type(p) for p in pm.passes]
        self.assertEqual(
            pass_types,
            [InsertedPassX, InsertedPassY, TargetPassA, TargetPassB],
        )

    def test_insert_before_nonexistent_target_raises(self) -> None:
        """Test that inserting before a nonexistent target raises ValueError."""
        pm = create_test_pass_manager()

        pm.add_pass(TargetPassA())

        pm.insert_passes_before(TargetPassB, [InsertedPassX()])
        with self.assertRaisesRegex(ValueError, "TargetPassB not found"):
            pm._apply_pass_insertions()
...
class TestInsertPassesAfter(unittest.TestCase):
    ...
    def test_insert_single_pass_after_target(self) -> None:
        ...
        pass_types = [type(p) for p in pm.passes]
        self.assertEqual(pass_types, [TargetPassA, InsertedPassX, TargetPassB])
    ...
    def test_insert_after_last_pass(self) -> None:
        ...
        pass_types = [type(p) for p in pm.passes]
        self.assertEqual(pass_types, [TargetPassA, TargetPassB, InsertedPassX])

    def test_insert_after_same_target_accumulates(self) -> None:
        """Test that multiple insert_passes_after calls for the same target
        accumulate.
        """
        pm = create_test_pass_manager()
        ...
        self.assertEqual(
            pass_types,
            [TargetPassA, InsertedPassX, InsertedPassY, TargetPassB],
        )

    def test_insert_after_nonexistent_target_raises(self) -> None:
        """Test that inserting after a nonexistent target raises ValueError."""
        pm = create_test_pass_manager()
...
class TestApplyPassInsertions(unittest.TestCase):
    ...
    def test_nonexistent_target_error_includes_available_passes(self) -> None:
        """Test that the ValueError lists available passes for debugging."""
        pm = create_test_pass_manager()

        pm.add_pass(TargetPassA())
        pm.add_pass(TargetPassB())

        pm.insert_passes_before(InsertedPassX, [InsertedPassY()])

        with self.assertRaises(ValueError) as ctx:
            pm._apply_pass_insertions()

        error_msg = str(ctx.exception)
        self.assertIn("InsertedPassX not found", error_msg)
        self.assertIn("TargetPassA", error_msg)
        self.assertIn("TargetPassB", error_msg)

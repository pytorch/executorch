...
def _build_post_rescale_fixture(
    *,
    ...
    gamma_output_zp: int | None = None,
    add_output_zp: int | None = None,
    ...
    (
        ...
        gamma_input_zp,
        gamma_output_scale,
        fixture_gamma_output_zp,
        beta_input_zp,
        add_activation_scale,
        add_beta_scale,
        add_output_scale,
    ) = _fixture_rescale_params(actual_dyt_identity_qparams)
    resolved_gamma_output_zp = (
        fixture_gamma_output_zp if gamma_output_zp is None else gamma_output_zp
    )
    resolved_add_output_zp = (
        _zp_of(table_qargs) if add_output_zp is None else add_output_zp
    )
    ...
    with graph.inserting_before(output):
        ...
        mul_output_rescale = graph.call_function(
            exir_ops.backend.tosa.RESCALE.default,
            (mul, torch.int8, [gamma_output_scale], 0, resolved_gamma_output_zp),
        )
        ...
        add_output_rescale = graph.call_function(
            exir_ops.backend.tosa.RESCALE.default,
            (add, torch.int8, [add_output_scale], 0, resolved_add_output_zp),
        )
    ...
    conv.meta["input_qparams"] = {
        0: _qargs(
            ...
            zp=(resolved_add_output_zp if conv_input_zp is None else conv_input_zp),
        ),
...
def test_identity_affine_preserves_convolution_input_zero_point() -> None:
    table = torch.zeros(256, dtype=torch.int8)
    exported_program, _, _ = _build_post_rescale_fixture(
        table=table,
        gamma_code=1,
        beta_code=-7,
        actual_dyt_identity_qparams=False,
    ...
    input_qparams = cast(dict[int, QuantArgs], conv.meta["input_qparams"])
    table_qparams = cast(dict[int, QuantArgs], table_node.meta["output_qparams"])

    assert result.modified
    assert table_qparams[0].get_zp_per_tensor() == 0
    assert input_qparams[0].get_zp_per_tensor() == 7
    assert not any("aten.mul" in target for target in _call_targets(exported_program))
    assert not any("aten.add" in target for target in _call_targets(exported_program))


def test_identity_gamma_preserves_downstream_rescale_input_zero_point() -> None:
    table = torch.ones(256, dtype=torch.int8)
    exported_program, _, _ = _build_post_rescale_fixture(
        table=table,
        gamma_code=-6,
        beta_code=0,
    ...
    activation_rescale = cast(Node, add.args[0])
    table_node = cast(Node, activation_rescale.args[0])
    table_qparams = cast(dict[int, QuantArgs], table_node.meta["output_qparams"])

    assert result.modified
    assert table_node.target == exir_ops.backend.tosa.TABLE.default
    assert table_qparams[0].get_zp_per_tensor() == 0
    assert activation_rescale.args[3] == 7
    assert not any("aten.mul" in target for target in _call_targets(exported_program))

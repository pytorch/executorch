...
import torch
from executorch.backends.arm._passes.replace_inf_values_pass import ReplaceInfValuesPass
...
def test_replace_inf_values_pass_clamps_inf_constants():
    """
    Trace a module with infinities, run ReplaceInfValuesPass, and expect the buffer and scalar
    literals to be clamped to ±255 with no infinities left.
    """
    gm = fx.symbolic_trace(ModuleWithInf())

    result = ReplaceInfValuesPass().call(gm)
...
def test_replace_inf_values_respects_disallowed_nodes():
    ...
    replace_inf = ReplaceInfValuesPass()

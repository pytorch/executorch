        ...
        for spec in self.graph_module.lowered_module_0.compile_specs:
            if spec.key == "output_format":
                if spec.value == b"tosa":
                    ...
                    to_print = _format_dict(to_print)
                    to_print = f"\n TOSA deserialized: \n{to_print}"
...
class ArmTester(Tester):
    ...
    def get_graph(self, stage: str = None) -> Graph:
        if stage is None:
            stage = self.cur
        artifact = self.get_artifact(stage)
        if self.cur == self.stage_name(ToEdge) or self.cur == self.stage_name(
            Partition
        ):
            graph = artifact.exported_program().graph
        elif self.cur == self.stage_name(Export) or self.cur == self.stage_name(
            Quantize
        ):

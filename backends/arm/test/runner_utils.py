...
# Remove these filters when the VKML capability advertisement is fixed.
# See MLETORCH-2584.
# The same issue with 09748 error, MLETORCH-2582.
# The last 2 errors are the reported bug:
# Incomplete descriptor layout generated by
# Model Converter for duplicated custom-shader inputs.
# MLETORCH-2585
KNOWN_VKML_VALIDATION_VUIDS = (
...
def run_vkml_emulation_layer(
    ...
    runtime_env = _get_vkml_runtime_env()
    result = _run_cmd(cmd_line, env=runtime_env)

    result_stdout = result.stdout.decode(errors="replace")
    result_stderr = result.stderr.decode(errors="replace")

    if _vulkan_validation_requested():
        _raise_on_vulkan_validation_errors(result_stdout, result_stderr)
...
def _vulkan_validation_requested() -> bool:
    value = os.environ.get(VULKAN_VALIDATION_ENV)
    if value is None:
        return False
...
def _add_known_vkml_validation_filters(env: dict[str, str]) -> None:
    """Mute only known VKML validation errors; preserve all other VVL errors."""
    existing = [
        value.strip()
        for value in env.get(VULKAN_VALIDATION_MESSAGE_FILTER_ENV, "").split(
            os.path.pathsep
        )
        if value.strip()
    ]

    for vuid in KNOWN_VKML_VALIDATION_VUIDS:
        if vuid not in existing:
            existing.append(vuid)

    env[VULKAN_VALIDATION_MESSAGE_FILTER_ENV] = os.path.pathsep.join(existing)


def _enable_vulkan_validation(env: dict[str, str]) -> dict[str, str]:
    """Enable Khronos Vulkan validation for VKML test subprocesses.

    The layer is injected through the Vulkan loader rather than being added to
    VGFBackend's VkInstanceCreateInfo, so production runtime behavior is
    unchanged.

    """
    if not _vulkan_validation_requested():
        return env

    env = env.copy()

    _add_known_vkml_validation_filters(env)

    # Make the Khronos validation manifest visible when setup_path.sh has set
    # VK_LAYER_PATH to only the ML emulation-layer manifests. VK_LAYER_PATH
    # overrides the Vulkan loader's normal layer search path.
    vulkan_sdk_roots = [
        entry for entry in env.get("VULKAN_SDK", "").split(os.path.pathsep) if entry
    ]
    if vulkan_sdk_roots:
        validation_layer_dir = (
            Path(vulkan_sdk_roots[0]) / "share/vulkan/explicit_layer.d"
        )
        validation_manifest = validation_layer_dir / "VkLayer_khronos_validation.json"
        if validation_manifest.is_file():
            validation_layer_dir_text = str(validation_layer_dir)
            for variable in ("VK_LAYER_PATH", "VK_ADD_LAYER_PATH"):
                existing_paths = [
                    entry
                    for entry in env.get(variable, "").split(os.path.pathsep)
                    if entry and entry != validation_layer_dir_text
                ]
                env[variable] = os.path.pathsep.join(
                    [validation_layer_dir_text, *existing_paths]
                )

    # Vulkan instance layers are ordered from closest to the application
    # outward. Keep validation first and remove any pre-existing duplicate so
    # the Python runner matches test_arm_backend.sh.
    existing_layers = [
        layer
        for layer in env.get("VK_INSTANCE_LAYERS", "").split(os.path.pathsep)
        if layer and layer != VULKAN_VALIDATION_LAYER
    ]
    env["VK_INSTANCE_LAYERS"] = os.path.pathsep.join(
        [VULKAN_VALIDATION_LAYER, *existing_layers]
    )

    # Make validation output deterministic and parseable by the test runner.
    # These are Khronos Validation Layer settings.
    env.setdefault("VK_KHRONOS_VALIDATION_REPORT_FLAGS", "error")
    env.setdefault(
...
def _raise_on_vulkan_validation_errors(stdout: str, stderr: str) -> None:
    """Fail a VKML test when Khronos validation reports an API error."""
    combined = "\n".join(part for part in (stdout, stderr) if part)

    error_lines = [
        line
        for line in combined.splitlines()
        if "Validation Error:" in line or "ERROR : VALIDATION" in line
    ]
    ...
    raise RuntimeError(
        "Vulkan validation failed during VGF execution.\n\n"
        f"{summary}\n\n"
...
def _get_vkml_runtime_env() -> dict[str, str]:
    ...
    if sdk_root is None:
        return _enable_vulkan_validation(env)

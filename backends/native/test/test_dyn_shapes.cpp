 ...
 *   - MetalEngine kernels reading the actual shape via
...
bool run_one(Module& mod, int32_t batch) {
  ...
  auto input_tensor =
      from_blob(data.data(), sizes, ::executorch::aten::ScalarType::Float);

  auto outputs_result = mod.execute("forward", {EValue(input_tensor)});
  if (!outputs_result.ok()) {
    std::fprintf(
        stderr,
        "  batch=%d: execute() failed: 0x%x\n",
        batch,
        static_cast<unsigned>(outputs_result.error()));
    return false;
  }
  ...
  for (size_t i = 0; i < out_sizes.size(); ++i) {
    std::printf("%s%d", i ? ", " : "", static_cast<int>(out_sizes[i]));
  }
  std::printf("]");

  // Verify: shape should be [batch, 5].
  bool shape_ok =
      (out_sizes.size() == 2 && out_sizes[0] == batch && out_sizes[1] == 5);
...
int main(int argc, char** argv) {
  ...
  if (load_err != Error::Ok) {
    std::fprintf(
        stderr, "load_forward failed: 0x%x\n", static_cast<unsigned>(load_err));
    return 1;
  }

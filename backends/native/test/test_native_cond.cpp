...
bool run_branch(Module& module, bool pred, float expected) {
  ...
  auto pred_tensor =
      from_blob(pred_data.data(), {}, ::executorch::aten::ScalarType::Bool);
...
int main() {
  ...
  std::string pte_path =
      env_pte ? std::string(env_pte) : std::string("/tmp/native_cond.pte");
  ...
  if (!run_branch(module, true, 2.0f))
    return 3;
  if (!run_branch(module, false, 1.0f))
    return 3;

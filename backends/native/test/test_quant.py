...
class FuseGGUFPassTest(unittest.TestCase):
    def _opaque_constants(self, program):
        return [
            c
            for c in (program.methods[0].constants or [])
            if c.meta.quant is not None
            and isinstance(c.meta.quant.scheme, OpaqueQuantization)
        ]

    def test_gguf_linear_serializes_as_packed_linear(self):
        # A GGUF-quantized linear serializes as a plain linear over a weight
        # constant tagged with an opaque codec, dropping the dequantize.
        ...
        packed = self._opaque_constants(program)
    ...
    def test_gguf_embedding_serializes_as_packed_embedding(self):
        # The embedding counterpart: plain embedding over an opaque quantized weight.

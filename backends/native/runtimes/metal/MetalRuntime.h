...
// Forward declaration: keep MetalRuntime.h pure-C++; the .mm
...
/**
 * Metal Runtime — Apple-silicon accelerator using the existing
 ...
 *
 * v1 constraint: at most one non-CPU provider per process. MetalRuntime
 * is mutually exclusive with FakeAccelProvider in
 * make_default_runtimes().
 */

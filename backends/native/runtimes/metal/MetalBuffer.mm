...
MetalBuffer::~MetalBuffer() {
  if (mode_ == Mode::Owned && ptr_ && stream_) {
    stream_->allocator().free(ptr_);
  } else if (mode_ == Mode::NdmAlias) {
    ndm_buffer_.Free();
    // Note: we don't unregister from MetalStream's ptrToBuffer_ here.
    // For Owned mode, stream_->allocator().free() handles pool return AND ptrToBuffer_
...
void MetalBuffer::re_alias(void* ptr, size_t bytes) {
  ...
  // Note: caller is responsible for stream_->allocator().registerExternalBuffer(ptr)

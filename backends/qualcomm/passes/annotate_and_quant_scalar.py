...
class AnnotateAndQuantScalar(ExportPass):
    ...
    def _update_scalar_node_attrs(
        ...
        quant_attrs["scale"] = val.div(quant_attrs["quant_max"] - quant_attrs["quant_min"])
        return quant_attrs
    ...
    def _traverse_binary_node(self, graph_module: torch.fx.GraphModule):
        src_partitions = get_source_partitions(graph_module.graph, self.binary_op_sources)
        ...
        for src_partition in src_partitions:
            ...
            if output.meta.get(self.quant_attrs_key) and len(src_partition.input_nodes) == 1:
                ...
                scalar_quant_attrs = self._update_scalar_node_attrs(graph_module, source_scalar_node, q_node_attrs)
                self._annotate_scalar_node(graph_module, scalar_node, scalar_quant_attrs)

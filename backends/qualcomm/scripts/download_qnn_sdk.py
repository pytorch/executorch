...
def _download_archive(url: str, archive_path: pathlib.Path) -> bool:
    """Robust streaming download with retries and progress output."""
    ...
    try:
        ...
        logger.error(f"\n[QNN] Error during download: {e}")
        logger.exception("Error during download: %s", e)
        return False

    if archive_path.exists() and archive_path.stat().st_size == 0:
        logger.error("[QNN] Downloaded file is empty!")
        return False
    elif not archive_path.exists():
        logger.error("[QNN] File was not downloaded!")
        return False

...
def _decode_qnn_options(compile_specs: Any) -> Optional[Any]:
    ...
    Args:
        compile_specs: The value passed as ``compile_specs``; any shape is
    ...
    Returns:
        ...
        Returning ``None`` rather than raising keeps this usable as a
    ...
    except Exception:  # noqa: BLE001 - a stub may not be iterable or decodable
        return None
...
def _verify_target_matches_specs(
    ...
    Args:
        ...
        soc_model: The SoC the caller believes it is targeting.
        backend_type: The backend the caller believes it is targeting.

    Raises:
        ValueError: If either value contradicts the specs.
    """
    ...
    if soc_model is not None and soc_model != spec_soc_model:
        raise ValueError(
    ...
    if backend_type is not None and backend_type != spec_backend_type:
        raise ValueError(

    ...
    def build_parser(cls, **parser_kwargs: Any) -> argparse.ArgumentParser:
        ...
        for spec in fields(cls):
            ...
            if spec.name in _LIST_VALUED_FIELDS:
                ...
                # int/float fields keep their type so the parser converts; the
                # rest are strings. ``None`` defaults carry no type information,
                # so they stay strings and the consumer coerces.
                numeric = isinstance(default, (int, float))
                kwargs["type"] = type(default) if numeric else str

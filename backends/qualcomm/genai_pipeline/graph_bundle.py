...
#: ``None``. ``compilation/pass_policy.py`` indexes both, so a mapping missing
#: either one is rejected at construction rather than at lowering.
...
class GraphBundle:
    ...
    * ``compile_spec`` -- built by the compilation stage from ``ControlArgs``
      and the component the graph belongs to, since weight sharing and
      multi-context are properties of how graphs are grouped into a ``.pte``
      rather than of a graph itself.
    * ``dep_table`` / ``passes_job`` -- derived by the compilation stage from
      ``meta``, ``ControlArgs`` and the shard count, for the same reason: graph
      sharding and quantized-IO tagging are settings of the lowering call. The
      shard count that inserts ``SplitGraph`` is the one that sets the
      multi-context compile spec, so deriving both in one place keeps them from
      disagreeing -- a model that is sharded but not split still compiles.
    * ``dynamic_shapes`` -- HTP has no dynamic shapes, and the parameter was
      removed from this package at review request.
    * ``skip_node_id_set`` / ``skip_node_op_set`` -- per-run partitioning
      overrides today, so they travel in ``extra_options``. They belong here
      only once two graphs of one model need different values.
    ...
    Attributes:
        ...
        quant_io_dtypes: The graph-boundary dtypes quantization chose, as
            ...
            * **The component never derives dtypes from a recipe.** Encoder
            ...
            ``compilation/pass_policy.py`` distinguishes the two by component,
            since only the component knows whether a recipe was meant to supply
            these at all.

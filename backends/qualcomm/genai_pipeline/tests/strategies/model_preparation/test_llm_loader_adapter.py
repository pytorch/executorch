...
class TestLLMLoaderAdapter(unittest.TestCase):
    def setUp(self):
        self.model_config = MagicMock()
        self.model_config.repo_id = None
        self.control_args = MagicMock()
        self.control_args.checkpoint = "/tmp/checkpoint.pt"
        self.control_args.model = "test_model"
        self.adapter = LLMLoaderAdapter(self.model_config, self.control_args)

    def test_load_model_builds_graphs_and_loads_checkpoint_into_each_graph(self):
        calibration_module = _make_module("calibration")
        decode_module = _make_module("decode")
        ...
        with patch(
            "executorch.backends.qualcomm.genai_pipeline.strategies."
            "model_preparation.llm_loader_adapter.torch.load",
            return_value=state_dict,
        ) as torch_load:
            result = self.adapter.load_model(
                "test_model",
                extra_options={
                    "model_arch": model_arch,
                    "weight_transforms": {ARTIFACT_TEXT_DECODER: [transform]},
                },
            )
    ...
    def test_load_model_requires_model_arch(self):
        with self.assertRaises(ValueError):
            self.adapter.load_model("test_model", extra_options={})

    def test_load_state_dict_uses_injected_hf_loader(self):
        state_dict = {"weight": MagicMock(name="weight")}
        state_dict_loader = MagicMock(return_value=state_dict)
        self.control_args.checkpoint = None
        self.model_config.repo_id = "test/repo"

        with patch(
            "executorch.backends.qualcomm.genai_pipeline.strategies."
            "model_preparation.llm_loader_adapter.torch.load"
        ) as torch_load:
            result = self.adapter._load_state_dict(state_dict_loader)

        state_dict_loader.assert_called_once_with("test/repo")
        torch_load.assert_not_called()
        self.assertIs(result, state_dict)

    def test_load_model_uses_component_keyed_state_dict_loader(self):
        module = _make_module("decoder")
        state_dict = {"weight": MagicMock(name="weight")}
        state_dict_loader = MagicMock(return_value=state_dict)
        self.control_args.checkpoint = None
        self.model_config.repo_id = "test/repo"
        ...
        result = self.adapter.load_model(
            ...
            extra_options={
                "model_arch": {
                    ARTIFACT_TEXT_DECODER: {
                        DEFAULT_GRAPH_NAME: MagicMock(return_value=module),
                    }
                },
        ...
        self.assertEqual(
            result,
            {ARTIFACT_TEXT_DECODER: {DEFAULT_GRAPH_NAME: module}},
        )

    def test_get_metadata_reads_module_when_it_exposes_metadata(self):
        with_metadata = MagicMock()
        with_metadata.get_metadata.return_value = {"get_n_layers": 2}

        result = self.adapter.get_metadata(with_metadata)

        self.assertEqual(result, {"get_n_layers": 2})
        with_metadata.get_metadata.assert_called_once_with()

    def test_get_metadata_returns_empty_when_module_exposes_none(self):
        without_metadata = MagicMock(spec=[])

        result = self.adapter.get_metadata(without_metadata)
...
class TestGetExampleInputs(unittest.TestCase):
    """The static LLM graph module owns its export signature."""

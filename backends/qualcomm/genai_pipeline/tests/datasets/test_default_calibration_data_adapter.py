...
class TestGenerateCalibrationData(unittest.TestCase):
    ...
    def setUp(self):
        self.adapter = DefaultCalibrationDataAdapter(num_samples=TEST_NUM_SAMPLES)

    def test_generates_requested_number_of_samples(self):
        data = self.adapter.generate_calibration_data(
            _make_tokenizer(), extra_options={"max_context_len": TEST_SEQ_LENGTH}
        )
        self.assertEqual(len(data[ARTIFACT_TEXT_DECODER]), TEST_NUM_SAMPLES)

    def test_sample_shape_and_dtype(self):
        data = self.adapter.generate_calibration_data(
            _make_tokenizer(), extra_options={"max_context_len": TEST_SEQ_LENGTH}
        )
        input_ids, attention_mask = data[ARTIFACT_TEXT_DECODER][0]
    ...
    def test_tokens_within_vocab_range(self):
        ...
        data = self.adapter.generate_calibration_data(
            ...
            extra_options={"max_context_len": TEST_SEQ_LENGTH},
        )
        for input_ids, _ in data[ARTIFACT_TEXT_DECODER]:
    ...
    def test_seed_makes_generation_reproducible(self):
        kwargs = {
            "extra_options": {
                "max_context_len": TEST_SEQ_LENGTH,
                "seed": 7,
            },
        }
        ...
        for (ids_a, _), (ids_b, _) in zip(
            first[ARTIFACT_TEXT_DECODER], second[ARTIFACT_TEXT_DECODER]
        ):
    ...
    def test_provided_list_dataset_is_returned_unchanged(self):
        ...
        self.assertIs(result[ARTIFACT_TEXT_DECODER], dataset)

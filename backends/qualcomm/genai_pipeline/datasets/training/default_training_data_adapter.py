...
class DefaultTrainingDataAdapter:
    """QAT training data provider.

    Training data is not synthesized: random tokens carry no learning signal.
    Uses injected dataset loaders when present; otherwise callers enabling QAT
    must supply data via ``extra_options["training_data"]``.
    ...
    def generate_training_data(
        ...
        """Return component-keyed QAT training data."""
        ...
        if training_data is not None:
            ...
            return (
                ...
                else {ARTIFACT_TEXT_DECODER: training_data}
            )
        ...
        if not self._dataset_loaders:
            raise ValueError(
                ...
                "extra_options['training_data'] or inject corpus sources. For "
                "PTQ, skip QAT entirely and use the calibration adapter instead."
            )

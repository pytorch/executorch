...
def get_default_8bit_qnn_ptq_config(act_symmetric: bool = False) -> QuantizationConfig:
    extra_args: Dict[str, Any] = {"eps": 2**-12}

    act_quantization_spec = QuantizationSpec(
        dtype=torch.uint8,
        quant_min=None if act_symmetric else torch.iinfo(torch.uint8).min,
        quant_max=None if act_symmetric else torch.iinfo(torch.uint8).max,
        qscheme=(
...
def annotate_div(node: Node, quantization_config: QuantizationConfig) -> None:
    def _derived_inp1_const_div_quant_spec(
        ...
        return DerivedQuantizationSpec(
            ...
            quant_min=output_qspec.quant_min,
            quant_max=output_qspec.quant_max,
            ch_axis=0,
            qscheme=output_qspec.qscheme,
        )

    if all([type(a) == Node for a in node.args]):
        annotate_binary(node, quantization_config)
    # special constant divisor case
    elif type(node.args[0]) == Node and isinstance(node.args[1], numbers.Number):
        if _is_annotated([node]):
            return
...
def annotate_sigmoid(node: Node, quantization_config: QuantizationConfig) -> None:
    ...
    scale = 1 / (out_qconf.quant_max - out_qconf.quant_min + 1)

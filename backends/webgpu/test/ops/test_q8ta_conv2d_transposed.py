...
"""`et_vk.q8ta_conv2d_transposed` module + configs: int8 transposed conv.

An `nn.ConvTranspose2d` (groups == 1, dilation == 1) through XNNPACK static PT2E
lowers to a delegated `quantize_per_tensor -> q8ta_conv2d_transposed ->
...
def make_q8ta_conv2d_transposed_module(
    ic, oc, k, h, w, stride=2, padding=0, output_padding=0, bias=True, n=1, seed=0
):
...
class Q8taConv2dTransposedTest(unittest.TestCase):
    def test_delegates(self) -> None:
        m = make_q8ta_conv2d_transposed_module(8, 8, 2, 8, 8, stride=2)
        ...
        self.assertIn(b"q8ta_conv2d_transposed", et.buffer)

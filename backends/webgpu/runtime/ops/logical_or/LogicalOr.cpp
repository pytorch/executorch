...
static_assert(
    sizeof(LogicalOrParams) == 16,
    "LogicalOrParams must be 16 bytes");

// out = a | b (bool OR); serves logical_or + bitwise_or (mirrors Vulkan).
void logical_or_op(WebGPUGraph& graph, const std::vector<int>& args) {
  ...
  if (graph.get_value_type(a_id) != WebGPUGraph::ValueType::Tensor ||
    ...
    throw std::runtime_error("logical_or: a/b/out is not a tensor");
  }
  ...
  if (a_tensor.buffer == nullptr || b_tensor.buffer == nullptr ||
    ...
    throw std::runtime_error("logical_or: null buffer binding");
  }
  ...
  if (!a_tensor.is_int || !b_tensor.is_int || !out_tensor.is_int ||
    ...
    throw std::runtime_error("logical_or: a/b/out must be 1-byte bool tensors");
  }
  ...
  if (numel == 0u || numel % 4u != 0u || numel > UINT32_MAX) {
    throw std::runtime_error("logical_or: numel must be a nonzero mult of 4");
  }
  if (a_tensor.nbytes != numel || b_tensor.nbytes != numel) {
    throw std::runtime_error("logical_or: a/b/out numel mismatch (same-shape)");
  }

  LogicalOrParams params = {};
  params.num_words = static_cast<uint32_t>(numel / 4u);

  uint32_t wg_size =
      utils::clamp_workgroup_size(device, kLogicalOrWorkgroupSizeX);
  utils::WgCount workgroup_count = utils::compute_2d_workgroup_count(
      device, params.num_words, wg_size, "logical_or");
  ...
  WGPUBuffer uniform_buffer =
      utils::make_uniform(device, &params, sizeof(LogicalOrParams));
  ...
  auto resize = [a_id, b_id, out_id, wg_size, dispatch_idx, params_buf](
                    WebGPUGraph& g) {
    ...
    if (n == 0u || n % 4u != 0u || n > UINT32_MAX ||
      ...
      throw std::runtime_error("logical_or(resize): numel must be a mult of 4");
    }
    ...
    const utils::WgCount wgc = utils::compute_2d_workgroup_count(
        g.device(), p.num_words, wg_size, "logical_or");
...
WEBGPU_REGISTER_OPERATORS {
  WEBGPU_REGISTER_OP(aten.logical_or.default, logical_or_op);
  WEBGPU_REGISTER_OP(aten.bitwise_or.Tensor, logical_or_op);
}

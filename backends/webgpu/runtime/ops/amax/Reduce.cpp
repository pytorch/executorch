...
void amax_impl(WebGPUGraph& graph, const std::vector<int>& args) {
  // aten.amax.default args: [in, dim, keepdim, out]
  ...
  if (in_tensor.buffer == nullptr || out_tensor.buffer == nullptr) {
    throw std::runtime_error("amax: null buffer binding");
  }
  if (in_tensor.is_int || out_tensor.is_int) {
    throw std::runtime_error("amax: int dtype unsupported");
  }
  ...
  if (reduce_size == 0u ||
    ...
    throw std::runtime_error("amax: shape mismatch (num_rows * reduce_size)");
  }
  ...
  WGPUBuffer uniform_buffer =
      utils::make_uniform(device, &params, sizeof(AmaxParams));
  graph.add_uniform_buffer_bytes(sizeof(AmaxParams));

  WGPUShaderSourceWGSL wgsl_desc = {};
  wgsl_desc.chain.sType = WGPUSType_ShaderSourceWGSL;
  wgsl_desc.code = {kAmaxWGSL, WGPU_STRLEN};
  ...
  bg_entries[2].size = sizeof(AmaxParams);
  ...
  const size_t dispatch_idx = graph.add_dispatch(
      {pipeline, bind_group, workgroup_count.x, "amax", workgroup_count.y});
...
WEBGPU_REGISTER_OPERATORS {
  WEBGPU_REGISTER_OP(aten.amax.default, amax_impl);
}

...
void decompose(
  ...
  if (ndim == 0 || dim < 0 || dim >= ndim) {
    throw std::runtime_error("WebGPU softmax: dim out of range");
  }
  uint64_t o = 1, in = 1;
...
void softmax_impl(WebGPUGraph& graph, const std::vector<int>& args) {
  ...
  if (in.dims.empty()) {
    throw std::runtime_error("WebGPU softmax: scalar input unsupported");
  }
  if (graph.get_value_type(dim_id) != WebGPUGraph::ValueType::Int) {
    throw std::runtime_error("WebGPU softmax: dim must be an int");
  }
  ...
  if (outer == 0 || r == 0 || inner == 0) {
    throw std::runtime_error("WebGPU softmax: zero-sized reduction");
  }
  ...
  if (in.nbytes != numel * sizeof(float) ||
    ...
    throw std::runtime_error("WebGPU softmax: fp32-only (byte-size mismatch)");
  }

  const uint64_t lines = static_cast<uint64_t>(outer) * inner;
  if (lines > UINT32_MAX) {
    throw std::runtime_error(
        "WebGPU softmax: line count exceeds dispatch limit");
  }

  const uint32_t wg_size =
      utils::clamp_workgroup_size(device, kSoftmaxWorkgroupSizeX);
  const uint32_t workgroup_count = utils::compute_1d_workgroup_count(
      device, static_cast<uint32_t>(lines), wg_size, "softmax");
  ...
  WGPUBuffer uniform_buffer =
      utils::make_uniform(device, &params, sizeof(SoftmaxParams));
  graph.add_uniform_buffer_bytes(sizeof(SoftmaxParams));

  WGPUShaderSourceWGSL wgsl_desc = {};
  wgsl_desc.chain.sType = WGPUSType_ShaderSourceWGSL;
  wgsl_desc.code = {kSoftmaxWGSL, WGPU_STRLEN};
  ...
  bg_entries[2].size = sizeof(SoftmaxParams);
  ...
  const size_t dispatch_idx =
      graph.add_dispatch({pipeline, bind_group, workgroup_count, "softmax"});
  ...
  graph.add_tensor_resize_hook(
      ...
      [in_id, out_id, dim, build_numel, wg_size, dispatch_idx, params_buf](
        ...
        decompose(d, dim, o, rr, n);
        if (o == 0u || rr == 0u || n == 0u) {
          throw std::runtime_error("WebGPU softmax: live zero-sized reduction");
        }
        // Buffers were sized for build-time numel; reject a larger live size.
        if (static_cast<uint64_t>(o) * rr * n > build_numel) {
          throw std::runtime_error(
              "WebGPU softmax: live numel exceeds build max");
        }
...
WEBGPU_REGISTER_OPERATORS {
  WEBGPU_REGISTER_OP(aten._softmax.default, softmax_impl);
}

...
// @generated from rms_norm_vec4_add.wgsl - DO NOT EDIT.
// wgsl-sha256: d4855cb6c90093ba216ae4a877567a3f70a5ec0d4b439f4fc012bedf24fc2027
inline constexpr const char* kRmsNormVec4AddWGSL = R"(
// Copyright (c) Meta Platforms, Inc. and affiliates.
// All rights reserved.
//
// This source code is licensed under the BSD-style license found in the
...
// rms_norm_vec4 with the immediately following aten.add.Tensor folded in.
// The reduction and the normalized store are byte-for-byte the unfused vec4
// kernel; the add evaluates the same expression binary_add.wgsl does, at the
// same element index. The host only takes this route when alpha == 1.0, which
// makes `fuse.alpha * n` exact under any fma contraction the backend chooses.
// t_out is still written, so no consumer of it is assumed dead.
...
inline constexpr uint32_t kRmsNormVec4AddWorkgroupSizeX = 64;
inline constexpr uint32_t kRmsNormVec4AddWorkgroupSizeY = 1;
inline constexpr uint32_t kRmsNormVec4AddWorkgroupSizeZ = 1;

...
static_assert(
    sizeof(Q8taConvTParams) == 96,
    "Q8taConvTParams must match the WGSL Params struct (96 bytes)");
...
// int8 transposed conv (groups==1, dilation==1); gather form; mirrors Vulkan.
void q8ta_conv2d_transposed_impl(
  ...
  if (graph.get_value_type(in_id) != WebGPUGraph::ValueType::Tensor ||
    ...
    throw std::runtime_error(
        "q8ta_conv2d_transposed: in/weight/scales/out not tensor");
  }
  const bool has_bias =
  ...
  if (graph.get_value_type(act_id) != WebGPUGraph::ValueType::String ||
    ...
    throw std::runtime_error(
        "q8ta_conv2d_transposed: only activation='none' supported");
  }
  ...
  if (in_tensor.buffer == nullptr || weight_tensor.buffer == nullptr ||
    ...
    throw std::runtime_error("q8ta_conv2d_transposed: null buffer binding");
  }
  if (in_tensor.dims.size() != 4 || out_tensor.dims.size() != 4 ||
      weight_tensor.dims.size() != 2) {
    throw std::runtime_error(
        "q8ta_conv2d_transposed: in/out must be 4D, weight 2D");
  }
  ...
  // Transposed schema inserts output_padding at 12, shifting dilation to 13.
  const auto [kernel_h, kernel_w] = pair_or_throw(
      graph.get_int_list(args.at(9)), "q8ta_conv2d_transposed: kernel_size");
  const auto [stride_h, stride_w] = pair_or_throw(
      graph.get_int_list(args.at(10)), "q8ta_conv2d_transposed: stride");
  const auto [pad_h, pad_w] = pair_or_throw(
      graph.get_int_list(args.at(11)), "q8ta_conv2d_transposed: padding");
  const auto [dil_h, dil_w] = pair_or_throw(
      graph.get_int_list(args.at(13)), "q8ta_conv2d_transposed: dilation");
  const int groups = graph.get_int(args.at(14));
  if (groups != 1) {
    throw std::runtime_error(
        "q8ta_conv2d_transposed: only groups==1 supported");
  }
  if (dil_h != 1 || dil_w != 1) {
    throw std::runtime_error(
        "q8ta_conv2d_transposed: only dilation==1 supported");
  }
  if (stride_h < 1 || stride_w < 1) {
    throw std::runtime_error("q8ta_conv2d_transposed: stride must be >= 1");
  }
  ...
  if (static_cast<uint64_t>(out_tensor.dims.at(0)) != N ||
    ...
    throw std::runtime_error(
        "q8ta_conv2d_transposed: output must be [N, OC, H_out, W_out]");
  }
  if (weight_row_stride < Kh * Kw * IC) {
    throw std::runtime_error(
        "q8ta_conv2d_transposed: weight row stride < Kh*Kw*IC");
  }
  const uint64_t out_numel = N * OC * H_out * W_out;
  if (out_numel == 0 || W_out % 4 != 0) {
    throw std::runtime_error(
        "q8ta_conv2d_transposed: W_out must be a nonzero multiple of 4");
  }
  ...
  if (out_numel > UINT32_MAX || in_numel > UINT32_MAX ||
    ...
    throw std::runtime_error("q8ta_conv2d_transposed: numel exceeds u32");
  }
  ...
  uint32_t wg_size =
      utils::clamp_workgroup_size(device, kQ8taConv2dTransposedWorkgroupSizeX);
  utils::WgCount workgroup_count = utils::compute_2d_workgroup_count(
      device, num_words, wg_size, "q8ta_conv2d_transposed");
  ...
  WGPUBuffer params_buf =
      utils::make_uniform(device, &params, sizeof(Q8taConvTParams));
  graph.add_uniform_buffer_bytes(sizeof(Q8taConvTParams));

  WGPUShaderSourceWGSL wgsl_desc = {};
  wgsl_desc.chain.sType = WGPUSType_ShaderSourceWGSL;
  wgsl_desc.code = {kQ8taConv2dTransposedWGSL, WGPU_STRLEN};
  ...
  bg[5].size = sizeof(Q8taConvTParams);

...
static_assert(
    sizeof(Q8taConvDwParams) == 80,
    "Q8taConvDwParams must match the WGSL Params struct (80 bytes)");
...
// int8 depthwise conv; per-channel windowed dot; mirrors Vulkan q8ta_conv2d_dw.
void q8ta_conv2d_dw_impl(WebGPUGraph& graph, const std::vector<int>& args) {
  ...
  if (graph.get_value_type(in_id) != WebGPUGraph::ValueType::Tensor ||
    ...
    throw std::runtime_error("q8ta_conv2d_dw: in/weight/scales/out not tensor");
  }
  const bool has_bias =
  ...
  if (graph.get_value_type(act_id) != WebGPUGraph::ValueType::String ||
    ...
    throw std::runtime_error(
        "q8ta_conv2d_dw: only activation='none' supported");
  }
  ...
  if (in_tensor.buffer == nullptr || weight_tensor.buffer == nullptr ||
    ...
    throw std::runtime_error("q8ta_conv2d_dw: null buffer binding");
  }
  if (in_tensor.dims.size() != 4 || out_tensor.dims.size() != 4 ||
      weight_tensor.dims.size() != 3) {
    throw std::runtime_error("q8ta_conv2d_dw: in/out must be 4D, weight 3D");
  }
  ...
  const auto [stride_h, stride_w] =
      pair_or_throw(graph.get_int_list(args.at(10)), "q8ta_conv2d_dw: stride");
  const auto [pad_h, pad_w] =
      pair_or_throw(graph.get_int_list(args.at(11)), "q8ta_conv2d_dw: padding");
  const auto [dil_h, dil_w] = pair_or_throw(
      graph.get_int_list(args.at(12)), "q8ta_conv2d_dw: dilation");
  const int groups = graph.get_int(args.at(13));

  const uint64_t N = static_cast<uint64_t>(in_tensor.dims.at(0));
  const uint64_t C = static_cast<uint64_t>(in_tensor.dims.at(1));
  const uint64_t H_in = static_cast<uint64_t>(in_tensor.dims.at(2));
  const uint64_t W_in = static_cast<uint64_t>(in_tensor.dims.at(3));
  const uint64_t Kh = static_cast<uint64_t>(weight_tensor.dims.at(0));
  const uint64_t Kw = static_cast<uint64_t>(weight_tensor.dims.at(1));
  const uint64_t OC = static_cast<uint64_t>(weight_tensor.dims.at(2));
  ...
  if (OC != C || static_cast<uint64_t>(groups) != C) {
    throw std::runtime_error(
        "q8ta_conv2d_dw: depthwise requires groups==C==OC");
  }
  if (static_cast<uint64_t>(out_tensor.dims.at(1)) != C ||
      static_cast<uint64_t>(out_tensor.dims.at(0)) != N) {
    throw std::runtime_error(
        "q8ta_conv2d_dw: output must be [N, C, H_out, W_out]");
  }
  const uint64_t out_numel = N * C * H_out * W_out;
  if (out_numel == 0 || W_out % 4 != 0) {
    throw std::runtime_error(
        "q8ta_conv2d_dw: W_out must be a nonzero multiple of 4");
  }
  if (out_numel > UINT32_MAX || N * C * H_in * W_in > UINT32_MAX) {
    throw std::runtime_error("q8ta_conv2d_dw: numel exceeds u32");
  }
  const uint64_t in_numel = N * C * H_in * W_in;
  const uint64_t weight_numel = Kh * Kw * OC;
  if (!in_tensor.is_int8 || in_tensor.nbytes != in_numel || in_numel % 4 != 0 ||
    ...
    throw std::runtime_error(
        "q8ta_conv2d_dw: int8 in/weight/out size mismatch");
  }
  if (scales_tensor.nbytes != OC * sizeof(float)) {
    throw std::runtime_error("q8ta_conv2d_dw: weight_scales must be fp32 [OC]");
  }
  if (has_bias) {
    const auto& b = graph.get_tensor(bias_id);
    if (b.buffer == nullptr || b.nbytes != OC * sizeof(float)) {
      throw std::runtime_error("q8ta_conv2d_dw: bias must be fp32 [OC]");
    }
  }

  Q8taConvDwParams params = {};
  ...
  uint32_t wg_size =
      utils::clamp_workgroup_size(device, kQ8taConv2dDwWorkgroupSizeX);
  utils::WgCount workgroup_count = utils::compute_2d_workgroup_count(
      device, num_words, wg_size, "q8ta_conv2d_dw");
  ...
  WGPUBuffer params_buf =
      utils::make_uniform(device, &params, sizeof(Q8taConvDwParams));
  graph.add_uniform_buffer_bytes(sizeof(Q8taConvDwParams));

  WGPUShaderSourceWGSL wgsl_desc = {};
  wgsl_desc.chain.sType = WGPUSType_ShaderSourceWGSL;
  wgsl_desc.code = {kQ8taConv2dDwWGSL, WGPU_STRLEN};
  ...
  bg[5].size = sizeof(Q8taConvDwParams);
...
WEBGPU_REGISTER_OPERATORS {
  WEBGPU_REGISTER_OP(et_vk.q8ta_conv2d_dw.default, q8ta_conv2d_dw_impl);
}

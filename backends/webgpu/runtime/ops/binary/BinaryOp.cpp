...
void add_binary_broadcast_op(
  ...
  if (in1_tensor.buffer == nullptr || in2_tensor.buffer == nullptr ||
    ...
    throw std::runtime_error(name + ": null buffer binding");
  }
  // fp32-only backend: reject int operands (would be read as f32).
  if (in1_tensor.is_int || in2_tensor.is_int || out_tensor.is_int) {
    throw std::runtime_error(name + ": int dtype unsupported");
  }
  ...
  if (out_tensor.nbytes !=
    ...
    throw std::runtime_error(name + ": non-fp32 operand (nbytes != numel * 4)");
  }
  ...
  auto resize = [in1_id, in2_id, out_id, wg_size, dispatch_idx, o_buf, a_buf,
    ...
    for (size_t i = 0; i < r; i++) {
      ...
      if (av != bv && av != 1 && bv != 1) {
        throw std::runtime_error(
            name + "(resize): operands are not broadcast-compatible");
      }
      out_d[i] = av > bv ? av : bv;
    }
    ...
    const std::string resize_label = name + "(resize)";
    const utils::WgCount wgc = utils::compute_2d_workgroup_count(
        g.device(), om.numel, wg_size, resize_label.c_str());

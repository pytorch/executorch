...
void div_impl(WebGPUGraph& graph, const std::vector<int>& args) {
  // div.Tensor has no alpha operand (unlike sub).
  ...
  if (out_tensor.dims.size() > kTensorMetaMaxNdim ||
    ...
    throw std::runtime_error("div: tensor rank exceeds 4 (MAX_NDIM)");
  }
  ...
  uint32_t wg_size =
      utils::clamp_workgroup_size(device, kBinaryDivWorkgroupSizeX);
  utils::WgCount workgroup_count =
      utils::compute_2d_workgroup_count(device, out_meta.numel, wg_size, "div");
  ...
  auto div_resize =
      [in1_id, in2_id, out_id, wg_size, dispatch_idx, o_buf, a_buf, b_buf](
        ...
        for (size_t i = 0; i < r; i++) {
          ...
          if (av != bv && av != 1 && bv != 1) {
            throw std::runtime_error(
                "div(resize): operands are not broadcast-compatible");
          }
          out_d[i] = av > bv ? av : bv;
        }
        ...
        const utils::WgCount wgc = utils::compute_2d_workgroup_count(
            g.device(), om.numel, wg_size, "div(resize)");
...
WEBGPU_REGISTER_OPERATORS {
  WEBGPU_REGISTER_OP(aten.div.Tensor, div_impl);
}

...
#undef FORCE_INLINE_APPLE
  ...
  STANDALONE_HOST_DEVICE constexpr T real() const {
    return real_;
  }
  constexpr void real(T value) {
    real_ = value;
  }
  STANDALONE_HOST_DEVICE constexpr T imag() const {
    return imag_;
  }
...
struct alignas(4) complex<Half> {
  ...
  // be constexpr
  STANDALONE_HOST_DEVICE explicit inline complex(
      const Half& real,
      const Half& imag)
  ...
  constexpr STANDALONE_HOST_DEVICE Half real() const {
    return real_;
  }
  constexpr STANDALONE_HOST_DEVICE Half imag() const {
    return imag_;
  }
...
#include <executorch/backends/aoti/slim/c10/util/complex_math.h> // IWYU pragma: keep
// utilities for complex types
#include <executorch/backends/aoti/slim/c10/util/complex_utils.h> // IWYU pragma: keep

...
#include <executorch/backends/aoti/slim/c10/util/llvmMathExtras.h>
#include <intrin.h>
#else
...
STANDALONE_ALWAYS_INLINE bool
mul_overflows(uint64_t a, uint64_t b, uint64_t* out) {
#if STANDALONE_HAS_BUILTIN_OVERFLOW()
...
STANDALONE_ALWAYS_INLINE bool
mul_overflows(int64_t a, int64_t b, int64_t* out) {
#if STANDALONE_HAS_BUILTIN_OVERFLOW()
  return __builtin_mul_overflow(a, b, out);
#else

...
TEST_P(FileDataLoaderTest, MoveCtor) {
  ...
  EXPECT_EQ(dl2.size().get(), contents.size());
  Result<FreeableBuffer> fb = dl2.load(
      /*offset=*/0,
      contents.size(),
      DataLoader::SegmentInfo(DataLoader::SegmentInfo::Type::Program));
  ASSERT_EQ(fb.error(), Error::Ok);
...
TEST_P(FileDataLoaderTest, EnnQueryFailure) {
  ...
  Result<FreeableBuffer> fb = fdl->load(
      0, 8, DataLoader::SegmentInfo(DataLoader::SegmentInfo::Type::Program));
  ASSERT_EQ(fb.error(), Error::Ok);
  // Test query with invalid pointer
  EnnBufferPtr out;
  void* invalid_ptr = reinterpret_cast<void*>(0xDEADBEEF);
  bool found = SharedMemoryManager::getInstance()->query(&out, invalid_ptr, 8);
  EXPECT_FALSE(found);
...
TEST_P(FileDataLoaderTest, EnnFreeFailure) {
  ...
  // Allocate and free with invalid ENN state
  {
    ...
    fb->Free(); // Free should fail gracefully
    EnnApi::getEnnApiInstance()->EnnInitialize();
  }
}

...
class PixelUnshuffle(torch.nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.module = torch.nn.PixelUnshuffle(downscale_factor=2)
...
class TestPixelUnshuffle(unittest.TestCase):
    def _test(self, module: torch.nn.Module, inputs):
        ...
        (
            tester.export()
            .check_count({"torch.ops.aten.pixel_unshuffle.default": 1})
    ...
    def test_fp32_pixel_unshuffle(self):
        inputs = (torch.randn(1, 4, 8, 8),)
        self._test(PixelUnshuffle(), inputs)

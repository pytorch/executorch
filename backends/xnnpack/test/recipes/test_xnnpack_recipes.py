...
class TestXnnpackRecipes(unittest.TestCase):
    ...
    # pyre-ignore [56]: Can't infer decorator type
    @testutil.data_provider(
        lambda: [
            {
                "export_recipe": ExportRecipe.get_recipe(
                    XNNPackRecipeType.INT8_DYNAMIC_PER_CHANNEL
                ),
            },
            {
                "export_recipe": ExportRecipe.get_recipe(
                    XNNPackRecipeType.INT8_DYNAMIC_PER_TENSOR
                ),
            },
        ]
    )
    def test_int8_dynamic_quant_recipe(self, export_recipe: ExportRecipe) -> None:
        with torch.no_grad():
            m_eager = TestHelperModules.TwoLinearModule().eval()
            example_inputs = [(torch.randn(9, 8),)]
            session = export(
    ...
    @testutil.data_provider(
        lambda: [
            {
                "export_recipe": ExportRecipe.get_recipe(
                    XNNPackRecipeType.INT8_STATIC_PER_CHANNEL
                ),
            },
            {
                "export_recipe": ExportRecipe.get_recipe(
                    XNNPackRecipeType.INT8_STATIC_PER_TENSOR
                ),
            },
        ]
    )
    def test_int8_static_quant_recipe(self, export_recipe: ExportRecipe) -> None:
        with torch.no_grad():
            m_eager = TestHelperModules.TwoLinearModule().eval()
            example_inputs = [(torch.randn(9, 8),)]
            session = export(
    ...
    def test_8a4w_recipe(self, export_recipe: ExportRecipe) -> None:

...
class ConvTranspose2dSeq(torch.nn.Module):
    def __init__(self):
        super().__init__()
        ...
        self.second = torch.nn.ConvTranspose2d(
            in_channels=3,
            out_channels=2,
            kernel_size=(3, 3),
            padding=1,
...
class ConvTranspose2dBatchNorm(torch.nn.Module):
    ...
    def get_inputs(self):
        return (torch.randn(2, 2, 5, 5),)


class ConvTranspose2dPermute(torch.nn.Module):
...
class TestConvTranspose2d(unittest.TestCase):
    def _test(
        ...
        check_delegate: bool = True,
        check_quantized: bool = True,
    ):
        ...
        (
            tester.export()
            .check_count({"torch.ops.aten.conv_transpose2d": conv_count})
    ...
    def test_fp16_conv_transpose2d(self) -> None:
        for has_bias in (True, False):
            self._test(ConvTranspose2d(bias=has_bias, dtype=torch.float16))

    def test_fp32_conv_transpose2d(self) -> None:
        for has_bias in (True, False):
            self._test(ConvTranspose2d(bias=has_bias))

    def test_fp32_conv_transpose2d_permute(self) -> None:
        for perm_order in list(itertools.permutations([0, 1, 2, 3])):
            self._test(ConvTranspose2dPermute(perm_order))

    def test_qs8_conv_transpose2d_test(self) -> None:
        for has_bias in (True, False):
            self._test(
    ...
    def test_qs8_conv_transpose2d_per_channel(self) -> None:
        self._test(
            ConvTranspose2d(),
    ...
    def test_fp32_conv_transpose2d_seq(self) -> None:
        self._test(ConvTranspose2dSeq(), conv_count=2)

    def test_qs8_conv_transpose2d_seq(self) -> None:
        self._test(
            ConvTranspose2dSeq(),
            conv_count=2,
            quant_config=get_symmetric_quantization_config(),
    ...
    def test_fp32_conv_transpose2d_depthwise(self):
    ...
    def test_fp32_conv_transpose2d_bn(self):
        ...
        self._test(
            ConvTranspose2dBatchNorm(in_features=2, out_features=2, kernel_size=(2, 2))
        )

    def test_fp32_conv_transpose2d_bn_hardtanh_mean_sequence(self):
        """
        ...
        class ConvTranspose2dBatchNormHardTanh(torch.nn.Module):
        ...
        self._test(
            ConvTranspose2dBatchNormHardTanh(
                in_channels=2, out_channels=1, kernel_size=(2, 2)
            )
        )

    def test_qs8_conv2d_bn(self):
        self._test(
            ConvTranspose2dBatchNorm(),
            quant_config=get_symmetric_quantization_config(),
            conv_count=2,
            check_quantized=False,  # XNNPackQuantizer does not this pattern
        )

    def test_qs8_conv_transpose2d_relu(self):
    ...
    def test_qs8_conv_transpose2d_dw_relu(self):
        # Depthwise Convolution Requirements:
        # - Groups must equal In Channels
        # - Out Channels must be a positive multiple of In Channels
        groups = 2
        ...
        for per_channel_quant in (False, True):
            ...
            self._test(
                ...
                check_delegate=not per_channel_quant,
            )

    def test_qs8_conv_transpose2d_relu_seq(self):
        class ConvReLUSeq(torch.nn.Module):
            def __init__(self):
                super().__init__()
                self.model = torch.nn.Sequential(
                    torch.nn.ConvTranspose2d(1, 1, 1),
                    torch.nn.ReLU(),
                    torch.nn.ConvTranspose2d(1, 64, 1),
                    torch.nn.ReLU(),
                )
    ...
    def test_qs8_conv_transpose2d_relu_multi_users(self):
        class ConvTranspose2dReluMultiUsers(torch.nn.Module):
            def __init__(self):
                super().__init__()
                self.conv1 = torch.nn.ConvTranspose2d(1, 1, 1)
                self.conv2 = torch.nn.ConvTranspose2d(1, 64, 1)
                self.relu = torch.nn.ReLU()

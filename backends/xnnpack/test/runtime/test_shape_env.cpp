...
TEST(TestShapeEnv, single_sym_solves) {
  ...
  EXPECT_TRUE(
      env.specialize(
          {specs.data(), specs.size()}, {values.data(), values.size()}) ==
      Error::Ok);
  EXPECT_EQ(env.bounds[0].min, 42u);
  ASSERT_TRUE(env.bounds[0].max.has_value());
  EXPECT_EQ(*env.bounds[0].max, 42u);
}
...
TEST(TestShapeEnv, sym_with_offset) {
  ...
  EXPECT_TRUE(
      env.specialize(
          {specs.data(), specs.size()}, {values.data(), values.size()}) ==
      Error::Ok);
  EXPECT_EQ(env.bounds[0].min, 10u);
  ASSERT_TRUE(env.bounds[0].max.has_value());
  EXPECT_EQ(*env.bounds[0].max, 10u);
}
...
TEST(TestShapeEnv, same_sym_consistent) {
  ...
  EXPECT_TRUE(
      env.specialize(
          {specs.data(), specs.size()}, {values.data(), values.size()}) ==
      Error::Ok);
  EXPECT_EQ(env.bounds[0].min, 5u);
  ASSERT_TRUE(env.bounds[0].max.has_value());
  EXPECT_EQ(*env.bounds[0].max, 5u);
}
...
TEST(TestShapeEnv, multiple_syms) {
  ...
  EXPECT_TRUE(
      env.specialize(
          {specs.data(), specs.size()}, {values.data(), values.size()}) ==
      Error::Ok);
  EXPECT_EQ(env.bounds[0].min, 3u);
  EXPECT_EQ(*env.bounds[0].max, 3u);
  EXPECT_EQ(env.bounds[1].min, 7u);
  EXPECT_EQ(*env.bounds[1].max, 7u);
}
...
TEST(TestShapeEnv, multi_term_skipped) {
  ...
  EXPECT_EQ(env.bounds[0].min, 1u);
  EXPECT_FALSE(env.bounds[0].max.has_value());
  EXPECT_EQ(env.bounds[1].min, 1u);
  EXPECT_FALSE(env.bounds[1].max.has_value());
}
...
TEST(TestShapeEnv, repeated_specialize_resets) {
  ...
  EXPECT_TRUE(
      env.specialize(
          {specs.data(), specs.size()}, {values1.data(), values1.size()}) ==
      Error::Ok);
  EXPECT_EQ(env.bounds[0].min, 42u);

  EXPECT_TRUE(
      env.specialize(
          {specs.data(), specs.size()}, {values2.data(), values2.size()}) ==
      Error::Ok);
  EXPECT_EQ(env.bounds[0].min, 7u);
  EXPECT_EQ(*env.bounds[0].max, 7u);
}

        ...
        if _is_annotated([node]) is False:  # type: ignore[list-item]
            ...
            if bias_node:
                bias_qspec = get_bias_qspec(
                    quantization_config, node, act_node, weight_node
                )
                _annotate_input_qspec_map(
                    node,
                    bias_node,
...
def _annotate_linear_relu(
    ...
    for node in gm.graph.nodes:
        ...
        if isinstance(bias, Node):
            bias_qspec = get_bias_qspec(
                quantization_config, linear_node, input_act, weight
            )
    ...
    for n in gm.graph.nodes:
        ...
        if isinstance(bias, Node):
            input_qspec_map[bias] = get_bias_qspec(
                quantization_config, conv_node, input_act, weight
            )
            partition.append(bias)
    ...
    for match in matches:
        ...
        if bias_node is not None:
            input_qspec_map[bias_node] = get_bias_qspec(
                quantization_config, conv_node, input_node, weight_node
            )
        conv_node.meta["quantization_annotation"] = QuantizationAnnotation(
            input_qspec_map=input_qspec_map,
            _annotated=True,

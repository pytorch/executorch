...
from executorch.exir.backend.utils import WhyNoPartition, is_shape_dynamic
...
class MaxPool2dConfig(GenericNodePartitionerConfig):
    ...
    def check_constraints(self, node: torch.fx.Node, ep: ExportedProgram) -> bool:
        ...
        if stride[0] > kernel_size[0] or stride[1] > kernel_size[1]:
            why(node, reason=f"stride ({stride}) must be less than or equal to kernel size ({kernel_size})")
            return False

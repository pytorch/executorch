...
class GEMMConfig(XNNPartitionerConfig):
    """
    GEMM ops like Convolution, Addmm, Linear, mostly behave in the same way, in which we
    ...
    def _detect_precision(self, node: torch.fx.Node) -> ConfigPrecisionType:
        ...
        if not is_dequant(weight_node):
            return ConfigPrecisionType.FP32

        activation = get_input_node(node, self.act_idx)
        if is_dynamic_q(activation):
    ...
    def _get_weight_deps(
        ...
        if precision == ConfigPrecisionType.FP32:
            ...
            dq_weight = get_input_node(node, self.weight_idx)
            if not is_dequant(dq_weight):
                return False, []
            gemm_deps.append(dq_weight)
            weight = get_input_node(dq_weight, 0)
            if not is_param_node(ep, weight):
                return False, []
            gemm_deps.append(weight)

            if is_per_channel(dq_weight):
                if len(dq_weight.all_input_nodes) <= 1:
                    # Expected channel quantized to have scale/zp nodes
                    return False, []

                gemm_deps.extend(dq_weight.all_input_nodes[1:3])
            return (True, gemm_deps)

    ...
    def _overwrite_precision(self, node: torch.fx.Node):
        ...
        if precision not in self.enabled_precision_types:
            ...
            if self.enabled_precision_types == [ConfigPrecisionType.FP32]:
                ...
                if precision in [ConfigPrecisionType.STATIC_QUANT, ConfigPrecisionType.DYNAMIC_QUANT]:
...
class LinearConfig(GEMMConfig):
    ...
    def _get_weight_deps(
        ...
        overwritten_linear_precision, precision = self._overwrite_precision(node)
        if precision == ConfigPrecisionType.FP32 and (
            self.force_fp32_dynamic_linear or overwritten_linear_precision):
            # if force fp32_dynamic_linear is enabled or overwriting precision to fp32, then we

...
#define REMAP_ID(map, key, out_var)                                  \
  uint32_t out_var;                                                  \
  do {                                                               \
    auto _remap_it = (map).find((key));                              \
    ET_CHECK_OR_RETURN_ERROR(                                        \
        _remap_it != (map).end(),                                    \
        Internal,                                                    \
        "Remapped id not found for key %u",                          \
        static_cast<uint32_t>(key));                                 \
    out_var = _remap_it->second;                                     \
  } while (0)
...
#define XNN_FLAG_MAYBE_PACK_FOR_QB4W_GEMM 0x00000100;
  ...
  xnn_status status = xnn_define_convert(
      subgraph_ptr, cvt_input_id, cvt_output_id, flags);
...
Error defineExpNode(
  ...
  xnn_status status = xnn_define_exp(
      subgraph_ptr, exp_input, exp_output, graph_node->flags());
...
Error definePReLUNode(
  ...
  xnn_status status = xnn_define_prelu(
      subgraph_ptr, prelu_input1, prelu_input2, prelu_output,
      graph_node->flags());
...
ET_NODISCARD Error XNNCompiler::compileModel(
  ...
  ET_CHECK_OR_RETURN_ERROR(
      flatbuffer_graph != nullptr,
      InvalidProgram,
      "Failed to deserialize XNNPACK flatbuffer graph");

  ET_CHECK_OR_RETURN_ERROR(
      flatbuffer_graph->xvalues() != nullptr,
      InvalidProgram,
      "XNNPACK flatbuffer graph has null xvalues");

  ET_CHECK_OR_RETURN_ERROR(
      flatbuffer_graph->xnodes() != nullptr,
      InvalidProgram,
      "XNNPACK flatbuffer graph has null xnodes");
  ...
  uint32_t num_values = flatbuffer_graph->xvalues()
      ? flatbuffer_graph->xvalues()->size()
      : 0;
  ET_CHECK_OR_RETURN_ERROR(

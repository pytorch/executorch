...
def quantized_linear_out_meta(
    ...
    assert weight_zero_point.numel() in [
        1,
        out_channels,
    ], f"weight_zero_point must be per-tensor (1) or per-channel ({out_channels}), got {weight_zero_point.numel()}"
    assert weight_multiplier.numel() in [
        1,
        out_channels,
    ], f"weight_multiplier must be per-tensor (1) or per-channel ({out_channels}), got {weight_multiplier.numel()}"
    assert weight_shift.numel() in [
        1,
        out_channels,
    ], f"weight_shift must be per-tensor (1) or per-channel ({out_channels}), got {weight_shift.numel()}"

...
def test_nhwc_add_channel_broadcast_matches_legacy_layout():
    x = torch.randint(-8, 8, (1, 3, 7, 9), dtype=torch.int8)
    bias = torch.randint(-4, 4, (1, 3, 1, 1), dtype=torch.int8)
    ...
    explicit = _run_add(
        torch.ops.cortex_m.quantized_add_nhwc,
...
def test_nhwc_mul_channel_broadcast_matches_legacy_layout():
    x = torch.randint(-8, 8, (1, 3, 7, 9), dtype=torch.int8)
    bias = torch.randint(-4, 4, (1, 3, 1, 1), dtype=torch.int8)
    ...
    explicit = _run_mul(
        torch.ops.cortex_m.quantized_mul_nhwc,
        x.permute(0, 2, 3, 1).contiguous(),
        bias.permute(0, 2, 3, 1).contiguous(),
    )

...
def test_explicit_layout_rejects_per_channel_activation_qparams():
    graph = torch.fx.Graph()
    node = graph.call_function(exir_ops.edge.aten.convolution.default, args=())
    node.meta["val"] = torch.randn(1, 4, 8, 8)
    per_tensor = SimpleNamespace(per_channel=False)
    per_channel = SimpleNamespace(per_channel=True, axis=1)
    filter_fn = (
        CortexMExplicitLayoutPass(None)
        .op_map[exir_ops.edge.aten.convolution.default]
        .filter_fn
    )
    assert filter_fn is not None

    node.meta["input_qparams"] = {0: per_channel}
    node.meta["output_qparams"] = {0: per_tensor}
    assert not filter_fn(node)

    node.meta["input_qparams"] = {0: per_tensor}
    node.meta["output_qparams"] = {0: per_channel}
    assert not filter_fn(node)

    node.meta["output_qparams"] = {0: per_tensor}
    assert filter_fn(node)
...
def test_channel_mul_is_remapped_inside_explicit_layout_region():
    x = torch.randn(1, 3, 8, 8)
...
def test_explicit_nhwc_channel_add_runs_on_fvp():
    x = torch.linspace(-5, 5, steps=1 * 3 * 7 * 9).reshape(1, 3, 7, 9)
    _run_explicit_layout_on_fvp(
        ConvBiasConv(),
        (x,),
...
def test_explicit_nhwc_channel_mul_runs_on_fvp():
    x = torch.linspace(-5, 5, steps=1 * 3 * 7 * 9).reshape(1, 3, 7, 9)
    _run_explicit_layout_on_fvp(
        ConvMulConv(),
        (x,),

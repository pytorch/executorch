...
test_cases = {
    ...
    # Divisor spans down to ~1 quantization step off its zero point, so the
    # smallest denominator produces a very large quotient — exercises the
    # kernel's float round/clamp path (a naive int32 cast would overflow).
    "tensor_small_denom": McuTestCase(
        CortexMTensorDiv(),
        (ramp_tensor(1, 10, (8,)), ramp_tensor(0.05, 2.5, (8,))),
    ),

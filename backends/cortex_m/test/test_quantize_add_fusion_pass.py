...
from executorch.backends.cortex_m.passes.quantized_add_fusion_pass import (
    QuantizedAddFusionPass,
)
...
class TestQuantizedAddFusionPass(unittest.TestCase):
    """
    Test suite for the QuantizedAddFusionPass which fuses dequantize->add->quantize patterns
    ...
    def _apply_passes(self, edge_program):
        """Apply both ReplaceQuantNodesPass and QuantizedAddFusionPass"""
        # Step 1: Replace quant nodes
        replace_pass = ReplaceQuantNodesPass()
        intermediate_program = edge_program.transform([replace_pass])
    ...
    def test_broadcastable_shapes(self):
        ...
        # Verify numerical equivalence
        # output = transformed_graph(*inputs)
        # expected = BroadcastAddModel()(*inputs)
        # # If output or expected is a tuple, extract the first element

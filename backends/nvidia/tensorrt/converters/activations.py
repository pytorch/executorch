...
def convert_softmax(
    ...
    # Handle negative dim - use get_node_shape for reliability with dynamic shapes
...
def convert_log_softmax(
    ...
    # Handle negative dimension - use get_node_shape for reliability with dynamic shapes
    input_shape = get_node_shape(input_node) or tuple(input_trt.shape)
    ndim = len(input_shape)
    if dim < 0:
        dim = ndim + dim
...
def convert_hardswish(
    ...
    """
    Convert PyTorch hardswish to TensorRT composite layers.

    Hardswish is defined as: x * relu6(x + 3) / 6
    Which is: x * min(max(x + 3, 0), 6) / 6

    This is critical for MobileNetV3 support.

    Args:
        node: FX node representing the hardswish operation.
        network: TensorRT network definition.
        input_map: Mapping from FX nodes to TensorRT tensors.
    ...
    if add_3_layer is None:
        raise RuntimeError(f"Failed to create add_3 layer for hardswish {node.name}")
    add_3_layer.name = f"hardswish_add3_{node.name}"
    add_3_output = add_3_layer.get_output(0)

    # Step 2: ReLU (max(x + 3, 0))
    relu_layer = network.add_activation(add_3_output, trt.ActivationType.RELU)
    if relu_layer is None:
        raise RuntimeError(f"Failed to create relu layer for hardswish {node.name}")
    relu_layer.name = f"hardswish_relu_{node.name}"
    relu_output = relu_layer.get_output(0)

    # Step 3: Min with 6 (clip to 6, implementing relu6)
    const_6 = _create_scalar_constant(
        network, 6.0, f"{node.name}_const6", target_ndims=input_ndims
    )
    min_layer = network.add_elementwise(
        relu_output, const_6, trt.ElementWiseOperation.MIN
    )
    if min_layer is None:
        raise RuntimeError(f"Failed to create min layer for hardswish {node.name}")
    min_layer.name = f"hardswish_min6_{node.name}"
    min_output = min_layer.get_output(0)

    # Step 4: Divide by 6 (reuse const_6)
    div_layer = network.add_elementwise(
        min_output, const_6, trt.ElementWiseOperation.DIV
    )
    if div_layer is None:
        raise RuntimeError(f"Failed to create div layer for hardswish {node.name}")
    div_layer.name = f"hardswish_div6_{node.name}"
    div_output = div_layer.get_output(0)

    # Step 5: Multiply by input (x * relu6(x + 3) / 6)
    mul_layer = network.add_elementwise(
        input_trt, div_output, trt.ElementWiseOperation.PROD
    )
    if mul_layer is None:
        raise RuntimeError(f"Failed to create mul layer for hardswish {node.name}")
    mul_layer.name = f"hardswish_{node.name}"

    logger.debug(f"[TensorRT] Created hardswish composite: {mul_layer.name}")

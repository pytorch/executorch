...
def convert_bmm(
    ...
    """
    if len(node.args) < 2:
        raise ValueError(f"aten.bmm requires at least 2 arguments, got {len(node.args)}")

    lhs_arg = node.args[0]
    rhs_arg = node.args[1]

    if not isinstance(lhs_arg, torch.fx.Node) or lhs_arg not in input_map:
        raise ValueError(f"LHS input not found in input_map for bmm node {node.name}")
    if not isinstance(rhs_arg, torch.fx.Node) or rhs_arg not in input_map:
        raise ValueError(f"RHS input not found in input_map for bmm node {node.name}")
    ...
    if layer is None:
        raise RuntimeError(f"Failed to create MatrixMultiply layer for bmm node {node.name}")

...
DEFINE_bool(warm_up, false, "Pre-run before inference.");
...
int main(int argc, char** argv) {
  ...
  if (input_list.is_open()) {
    ...
    while (std::getline(input_list, file_path)) {
      ...
      std::vector<exec_aten::Tensor> tensors;
      for (int input_index = 0; input_index < num_inputs; ++input_index) {
        exec_aten::Tensor t = method->get_input(input_index).toTensor();
        ...
        fin.close();
        std::memcpy(t.mutable_data_ptr<char>(), input_data.data(), t.nbytes());
        tensors.push_back(t);
      }

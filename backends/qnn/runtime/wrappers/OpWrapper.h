...
#include <executorch/backends/qnn/runtime/wrappers/ParamWrapper.h>
#include <executorch/backends/qnn/runtime/wrappers/QuantizeParamsWrapper.h>
#include <executorch/backends/qnn/runtime/wrappers/ScalarParamWrapper.h>
#include <executorch/backends/qnn/runtime/wrappers/TensorParamWrapper.h>
#include <executorch/backends/qnn/runtime/wrappers/TensorWrapper.h>
...
class OpWrapper final {
 public:
  explicit OpWrapper(std::string name, std::string package_name,
                     std::string op_type)
      : name_(std::move(name)),
  ...
  void AddTensorParam(const std::string& name, Qnn_DataType_t data_type,
                      ...
                      const void* data, bool copy_data = false) {
    ...
    std::shared_ptr<TensorWrapper> tensor_wrapper = CreateTensorWrapper(
        QNN_TENSOR_TYPE_STATIC, data_type, std::move(quantize_param_wrapper),
        rank, dims, kBytes, data, copy_data);
  ...
  const std::string GetOpType() { return op_type_; }

...
#include <executorch/backends/qnn/runtime/python/PyQnnWrapperAdaptor.h>
...
std::unique_ptr<QuantizeParamsWrapper> CreateQuantizationParamWrapper(
    const Qnn_QuantizationEncoding_t& encoding, py::dict& quant_info) {
...
std::shared_ptr<TensorWrapper> CreateTensorWrapper(
  ...
  if (data.size() == 0) {
    return CreateTensorWrapper(tensor_name, tensor_type, data_type,
                               std::move(quantize_param_wrapper), rank,
                               dims.data(), 0, nullptr, copy_data);
  }
  return CreateTensorWrapper(tensor_name, tensor_type, data_type,
                             std::move(quantize_param_wrapper), rank,
                             dims.data(), 0, data.data(), copy_data);
}

PYBIND11_MODULE(PyQnnWrapperAdaptor, m) {
  py::enum_<Qnn_TensorType_t>(m, "Qnn_TensorType_t")
      .value("QNN_TENSOR_TYPE_APP_WRITE",
             Qnn_TensorType_t::QNN_TENSOR_TYPE_APP_WRITE)
      .value("QNN_TENSOR_TYPE_APP_READ",
             Qnn_TensorType_t::QNN_TENSOR_TYPE_APP_READ)
      .value("QNN_TENSOR_TYPE_APP_READWRITE",
      ...
      .value("QNN_TENSOR_TYPE_UNDEFINED",
  ...
  py::enum_<Qnn_QuantizationEncoding_t>(m, "Qnn_QuantizationEncoding_t")
      .value("QNN_QUANTIZATION_ENCODING_UNDEFINED",
             Qnn_QuantizationEncoding_t::QNN_QUANTIZATION_ENCODING_UNDEFINED)
      .value("QNN_QUANTIZATION_ENCODING_SCALE_OFFSET",
             Qnn_QuantizationEncoding_t::QNN_QUANTIZATION_ENCODING_SCALE_OFFSET)
      .value("QNN_QUANTIZATION_ENCODING_AXIS_SCALE_OFFSET",
      ...
      .value("QNN_QUANTIZATION_ENCODING_BW_AXIS_SCALE_OFFSET",
             Qnn_QuantizationEncoding_t::
  ...
  py::class_<PyQnnOpWrapper, std::shared_ptr<PyQnnOpWrapper>>(m,
      ...
      .def("AddInputTensors", &PyQnnOpWrapper::AddInputTensors,
           ...
           py::arg("tensors"))
      .def("AddOutputTensors", &PyQnnOpWrapper::AddOutputTensors,
           "A function which add output tensor wrapper into op wrapper",
           py::arg("tensors"))
      .def("AddTensorParam", &PyQnnOpWrapper::AddTensorParam,
           "A function which add tensor parameter into op wrapper",
           py::arg("name"), py::arg("data_type"), py::arg("rank"),
           py::arg("dims"), py::arg("data"), py::arg("copy_data"))
      .def("AddScalarParam", &PyQnnOpWrapper::AddScalarParam,
           "A function which add scalar parameter into op wrapper",
           py::arg("name"), py::arg("data_type"), py::arg("attrData"))
      .def("GetOpWrapper", &PyQnnOpWrapper::GetOpWrapper,

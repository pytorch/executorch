...
Error QnnBackendCache::GetQnnGraphInfoFromBinary() {
  ...
  if (error != QNN_SUCCESS) {
    QNN_EXECUTORCH_LOG(kLogLevelWarn,
                       "[Qnn Execu Torch] Failed to interpret QNN Context "
                       "binary. Error code %d",
  ...
  if (binaryinfo->version == QNN_SYSTEM_CONTEXT_BINARY_INFO_VERSION_1) {
    ...
    QNN_EXECUTORCH_LOG(kLogLevelWarn,
                       "[Qnn Execu Torch] Unknown QNN BinaryInfo version %d.",
                       binaryinfo->version);
    return Error::Internal;
  }
  ...
  if (num_graphs > 1) {
    QNN_EXECUTORCH_LOG(
        kLogLevelWarn,
        "[Qnn Execu Torch] The context binary contains %lu graphs. But now "
  ...
  if (graph[0].version != QNN_SYSTEM_CONTEXT_GRAPH_INFO_VERSION_1) {
    QNN_EXECUTORCH_LOG(kLogLevelWarn,
                       "[Qnn Execu Torch] Unknown QNN GraphInfo version %d.",
                       graph[0].version);
    return Error::Internal;
  }
...
QnnBackendCache::QnnBackendCache(
  ...
  if (qnn_context_blob_.buffer == nullptr) {
    ...
    QNN_EXECUTORCH_LOG(kLogLevelInfo,
                       "[Qnn Execu Torch] Caching: Caching is in SAVE MODE.");
    return;
  }

  if (qnn_sys_impl_.Load() != Error::Ok) {
    QNN_EXECUTORCH_LOG(kLogLevelError,
                       "[Qnn Execu Torch] Failed to Load QnnSystem "
  ...
  if (error != QNN_SUCCESS) {
    QNN_EXECUTORCH_LOG(
        kLogLevelError,
        "[Qnn Execu Torch] Failed to create Qnn "
  ...
  state_ = DESERIALIZE;
  QNN_EXECUTORCH_LOG(kLogLevelInfo,
                     "[Qnn Execu Torch] Caching: Caching is in RESTORE MODE.");
  Error status = GetQnnGraphInfoFromBinary();
  if (status == Error::Internal) {
    QNN_EXECUTORCH_LOG(
        kLogLevelError,
        "[Qnn Execu Torch] Failed to parse QNN Graph Info. The cache "
...
QnnBackendCache::~QnnBackendCache() {
  ...
  if (sys_context_handle_ != nullptr) {
    ...
    if (error != QNN_SUCCESS) {
      QNN_EXECUTORCH_LOG(
          kLogLevelWarn,
          "[Qnn Execu Torch] Failed to free QNN system context.");
    }

...
class TosaBackend( BackendDetails ):
    ...
    def preprocess(
        ...
        def dbg_fail(  node, tosa_fb ):
            ...
            dbg_node(node)
            raise RuntimeError( f"TOSA Internal Error on node, enable logging for further info" )
        ...
        for node in edge_program.graph.nodes:
            if node.op == "call_function":
                ...
                tosa_fb.currRegion.currBasicBlock.addTensor( outp.name, outp.shape, outp.dtype )
                ...
                if op:
                    ...
                    tosa_fb.addOperator( op, [ inputs[0].name, inputs[1].name ], [ outp.name ], attr )
                else:
                    ...
                    if exir_ops.edge.aten.addmm.default == node.target:
                        ...
                        input_reshape_res = tosa_fb.addIntermediate((1,) + input.shape, outp.dtype)
                        attr_input = ts.TosaSerializerAttribute()
                        attr_input.ReshapeAttribute((1, ) + input.shape)
                        tosa_fb.addOperator( TosaOp.Op().RESHAPE, [input.name], [input_reshape_res.name], attr_input)

                        # Reshape weight tensor
                        weight_reshape_res = tosa_fb.addIntermediate((1, ) + weight.shape, outp.dtype)
                        attr_weight = ts.TosaSerializerAttribute()
                        attr_weight.ReshapeAttribute((1, ) + weight.shape)
                        tosa_fb.addOperator( TosaOp.Op().RESHAPE, [weight.name], [weight_reshape_res.name], attr_weight)

                        # Reshape bias tensor
                        bias_reshape_res = tosa_fb.addIntermediate((1, 1,) + bias.shape, outp.dtype)
                        attr_bias = ts.TosaSerializerAttribute()
                        attr_bias.ReshapeAttribute((1, 1, ) + bias.shape)
                        tosa_fb.addOperator( TosaOp.Op().RESHAPE, [bias.name], [bias_reshape_res.name], attr_bias )
                        ...
                        # Add MatMulOp
                        tosa_fb.addOperator( TosaOp.Op().MATMUL,
                        ...
                        tosa_fb.addOperator( TosaOp.Op().ADD, [ bias_reshape_res.name, mm_res.name ], [ add_res.name ], None )

                        # Reshape final result to original shape
                        attr_out = ts.TosaSerializerAttribute()
                        attr_out.ReshapeAttribute(outp.shape)
                        tosa_fb.addOperator(TosaOp.Op().RESHAPE, [add_res.name], [outp.name], attr_out)
                    elif exir_ops.edge.aten.permute_copy.default == node.target:
                        attr = ts.TosaSerializerAttribute()
                        attr.TransposeAttribute( inputs[1].special )
                        tosa_fb.addOperator( TosaOp.Op().TRANSPOSE, [ inputs[0].name ], [ outp.name ], attr )
                    elif exir_ops.edge.aten.hardtanh.default == node.target:
                        attr = ts.TosaSerializerAttribute()
                        attr.ClampAttribute(tosa_fb.builder, int(inputs[1].threshold), int(inputs[2].threshold), inputs[1].threshold, inputs[2].threshold)
                        tosa_fb.addOperator( TosaOp.Op().CLAMP, [ inputs[0].name ], [ outp.name ], attr)
                        ...
                        input_reshaped = tosa_fb.addIntermediate(input_shape_NHWC, outp.dtype)
                        tosa_fb.addOperator(TosaOp.Op().RESHAPE, [inputs[0].name], [input_reshaped.name], attr_input_reshape)
                        ...
                        conv2d_res = tosa_fb.addIntermediate(out_shape_TOSA_CONV2D, outp.dtype)
                        tosa_fb.addOperator(TosaOp.Op().CONV2D, [input_reshaped.name, inputs[1].name, inputs[2].name], [ conv2d_res.name ], attr)
                        ...
                        tosa_fb.addOperator(TosaOp.Op().TRANSPOSE, [conv2d_res.name], [outp.name], attr_output_transpose)
                        ...
                        tosa_fb.addOperator( TosaOp.Op().RECIPROCAL, [ inputs[1].name ], [ recip.name ] )

                        attr = ts.TosaSerializerAttribute()
                        attr.MulAttribute( 0 )
                        tosa_fb.addOperator( TosaOp.Op().MUL, [ inputs[0].name, recip.name ], [ outp.name ], attr )
                    elif exir_ops.edge.aten._native_batch_norm_legit_no_training.default == node.target:
                        ...
                        assert 0.1 == momentum.threshold, "Expected 0.1 momentum, not currently encoded into TOSA"
                        ...
                        mean_reshaped_res = tosa_fb.addIntermediate((1, ) + running_mean.shape + (1, 1, ), input_dtype)
                        attr_mean = ts.TosaSerializerAttribute()
                        attr_mean.ReshapeAttribute((1,) + running_mean.shape + (1, 1, ))
                        tosa_fb.addOperator( TosaOp.Op().RESHAPE, [running_mean.name], [mean_reshaped_res.name], attr_mean )

                        # Subtract mean
                        int1 = tosa_fb.addIntermediate( input_shape, input_dtype )
                        tosa_fb.addOperator( TosaOp.Op().SUB, [ activations.name, mean_reshaped_res.name], [ int1.name ] )
                        # Adding eplison to variance
                        epsilon_const = tosa_fb.addConst( [1], input_dtype, [epsilon.threshold])
                        int2 = tosa_fb.addIntermediate( running_var.shape, input_dtype )
                        tosa_fb.addOperator( TosaOp.Op().ADD, [ running_var.name, epsilon_const.name ], [ int2.name ] )
                        ...
                        var_reshaped_res = tosa_fb.addIntermediate((1, ) + running_var.shape + (1, 1, ), input_dtype)
                        attr_var = ts.TosaSerializerAttribute()
                        attr_var.ReshapeAttribute((1,) + running_var.shape + (1, 1, ))
                        tosa_fb.addOperator( TosaOp.Op().RESHAPE, [int3.name], [var_reshaped_res.name], attr_var )

                        # Multiple shifted activations with reciprocal variance
                        # int4 = tosa_fb.addIntermediate( input_shape, input_dtype )
                        tosa_fb.addOperator( TosaOp.Op().MUL, [ int1.name, var_reshaped_res.name ], [ outp.name ], attr_torch_to_tosa( TosaOp.Op().MUL, node ))
                        ...
                        attr = ts.TosaSerializerAttribute()
                        attr.PoolAttribute(kernel=kernel_size_list, stride=stride_size_list, pad=pad_size_list,
                                            input_zp=0, output_zp=0, accum_dtype=8) #FP32 accum type
                        ...
                        transeposed_input_shape = [input_tensor.shape[i] for i in NHWC_Order]
                        input_transposed = tosa_fb.addIntermediate(transeposed_input_shape, outp.dtype)
                        tosa_fb.addOperator(TosaOp.Op().TRANSPOSE, [input_tensor.name], [input_transposed.name], attr_input_transpose)

                        avg_pool2d_res_shape = [outp.shape[i] for i in NHWC_Order]
                        avg_pool2d_res = tosa_fb.addIntermediate(avg_pool2d_res_shape, outp.dtype)
                        tosa_fb.addOperator( TosaOp.Op().AVG_POOL2D, [ input_transposed.name ], [ avg_pool2d_res.name ], attr )

                        # TOSA is [N, H, W, C], Transpose back to Torch's [N, C, H, W]
                        NCHW_Order = [0, 3, 1, 2]
                        attr_output_transpose = ts.TosaSerializerAttribute()
                        attr_output_transpose.TransposeAttribute(NCHW_Order)
                        tosa_fb.addOperator(TosaOp.Op().TRANSPOSE, [avg_pool2d_res.name], [outp.name], attr_output_transpose)
                ...
                if out in edge_program.graph_signature.inputs_to_parameters:
                    ...
                    tosa_fb.addConst( inputs[0].shape, inputs[0].dtype, weight_values, name=out )
                    ...
                    # Input argument
                    tensor = ts.TosaSerializerTensor( inputs[0].name, inputs[0].shape, inputs[0].dtype,
                                                      data=None, placeholderFilename=inputs[0].name+".npy" )
                    tosa_fb.addInputTensor( tensor )
                continue

            elif node.op == "output":
                for output in node.args[0]:
                    tosa_fb.addOutputTensor( tosa_fb.currRegion.currBasicBlock.tensors[output.name] )
                continue

...
def tosa_run_test( op, profile = TosaProfile.MI ):
    #
    ...
    model = TestList[op]
    if model.inputs.get(profile) == None:
        print("\033[96m" + "Skipping test as no inputs for this TOSA profile"+ "\033[0m")
        return
    ...
    cmd_flatc = "flatc" + " -o " + TOSA_OUT_PATH +  " --raw-binary -t ./backends/tosa/serialization_lib/schema/tosa.fbs -- ./" + TOSA_OUT_PATH + "/output.tosa"
    ...
    if np.allclose(tosa_output, torch_output, 1e-1, equal_nan=True) == True:
        print('\033[92m' + 'Torch and Tosa Reference results are matching for operator: ' + op + '\033[0m')
    else:
    ...
    if profile == TosaProfile.BI:
        ...
        print("\033[96m" + "Skipping Vela test on non-BI profile.".format(op) + "\033[0m")

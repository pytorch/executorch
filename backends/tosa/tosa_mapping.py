...
def map_dtype( data_type ):
    unsupported = ( torch.float64, torch.double, torch.complex64, torch.cfloat, torch.complex128, torch.cdouble,
                    torch.uint8, torch.int64, torch.long )

    dmap = { torch.float32: ts.DType.FP32,
    ...
    assert rtype != None, "Unknown type"
    return rtype
...
class TosaArg():
    ...
    def process_node( self, argument ):
        ...
        assert argument.meta.get('val') != None

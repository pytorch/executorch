...
Instruction load_instruction(
  ...
  switch (op_type) {
    ...
    case mlx_delegate::OpNode_BitwiseOrNode: {
      auto fb = fb_instr->op_as_BitwiseOrNode();
      if (!fb) {{
        throw std::runtime_error("FlatBuffer op_type/payload mismatch for {class_name}");
      }}
      BitwiseOrNode node;
    ...
    case mlx_delegate::OpNode_IfNode: {
      auto fb = fb_instr->op_as_IfNode();
      if (!fb) {{
        throw std::runtime_error("FlatBuffer op_type/payload mismatch for {class_name}");
      }}
      IfNode node;
      node.cond = convert_int_or_vid(fb->cond());
      node.then_chain_idx = fb->then_chain_idx();
      node.else_chain_idx = fb->else_chain_idx();
      instr.op = OpCode::IF;
      instr.node = std::move(node);
      break;
    }

    case mlx_delegate::OpNode_RandomBitsNode: {
      auto fb = fb_instr->op_as_RandomBitsNode();
      if (!fb) {{
        throw std::runtime_error("FlatBuffer op_type/payload mismatch for {class_name}");
      }}
      RandomBitsNode node;
    ...
    case mlx_delegate::OpNode_MetalKernelNode: {
      ...
      node.source = strpool.intern(fb->source());
      if (fb->inputs()) {
      ...
      node.header = strpool.intern(fb->header());
      if (fb->input_names()) {

...
def inspect_pte_file(pte_path: Union[str, Path]) -> Dict:
    ...
    """
    from executorch.backends.mlx.pte_inspector import (
...
def print_mlx_graph_summary(pte_path: Union[str, Path]) -> None:
    ...
    """
    from executorch.backends.mlx.pte_inspector import show_mlx_instructions
...
def find_build_dir():
    ...
    for candidate in candidates:
        runner_path = candidate / "backends" / "mlx" / "test" / "op_test_runner"
...
def find_op_test_runner() -> Path:
    ...
    candidates = [
        ...
        / "test"
        / "op_test_runner",
        executorch_root / "cmake-out" / "backends" / "mlx" / "test" / "op_test_runner",
        executorch_root / "build" / "backends" / "mlx" / "test" / "op_test_runner",
    ]

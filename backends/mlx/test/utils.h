...
struct CacheArgs {
  ...
  ::executorch::extension::llm::cache::CacheConfig config;
};

inline CacheArgs flat_config(
    int capacity,
    int n_layers,
    int n_kv_heads,
    int head_dim,
  ...
  CacheArgs args{
      cache::CacheGeometry{std::vector<cache::LayerGeometry>(
          n_layers,
          cache::LayerGeometry{
              cache::LayerPolicy{cache::LayerPolicy::Kind::Flat, 0},
              n_kv_heads,
              head_dim})},
      cache::CacheConfig{capacity, kv_dtype}};
  if (initial_capacity) {
    args.config.initial_capacity = *initial_capacity;
  }
  return args;
}

inline CacheArgs ring_config(
    int capacity,
    int window,
    int max_write,
    int n_kv_heads,
  ...
  args.geometry.layers[0].policy =
      ::executorch::extension::llm::cache::LayerPolicy{
          ::executorch::extension::llm::cache::LayerPolicy::Kind::Ring, window};
  args.config.max_write = max_write;
  return args;
}

template <class T>
T make_cache(CacheArgs args) {
  return T(args.geometry, args.config);
}

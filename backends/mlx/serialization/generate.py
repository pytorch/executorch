...
def _file_header(comment: str, description: str = "") -> List[str]:
    ...
    lines = [
        ...
        f"{comment}",
        f"{comment} Source:    backends/mlx/serialization/schema.fbs",
        f"{comment} Generator: backends/mlx/serialization/generate.py",
        f"{comment}",
        f"{comment} To regenerate, run from the executorch root:",
        f"{comment}     python backends/mlx/serialization/generate.py",
        f"{comment}",
        sep,
    ]
    if description:
...
def _generate_op_builder_method(table: FBSTable) -> str:
    ...
    lines = [
        ...
        f"        from executorch.backends.mlx.serialization._generated.mlx_delegate import {class_name} as {fb_module_name}",
...
"""
        ...
        for export in sorted(exports):
            init_content += f"from executorch.backends.mlx.serialization._generated.mlx_delegate.{export} import {export}\n"

...
TEST_F(TypedSettersTest, CombinedDispatchProducesCorrectOutput) {
  ...
  float* input = static_cast<float*>(stream_->allocator().alloc(numel * sizeof(float)));
  float* output = static_cast<float*>(stream_->allocator().alloc(numel * sizeof(float)));
  ...
  stream_->recorder().beginDispatch(kernel_)
      .setInput(0, input, numel * sizeof(float))
      .setOutput(1, output, numel * sizeof(float))
      .setBytes<float>(2, scale)
      .setVectorBytes(3, bias_array)
      .setBytes<uint32_t>(4, numel)
      .setBytes<uint32_t>(5, bias_len)
      .run(uvec3(1, 1, 1), uvec3(64, 1, 1));
  stream_->sync();
  ...
  stream_->allocator().free(input);
  stream_->allocator().free(output);
}
...
TEST_F(TypedSettersTest, MultipleDispatchesOnSameStream) {
  for (uint32_t numel : {16u, 32u, 64u, 128u}) {
    ...
    stream_->recorder().beginDispatch(kernel_)
        ...
        .setOutput(1, output, numel * sizeof(float))
        .setBytes<float>(2, scale)
        .setVectorBytes(3, bias)
        .setBytes<uint32_t>(4, numel)
        .setBytes<uint32_t>(5, bias_len)
        .run(uvec3(1, 1, 1), uvec3(tg, 1, 1));
    stream_->sync();
...
TEST_F(HazardTrackingTest, IndependentDispatchesSkipBarrier) {
  ...
  float* in_a = static_cast<float*>(stream_->allocator().alloc(numel * sizeof(float)));
  float* out_a = static_cast<float*>(stream_->allocator().alloc(numel * sizeof(float)));
  float* in_b = static_cast<float*>(stream_->allocator().alloc(numel * sizeof(float)));
  float* out_b = static_cast<float*>(stream_->allocator().alloc(numel * sizeof(float)));
  for (uint32_t i = 0; i < numel; ++i) {
    in_a[i] = 1.0f; in_b[i] = 2.0f;
  }

  uint64_t barriers_before = stream_->recorder().barrierStats().inserted;
  uint64_t skipped_before = stream_->recorder().barrierStats().skipped;

  runDispatch(stream_.get(), kernel_, in_a, out_a, numel);
  runDispatch(stream_.get(), kernel_, in_b, out_b, numel);
  stream_->sync();

  uint64_t barriers_inserted = stream_->recorder().barrierStats().inserted - barriers_before;
  uint64_t barriers_skipped = stream_->recorder().barrierStats().skipped - skipped_before;
  ...
  stream_->allocator().free(in_a); stream_->allocator().free(out_a);
  stream_->allocator().free(in_b); stream_->allocator().free(out_b);
}
...
TEST_F(HazardTrackingTest, RAWHazardInsertsBarrier) {
  constexpr uint32_t numel = 16;
  float* a = static_cast<float*>(stream_->allocator().alloc(numel * sizeof(float)));
  float* b = static_cast<float*>(stream_->allocator().alloc(numel * sizeof(float)));
  float* c = static_cast<float*>(stream_->allocator().alloc(numel * sizeof(float)));
  ...
  stream_->allocator().free(a); stream_->allocator().free(b); stream_->allocator().free(c);
}
...
TEST_F(HazardTrackingTest, WAWHazardInsertsBarrier) {
  ...
  float* a = static_cast<float*>(stream_->allocator().alloc(numel * sizeof(float)));
  float* b = static_cast<float*>(stream_->allocator().alloc(numel * sizeof(float)));
  float* x = static_cast<float*>(stream_->allocator().alloc(numel * sizeof(float)));
  for (uint32_t i = 0; i < numel; ++i) {
    a[i] = 7.0f; b[i] = 11.0f;
  }
  ...
  stream_->allocator().free(a); stream_->allocator().free(b); stream_->allocator().free(x);
}
...
TEST_F(HazardTrackingTest, FlushBoundaryResetsHazardTracker) {
  ...
  stream_->allocator().free(a); stream_->allocator().free(b);
}

...
TEST_F(StreamLifecycleTest, AllocReturnsNonNullPointer) {
  MetalStream stream;
  void* p = stream.allocator().alloc(1024);
  EXPECT_NE(p, nullptr);
  stream.allocator().free(p);
}
...
TEST_F(StreamLifecycleTest, ManyAllocsFreedAtEndDoesNotLeak) {
  ...
  std::vector<void*> ptrs;
  for (int i = 0; i < 50; ++i) {
    void* p = stream.allocator().alloc(256 + i * 16);
    ASSERT_NE(p, nullptr);
    ptrs.push_back(p);
  }
  for (void* p : ptrs) {
...
TEST_F(StreamLifecycleTest, BuffersOutliveStream) {
  ...
  // test (no leak detector); it just verifies no crash on destruct.
  {
    MetalStream stream;
    for (int i = 0; i < 20; ++i) {
      void* p = stream.allocator().alloc(128);
      ASSERT_NE(p, nullptr);
    }
...
TEST_F(StreamLifecycleTest, RegisterExternalBufferReturnsTrue) {
  ...
  bool ok = stream.allocator().registerExternalBuffer(data.data(), data.size() * sizeof(int32_t));
  EXPECT_TRUE(ok);
  // Lookup should succeed.
  id<MTLBuffer> b = stream.allocator().bufferMtlForPtr(data.data(), data.size() * sizeof(int32_t));
  EXPECT_NE(b, nil);
}
...
TEST_F(StreamLifecycleTest, RegisterExternalBufferTwiceIsIdempotent) {
  ...
  EXPECT_TRUE(stream.allocator().registerExternalBuffer(data.data(), data.size() * sizeof(int32_t)));
...
TEST_F(StreamLifecycleTest, BufferForPtrAutoRegisters) {
  ...
  id<MTLBuffer> b = stream.allocator().bufferMtlForPtr(data.data(), data.size() * sizeof(float));
  EXPECT_NE(b, nil);
}
...
TEST_F(StreamLifecycleTest, AllocWaitFreeRoundtrip) {
  ...
  MetalStream stream;
  void* p = stream.allocator().alloc(2048);
  ASSERT_NE(p, nullptr);
  stream.wait();
  stream.allocator().free(p);
}

// Lock in the arch-suffix → flush-interval mapping. Future Apple-silicon
// naming changes that drop one of these suffixes regress visibly here.
TEST(DefaultFlushIntervalForArchSuffixTest, KnownSuffixesMapToTunedIntervals) {
  using executorch::backends::metal_v2::defaultFlushIntervalForArchSuffix;
  EXPECT_EQ(defaultFlushIntervalForArchSuffix('p'), 20)
      << "iPhone (suffix 'p') should be conservative";
  EXPECT_EQ(defaultFlushIntervalForArchSuffix('g'), 40)
      << "Base/Pro (suffix 'g')";
  EXPECT_EQ(defaultFlushIntervalForArchSuffix('s'), 50)
      << "Max (suffix 's')";
  EXPECT_EQ(defaultFlushIntervalForArchSuffix('d'), 50)
      << "Ultra (suffix 'd')";
}

TEST(DefaultFlushIntervalForArchSuffixTest, UnknownSuffixFallsBackToBase) {
  using executorch::backends::metal_v2::defaultFlushIntervalForArchSuffix;
  // Any non-recognized suffix falls back to the Base/Pro interval (40).
  EXPECT_EQ(defaultFlushIntervalForArchSuffix('x'), 40);
  EXPECT_EQ(defaultFlushIntervalForArchSuffix('\0'), 40);
  EXPECT_EQ(defaultFlushIntervalForArchSuffix('Z'), 40);
}

...
//                         MTLComputeCommandEncoder (the default path).
...
class IComputeBackend {
 public:
  ...
  //===--------------------------------------------------------------------===//
  // Hazard-aware dispatch (used by typed-setter path on MetalStream).
  ...
  //===--------------------------------------------------------------------===//
  // Completion handler hook.
  //
  // Register a callback to fire when the NEXT commit()'s CB completes.
  // The handler runs on a Metal driver thread (MTL3:
  // [cb addCompletedHandler:], MTL4: MTL4CommitOptions::
  // addFeedbackHandler:); callers MUST be thread-safe.
  //===--------------------------------------------------------------------===//

  virtual void addCompletionHandler(std::function<void()> handler) = 0;

  //===--------------------------------------------------------------------===//
  // Legacy CB seam — returns a non-null ILegacyCommandBufferProvider* iff
  // this backend implements the legacy MTLCommandBuffer model (MTL3 only).
  // MTL4 backends return nullptr. Lets callers (MetalCommandRecorder,
  // MpsInterop) reach the MTL3-specific surface without dynamic_cast.
  //===--------------------------------------------------------------------===//
  virtual class ILegacyCommandBufferProvider* legacyCommandBufferProvider() {
    return nullptr;
  }
};
...
// (and MPSGraph + MTL4 are build-flag mutually exclusive — see MpsInterop.h).
//
// Under MTL4 builds (ET_METAL_USE_MPSGRAPH=0) MpsInterop itself is compiled
// out and nobody references this interface.
//===----------------------------------------------------------------------===//

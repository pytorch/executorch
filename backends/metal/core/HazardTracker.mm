...
bool HazardTracker::needsBarrierForPending() const {
  ...
  // Same check for outputs against writers — any overlap is a WAW hazard.
  // Also check outputs against readers — overlap is a WAR hazard
  // (subsequent write would race a prior read on MTL4 concurrent dispatch).
  for (const auto& out : pending_outputs_) {
    ...
    auto rit = readers_.find(out.parent_mtl);
    if (rit != readers_.end() && overlapsAny(rit->second, out.lo, out.hi)) {
      return true;
    }
  }
  return false;
...
void HazardTracker::commitPending(bool barrier_inserted) {
  // Promote pending outputs into writers_ (merged per parent), pending
  // inputs into readers_ (merged per parent). Both maps are needed:
  //   - writers_ catches RAW (next read of a written range) and WAW
  //     (next write of a written range).
  //   - readers_ catches WAR (next write of a read range) — required on
  //     MTL4 because dispatches in the same encoder run concurrently.
  for (const auto& out : pending_outputs_) {
  ...
  for (const auto& in : pending_inputs_) {
    auto& bucket = readers_[in.parent_mtl];
    if (bucket.empty()) {
      bucket.push_back(in);
    } else {
      insertMerged(bucket, in.lo, in.hi);
    }
  }
...
std::vector<HazardTracker::Range> HazardTracker::writersSnapshot() const {
  ...
  // dump. Only called when debug logging is active.

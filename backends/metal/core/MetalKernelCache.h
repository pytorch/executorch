...
class MetalKernelCache {
 public:
  ...
  // it. No mutator should be called on the
  // returned pointer.
  ...
  // Insert `lib` into the library sub-store under `key`. ARC-managed:
  // the cache retains internally on insert; the caller's reference is
  // independent (caller still owns whatever +1 they had). On lost-race
  // duplicate the cache makes no insertion and the caller's reference
  // is unchanged.
  void insertLibrary(const std::string& key, id<MTLLibrary> lib);
  // Insert-or-find: returns the cache's stored library for `key`,
  ...
  // Insert `pso` into the PSO sub-store under `key`. Same ARC-managed
  // contract as insertLibrary.

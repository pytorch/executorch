...
class MetalMTL3Backend : public IComputeBackend,
 ...
 public:
  ...
  // ILegacyCommandBufferProvider hook — return self.
  ILegacyCommandBufferProvider* legacyCommandBufferProvider() override {
    return this;
  }
 ...
 private:
  ...
  // time via [cb addCompletedHandler:^(...){ run all }].
  // Cleared after attach. Per the per-CB unpin contract.
  std::vector<std::function<void()>> pendingCompletionHandlers_;
};

// Test-only: drop the cached useConcurrentEncoder() probe so the next
// call re-reads ET_METAL_USE_METAL3_CONCURRENT.

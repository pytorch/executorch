...
// Reuses MTLBuffer instances across allocator.alloc()/free() cycles to avoid
...
//
// Residency-quiet:
//   No method on MetalBufferPool calls into ResidencyManager. Pool
//   eviction (evictOldest / clear) just drops the strong MTLBuffer
//   ref; the residency set's addAllocation: retains the buffer for as
//   long as it's a member. Pool prewarm seeds buffers into the cache
//   without pinning. Residency lifecycle is driven exclusively by per-CB
//   binds (MetalCommandRecorder) and long-lived registrations (Heap
//   arena, External Permanent).
//===----------------------------------------------------------------------===//
...
public:
  ...
  /// Return buffer to pool. Buffers larger than maxBytes()/2 are dropped
  /// instead of cached: caching one giant buffer would crowd out smaller
  /// reusable entries (anti-monopoly heuristic). Callers that release
  /// such buffers will pay a fresh newBufferWithLength: cost on the next
  /// equivalent acquire().

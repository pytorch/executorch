...
inline std::string
buildQmvFastName(mlx_jit::JitDtype dtype, int group_size, int bits) {
  std::ostringstream s;
  s << "affine_qmv_fast_" << mlx_jit::typeToTemplateArg(dtype) << "_gs"
    << group_size << "_b" << bits << "_batch0";
  return s.str();
}
inline std::string
buildQmvName(mlx_jit::JitDtype dtype, int group_size, int bits) {
  std::ostringstream s;
  s << "affine_qmv_" << mlx_jit::typeToTemplateArg(dtype) << "_gs" << group_size
    << "_b" << bits << "_batch0";
  return s.str();
}
inline std::string
buildQmvQuadName(mlx_jit::JitDtype dtype, int group_size, int bits, int D) {
  std::ostringstream s;
  s << "affine_qmv_quad_" << mlx_jit::typeToTemplateArg(dtype) << "_gs"
    << group_size << "_b" << bits << "_d" << D << "_batch0";
  return s.str();
}
inline std::string buildQmmTName(
    mlx_jit::JitDtype dtype,
    int group_size,
    int bits,
    bool aligned_N) {
  std::ostringstream s;
  s << "affine_qmm_t_" << mlx_jit::typeToTemplateArg(dtype) << "_gs"
    << group_size << "_b" << bits << "_alN" << (aligned_N ? '1' : '0')
    << "_batch0";
  return s.str();
}
inline std::string buildQmmTSplitKName(
    mlx_jit::JitDtype dtype,
    int group_size,
    int bits,
    bool aligned_N) {
  std::ostringstream s;
  s << "affine_qmm_t_splitk_" << mlx_jit::typeToTemplateArg(dtype) << "_gs"
    << group_size << "_b" << bits << "_alN" << (aligned_N ? '1' : '0');
  return s.str();
}
inline std::string buildQmmTNaxName(
    mlx_jit::JitDtype dtype,
    int group_size,
    int bits,
    bool aligned_N) {
  std::ostringstream s;
  s << "affine_qmm_t_nax_" << mlx_jit::typeToTemplateArg(dtype) << "_gs"
    << group_size << "_b" << bits << "_alN" << (aligned_N ? '1' : '0')
...
inline std::string buildSplitKAccumName(mlx_jit::JitDtype dtype) {
  std::ostringstream s;
  s << "gemm_splitk_accum_" << mlx_jit::typeToTemplateArg(dtype) << "_to_"
    << mlx_jit::typeToTemplateArg(dtype);
  return s.str();
}
...
inline void dispatchAffineQmmTViaMlxJit(
  ...
  stream->recorder()
      ...
      .run(
          uvec3{uint32_t((N + BN - 1) / BN), uint32_t((M + BM - 1) / BM), 1u},
...
inline void dispatchAffineQmmTNaxViaMlxJit(
  ...
  ET_CHECK_MSG(
      K % 64 == 0,
      "dispatchAffineQmmTNaxViaMlxJit requires K %% 64 == 0, got K=%d",
      K);
...
//   3. Allocate intermediate [split_k, M, N] in `dtype` via
...
inline bool dispatchAffineQmmTSplitKViaMlxJit(
    ...
    executorch::aten::ScalarType dtype) {
  ET_CHECK_MSG(
      M > 1, "dispatchAffineQmmTSplitKViaMlxJit requires M>1, got %d", M);
  ...
  const std::string kname =
      buildQmmTSplitKName(jdt, group_size, bits, aligned_N);
  auto pso =
      mlx_jit::shared(stream->compiler())
  ...
  stream->recorder()
      ...
      .run(
          uvec3{uint32_t(n_tiles), uint32_t(m_tiles), uint32_t(split_k)},
          uvec3{32u, 2u, 2u});
  ...
  stream->recorder()
      ...
      .run(uvec3{uint32_t((N + 31) / 32), uint32_t(M), 1u}, uvec3{32u, 1u, 1u});

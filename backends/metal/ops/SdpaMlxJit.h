...
inline void dispatchSdpaVectorViaMlxJit(
  ...
  d.setInput(0, Q.const_data_ptr(), Q.nbytes());
  d.setInput(1, K.const_data_ptr(), K.nbytes());
  d.setInput(2, V.const_data_ptr(), V.nbytes());
  d.setOutput(3, O.mutable_data_ptr(), O.nbytes());
  d.setBytes<int32_t>(4, gqa_factor);
  d.setBytes<int32_t>(5, N);
  // Strides are size_t (8B) per kernel signature.
  d.setBytes<int64_t>(6, kvHeadStride(K));
  d.setBytes<int64_t>(7, seqStride(K));
  d.setBytes<int64_t>(8, kvHeadStride(V));
  d.setBytes<int64_t>(9, seqStride(V));
  d.setBytes<float>(10, scale);
  if (has_mask) {
...
inline void dispatchSdpaVector2PassViaMlxJit(
  ...
  d1.setInput(1, K.const_data_ptr(), K.nbytes());
  d1.setInput(2, V.const_data_ptr(), V.nbytes());
  d1.setOutput(3, intermediate_ptr, intermediate_bytes);
  d1.setOutput(4, sums_ptr, partials_bytes);
  d1.setOutput(5, maxs_ptr, partials_bytes);
  d1.setBytes<int32_t>(7, N);
  d1.setBytes<int64_t>(8, kvHeadStride(K));
  d1.setBytes<int64_t>(9, seqStride(K));
  d1.setBytes<int64_t>(10, kvHeadStride(V));
  d1.setBytes<int64_t>(11, seqStride(V));
  d1.setBytes<float>(12, scale);
  if (has_mask) {
  ...
  stream->allocator().free(intermediate_ptr);
  stream->allocator().free(sums_ptr);
  stream->allocator().free(maxs_ptr);
}

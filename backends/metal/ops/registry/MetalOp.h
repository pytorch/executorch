...
class MetalOp {
 public:
  ...
  /// Per-op dtype support. Pure virtual — every op must declare its
  /// supported dtypes explicitly. There is intentionally no default:
  /// a silent fp32-only fallback would hide ops that should accept
  /// fp16/bf16 (or quantized integer types) at the type-system layer.
  virtual bool supports(ScalarType dtype) const = 0;

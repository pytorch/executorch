...
inline bool isRowContiguous(const Tensor& t) {
  ...
  for (int i = static_cast<int>(sizes.size()) - 1; i >= 0; --i) {
    if (sizes[i] == 1)
      continue; // size-1 dim's stride is irrelevant
...
inline ElementwiseVariant classifyBinary(const Tensor& a, const Tensor& b) {
  ...
  if (a_scalar && b_scalar)
    return ElementwiseVariant::ScalarScalar;
  if (a_scalar && isRowContiguous(b))
    return ElementwiseVariant::ScalarVector;
  if (b_scalar && isRowContiguous(a))
    return ElementwiseVariant::VectorScalar;

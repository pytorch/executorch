...
// The `kernelName` strings follow MLX's exact naming conventions
// (matmul.cpp:216-254) so the on-disk shader cache layout matches MLX's;
// this also means the [[host_name(...)]] in the JIT instantiation lines
// up with the lookup. PSO disambiguation by FC values is automatic via
// `fcs.fingerprint()` in MetalKernelCompiler::getOrCompilePsoFromSource —
// we don't need MLX's separate `hash_name` parameter.
//===----------------------------------------------------------------------===//
...
class KernelLoader {
 public:
  ...
  // `kernelName` is the library cache key + the [[host_name]] in the single
  // emitted template instantiation; e.g.
  ...
  // PSO cache disambiguation by FC values is automatic via fcs.fingerprint()
  // inside MetalKernelCompiler::getOrCompilePsoFromSource — we don't take
  // MLX's `hash_name` parameter (their string-keyed PSO cache needs it; ours
  // derives the equivalent suffix from the FC tuple itself).
  ...
  // Mirrors MLX's get_steel_gemm_splitk_nax_kernel. align_K is a runtime
  // branch in the kernel, NOT a template arg or FC, so it must NOT enter
  // the cache key (MLX includes it in their hash_name and over-caches; we
  // correctly dedup).
  ...
  //                    bool_mask, float_mask, has_sinks, blocks (int slot 26).
  // `blocks` is an int FC unique to pass 1 — pass 2 reads `blocks` from
  // buffer 4 instead. PSO cache key automatically includes `blocks` via
  // fcs.fingerprint() (the int section renders as "#26=<N>,").
  ...
  //   No FCs in v0 (always non-batched; alignment baked into `_fast`
  //   selection).
  ...
  //   Template signature: affine_qmv_quad<T, group_size, bits, D,
  //   batched=false>
  // Used when M=1 AND bits==4 AND D ∈ {64, 128} (per MLX's instantiation
  // table).
  ...
  // ---- Mirrors MLX's affine_qmm_t_nax kernel template
  // (quantized_nax.h:1194+).
  //   Template signature:

...
{
    if (executorch::runtime::isFloatingType(a_type) &&  executorch::runtime::isFloatingType(b_type))
    {
...
Tensor& div_out(KernelRuntimeContext& ctx,
                const Tensor& a,
                const Tensor& b,
                Tensor& out)
{
...
Tensor& div_out_mode(KernelRuntimeContext& ctx,
                     const Tensor& a,
                     const Tensor& b,
                     ...
                     Tensor& out)
{
    ...
    // Check Dim Order
    ET_KERNEL_CHECK(
      ctx, executorch::runtime::tensors_have_same_dim_order(a, b, out),
      InvalidArgument, out);
    ...
    {    
        ET_SWITCH_REAL_TYPES(compute_type, ctx, op_name, CTYPE_COMPUTE, [&]() {
            ...
            const CTYPE_COMPUTE val_a, const CTYPE_COMPUTE val_b) {
            if ( executorch::runtime::is_integral_type<CTYPE_COMPUTE, /*includeBool=*/true>::value) {
            if (val_b == 0) {
            ...
            // it must be floor.
            value = torch::executor::native::utils::floor_divide(val_a, val_b);
            }
            return value;
            },
            ctx,
...
Tensor& div_scalar_out(KernelRuntimeContext& ctx,
                       const Tensor& a,
                       const Scalar& b,
                       Tensor& out)
{
...
Tensor& div_scalar_mode_out(KernelRuntimeContext& ctx,
                            const Tensor& a,
                            const Scalar& b,
                            exec_aten::optional<exec_aten::string_view> mode,
                            Tensor& out)

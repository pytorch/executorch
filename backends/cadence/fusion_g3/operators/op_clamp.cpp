...
Tensor& clamp_out(
  ...
  ScalarType min_type =
      has_min ? torch::executor::native::utils::get_scalar_dtype(min_opt.value()) : in_type;
  ScalarType max_type =
      has_max ? torch::executor::native::utils::get_scalar_dtype(max_opt.value()) : in_type;
  ...
  // Resize
  ET_KERNEL_CHECK(
      ctx, executorch::runtime::resize_tensor(out, in.sizes()) == Error::Ok, InvalidArgument, out);
#endif
  ...
  if (!(((in_type == ScalarType::Float) ||
         (in_type == ScalarType::Short) ||
         (in_type == ScalarType::Char) ||
         (in_type == ScalarType::Byte)) &&
        (in_type == out_type))) {
    optimized = false;
  }
  ...
  if(has_max)
  {
    if ((max_opt.value().isFloatingPoint()) && (in.scalar_type() == ScalarType::Short)) {
      optimized = false;
    }
  
    if ((max_opt.value().isFloatingPoint()) && (in.scalar_type() == ScalarType::Char)) {
      optimized = false;
    }
  
    if ((max_opt.value().isFloatingPoint()) && (in.scalar_type() == ScalarType::Byte)) {
      optimized = false;
    }
  }
  ...
  if(has_min)
  {
    if ((min_opt.value().isFloatingPoint()) && (in.scalar_type() == ScalarType::Short)) {
      optimized = false;
    }
  
    if ((min_opt.value().isFloatingPoint()) && (in.scalar_type() == ScalarType::Char)) {
      optimized = false;
    }
  
    if ((min_opt.value().isFloatingPoint()) && (in.scalar_type() == ScalarType::Byte)) {
      optimized = false;
    }
  }
  ...
  else 
  {
    ...
    if (has_min) {
      common_type = torch::executor::native::utils::promote_type_with_scalar(common_type, min_opt.value());
    }
    if (has_max) {
      common_type = torch::executor::native::utils::promote_type_with_scalar(common_type, max_opt.value());
    }
          ...
          [has_min, min_opt, has_max, max_opt](const CTYPE_COMPUTE val_in) {
            ...
            if (has_min) {
              val_out = torch::executor::native::utils::max_override(
                  val_out, torch::executor::native::utils::scalar_to<CTYPE_COMPUTE>(min_opt.value()));
            }
            if (has_max) {
              val_out = torch::executor::native::utils::min_override(
                  val_out, torch::executor::native::utils::scalar_to<CTYPE_COMPUTE>(max_opt.value()));
            }
            return val_out;
          },
          ctx,
...
Tensor& clamp_tensor_out(
  ...
  ET_KERNEL_CHECK(
      ctx,
      torch::executor::resize_to_broadcast_target_size(in, min, max, out) == Error::Ok,
      InvalidArgument,
      out);
#endif
  ...
  if ((in.scalar_type() == ScalarType::Float) && (optimized)) {
    ...
    if (has_min) {
      common_type = executorch::runtime::promoteTypes(common_type, min.scalar_type());
    }
    if (has_max) {
      common_type = executorch::runtime::promoteTypes(common_type, max.scalar_type());
    }
    ...
    ET_SWITCH_REALB_TYPES(compute_type, ctx, op_name, CTYPE_COMPUTE, [&]() {
      torch::executor::native::utils::apply_tritensor_elementwise_fn<CTYPE_COMPUTE, op_name>(
          [has_min, has_max](
            ...
            CTYPE_COMPUTE val_out = val_in;
            if (has_min) {
              val_out = torch::executor::native::utils::max_override(val_out, val_min);
            }
            if (has_max) {
              val_out = torch::executor::native::utils::min_override(val_out, val_max);
            }
            return val_out;
          },
          ctx,

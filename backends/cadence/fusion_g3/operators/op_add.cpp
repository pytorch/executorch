...
Tensor& add_out(KernelRuntimeContext& ctx,
                const Tensor& a,
                const Tensor& b,
                const Scalar& alpha,
                Tensor& out)
{
    ...
    ET_KERNEL_CHECK(
      ...
      (canCast(common_type, out.scalar_type()) &&
       torch::executor::check_alpha_type(torch::executor::native::utils::
       get_scalar_dtype(alpha), common_type)),
      InvalidArgument,
      out);
    ...
    if(compute_type == ScalarType::Int)
    {
        ...
        if(broadcast)
        {
            xa_nn_elm_add_broadcast_5D_32x32_32(out_data, out_shape,
                inp1_data, inp1_shape, inp2_data, inp2_shape, max_dim, alpha_val);
        }
        else
        {
             xa_nn_elm_add_32x32_32(out_data, inp1_data, inp2_data, alpha_val, out.numel());
        }
    }
    else if(compute_type == ScalarType::Float)
    {
        ...
        if(broadcast)
        {
            xa_nn_elm_add_broadcast_5D_f32xf32_f32(out_data, out_shape, inp1_data,
                 inp1_shape, inp2_data, inp2_shape, max_dim, alpha_val);
        }
        else
        {
            xa_nn_elm_add_f32xf32_f32(out_data, inp1_data, inp2_data, alpha_val, out.numel());
        }
    }
    else
    {
...
Tensor& add_scalar_out(KernelRuntimeContext& ctx,
                       const Tensor& a,
                       const Scalar& b,
                       const Scalar& alpha,
                       Tensor& out)
{
    ...
    if(compute_type == ScalarType::Int)
    {
        ...
        xa_nn_elm_add_scalar_32x32_32(out_data, inp1_data, inp2_val,
                        alpha_val, out.numel());
    }
    else if(compute_type == ScalarType::Float)
    {
        ...
        xa_nn_elm_add_scalar_f32xf32_f32(out_data, inp1_data, inp2_val,
                        alpha_val, out.numel());
    }
    else
    {
        ET_SWITCH_REALB_TYPES(compute_type, ctx, op_name, CTYPE_COMPUTE, [&]() {
            ...
            [b, alpha](const CTYPE_COMPUTE val_a) {
              CTYPE_COMPUTE val_b = torch::executor::native::
              utils::scalar_to<CTYPE_COMPUTE>(b);
              CTYPE_COMPUTE val_alpha = torch::executor::native::
              utils::scalar_to<CTYPE_COMPUTE>(alpha);
              return val_a + val_alpha * val_b;
            },
            ctx,
            a,

...
void layer_norm(
    ...
    size_t normalized =  executorch::runtime::getTrailingDims(input, dim) * dim_size;
        ...
        Tensor& rstd_out)
{
    ...
    ET_KERNEL_CHECK(
      ctx, executorch::runtime::tensor_is_default_dim_order(input), InvalidArgument, ret_val);

    ET_KERNEL_CHECK(
      ctx,
      executorch::runtime::tensors_have_same_dim_order(input, out, mean_out, rstd_out),
      InvalidArgument,
      ret_val);
    ...
    ET_KERNEL_CHECK(
            ctx,
            executorch::runtime::resize_tensor(mean_out, {mean_rstd_sizes,
            mean_rstd_ndim}) == Error::Ok,
            InvalidArgument,
            ret_val);

    ET_KERNEL_CHECK(
            ctx,
            executorch::runtime::resize_tensor(rstd_out, {mean_rstd_sizes,
            mean_rstd_ndim}) == Error::Ok,
            InvalidArgument,
            ret_val);
    ...
    if(out.scalar_type() == ScalarType::Float)
    {
        ...
        xa_nn_native_layer_norm_f32_f32(out_data, mean_data, rstd_data,
                                        inp_data, input_shape, input.dim(),
                                        dim, weight_data, bias_data,
                                        (float)eps);

...
Tensor& slice_copy_Tensor_out(KernelRuntimeContext& ctx,
                              const Tensor& in,
                              int64_t dim,
                              ...
                              Tensor& out)
{
    ...
    int64_t length = torch::executor::adjust_slice_indices(in.size(dim),
                                                    &start, &end, step);
...
#ifdef OP_ARG_CHECK
    ET_KERNEL_CHECK(
      ctx, torch::executor::check_slice_copy_args(in, dim, step, out),
      InvalidArgument, out);
    ...
    torch::executor::get_slice_copy_out_target_size(in, dim, length,
                            target_sizes, &target_ndim);
    ET_KERNEL_CHECK(
      ctx,
      executorch::runtime::resize_tensor(out,
      {target_sizes, target_ndim}) == Error::Ok,
      InvalidArgument,
      out);
#endif

 ...
 */
void check_quantize_per_tensor_args(const Tensor& input,
                                    int64_t quant_min,
                                    int64_t quant_max,
                                    ScalarType dtype,
                                    Tensor& out)
    ...
    if(is_asym_quant)
    {
        if (out.scalar_type() == ScalarType::Byte)
        {
            ...
            xa_nn_elm_quantize_f32_asym8u(
                out_data, input_data, inp_shape, input.dim(), axis,
                 scale_data, zero_point_data, quant_min, quant_max);
        }
                  ...
                  case ScalarType::out_dtype: {                                                \
                    ...
                    for (size_t i = 0; i < input_numel; i++) {                                 \
                      ...
                      out_data_ptr[i] = quantize_val<OUT_CTYPE, IN_CTYPE>(                     \
                          (double)*scale_data, (int64_t)*zero_point_data, value,               \
    ...
    else
    {
        if (out.scalar_type() == ScalarType::Byte)
        {
            ...
            xa_nn_elm_quantize_f32_sym8u(
                out_data, input_data, inp_shape, input.dim(), axis,
                 scale_data, quant_min, quant_max);
        }
        else if (out.scalar_type() == ScalarType::Char)
        {
            int8_t* out_data = out.mutable_data_ptr<int8_t>();
...
Tensor& quantize_per_tensor_tensor_args_out(KernelRuntimeContext& context,
                                            const Tensor& input,
                                            const Tensor& scale,
                                            ...
                                            Tensor& out)
{
    ...
    quantize_per_tensor_out(context,
      input,
...
Tensor& quantize_per_tensor_tensor_args_out(const Tensor& input,
                                            const Tensor& scale,
                                            const Tensor& zero_point,
                                            int64_t quant_min,
                                            int64_t quant_max,
...
Tensor& quantize_per_channel_out(const Tensor& input,
                                const Tensor& scale,
                                const Tensor& zero_point,
                                int64_t axis,
                                int64_t quant_min,
    ...
    Tensor& out)
{
    ...
    torch::executor::TensorImpl reshaped_input_impl = executorch::runtime::etensor::TensorImpl(
                              input.scalar_type(),
                              2,
                              input_sizes.data(),
                              input_data,

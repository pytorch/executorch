        ...
        Tensor& out)
{
    ...
    if(compute_type == ScalarType::Int)
    {
        ...
        if(broadcast)
        {
            xa_nn_elm_mul_broadcast_5D_32x32_32(out_data, out_shape, inp1_data,
                          inp1_shape, inp2_data, inp2_shape, max_dim);
        }
        else
        {
    ...
    else if(compute_type == ScalarType::Float)
    {
        ...
        if(broadcast)
        {
            xa_nn_elm_mul_broadcast_5D_f32xf32_f32(out_data, out_shape, inp1_data,
                           inp1_shape, inp2_data, inp2_shape, max_dim);
        }
        else
        {
...
Tensor& mul_scalar_out(KernelRuntimeContext& ctx,
                       const Tensor& a,
                       const Scalar& b,
                       Tensor& out)
{
    ...
    // Compute Dtype
    ScalarType compute_type = torch::executor::native::
                            utils::get_compute_type(common_type);
    ...
    else if(compute_type == ScalarType::Float)
    {
        ...
        xa_nn_elm_mul_scalar_f32xf32_f32(out_data, inp1_data, inp2_val, out.numel());
    }
    else
    {
        ET_SWITCH_REALB_TYPES(compute_type, ctx, op_name, CTYPE_COMPUTE, [&]() {
            const CTYPE_COMPUTE val_b = torch::executor::native::

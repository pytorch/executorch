...
class TestTypeDispatchPasses(unittest.TestCase):
    ...
    def test_int8_dispatch_quantized_conv_nchw_1d_depthwise(self) -> None:
        """Test int8 x int8 inputs for 1D depthwise conv should dispatch to 1d_depthwise_asym8sxsym8s_asym8s variant for quantized_conv_nchw"""
        # 1D Depthwise convolution: groups == input_channels
        x = torch.randint(-128, 127, (1, 3, 8), dtype=torch.int8)
        w = torch.randint(
            -128, 127, (3, 1, 3), dtype=torch.int8
        )  # groups=3, input_channels=3
        b = torch.randint(-2147483648, 2147483647, (3,), dtype=torch.int32)
        gm = single_op_builder(
            placeholders=(x, w, b),
        ...
        # Should be replaced with 1D int8 depthwise specific variant
        self.assertEqual(
            count_node(
                gm,
                exir_ops.edge.cadence.quantized_conv_nchw_1d_depthwise_asym8sxsym8s_asym8s.per_tensor,
            ),
            1,
        )

    def test_uint8_dispatch_quantized_conv_nchw_1d_depthwise(self) -> None:
        """Test uint8 x uint8 inputs for 1D depthwise conv should dispatch to 1d_depthwise_asym8uxasym8u_asym8u variant for quantized_conv_nchw"""
        # 1D Depthwise convolution: groups == input_channels
        x = torch.randint(0, 255, (1, 3, 8), dtype=torch.uint8)
        w = torch.randint(
            0, 255, (3, 1, 3), dtype=torch.uint8
        )  # groups=3, input_channels=3
        b = torch.randint(-2147483648, 2147483647, (3,), dtype=torch.int32)
        gm = single_op_builder(
            placeholders=(x, w, b),
        ...
        # Should be replaced with 1D uint8 depthwise specific variant
        self.assertEqual(
            count_node(
                gm,
                exir_ops.edge.cadence.quantized_conv_nchw_1d_depthwise_asym8uxsym8u_asym8u.per_tensor,
            ),
            1,
        )

    def test_int8_dispatch_quantized_conv_nhwc_1d_depthwise(self) -> None:
        """Test int8 x int8 inputs for 1D depthwise conv should dispatch to 1d_depthwise_asym8sxsym8s_asym8s variant for quantized_conv_nhwc"""
        # 1D Depthwise convolution: groups == input_channels
        x = torch.randint(-128, 127, (1, 8, 3), dtype=torch.int8)
        w = torch.randint(
            -128, 127, (3, 3, 1), dtype=torch.int8
        )  # groups=3, input_channels=3
        b = torch.randint(-2147483648, 2147483647, (3,), dtype=torch.int32)
        gm = single_op_builder(
            placeholders=(x, w, b),
        ...
        self.assertEqual(
            count_node(
                gm,
                exir_ops.edge.cadence.quantized_conv_nhwc_1d_depthwise_asym8sxsym8s_asym8s.per_tensor,
            ),
            1,
        )

    def test_uint8_dispatch_quantized_conv_nhwc_1d_depthwise(self) -> None:
        """Test uint8 x uint8 inputs for 1D depthwise conv should dispatch to 1d_depthwise_asym8uxasym8u_asym8u variant for quantized_conv_nhwc"""
        # 1D Depthwise convolution: groups == input_channels
        x = torch.randint(0, 255, (1, 8, 3), dtype=torch.uint8)
        w = torch.randint(
            0, 255, (3, 3, 1), dtype=torch.uint8
        )  # groups=3, input_channels=3
        b = torch.randint(-2147483648, 2147483647, (3,), dtype=torch.int32)
        gm = single_op_builder(
            placeholders=(x, w, b),
        ...
        self.assertEqual(
            count_node(
                gm,
                exir_ops.edge.cadence.quantized_conv_nhwc_1d_depthwise_asym8uxsym8u_asym8u.per_tensor,
            ),
            1,
        )

...
def add_constant_placeholder(
    ...
    first_user_input = next(
        (n for n in ep.graph.nodes if n.op == "placeholder" and n.name in user_inputs),
        None,
    )
...
def tensor_qparam_overload(op: OpOverload) -> OpOverload:
    ...
    assert packet is not None and hasattr(
        packet, "default"
    ), f"no tensor-qparam overload registered for {name}; per-channel needs one"
    return packet.default


def get_weight_scale(gm: fx.GraphModule, dq_weight: fx.Node) -> float | torch.Tensor:
...
def get_weight_zero_point(gm: fx.GraphModule, dq_weight: fx.Node) -> int | torch.Tensor:

...
void quantized_linear_out(
  ...
  const auto leading_dims =
      cadence::impl::cpu::native::getLeadingDims(src, src.dim() - 1);

  ET_CHECK_MSG(
      out_multiple.numel() == 1, "out_multiplier should have one element");
  ET_CHECK_MSG(
  ...
  const float out_scale =
      -out_multiplier[0] * 1.0 / (1 << 31) * pow(2, out_shift[0]);

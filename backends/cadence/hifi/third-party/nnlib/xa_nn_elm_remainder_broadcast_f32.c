...
DISCARD_FUN_FOR_NONVOID_RETURN(
    WORD32,
    xa_nn_elm_remainder_f32xf32_f32,
    (FLOAT32 * p_out,
...
#if HAVE_VFPU
static void internal_elm_remainder_broadcast_2D_f32xf32_f32(
    FLOAT32* __restrict__ p_out,
    const FLOAT32* __restrict__ p_inp1,
    const FLOAT32* __restrict__ p_inp2,
    WORD32 out_lc,
  ...
  if (sign_flag) {
    for (i = 0; i < out_lc; i++) {
      ...
      if (((((unsigned)p_a) & 7) == 0) && ((((unsigned)p_b) & 7) == 0) &&
          ((((unsigned)p_c) & 7) == 0)) {
...
static void internal_elm_remainder_broadcast_f32xf32_f32(
    FLOAT32* __restrict__ p_out,
    const FLOAT32* __restrict__ p_inp1,
    const FLOAT32* __restrict__ p_inp2,
    WORD32 num_elm,
...
DISCARD_FUN_FOR_NONVOID_RETURN(
    WORD32,
    xa_nn_elm_remainder_broadcast_4D_f32xf32_f32,
    (FLOAT32 * p_out,
...
WORD32 xa_nn_elm_remainder_broadcast_4D_f32xf32_f32(
  ...
  xtbool sign_flag;
  for (i = 0; i < 4; i++) {
    if ((p_inp1_shape[i] != p_inp2_shape[i] && p_inp1_shape[i] != 1 &&
         p_inp2_shape[i] != 1) ||
        (p_out_shape[i] !=
         (p_inp1_shape[i] > p_inp2_shape[i] ? p_inp1_shape[i]
                                            : p_inp2_shape[i]))) {
      return -1;
    }
  }
  ...
  if (need_broadcast == 0) {
    ...
    internal_elm_remainder_broadcast_2D_f32xf32_f32(
        p_out, p_inp1, p_inp2, 1, p_out_shape[0] * inp1_strides[0], sign_flag);
    ...
    for (itr0 = 0; itr0 < p_out_shape[0]; itr0++) {
      ...
      for (itr1 = 0; itr1 < p_out_shape[1]; itr1++) {
        internal_elm_remainder_broadcast_2D_f32xf32_f32(
            p_out_tmp, p_inp1_tmp0, p_inp2_tmp0, out_lc, in_lc, sign_flag);
    ...
    for (itr0 = 0; itr0 < p_out_shape[0]; itr0++) {
      ...
      for (itr1 = 0; itr1 < p_out_shape[1]; itr1++) {
        ...
        for (itr2 = 0; itr2 < p_out_shape[2]; itr2++) {
          {
            internal_elm_remainder_broadcast_f32xf32_f32(
                p_out_tmp, p_inp1_tmp1, p_inp2_tmp1, p_out_shape[3], sign_flag);
          }

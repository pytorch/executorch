...
void xa_opt_quantized_conv_nchw_1d_asym8uxsym8u_asym8u(
    ...
    Tensor& out) {
  ET_CHECK_MSG(
      groups == 1, "Only groups=1 supported for regular 1D convolution");
...
void quantized_conv_nchw_1d_asym8uxsym8u_asym8u_per_tensor_out(
    KernelRuntimeContext& ctx,
    const Tensor& input,
    const Tensor& weight,
    const Tensor& bias,
    ...
    Tensor& out) {
  xa_opt_quantized_conv_nchw_1d_asym8uxsym8u_asym8u(
      ctx,
      input,
      weight,
      bias,

...
// Specialized 1D depthwise NHWC convolution for uint8 x uint8 -> uint8
void xa_opt_quantized_conv_nhwc_1d_depthwise_asym8uxsym8u_asym8u(
  ...
  WORD32 input_width = input.size(1);  // NHWC: input shape is [N, W, C]
  WORD32 input_channels = input.size(2);
  WORD32 kernel_height = 1;
  WORD32 kernel_width = weight.size(1);  // NHWC: weight shape is [C_out, K, C_in]
  WORD32 out_channels = weight.size(0);
  WORD32 out_height = 1;
  WORD32 out_width = out.size(1);  // NHWC: out shape is [N, W_out, C_out]
...
void quantized_conv_nhwc_1d_depthwise_asym8uxsym8u_asym8u_per_tensor_out(
    ...
    Tensor& out) {
  xa_opt_quantized_conv_nhwc_1d_depthwise_asym8uxsym8u_asym8u(
      ctx,
      input,
      weight,
      bias,

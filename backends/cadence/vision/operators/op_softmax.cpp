...
Tensor& _softmax_out(
  ...
  if (optimized){
    ...
    if (ping_pong_process) {
      ...
      idma_copy_2d_desc(0, inpData[pp_swap], ptr_in, 4 * stride, DESC_IDMA_PRIOR_H, 1, 0, 0);
      pp_swap = 1;
      ...
      for (int i = 0; i < (outer_size - 1); i++){ 
          ...
          IDMA_HW_WAIT_ALL(1);
          idma_copy_2d_desc(1, ptr_out, outData[pp_swap], 4 * stride, DESC_IDMA_PRIOR_H, 1, 0, 0);
      ...
      stride = size;
        ...
        idma_copy_2d_desc(0, data_dram0, ptr_in, 4 * stride, DESC_IDMA_PRIOR_H, 1, 0, 0);
        IDMA_HW_WAIT_ALL(0);

		    vsoftmaxf(data_dram1, data_dram0, stride);

        idma_copy_2d_desc(1, ptr_out, data_dram1, 4 * stride, DESC_IDMA_PRIOR_H, 1, 0, 0);        
        IDMA_HW_WAIT_ALL(1);

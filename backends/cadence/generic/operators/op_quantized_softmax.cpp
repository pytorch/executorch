 ...
 *   1. Dequantize: Convert quantized input to float using in_scale and
 * in_zero_point
 *   2. Find max: Compute max over unmasked positions (for numerical stability)
 *   3. Exp & sum: Compute exp(x - max) and accumulate sum for unmasked
 * positions
 *   4. Normalize: Divide by sum to get probabilities
 *   5. Quantize: Convert back to quantized output using out_scale and
 * out_zero_point
 *
...
void quantized_softmax_per_tensor_(
  ...
  int64_t lastUnmaskedPos =
      -1; // Tracks highest unmasked index for incremental updates

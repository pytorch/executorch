...
class RemovePermutesAroundElementwiseOps(ExportPass):
    ...
    class Subgraph:
        ...
        # Open graph boundaries used only for structural layout-copy propagation.
    ...
    def _boundary_rewrite_is_cost_safe(self, subgraph: Subgraph) -> bool:
        ...
        removed_copies = {permute for permute, _ in subgraph.edges_in} | {
    ...
    def _insert_output_boundary_permutations(self, subgraph: Subgraph) -> None:
        ...
        for (producer, permutation), outputs in groups.items():
            with producer.graph.inserting_before(outputs[0]):
    ...
    def update_pad(
        ...
        if node.target == exir_ops.edge.channels_last.constant_pad_nd.default:
            return
        ...
        if layout_region and node.target == exir_ops.edge.aten.constant_pad_nd.default:
            node.target = exir_ops.edge.channels_last.constant_pad_nd.default

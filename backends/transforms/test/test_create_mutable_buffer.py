...
class TestMutableBufferCreation(unittest.TestCase):
    ...
    def test_create_mutable_buffer(self):
        ...
        with graph.inserting_after(input_node):
            add_node = graph.create_node(  # noqa F481
                "call_function",
...
class TestRegisterMutableBufferPass(unittest.TestCase):
            ...
            for node in graph_module.graph.nodes:
                if (
                    ...
                    with graph_module.graph.inserting_after(node):
                        ...
                        # Update the output of the graph. This is a bit hacky, but it is done through `to_edge`.
                        # We have to mimic it here for testing against eager.
                        output_node = graph_module.graph.output_node()
                        outputs = list(output_node.args[0])
                        if self.return_buffer:
                            outputs.insert(0, buf_node)
                        else:  # return the index_put node
                            outputs.insert(0, index_put_node)
    ...
    def _test_edge_pass(
        self, model, example_inputs, num_lifted_args=1, return_buffer=False
    ):
        ...
        transformed_ep = edge_program.transform(
            passes=[
                TestRegisterMutableBufferPass.HelperPass(
                    edge_program.exported_program(), buffer_tensor, return_buffer=True
                )
            ]
        )
    ...
    def test_single(self):
        ...
        graph_model = CustomModuleGraph().eval()
        et1 = self._test_edge_pass(graph_model, example_inputs, return_buffer=True)

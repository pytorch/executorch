...
from executorch.backends.apple.coreml.quantizer._annotation_config import (
    ...
    for node in nodes:
        if node is not None:
            if Q_ANNOTATION_KEY not in node.meta:
                node.meta[Q_ANNOTATION_KEY] = _QuantizationAnnotation()
    ...
    class Pattern(_torch.nn.Module):
        def forward(self, input, weight, bias):
            ...
            if act_fn is not None:
                ...
                output = (
                    act_fn(output, inplace=True) if act_in_place else act_fn(output)
                )
...
def _get_weighted_mod_bn_pattern(
    ...
    class Pattern(_torch.nn.Module):
        def forward(
            self, input, weight, bias, bn_weight, bn_bias, bn_run_mean, bn_run_var
        ):
            mod_out = mod_fn(input, weight, bias)
...
def annotate_weighted_mod_pattern(  # noqa: C901
    ...
    for match in matches:
        ...
        if output_node is None:
            mod_node.meta[Q_ANNOTATION_KEY] = _QuantizationAnnotation(
            ...
            if not _is_annotated([output_node]):
                ...
                output_node.meta[Q_ANNOTATION_KEY] = _QuantizationAnnotation(
                    output_qspec=output_qspec,
                    _annotated=True,
                )
...
def annotate_binary_op_act_pattern(
    ...
    for match in matches:
        ...
        if output_node is None:
            binary_op_node.meta[Q_ANNOTATION_KEY] = _QuantizationAnnotation(
                input_qspec_map=input_qspec_map,
...
def annotate_unary_shared_observer_ops(
    ...
    for _, op_partitions in partitions.items():
        for partition in op_partitions:
            ...
            if (
                Q_ANNOTATION_KEY not in input_node.meta
                or not input_node.meta[Q_ANNOTATION_KEY]._annotated
                or input_node.meta[Q_ANNOTATION_KEY].output_qspec is None
            ...
            op_node.meta[Q_ANNOTATION_KEY] = _QuantizationAnnotation(
                input_qspec_map={

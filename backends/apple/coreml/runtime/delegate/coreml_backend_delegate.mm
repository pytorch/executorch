...
// Format identifier for JSON reference to NamedDataStore
constexpr const char* kNamedDataReferenceKeyField = "key";
...
bool isNamedDataReference(const void* data, size_t size) {
    ...
    json j = json::parse(content, nullptr, false); // false = don't throw on error
    ...
    return j.contains("version") && j.contains("key") && j["key"].is_string();
}

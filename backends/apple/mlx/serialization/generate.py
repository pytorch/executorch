...
def generate_python_schema(schema: FBSSchema) -> str:
...
def generate_python_serializers(schema: FBSSchema) -> str:
    ...
    op_type_names_lines = ['# FlatBuffer union indices: 0 = NONE, then 1-indexed from union order']
...
def _get_field_kind(fld: FBSField, table: FBSTable) -> str:
    ...
    # Check if this field has a corresponding _is_set field in the table
    has_is_set_field = any(f.name == f"{fld.name}_is_set" for f in table.fields)
    ...
    if t == 'FloatOrVid':
        return "float_or_vid"
    if t == "DTypeId":
        # Check for _is_set sentinel pattern (for when None vs 0 matters)
        if has_is_set_field:
            return "optional_dtype"  # Use _is_set to check
        # All dtype fields are plain values - Python serializer ensures value is present
        return "dtype"
    ...
    if t in ('float', 'double'):
        # Check for _is_set sentinel pattern (for when None vs 0 matters)
        if has_is_set_field:
            return "optional_float"  # Use _is_set to check
        # All float fields are plain values - Python serializer ensures value is present
        return "float"
    if t == 'bool':
        return "bool"
    if t == 'string':
...
def generate_cpp_loader_h(schema: FBSSchema) -> str:
    ...
    if dtype_enum:
        ...
        for name, val in dtype_enum.values:
            lines.append(f'  {name},')
        lines.append('};')
        lines.append('')
...
def _fbs_type_to_cpp(fbs_type: str, required: bool, table: Optional['FBSTable'] = None, fld: Optional['FBSField'] = None) -> str:
    """Convert FBS type to C++ type.

    Args:
        fbs_type: The FlatBuffer type string
        required: Whether the field is required
        table: Optional table context for checking _is_set fields
...
def _generate_loader_case(table: FBSTable) -> List[str]:
    ...
    for fld in table.fields:
        ...
        if kind == "tid":
            ...
            # Field with _is_set pattern - check if set before assigning
            lines.append(f'      if (fb->{fb_field_name}_is_set()) {{')
            lines.append(f'        node.{fld.name} = fb->{fb_field_name}();')
            lines.append(f'      }}')
        elif kind == "str":
            lines.append(f'      node.{fld.name} = fb->{fb_field_name}() ? fb->{fb_field_name}()->str() : "";')
        elif kind == "optional_str":
            lines.append(f'      if (fb->{fb_field_name}()) {{')
            lines.append(f'        node.{fld.name} = fb->{fb_field_name}()->str();')
            lines.append(f'      }}')
        elif kind == "dtype":
            lines.append(f'      node.{fld.name} = convert_dtype(fb->{fb_field_name}());')
        elif kind == "optional_dtype":
            # Field with _is_set pattern - check if set before assigning
            lines.append(f'      if (fb->{fb_field_name}_is_set()) {{')
            lines.append(f'        node.{fld.name} = convert_dtype(fb->{fb_field_name}());')
...
def main():
    parser = argparse.ArgumentParser(description="Generate MLX delegate code from schema.fbs")
    parser.add_argument(
        "--flatc",
        default="flatc",
    ...
    print(f"  Found {len(schema.enums)} enums, {len(schema.structs)} structs, "

...
Usage:
    ...
    python -m executorch.backends.apple.mlx.test.test_add run
    ...
    python -m executorch.backends.apple.mlx.test.test_add run --dynamic-batch

    # Tensor + scalar with dynamic batch:
    python -m executorch.backends.apple.mlx.test.test_add run --scalar 2.5 --dynamic-batch
"""
...
def main():
    parser = argparse.ArgumentParser(description="Test add op on MLX delegate")
    parser.add_argument(
        "action",

...
Usage:
    python -m executorch.backends.apple.mlx.examples.whisper.run_whisper \
...
def run_whisper_inference(  # noqa: C901
    ...
    logger.info(f"Dtype: {metadata['dtype']}, Quantize: {metadata['quantize']}")
    logger.info(f"Max decoder seq len: {metadata['max_decoder_seq_len']}")
    logger.info(f"Num decoder layers: {metadata['num_decoder_layers']}")
    ...
    logger.info(f"Start token: {sot_id}")
    logger.info(f"EOS token: {eos_id}")
    logger.info(f"Forced tokens: {forced_tokens_dict}")
    ...
    for step in range(max_new_tokens):
        ...
        if current_pos in forced_tokens_dict:
            ...
            if step < 5:
                token_str = processor.tokenizer.convert_ids_to_tokens([next_token_id])
                logger.info(
                    f"[step {step}] pos={current_pos}, FORCED: {next_token_id} -> {token_str}"
                )
        else:
            # Sample from logits (greedy for now)
            next_token_id = torch.argmax(logits[0, -1, :]).item()
            if step < 5:
                token_str = processor.tokenizer.convert_ids_to_tokens([next_token_id])
                logger.info(
                    f"[step {step}] pos={current_pos}, SAMPLED: {next_token_id} -> {token_str}"
                )

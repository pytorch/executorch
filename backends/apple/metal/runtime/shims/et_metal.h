...
class ETMetalKernelFunction {
 public:
  ...
  void startEncoding();
  // Binds a tensor's buffer. A strided view (see metal_record_strided_view) is
  // bound as a packed copy, which is what hand-written kernels expect, unless
  // `strided_view_in_place` is set: kernels generated by inductor index the
  // view's real layout and must see the memory itself.
  void setArg(
      unsigned idx,
      const executorch::runtime::etensor::Tensor& tensor,
      bool strided_view_in_place = false);

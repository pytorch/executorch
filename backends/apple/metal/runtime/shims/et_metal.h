...
class ETMetalKernelFunction {
 public:
  ...
  void startEncoding();
  // How a kernel uses a tensor argument. It decides what a strided view (see
  // metal_record_strided_view) is bound as.
  enum class ArgAccess {
    // A hand-written kernel reads the tensor as dense: a strided view is bound
    // as a packed copy of its elements.
    kRead,
    // A hand-written kernel writes the tensor, or reads and writes it: a
    // strided view is refused, since writes to a copy would be lost.
    kWrite,
    // A kernel generated by inductor, which indexes the tensor with the
    // strides it was compiled with: a strided view is bound where it is.
    kStridedInPlace,
  };

  // Binds a tensor's buffer. Throws std::runtime_error if a strided view cannot
  // be bound the way `access` asks.
  void setArg(
      unsigned idx,
      const executorch::runtime::etensor::Tensor& tensor,
      ArgAccess access = ArgAccess::kRead);
 ...
 private:
  ...
  // view, changes the pipeline state and whatever it binds; restoreBindings()
  // puts all of this function's back.
  struct Binding {
  ...
  void bindBuffer(unsigned idx, MTLBuffer_t buffer, size_t offset);
  void bindBytes(unsigned idx, const void* data, size_t size);
...
// gathered: no empty dim, no negative stride, at most 16 dims, and fewer than
// 2^32 elements both in the view and in the memory it spans.
bool metal_can_pack_strided_view(
    const std::vector<int64_t>& sizes,
    const std::vector<int64_t>& strides);

// A view whose elements are not densely packed, such as one half of a
// channels-last tensor chunked along C. It stays where it is, inside its
// parent's buffer: kernels generated by inductor index it with the strides they
// were compiled with, and both read and write through it, so handing them a
// packed copy would send their indices past the end of the copy and lose their
...
// cannot gather (metal_can_pack_strided_view).
bool metal_record_strided_view(
    const void* tensor,
    std::vector<int64_t> sizes,
    std::vector<int64_t> strides);
// Makes `to` a strided view like `from`, if `from` is one.

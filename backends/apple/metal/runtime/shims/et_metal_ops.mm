...
// All tensors should have their data_ptr directly in the map since we materialize
// all offset/broadcast cases in aoti_torch__reinterpret_tensor
...
AOTITorchError aoti_torch_mps_addmm_out(
  ...
  @autoreleasepool {
    try {
      ...
      int64_t M = mat1_tensor->sizes()[0];
      int64_t K = mat1_tensor->sizes()[1];
      int64_t N = mat2_tensor->sizes()[1];

      if (mat1_tensor->sizes()[1] != mat2_tensor->sizes()[0]) {
        ET_LOG(Error, "aoti_torch_mps_addmm_out: incompatible matrix sizes");
        return Error::InvalidArgument;
      }
      ...
      ETMetalStream* stream = getCurrentMetalStream();
      if (!stream) {
        ET_LOG(Error, "aoti_torch_mps_addmm_out: Failed to get current Metal stream");
        return Error::Internal;
      }
      ...
      // Get Metal buffers
      id<MTLBuffer> input_buffer = get_mtl_buffer(input_tensor, "aoti_torch_mps_addmm_out", "input");
      id<MTLBuffer> mat1_buffer = get_mtl_buffer(mat1_tensor, "aoti_torch_mps_addmm_out", "mat1");
      id<MTLBuffer> mat2_buffer = get_mtl_buffer(mat2_tensor, "aoti_torch_mps_addmm_out", "mat2");
      id<MTLBuffer> out_buffer = get_mtl_buffer(out_tensor, "aoti_torch_mps_addmm_out", "out");
      ...
      if (dtype == static_cast<int32_t>(SupportedDTypes::FLOAT32)) {
        ...
        ET_LOG(Error, "aoti_torch_mps_addmm_out: Unsupported data type: %d", dtype);
        return Error::InvalidArgument;
      }

      // Create MPSGraph for addmm: out = beta * input + alpha * (mat1 @ mat2)
      ...
      // Handle input shape - it could be 1D (bias) or 2D
      NSArray<NSNumber*>* inputShape;
      if (input_tensor->dim() == 1) {
        inputShape = @[@(input_tensor->sizes()[0])];
      } else {
        inputShape = @[@(input_tensor->sizes()[0]), @(input_tensor->sizes()[1])];
      }

      MPSGraphTensor* mat1Placeholder = [mpsGraph placeholderWithShape:mat1Shape
                                                              dataType:mps_dtype
                                                                  name:@"mat1"];
      ...
      // Compute mat1 @ mat2
      MPSGraphTensor* mmResult = [mpsGraph matrixMultiplicationWithPrimaryTensor:mat1Placeholder
                                                                 secondaryTensor:mat2Placeholder
                                                                            name:@"mm"];

      // Scale mm result by alpha
      MPSGraphTensor* alphaConst = [mpsGraph constantWithScalar:(double)alpha
                                                       dataType:mps_dtype];
      MPSGraphTensor* scaledMM = [mpsGraph multiplicationWithPrimaryTensor:mmResult
                                                           secondaryTensor:alphaConst
                                                                      name:@"scaled_mm"];

      // Scale input by beta
      MPSGraphTensor* betaConst = [mpsGraph constantWithScalar:(double)beta
                                                      dataType:mps_dtype];
      MPSGraphTensor* scaledInput = [mpsGraph multiplicationWithPrimaryTensor:inputPlaceholder
                                                              secondaryTensor:betaConst
                                                                         name:@"scaled_input"];

      // Add: out = beta * input + alpha * (mat1 @ mat2)
      MPSGraphTensor* addResult = [mpsGraph additionWithPrimaryTensor:scaledInput
                                                      secondaryTensor:scaledMM
                                                                 name:@"addmm_result"];

      // Create tensor data
      MPSGraphTensorData* mat1Data = [[MPSGraphTensorData alloc] initWithMTLBuffer:mat1_buffer
                                                                             shape:mat1Shape
                                                                          dataType:mps_dtype];
      MPSGraphTensorData* mat2Data = [[MPSGraphTensorData alloc] initWithMTLBuffer:mat2_buffer
                                                                             shape:mat2Shape
      ...
      NSDictionary* results = @{addResult: outputData};

      @try {
        stream->executeMPSGraph(mpsGraph, feeds, results, SyncType::COMMIT);
      } @catch (NSException *exception) {
        ET_LOG(Error, "aoti_torch_mps_addmm_out: NSException: %s - %s",
              [[exception name] UTF8String], [[exception reason] UTF8String]);
        [mat1Data release];
        [mat2Data release];
        [inputData release];

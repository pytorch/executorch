...
The Inspector APIs provides a convenient interface for analyzing the
contents of `ETRecord <placeholder_link>`__ and
`ETDump <placeholder_link>`__, helping developers get insights on model
architecture and performance statistics. It’s build on top of the
``EventBlock`` data structure, which organizes a group of ``Event``\ s
for easy access to details of profiling events.

There are multiple ways in which users can interact with the Inspector
APIs: - By using public methods provided by the ``Inspector`` class. -
By accessing the public attributes of the ``Inspector``, ``EventBlock``,
and ``Event`` classes. - By using a CLI tool for basic functionalities.
...
Initialize an Inspector instance from the provided ETDump path and an
optional ETRecord path.
...
-  Inspector instance with the underlying ``EventBlock``\ s populated
   with data from ETDump and ETRecord.
...
Displays the underlying EventBlocks in a structured tabular format, with
each row representing an Event.

.. autofunction:: sdk.inspector.Inspector.print_data_tabular
...
Returns the total average compute time of all operators within the
specified module.
...
-  ``int`` : Sum of the average compute time (in seconds) of all
   operators within the module with “module_name”.
...
Access helper for ETRecord, defaults to returning the Edge Dialect
program.
...
-  ``graph`` : Optional name of the graph to access. If None, returns
   the Edge Dialect program.
...
An EventBlock contains a collection of events associated with a
particular profiling/debugging block retrieved from the runtime. Each
EventBlock represents a pattern of execution. For example, model
initiation and loading lives in a single EventBlock. If there’s a
control flow, each branch will be represented by a separate EventBlock.

.. _public-attributes-1:

Public Attributes:
^^^^^^^^^^^^^^^^^^

-  ``name`` : Name of the profiling/debugging block.
-  ``events`` : List of events associated with the profiling/debugging
   block.
...
An Event corresponds to an operator instance with perf data retrieved
from the runtime and other metadata from ETRecord.

.. _public-attributes-2:

Public Attributes:
^^^^^^^^^^^^^^^^^^

-  ``name`` : Name of the profiling/debugging Event.
-  ``perf_data`` : Performance data associated with the event retrived
   from the runtime (available attributes: p50, p90, avg, min, max and
   median).
-  ``op_type`` : List of op types corresponding to the event.
-  ``instruction_id`` : Instruction id of the profiling event.
-  ``delegate_debug_identifier`` : Supplemental identifier used in
   combination with instruction id.
-  ``debug_handles`` : Debug handles in the model graph to which this
   event is correlated.
-  ``stack_trace`` : A dictionary mapping the name of each associated op
   to its stack trace.
-  ``module_hierarchy`` : A dictionary mapping the name of each
   associated op to its module hierarchy.
-  ``is_delegated_op`` : Whether or not the event was delegated.
-  ``delegate_backend_name`` : Name of the backend this event was
   delegated to.
...
``print_data_tabular``\ $ API mentioned earlier:

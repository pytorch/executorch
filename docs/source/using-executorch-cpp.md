...
The imported target route needs the same care. The package attaches `$ORIGIN` and
`@loader_path` entries that do survive installation, so an application deployed beside the
libraries keeps working, but the entry pointing at the wheel's own directory is removed
because that is where a linked library lives. CMake offers a one line remedy that keeps it:

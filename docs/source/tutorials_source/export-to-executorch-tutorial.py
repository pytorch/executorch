...
# ----------------
#
# To run this tutorial in Google Colab, you’ll need to install ExecuTorch::
#
...
#   # If git clone is still failing, make sure Colab -> Setting -> Github -> Access Private Repo is checked
...
# flattened graph (meaning the graph does not contain any module heirachy,
...
# cleaner representation. Specifically,
#
...
example_args = (torch.randn(1, 3), torch.randn(1, 3))
pre_autograd_aten_dialect = capture_pre_autograd_graph(M(), example_args)
aten_dialect: ExportedProgram = export(pre_autograd_aten_dialect, example_args)
edge_program: EdgeProgramManager = to_edge(aten_dialect)
exported_program = edge_program.exported_program()

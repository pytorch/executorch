...
It should be provided to the `Inspector API <dev-tools-inspector.html>`__ to link back performance and debug data sourced from the runtime back to the Python source code.
...
.. currentmodule:: executorch.dev_tools.etrecord._etrecord
.. autofunction:: generate_etrecord

Using an ``ETRecord``
---------------------

Pass the ``ETRecord`` as an optional argument into the `Inspector API <dev-tools-inspector.html>`__ to access this data and  do post-run analysis.

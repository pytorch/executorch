...
- `op_linear_quantizer_config`: Post-training weight compression applied by coremltools, as a dict (for example `{"mode": "linear_symmetric", "dtype": "int8", "granularity": "per_channel"}`).  By default this skips single-axis `gather` ops, so embedding and other lookup tables are left uncompressed, except where the table is also consumed by another op: a tied embedding and output projection is compressed with that op.  `gather_along_axis` and `gather_nd` tables are not skipped.
- `quantize_gather_tables`: Opt `gather` tables into `op_linear_quantizer_config`.  Defaults to `False`.  Note that this covers **every** gather whose table is a float constant, not only embedding layers: Core ML lowers `index_select` and plain constant indexing to `gather` too, so buffers such as a rotary position cache are compressed as well.  Tables with an integer dtype are skipped by this opt-in, which checks the dtype; a two-dimensional integer table would otherwise fail the conversion.  The tied-weight case above has no such check, so an integer table shared with another op is still compressed.

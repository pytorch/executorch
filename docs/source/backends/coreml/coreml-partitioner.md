...
- `op_linear_quantizer_config`: Post-training weight compression applied by coremltools, as a dict (for example `{"mode": "linear_symmetric", "dtype": "int8", "granularity": "per_channel"}`).  By default this skips `gather` ops, so embedding and other lookup tables are left uncompressed.
- `quantize_gather_tables`: Opt `gather` tables into `op_linear_quantizer_config`.  Defaults to `False`.  Note that this covers **every** gather whose table is a float constant, not only embedding layers: Core ML lowers `index_select` and plain constant indexing to `gather` too, so buffers such as a rotary position cache are compressed as well.  Tables with an integer dtype are always skipped, since the linear quantizer cannot compress them.

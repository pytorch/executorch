        ...
        if self._partitioners is not None and len(self._partitioners) > 0:
            with validation_disabled():
                if isinstance(self._partitioners, dict):
                    # `to_backend` takes one Partitioner per method, and only
                    # lowers the methods named in the dict, so apply each
                    # method's partitioners one at a time.
                    for method_name, method_partitioners in self._partitioners.items():
                        for partitioner in method_partitioners:

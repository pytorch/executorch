...
class TestQuantizeStage(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.mock_exported_program.module.return_value = self.mock_model
        self.exported_program_data = {
            "exported_program": {"forward": self.mock_exported_program}
        }
        self.example_inputs = [(torch.randn(2, 10),)]
    ...
    def test_quantize_stage_missing_example_inputs(self) -> None:
        ...
        with self.assertRaises(ValueError) as context:
            stage.run(self.exported_program_data, {"example_inputs": {}})
        self.assertIn(
...
class TestExportSession(unittest.TestCase):
    ...
    def test_export_session_quantized_pipeline_with_quantizers(self) -> None:
        ...
        # The correct order is: export -> quantize -> export (re-export) -> edge_transform_and_lower -> executorch
        expected_stages = [
            "export",
            "quantize",
            "export",
...
class TestExportSessionPipelineExecution(unittest.TestCase):
    ...
    def test_pipeline_execution_order_quantized(
        ...
        # Create mock stages
        mock_export_stage1 = Mock()
        mock_export_stage1.name = "export"
        mock_export_stage1.get_artifacts.return_value = {"forward": Mock()}

        mock_quantize_stage = Mock()
        mock_quantize_stage.name = "quantize"
        mock_quantize_stage.get_artifacts.return_value = {"forward": Mock()}

        mock_export_stage2 = Mock()
        mock_export_stage2.name = "export"
        mock_export_stage2.get_artifacts.return_value = {"forward": Mock()}
        ...
        # Configure the mock classes to return our mock instances
        mock_export_stage_class.side_effect = [mock_export_stage1, mock_export_stage2]
        ...
        # Verify stages were called in the correct order
        mock_export_stage1.run.assert_called_once()
        mock_quantize_stage.run.assert_called_once()
        mock_export_stage2.run.assert_called_once()
        mock_edge_stage.run.assert_called_once()
        mock_executorch_stage.run.assert_called_once()

...
for (key, value) in products {
  ...
  // Broken into typed sub-expressions because the single chained initializer
  // exceeds the Swift manifest compiler's type-check budget.
  ...
  let resources: [Resource] = (value["resources"] as? [String] ?? []).filter {
    // Generated by scripts/build_apple_frameworks.sh and gitignored, so include
    // it only when present, the way the test fixtures below do, or a consumer's
    // resolve fails before the build script has produced it.
...
// are produced by scripts/build_apple_frameworks.sh and gitignored, so each is

...
for (key, value) in products {
  ...
  let target: Target = .target(
    ...
    resources: (value["resources"] as? [String] ?? []).filter {
      // Generated by scripts/build_apple_frameworks.sh and gitignored, so
      // include it only when present, the way the test fixtures below do, or a
      // consumer's resolve fails before the build script has produced it.

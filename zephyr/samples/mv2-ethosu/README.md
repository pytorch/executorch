# MobileNetV2 Image Classification with Ethos-U NPU

This sample demonstrates running a quantized MobileNetV2 image classification
model on the Arm Ethos-U NPU using ExecuTorch within a Zephyr RTOS application.

The model classifies a static RGB test input tensor with shape `[1, 3, 224, 224]`
(NCHW) into one of 1000 ImageNet classes and prints the top-5 predictions.

## Prerequisites

- Zephyr SDK with ExecuTorch module enabled
- Python 3.12+ with ExecuTorch, torchvision, and ethos-u-vela installed
- A board with Arm Ethos-U NPU (e.g., Corstone-300 FVP, Alif E7/E8 DevKit)

## Corstone-300 FVP (Ethos-U55)

> **Note:** Corstone-300 has only 2 MiB ISRAM. The MV2 allocator pools require
> ~3 MiB, so the build will link but FVP execution will fail at runtime.
> Use Corstone-320 (below) for end-to-end MV2 inference.

### Setup FVP paths

Source the environment generated by `examples/arm/setup.sh` and set the MAC
count before configuring the build.

Config Zephyr Corstone300 FVP
<!-- RUN setup_corstone300 -->
```
source modules/lib/executorch/examples/arm/arm-scratch/setup_path.sh
export ARMFVP_EXTRA_FLAGS="-C ethosu.num_macs=128"
```

### Export the model

Export a quantized INT8 MobileNetV2 model with Ethos-U55 delegation:

<!-- RUN test_ethos-u55_generate_pte -->
```
python -m executorch.backends.arm.scripts.aot_arm_compiler --model_name=mv2_untrained --quantize --delegate --target=ethos-u55-128 --output=mv2_u55_128.pte
```

### Build (link-check only)

<!-- RUN test_ethos-u55_build -->
```
west build -b mps3/corstone300/fvp modules/lib/executorch/zephyr/samples/mv2-ethosu -- -DET_PTE_FILE_PATH=mv2_u55_128.pte
```

## Corstone-320 FVP (Ethos-U85)

### Setup FVP paths

Source the environment generated by `examples/arm/setup.sh` and set the MAC
count before configuring the build.

These FVP command-line options are passed through the `ARMFVP_EXTRA_FLAGS`
environment variable. The sample does not set `ARMFVP_FLAGS` in its
`CMakeLists.txt`; the base `ARMFVP_FLAGS` come from the selected Zephyr board's
`board.cmake`. Zephyr captures these options when CMake runs; after changing
them, use `west build -c` to reconfigure.

Config Zephyr Corstone320 FVP
<!-- RUN setup_corstone320 -->
```
source modules/lib/executorch/examples/arm/arm-scratch/setup_path.sh
export ARMFVP_EXTRA_FLAGS="--simlimit 10 -C mps4_board.subsystem.ethosu.num_macs=256"
```

### Export the model

Export a quantized INT8 MobileNetV2 model with Ethos-U85 delegation:

<!-- RUN test_ethos-u85_generate_pte -->
```
python -m executorch.backends.arm.scripts.aot_arm_compiler --model_name=mv2_untrained --quantize --delegate --target=ethos-u85-256 --output=mv2_u85_256.pte
```

### Build

<!-- RUN test_ethos-u85_build -->
```
west build -b mps4/corstone320/fvp modules/lib/executorch/zephyr/samples/mv2-ethosu -- -DET_PTE_FILE_PATH=mv2_u85_256.pte
```

### Run on FVP

```
west build -t run
```

The sample leaves Zephyr running after `MobileNetV2 Demo Complete`.
`--simlimit 10` stops the FVP after ten seconds of simulated time; confirm that
the completion message appears before it exits. The Ethos-U model can report
NPU cycle counts, while CPU and memory-system timing are simplified. This
sample reports elapsed Zephyr time rather than NPU cycles; use hardware to
measure deployment latency.

## Alif Ensemble E8 DevKit

For boards with Ethos-U55-256 (e.g., Alif E8 HP core), use `--target=ethos-u55-256`:

```bash
python -m executorch.backends.arm.scripts.aot_arm_compiler \
    --model_name=mv2_untrained \
    --quantize \
    --delegate \
    --target=ethos-u55-256 \
    --output=mv2_ethosu.pte
```

```bash
west build -d build -b ensemble_e8_dk/ae822fa0e5597ls0/rtss_hp \
    modules/lib/executorch/zephyr/samples/mv2-ethosu -- \
    -DET_PTE_FILE_PATH=mv2_ethosu.pte
```

### Flash with west
```bash
west flash
```
If `west flash` does not work on your board, flash with Alif SE Tools instead;
see the [Zephyr Alif tutorial](../../../docs/source/zephyr_alif_tutorial.md).

### Run

After flashing, make sure the board has `SW4=U4`, open the Zephyr
console at 115200 baud, and reset again:

```bash
picocom -b 115200 /dev/ttyACM0
```

## Expected output

```
========================================
ExecuTorch MobileNetV2 Classification Demo
========================================

Ethos-U backend registered successfully
Model loaded, has 1 methods
Inference completed in <N> ms

--- Classification Results ---
Top-5 predictions:
  [1] class <id>: <score>
  [2] class <id>: <score>
  ...

MobileNetV2 Demo Complete
Inference time: <N> ms
========================================
```

When using `mv2_untrained`, the output class IDs will be arbitrary since the
model has no trained weights. Use `mv2` (requires torchvision pretrained
weights) for meaningful predictions.

## Memory requirements

The default configuration reserves 1.5 MiB each for the method and temporary
allocator pools. These defaults support this INT8 MobileNetV2 example with
Ethos-U delegation and CPU quantization/dequantization at the model boundary.
Adjust `CONFIG_EXECUTORCH_METHOD_ALLOCATOR_POOL_SIZE` and
`CONFIG_EXECUTORCH_TEMP_ALLOCATOR_POOL_SIZE` in `prj.conf`, in board-specific
`*.conf` files (for example, `boards/<board>.conf`), or via Zephyr's
`OVERLAY_CONFIG` mechanism for different model configurations.

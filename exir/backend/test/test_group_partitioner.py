...
class TestGroupPartitioner(unittest.TestCase):
    class TestOperatorSupport(OperatorSupportBase):
        ...
        def is_node_supported(self, submodules, node):  # submodules is required by interface
            if node.op == "get_attr":
                return True
    ...
    def create_both_partitioners(self, exported_program, op_support,
                                allows_single_node_partition=True,
    ...
    def run_and_compare_partitioners(self, group_partitioner, capability_partitioner, test_name=""):
        """
    ...
    def compare_partitions(self, partitions1, partitions2):
        ...
        if len(partitions1) != len(partitions2):
            print(f"Different number of partitions: {len(partitions1)} vs {len(partitions2)}")
            return False

        # Convert partitions to sets of node names for easier comparison
        partition_sets1 = [frozenset(node.name for node in p.nodes) for p in partitions1]
        partition_sets2 = [frozenset(node.name for node in p.nodes) for p in partitions2]
    ...
    def test_null_node_groups_complex_model(self):
        ...
        self.assertTrue(linear_relu_found or linear_tanh_found,
    ...
    def test_with_complex_graph_structure(self):
        ...
        # Add more supported nodes for this test
        op_support = self.TestOperatorSupport()
        op_support.add_supported_nodes(["add_1", "add_2", "linear_3", "linear_4", "linear_5"])
    ...
    def test_with_fusion_patterns(self):
        ...
        self.assertTrue(linear_relu_fusion, "Linear->ReLU fusion pattern should be preserved")
        self.assertTrue(linear_tanh_fusion, "Linear->Tanh fusion pattern should be preserved")
    ...
    def test_with_large_model(self):
        ...
        ] + ["sigmoid", "sigmoid_1", "sigmoid_2", "tanh_1", "tanh_2", "relu_1", "relu_2"])

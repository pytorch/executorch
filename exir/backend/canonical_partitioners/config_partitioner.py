...
class PartitionerConfig(ABC):
    ...
    The Config-Based Partitioner then uses these overriden methods to find nodes
...
class ConfigerationBasedPartitioner(Partitioner):
    def __init__(
        ...
        self.partitioner_configs: Dict[str, PartitionerConfig] = {}
        for config in partitioner_configs:
            target_name = config.target_name
            if target_name in self.partitioner_configs:
                other_config = self.partitioner_configs[target_name]
                raise RuntimeError(
                    f"PartitionerConfig: {config} and {other_config} have the same target_name"
                )
            else:
                self.partitioner_configs[target_name] = config
    ...
    def ops_to_not_decompose(
        ...
        def filter_fn(node: torch.fx.Node) -> bool:
            ...
            if target_name in self.partitioner_configs:
                config = self.partitioner_configs[target_name]
                # only filter_fn if config has original_aten
                if config.get_original_aten():
                    return self.partitioner_configs[target_name].check_constraints(
                        node, ep
                    )
        ...
        do_not_decomp = []
        for node_config in self.partitioner_configs.values():
    ...
    def get_matched_nodes_from_configs(
        ...
        for node in gm.graph.nodes:
            if node.op == "call_function":
                ...
                if target in self.partitioner_configs:
                    node_config = self.partitioner_configs[target]

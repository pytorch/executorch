...
class TestPasses(unittest.TestCase):
    ...
    def test_replace_view_copy_with_memory_view_pass(self) -> None:  # noqa: C901
        ...
        # Do transformation
        p = ReplaceViewCopyWithMemoryViewPass()
        p.set_program(ep)
        gm_res = p(gm)
        assert gm_res is not None
        gm = gm_res.graph_module
        ...
        # Only the static view_copy + parameter view_copy are replaced
        self.assertEqual(n_view_copy_after, 3)
        self.assertEqual(n_memory_view_after, 2)

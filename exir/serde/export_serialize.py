...
from torch._export.serde.upgrade import GraphModuleOpUpgrader
...
def _int_to_sympy_int(val) -> sympy.Expr:
    ...
    if val == -math.inf:
        return -sympy.oo
    return sympy.Integer(val)  # pyre-ignore
...
class GraphModuleSerializer:
        ...
        if node.target in _SYM_INT_OPS:
            ...
            ex_node = Node(
                ...
                inputs=self.serialize_inputs(
                    node.target, node.args, node.kwargs  # pyre-ignore
                ),
    ...
    def serialize_input(self, arg) -> Argument:  # noqa: C901
        ...
        if isinstance(arg, torch.fx.Node):
                ...
                if isinstance(attr, torch.Tensor):
                    ...
                    return Argument.create(
                        as_graph=GraphArgument(
                            name=arg.target, graph=graph  # pyre-ignore
                        )
                    )
                else:
                    raise SerializeError(
            ...
            if not (
                arg._has_method("__getstate__")  # pyre-ignore
        ...
        if spec.kind == ep.InputKind.USER_INPUT:
            ...
            return InputSpec.create(
                parameter=InputToParameterSpec(
                    ...
                    parameter_name=spec.target,  # pyre-ignore
                )
            )
            ...
            return InputSpec.create(
                tensor_constant=InputToTensorConstantSpec(
                    arg=TensorArgument(name=spec.arg.name),
                    tensor_constant_name=spec.target,  # pyre-ignore
                )
            )
        else:
        ...
        if spec.kind == ep.OutputKind.USER_OUTPUT:
            ...
            return OutputSpec.create(
                gradient_to_user_input=GradientToUserInputSpec(
                    arg=TensorArgument(name=spec.arg.name),
                    user_input_name=spec.target,  # pyre-ignore
                )
            )
        else:
            raise AssertionError(f"Unknown argument kind: {spec}")
    ...
    def serialize_outputs(self, node: torch.fx.Node) -> List[Argument]:  # noqa: C901
        ...
        if _is_single_tensor_return(node.target):  # pyre-ignore
            # e.g "-> Tensor"
            return [
                Argument.create(
                    as_tensor=self.serialize_tensor_output(node.name, meta_val)
        ...
        elif _is_single_tensor_list_return(node.target):  # pyre-ignore
            # e.g "-> Tensor[]"
            tensor_args = []
...
class GraphModuleDeserializer:
    ...
    def deserialize_sym_int(self, s: SymInt) -> Union[int, torch.SymInt]:
        if s.type == "as_expr":
            val = s.as_expr
            if val.expr_str in self.symbol_name_to_symbol:  # pyre-ignore
                ...
                if isinstance(sym, sympy.Symbol):
                    ...
                    if vr := self.symbol_name_to_range.get(val.expr_str):  # pyre-ignore
                        symbolic_shapes._constrain_symbol_range(
                            self.shape_env,  # pyre-ignore
                            sym,
            ...
            if val.hint is None:
                ...
                hint = val.hint.value  # pyre-ignore
    ...
    def deserialize_sym_bool(self, s: SymBool) -> Union[bool, torch.SymBool]:
        ...
        if s.type == "as_expr":
            expr = sympy.sympify(
                val.expr_str, locals=self.symbol_name_to_symbol  # pyre-ignore
            )
            return self.shape_env.create_symboolnode(expr)  # pyre-ignore
        elif s.type == "as_bool":
    ...
    def deserialize_graph(self, serialized_graph: Graph) -> torch.fx.Graph:
        ...
        for name, tensor_value in serialized_graph.tensor_values.items():
            meta_val = self.deserialize_tensor_meta(
                tensor_value, self.fake_tensor_mode  # pyre-ignore
            )
    ...
    def deserialize(
        ...
        self.shape_env = symbolic_shapes.ShapeEnv(  # pyre-ignore
            assume_static_by_default=True
        )
        self.fake_tensor_mode = FakeTensorMode(  # pyre-ignore
            allow_fallback_kernels=False,
            allow_non_fake_inputs=True,
            shape_env=self.shape_env,
        )
        self.symbol_name_to_symbol: Dict[str, sympy.Symbol] = {}  # pyre-ignore
        self.symbol_name_to_range = (  # pyre-ignore
            {} if symbol_name_to_range is None else symbol_name_to_range
        )
        self.constants = {} if constants is None else constants  # pyre-ignore
    ...
    def deserialize_input(self, inp: Argument) -> Any:  # noqa: C901
        value = inp.value
        typ_ = inp.type
        if typ_ == "as_none":
            ...
            return self.constants[inp.as_custom_obj.name]  # pyre-ignore
        else:
            raise SerializeError(f"Unhandled argument {inp}")
    ...
    def deserialize_outputs(self, serialized_node: Node, fx_node: torch.fx.Node):
        ...
        if len(returns) == 0:
            return
        if _is_single_tensor_return(fx_node.target):  # pyre-ignore
...
def _dict_to_dataclass(cls, data):
    ...
    if typing.get_origin(cls) == typing.Union and type(None) in typing.get_args(cls):
        ...
        obj = cls.create(**entry)
        ty = list(entry.keys())[0]
        field_type = cls.__annotations__[ty]
        setattr(obj, ty, _dict_to_dataclass(field_type, obj.value))
        return obj

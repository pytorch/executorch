...
class _Emitter(torch.fx.Interpreter):
    ...
    def _emit_spec(self, spec: ValueSpec) -> _EmitterValue:
        ...
        def _process(spec: LeafValueSpec) -> _AbstractValue:
            ...
            # Spec was already emitted (FX-level alias) — reuse the
            # existing Value so two FX nodes sharing one TensorSpec
            # also share one value_id in the lowered IR.
            existing_id = self.emitter_state.spec2id_dict.get(spec)
            if existing_id is not None:
                existing_evalue = self.emitter_state.values[existing_id]
                tensor = (
                    existing_evalue.val
                    if isinstance(existing_evalue.val, Tensor)
                    else None
                )
                return _AbstractValue(existing_id, tensor)

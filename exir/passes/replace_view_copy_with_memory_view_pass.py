...
class ReplaceViewCopyWithMemoryViewPass(PassBase):
    def __init__(self) -> None:
        super().__init__()
        self._program: Optional[ExportedProgram] = None
    ...
    def call(self, graph_module: torch.fx.GraphModule) -> PassResult:
        ...
        logging.debug(f"Replaced {n_replaced} view_copy nodes with memory.view nodes.")
    ...
    def set_program(self, program: ExportedProgram) -> None:
    ...
    def _is_replaceable_view_copy(self, node: torch.fx.Node) -> bool:
        ...
        if base.op == "placeholder":
            # For now, we only assume placeholder parameters are memory planned.
            # We cautiously assume that general user inputs and buffers are not memory planned.
            ...
            assert isinstance(self._program, ExportedProgram)
            if base.name in self._program.graph_signature.inputs_to_parameters:

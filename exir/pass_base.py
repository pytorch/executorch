...
# NOTE: dim_order_ops is intentionally excluded. Its output metadata (strides /
# dim_order) depends on the dim_order kwarg rather than on input metadata alone,
# so caching its FakeTensor dispatch corrupts stride propagation through retraced
# control-flow submodules (e.g. while_loop bodies), producing functionally
# transposed results. See test_compile_fix_broken_ops.
    ...
    try:
        ...
        try:
            ...
            def _is_nonbuiltin_bypass(v: object) -> bool:
                return (
                    isinstance(v, _DispatchCacheBypassEntry)
                    and v.reason == "non-builtin"
                )
        ...
        try:
            with fake_tensor_mode, dispatcher_mode:  # type: ignore[assignment, union-attr]
                with _extend_faketensor_cache_builtins():
                    return self.call_submodule(graph_module, tuple(inputs))
        finally:

...
def _get_contiguous_dim_order(ndim: int) -> List[int]:
    if ndim < 0:
        raise AssertionError(
            f"Unsupported rank for contiguous dim order. Now support ndim larger than or equal to 0, but got {ndim}"
        )
...
def _get_channels_last_dim_order(ndim: int) -> List[int]:
    ...
    raise AssertionError(
        f"Unsupported rank for channels last dim order. Now only support ndim equals to 4, but got  {ndim}"
    )

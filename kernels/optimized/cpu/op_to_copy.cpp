...
void float_to_bfloat16_range(
  ...
  // Integer rounding preserves subnormals regardless of FPCR, like BFloat16.
...
#pragma GCC unroll 4
  ...
  for (; i + kVectorWidth <= end; i += kVectorWidth) {
    ...
    // Avoid aliasing BFloat16 storage as a NEON vector.
...
bool convert_contiguous(const Tensor& self, Tensor& out) {
  ...
  if (numel > ::executorch::extension::internal::GRAIN_SIZE) {
    return ::executorch::extension::parallel_for(
...
Tensor& opt_to_copy_out(
  ...
  if (can_use_optimized_kernel) {
    ...
    const bool success = float_to_bfloat16

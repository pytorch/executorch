...
- [`//kernels/portable/edge_dialect_ops.yaml`](https://github.com/pytorch/executorch/blob/main/kernels/portable/edge_dialect_ops.yaml)
  - Add your entry here if your operator is **custom operator**, *mandatory* to ExecuTorch in edge dialect, and model *agnostic*.
    ...
    (e.g. `dim_order_ops::_to_dim_order_copy_out`, This operator, exclusive to the edge dialect, creates a tensor from another with a specific dimension order and data type. As the dimension order is unique to the edge dialect, this operator is considered edge-dialect-specific.)

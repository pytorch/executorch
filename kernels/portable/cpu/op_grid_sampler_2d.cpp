...
T get_pixel_value(
  ...
  return input_data
      [n * batch_stride + c * channel_stride + h * height_stride + w];
}
...
void grid_sampler_2d_impl(
  ...
  const T* input_data = input.data_ptr<T>(); // (N, C, inp_H, inp_W) shape
  T* out_data = out.data_ptr<T>(); // (N, C, out_H, out_W) shape
  const float* grid_data = grid.data_ptr<float>(); // (N, out_H, out_W, 2) shape
    ...
    for (const auto c : c10::irange(C)) {
      for (const auto h : c10::irange(out_H)) {
        for (const auto w : c10::irange(out_W)) {
          ...
          const int64_t grid_offset = n * grid_batch_stride +
              h * grid_height_stride + w * grid_width_stride;
          const float x = grid_data[grid_offset];
          const float y = grid_data[grid_offset + 1];

          // Calculate output index
          const int64_t out_idx = n * out_batch_stride +
              c * out_channel_stride + h * out_height_stride + w;

          // Apply interpolation method
          if (interpolation_mode == 0) { // Bilinear
            // Calculate corner indices and weights
            auto [ix_nw, lambda_x] =
                compute_source_index_and_weight(x, inp_W, align_corners);
            auto [iy_nw, lambda_y] =
                compute_source_index_and_weight(y, inp_H, align_corners);

            // Calculate bilinear weights
            float w_nw = (1.0f - lambda_x) * (1.0f - lambda_y);
            float w_ne = lambda_x * (1.0f - lambda_y);
            float w_sw = (1.0f - lambda_x) * lambda_y;
            float w_se = lambda_x * lambda_y;

            // Get corner pixel values with boundary checking
            T nw = get_pixel_value(
                input_data, n, c, iy_nw, ix_nw, inp_H, inp_W, padding_mode);
            T ne = get_pixel_value(
                input_data, n, c, iy_nw, ix_nw + 1, inp_H, inp_W, padding_mode);
            T sw = get_pixel_value(
                input_data, n, c, iy_nw + 1, ix_nw, inp_H, inp_W, padding_mode);
            T se = get_pixel_value(
                input_data,
                n,
                c,
                iy_nw + 1,
            ...
            out_data[out_idx] =
                static_cast<T>(nw * w_nw + ne * w_ne + sw * w_sw + se * w_se);

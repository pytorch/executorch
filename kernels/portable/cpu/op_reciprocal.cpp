...
Tensor& reciprocal_out(RuntimeContext& ctx, const Tensor& in, Tensor& out) {
  ...
  ET_SWITCH_REAL_TYPES(in.scalar_type(), ctx, "reciprocal", CTYPE_IN, [&] {
    ET_SWITCH_FLOAT_TYPES(out.scalar_type(), ctx, "reciprocal", CTYPE_OUT, [&] {
      apply_unary_map_fn(
          [](const CTYPE_IN val_in) {
            CTYPE_OUT xi = static_cast<CTYPE_OUT>(val_in);
            return static_cast<CTYPE_OUT>(1.0 / xi);
          },

...
bool check_as_strided_copy_args(
  ...
  ET_LOG_MSG_AND_RETURN_UNLESS(offset >= 0, "Negative storage offset");
...
bool check_cat_args(
  ...
  ET_LOG_AND_RETURN_UNLESS(tensors.size() > 0);
  ...
  for (size_t i = 0; i < tensors.size(); ++i) {
    ...
    ET_LOG_AND_RETURN_UNLESS(tensors_have_same_dim_order(tensors[i], out));
    ...
    ET_LOG_AND_RETURN_UNLESS(tensor_is_rank(tensors[ref_i], tensors[i].dim()));
  ...
  ET_LOG_AND_RETURN_UNLESS(dim >= 0);
...
bool check_expand_copy_args(
  ...
  ET_LOG_AND_RETURN_UNLESS(tensors_have_same_dtype(input, out));
...
bool check_permute_copy_args(const Tensor& in, IntArrayRef dims, Tensor& out) {
  ET_LOG_AND_RETURN_UNLESS(tensor_is_rank(in, dims.size()));
  ET_LOG_AND_RETURN_UNLESS(tensors_have_same_dtype(in, out));
  ...
  for (int i = 0; i < dims.size(); i++) {
    ET_LOG_AND_RETURN_UNLESS(tensor_has_dim(in, dims[i]));
    ...
    ET_LOG_AND_RETURN_UNLESS(dim < kTensorDimensionLimit && dim >= 0);
...
bool check_pixel_shuffle_args(
    ...
    Tensor& out) {
  ET_LOG_AND_RETURN_UNLESS(tensors_have_same_dtype(in, out));
  ET_LOG_AND_RETURN_UNLESS(tensor_has_rank_greater_or_equal_to(in, 3));
  ET_LOG_AND_RETURN_UNLESS(tensor_has_rank_greater_or_equal_to(out, 3));
  ET_LOG_AND_RETURN_UNLESS(upscale_factor > 0);
  ET_LOG_AND_RETURN_UNLESS(
...
bool check_pixel_unshuffle_args(
  ...
  ET_LOG_AND_RETURN_UNLESS(tensor_has_rank_greater_or_equal_to(in, 3));
  ET_LOG_AND_RETURN_UNLESS(tensor_has_rank_greater_or_equal_to(out, 3));
  ET_LOG_AND_RETURN_UNLESS(downscale_factor > 0);
  ET_LOG_AND_RETURN_UNLESS(in.size(in.dim() - 1) % downscale_factor == 0);
  ET_LOG_AND_RETURN_UNLESS(in.size(in.dim() - 2) % downscale_factor == 0);
  return true;
}
...
bool check_select_copy_out_args(
  ...
  ET_LOG_AND_RETURN_UNLESS(tensor_dim_has_index(in, dim, index));
  ET_LOG_AND_RETURN_UNLESS(tensors_have_same_dtype(in, out));
  return true;
}
...
bool check_stack_args(
  ...
  for (size_t i = 0; i < tensors.size(); i++) {
    ...
    ET_LOG_AND_RETURN_UNLESS(tensor_is_rank(tensors[i], tensors[0].dim()));
    for (size_t d = 0; d < tensors[i].dim(); d++) {
      ET_LOG_AND_RETURN_UNLESS(
  ...
  ET_LOG_AND_RETURN_UNLESS(dim >= 0 && dim < tensors[0].dim() + 1);
...
bool check_tril_args(const Tensor& in, Tensor& out) {
  ET_LOG_AND_RETURN_UNLESS(tensors_have_same_dtype(in, out));
  ET_LOG_AND_RETURN_UNLESS(tensor_has_rank_greater_or_equal_to(in, 2));
  return true;
}
...
bool check__to_dim_order_copy_args(
  ...
  if (dim_order.has_value()) {
    ...
    ET_LOG_AND_RETURN_UNLESS(out_dim_order.size() == input_dim_order.size());
    for (size_t i = 0; i < input_dim_order.size(); i++) {
      ET_LOG_AND_RETURN_UNLESS(out_dim_order[i] == input_dim_order[i]);
    }
  }
  return true;
}
...
bool check_unsqueeze_copy_args(
  ...
  ET_LOG_AND_RETURN_UNLESS(tensor_has_dim(out, dim));
  ...
  // 4. out.size(dim) == 1
  ET_LOG_AND_RETURN_UNLESS(input.dim() == out.dim() - 1);
...
bool check_view_copy_args(
    ...
    Tensor& out) {
  ET_LOG_AND_RETURN_UNLESS(size_int64_t.size() == out.sizes().size());
  ...
  ET_LOG_MSG_AND_RETURN_UNLESS(
      ...
      self.numel(),
      out.numel());
  ET_LOG_AND_RETURN_UNLESS(tensors_have_same_dtype(self, out));
...
bool get_view_copy_target_size(
  ...
  ET_LOG_AND_RETURN_UNLESS(size_int64_t.size() == dim);
...
bool check_diagonal_copy_args(
  ...
  ET_LOG_AND_RETURN_UNLESS(tensors_have_same_dtype(in, out));
  ET_LOG_AND_RETURN_UNLESS(tensor_has_rank_greater_or_equal_to(in, 2));
  ET_LOG_AND_RETURN_UNLESS(tensor_has_dim(in, dim1));
  ET_LOG_AND_RETURN_UNLESS(tensor_has_dim(in, dim2));
  if (dim1 < 0) {
    dim1 += nonzero_dim(in);
  }
  if (dim2 < 0) {
  ...
  ET_LOG_AND_RETURN_UNLESS(dim1 != dim2);
  return true;
}

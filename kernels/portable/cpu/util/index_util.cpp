...
bool check_index_select_args(
  ...
  ET_LOG_AND_RETURN_UNLESS(tensor_has_rank_smaller_or_equal_to(index, 1));
...
bool check_scatter_add_args(
  ...
  ET_LOG_AND_RETURN_UNLESS(tensors_have_same_dtype(self, src));
  ET_LOG_MSG_AND_RETURN_UNLESS(
      index.scalar_type() == ScalarType::Long,
      "Expected dypte int64 for index");
  ET_LOG_AND_RETURN_UNLESS(tensor_has_dim(self, dim));
...
bool check_select_scatter_args(
  ...
  ET_LOG_AND_RETURN_UNLESS(tensors_have_same_dtype(in, output));
  ...
  for (ssize_t d = 0; d < in.dim() - 1; d++) {
    if (d < dim) {
      ET_LOG_AND_RETURN_UNLESS(tensors_have_same_size_at_dims(in, d, src, d));
    } else {

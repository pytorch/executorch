...
#define GENERATE_SCALAR_OVERFLOW_TESTS(TEST_SUITE_NAME)                    \
  ...
  TEST_F(TEST_SUITE_NAME, LongTensorTooLargeScalarDies) {                  \
    /* 2^63 is one past the largest int64_t, so converting it is undefined \
     * unless the range check rejects it first. The add suites reject it   \
     * earlier, on the alpha type, so there this case only repeats their   \
     * existing floating-point alpha coverage. */                          \

...
Tensor& _cos_out(const Tensor& self, Tensor& out) {
...
void test_floating_point_cos_out(
  ...
  // clang-format off
  _cos_out(tf_in.make({1, 6}, { 0, 1, 3, 5, 10, 100 }), out);
...
void test_cos_invalid_output_dtype_dies() {
  ...
  ET_EXPECT_KERNEL_FAILURE(_cos_out(in, out));
}
...
TEST(OpCosOutKernelTest, MismatchedInputShapesDies) {
  ...
  ET_EXPECT_KERNEL_FAILURE(_cos_out(a, out));
}

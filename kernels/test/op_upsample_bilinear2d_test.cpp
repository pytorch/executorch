...
TEST_F(OpUpsampleBilinear2dTest, SmokeTestScales) {
  ...
  op_upsample_bilinear2d_vec_out(input, {}, false, OptionalArrayRef<double>({ scale_factors.data(), scale_factors.size() }), out);
...
TEST_F(OpUpsampleBilinear2dTest, SmokeTestAlignCornersScales) {
  ...
  const std::array<double, 2> scale_factors = {3.0 / 2, 4.0 / 3};
  op_upsample_bilinear2d_vec_out(input, {}, true, OptionalArrayRef<double>({ scale_factors.data(), scale_factors.size() }), out);
...
TEST_F(OpUpsampleBilinear2dTest, ZeroComputedOutputSizeDies) {
  ...
  ET_EXPECT_KERNEL_FAILURE(
      context_, op_upsample_bilinear2d_vec_out(input, {}, false, OptionalArrayRef<double>({scale_factors.data(), scale_factors.size()}), out));
}

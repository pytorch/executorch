...
Tensor&
_amin_out(const Tensor& in, ArrayRef<int64_t> dim, bool keepdim, Tensor& out) {
...
void test_amin_out_invalid_dimensions() {
  ...
  ET_EXPECT_DEATH(_amin_out(in, dim_list, /*keepdim=*/true, out), "");
...
void test_amin_out_invalid_shape() {
  ...
  out = tf.zeros({2, 1, 4});
  ET_EXPECT_DEATH(_amin_out(in, dim_list, /*keepdim=*/false, out), "");
}
...
void test_amin_out_dtype() {
  ...
  _amin_out(in, dim_list, /*keepdim=*/true, out);
  // clang-format off
  ...
  out = tf.zeros({2, 3});
  _amin_out(in, dim_list, /*keepdim=*/false, out);
  // clang-format off
  EXPECT_TENSOR_CLOSE(out, tf.make(
    {2, 3},
    {0, 0, 1, 0, 1, 2}));
  ...
  _amin_out(in, null_dim_list, /*keepdim=*/true, out);
  EXPECT_TENSOR_CLOSE(out, tf.make({1, 1, 1}, {2}));

  ArrayRef<int64_t> empty_dim_list{ArrayRef<int64_t>{}};
  _amin_out(in, empty_dim_list, /*keepdim=*/true, out);
  EXPECT_TENSOR_CLOSE(out, tf.make({1, 1, 1}, {2}));

  out = tf.zeros({});
  _amin_out(in, null_dim_list, /*keepdim=*/false, out);
  EXPECT_TENSOR_CLOSE(out, tf.make({}, {2}));

  _amin_out(in, empty_dim_list, /*keepdim=*/false, out);
  EXPECT_TENSOR_CLOSE(out, tf.make({}, {2}));
}
...
void test_amin_out_dtype<ScalarType::Bool>() {
  ...
  _amin_out(in, /*dim=*/-1, /*keepdim=*/true, out);
  // clang-format off
  EXPECT_TENSOR_CLOSE(
      out, tf_bool.make(
        {2, 3, 1},

...
TEST(OpEmbeddingOutTest, WeightWrongDimensionsDies) {
  ...
  // clang-format on
  ET_EXPECT_KERNEL_FAILURE(embedding_out(
      weight,
      indices,
      /*padding_idx=*/0,
      /*scale_grad_by_freq=*/false,
...
embedding_template = f"""
  ...
  embedding_out(weight, indices, $padding$, $scale$, $sparse$, out);
...
void test_dynamic_shape(
  ...
  embedding_out(weight, indices, 0, false, false, out);
  EXPECT_TENSOR_CLOSE(out, expected);
}

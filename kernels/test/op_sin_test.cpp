...
Tensor& _sin_out(const Tensor& self, Tensor& out) {
...
void test_floating_point_sin_out(
  ...
  // clang-format off
  _sin_out(tf_in.make({1, 6}, { 0, 1, 3, 5, 10, 100 }), out);
...
void test_sin_invalid_output_dtype_dies() {
  ...
  ET_EXPECT_KERNEL_FAILURE(_sin_out(in, out));
}
...
TEST(OpSinOutKernelTest, MismatchedInputShapesDies) {
  ...
  ET_EXPECT_KERNEL_FAILURE(_sin_out(a, out));
}

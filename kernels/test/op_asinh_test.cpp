...
Tensor& _asinh_out(const Tensor& self, Tensor& out) {
...
void test_floating_point_asinh_out(
  ...
  // clang-format off
  _asinh_out(tf_in.make({1, 6}, { 0, 1, 3, 5, 10, 100 }), out);
...
void test_asinh_invalid_output_dtype_dies() {
  ...
  ET_EXPECT_KERNEL_FAILURE(_asinh_out(in, out));
}
...
TEST(OpAsinhOutKernelTest, MismatchedInputShapesDies) {
  ...
  ET_EXPECT_KERNEL_FAILURE(_asinh_out(a, out));
}

...
class OpToTest : public OperatorTest {
 protected:
  ...
  void test_conversion_at_sizes(
    ...
    static_assert(
        (std::is_same_v<INPUT_CTYPE, float> &&
         std::is_same_v<OUTPUT_CTYPE, BFloat16>) ||
            (std::is_same_v<INPUT_CTYPE, BFloat16> &&
             std::is_same_v<OUTPUT_CTYPE, float>),
        "Only float/BFloat16 conversion pairs are supported");
...
TEST_F(OpToTest, FloatToBFloat16SubnormalsIgnoreFlushToZero) {
  ET_SKIP_IF(
      torch::executor::testing::SupportedFeatures::get()->is_aten,
      "ATen conversion may flush subnormals to zero");
  ...
  for (const bool flush_to_zero : {false, true}) {
    SCOPED_TRACE(::testing::Message() << "flush_to_zero=" << flush_to_zero);
    const uint64_t fpcr = flush_to_zero ? original_fpcr.value | kFlushToZero
                                        : original_fpcr.value & ~kFlushToZero;
    ...
    test_conversion_at_sizes<
        ...
        ScalarType::BFloat16>(
        {1, 7, 8, 9, 15, 16, 17},
        {float_from_bits(0x00018000), float_from_bits(0x80018000)},
        {BFloat16(0x0002, BFloat16::from_bits()),
         BFloat16(0x8002, BFloat16::from_bits())});
  }
}
#endif

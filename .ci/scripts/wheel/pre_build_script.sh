...
raw = sys.argv[1]
row = re.sub(r'[^0-9]', '', raw)
trains = {f'{major}{minor}' for major, minor in SUPPORTED_CUDA_VERSIONS}
if row in trains:
    print('cuda')
elif row and raw.lower().startswith(('cu', 'cuda')):
    # Names a CUDA train this project does not support. Reported rather than built as
    # CPU, because the row's name reaches the published index either way.
    print('unsupported')
else:
    print('cpu')
PY

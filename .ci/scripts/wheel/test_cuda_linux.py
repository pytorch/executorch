...
The build machines for these rows have no GPU, so this does not execute a model. What runs a
model on real hardware before a publication is a separate release-time step that a person owns
today, not an automated job wired into these workflows.
"""

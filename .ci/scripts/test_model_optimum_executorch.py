...
def test_text_generation(model_id, recipe, model_dir, run_only=False):
    command = [
        "optimum-cli",
        "export",
        "executorch",
...
def test_fill_mask(model_id, recipe, model_dir, run_only=False):
    command = [
        "optimum-cli",
        "export",
        "executorch",
...
def test_t5(model_id, recipe, model_dir, run_only=False):
...
def test_whisper(model_id, recipe, model_dir, run_only=False):
...
def test_vit(model_id, recipe, model_dir, run_only=False):
...
# Generated text (CoreML's b4w): Simply put, the theory of relativity states that the speed of light is the same for all observers, regardless of their relative motion. This means that the speed of light is the same for an observer moving with respect to the source of light as it is for an observer moving with respect to the observer. This is
# Generated text (torchao's b4w): Simply put, the theory of relativity states that the speed of light is the same for all observers. This is a very strange concept, and it is one of the most controversial and debated ideas in physics. The theory of relativity was first proposed by the German physicist Albert Einstein in 1905, and it
...
if __name__ == "__main__":
    ...
    model_registry = {
        "smollm": ("HuggingFaceTB/SmolLM2-135M", test_text_generation, "coreml_fp32_gpu"), # works
        "qwen3": ("Qwen/Qwen3-0.6B", test_text_generation, "coreml_fp32_gpu"), # works
        "olmo": ("allenai/OLMo-1B-hf", test_text_generation, "coreml_fp32_gpu"), # works
        "gemma3": ("unsloth/gemma-3-1b-it", test_text_generation, "coreml_fp32_gpu"), # does not export
        "phi4": ("microsoft/Phi-4-mini-instruct", test_text_generation, "coreml_fp32_gpu"), # fails to lower
        "llama3": ("NousResearch/Llama-3.2-1B", test_text_generation, "coreml_fp32_gpu"), # works
        "bert": ("google-bert/bert-base-uncased", test_fill_mask, "coreml_fp32_gpu"), # works
        "roberta": ("FacebookAI/xlmcl-roberta-base", test_fill_mask, "coreml_fp32_gpu"), # works
        "distilbert": ("distilbert/distilbert-base-uncased", test_fill_mask, "coreml_fp32_gpu"), # works
        "whisper": ("openai/whisper-tiny", test_whisper, "coreml_fp32_gpu"), # works
        "t5": ("google-t5/t5-small", test_t5, "coreml_fp32_gpu"), # CoreML runime failure
        "vit": ("google/vit-base-patch16-224", test_vit, "coreml_fp16_ne"), # works
    }

    if args.model_name not in model_registry:
        raise ValueError(f"Unknown model name: {args.model_name}. Available models: {model_registry.keys()}")
    
    with tempfile.TemporaryDirectory() as tmp_dir:
        model_id, test_func, recipe = model_registry[args.model_name]
        test_func(model_id=model_id, recipe=recipe, model_dir=tmp_dir, run_only=False)

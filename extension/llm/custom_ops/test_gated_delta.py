        ...
        for seq_len in (4, 1):  # T != 1 (chunked route) and T == 1 (decode route)
            with self.subTest(seq_len=seq_len):
                ...
                with tempfile.TemporaryDirectory() as tmp:
                    ...
                    # write_etdump_result_to_file only produces a file when the
                    # runtime was built with ET_EVENT_TRACER_ENABLED (buckconfig
                    # executorch.event_tracer_enabled=true, exercised by the CI
                    # variant labeled on this target and by local -c runs). The
                    # default tracer-off build records no events; skip the ETDump
                    # assertion there — execution + correctness above still ran.

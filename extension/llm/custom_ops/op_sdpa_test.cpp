...
op_gated_delta_rule(
    ...
    const executorch::aten::Tensor& key,
    const executorch::aten::Tensor& value,
    const executorch::aten::Tensor& decay,
    const executorch::aten::Tensor& beta,
...
// A scalar decay of [B, Hkv, T] must produce exactly what the channelwise
// kernel produces when the same value is broadcast across the K state rows.
void test_scalar_decay_matches_broadcast_channelwise(
    int32_t sequence_length,
    int32_t query_heads_per_kv) {
  TensorFactory<executorch::aten::ScalarType::Float> tfFloat;
  constexpr int32_t batch_size = 2;
  constexpr int32_t num_kv_heads = 2;
  constexpr int32_t k_head_dim = 6;
  constexpr int32_t v_head_dim = 4;
  const int32_t num_query_heads = num_kv_heads * query_heads_per_kv;

  auto fill = [](std::vector<float>& values, float scale, size_t period) {
    for (size_t i = 0; i < values.size(); ++i) {
      values[i] =
          static_cast<float>(i % period) - static_cast<float>(period / 2);
      values[i] *= scale;
    }
  };

  const size_t query_numel = static_cast<size_t>(batch_size) * num_query_heads *
      sequence_length * k_head_dim;
  const size_t key_numel = static_cast<size_t>(batch_size) * num_kv_heads *
      sequence_length * k_head_dim;
  const size_t value_numel = static_cast<size_t>(batch_size) * num_kv_heads *
      sequence_length * v_head_dim;
  const size_t beta_numel =
      static_cast<size_t>(batch_size) * num_kv_heads * sequence_length;
  const size_t state_numel =
      static_cast<size_t>(batch_size) * num_kv_heads * k_head_dim * v_head_dim;

  std::vector<float> query_values(query_numel);
  std::vector<float> key_values(key_numel);
  std::vector<float> value_values(value_numel);
  std::vector<float> beta_values(beta_numel);
  std::vector<float> scalar_decay_values(beta_numel);
  std::vector<float> initial_state_values(state_numel);
  fill(query_values, 0.11f, 7);
  fill(key_values, 0.13f, 5);
  fill(value_values, 0.17f, 9);
  fill(beta_values, 0.07f, 6);
  fill(initial_state_values, 0.05f, 11);
  for (size_t i = 0; i < beta_numel; ++i) {
    scalar_decay_values[i] = 0.5f + static_cast<float>(i % 5) * 0.1f;
  }

  // Same gate value repeated across every channel of the head.
  std::vector<float> channelwise_decay_values(beta_numel * k_head_dim);
  for (size_t i = 0; i < beta_numel; ++i) {
    std::fill(
        channelwise_decay_values.begin() + i * k_head_dim,
        channelwise_decay_values.begin() + (i + 1) * k_head_dim,
        scalar_decay_values[i]);
  }

  auto query = tfFloat.make(
      {batch_size, num_query_heads, sequence_length, k_head_dim}, query_values);
  auto key = tfFloat.make(
      {batch_size, num_kv_heads, sequence_length, k_head_dim}, key_values);
  auto value = tfFloat.make(
      {batch_size, num_kv_heads, sequence_length, v_head_dim}, value_values);
  auto beta =
      tfFloat.make({batch_size, num_kv_heads, sequence_length}, beta_values);
  auto initial_state = tfFloat.make(
      {batch_size, num_kv_heads, k_head_dim, v_head_dim}, initial_state_values);
  auto scalar_decay = tfFloat.make(
      {batch_size, num_kv_heads, sequence_length}, scalar_decay_values);
  auto channelwise_decay = tfFloat.make(
      {batch_size, num_kv_heads, sequence_length, k_head_dim},
      channelwise_decay_values);

  auto scalar_out =
      tfFloat.zeros({batch_size, num_query_heads, sequence_length, v_head_dim});
  auto scalar_state =
      tfFloat.zeros({batch_size, num_kv_heads, k_head_dim, v_head_dim});
  auto channelwise_out =
      tfFloat.zeros({batch_size, num_query_heads, sequence_length, v_head_dim});
  auto channelwise_state =
      tfFloat.zeros({batch_size, num_kv_heads, k_head_dim, v_head_dim});
  ...
  op_gated_delta_rule(
      ...
      channelwise_out,
      channelwise_state);
  ASSERT_EQ(channelwise_context.failure_state(), torch::executor::Error::Ok);

  EXPECT_TENSOR_CLOSE_WITH_TOL(scalar_out, channelwise_out, 1e-5, 1e-5);
  EXPECT_TENSOR_CLOSE_WITH_TOL(scalar_state, channelwise_state, 1e-5, 1e-5);
}
...
TEST(GatedDeltaRuleTest, RejectsMismatchedScalarDecay) {
  ...
  executorch::aten::Tensor key = tfFloat.ones({1, 1, 3, 2});
  executorch::aten::Tensor value = tfFloat.ones({1, 1, 3, 2});
  executorch::aten::Tensor decay = tfFloat.ones({1, 1, 2});
  executorch::aten::Tensor beta = tfFloat.ones({1, 1, 3});
  executorch::aten::Tensor initial_state = tfFloat.ones({1, 1, 2, 2});
  executorch::aten::Tensor out = tfFloat.zeros({1, 1, 3, 2});
...
TEST(GatedDeltaRuleTest, RejectsPartiallyOverlappingFinalStateOut) {
  ...
  executorch::aten::Tensor key = tfFloat.ones({1, 1, 1, 2});
  executorch::aten::Tensor value = tfFloat.ones({1, 1, 1, 2});
  executorch::aten::Tensor decay = tfFloat.ones({1, 1, 1, 2});
  executorch::aten::Tensor beta = tfFloat.ones({1, 1, 1});
  executorch::aten::Tensor initial_state = tfFloat.ones({1, 1, 2, 2});
  executorch::aten::Tensor out = tfFloat.zeros({1, 1, 1, 2});
  executorch::aten::Tensor final_state_out = tfFloat.zeros({1, 1, 2, 2});
  final_state_out.unsafeGetTensorImpl()->set_data(

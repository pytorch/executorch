...
TEST_F(MultimodalInputTest, ImageConstructorFromImage) {
  ...
  EXPECT_EQ(input.get_image().copy_uint8_data().size(), 224 * 224 * 3);
}
...
TEST_F(MultimodalInputTest, ImageConstructorFromRvalueImage) {
  ...
  size_t data_size = img.copy_uint8_data().size();
  ...
  EXPECT_EQ(input.get_image().copy_uint8_data().size(), data_size);
}
...
TEST_F(MultimodalInputTest, DifferentImageSizes) {
  ...
  EXPECT_EQ(input.get_image().copy_uint8_data().size(), 32 * 32);
}

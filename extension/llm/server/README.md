...
`TextLLMRunner` (and the planned `Session` primitives) underneath — each server
is a thin protocol shell over that engine. See `python/README.md` to run it.
...
prefix cache (`--enable-prefix-cache`). Unsupported params (`top_p`, `seed`,

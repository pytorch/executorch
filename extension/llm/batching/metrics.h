...
// Human-readable reports. Defined in metrics.cpp so a runner that never formats
// one does not pull <sstream> and <iomanip> into its translation units --
// everything above is trivially inline and allocation-free, these are not.
ET_EXPERIMENTAL std::string format_report(const GenerationMetrics& m);
ET_EXPERIMENTAL std::string format_report(const EngineMetrics& m);

...
class ModelConfig:
    ...
    def __post_init__(self):
        ...
        if (
            not math.isfinite(self.static_kv_cache_scale)
            or self.static_kv_cache_scale <= 0
        ):
            raise ValueError("static_kv_cache_scale must be finite and positive")
...
class LlmConfig:
    ...
    def from_args(cls, args: argparse.Namespace) -> "LlmConfig":  # noqa: C901
        ...
        if hasattr(args, "static_kv_cache_scale"):
            llm_config.model.static_kv_cache_scale = args.static_kv_cache_scale

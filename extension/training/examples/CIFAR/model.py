...
def train_model(
    ...
    # Initialize best testing loss to a high value for checkpointing on the best model
    ...
    for epoch in range(epochs):
        ...
        # Calculate the stats for average loss and accuracy for the entire epoch
        ...
        if test_loader is not None:
            ...
            if avg_test_loss < best_test_loss:
                ...
                print(
                    f"New best model saved with Testing loss: {avg_test_loss:.4f} and Testing accuracy: {test_accuracy:.2f}%"
                )
...
def fine_tune_executorch_model(
    ...
    Args:
        model_path (str): Path to the ExecutorTorch model file to be fine-tuned.
        save_path (str): Path where the fine-tuned model will be saved.
        train_loader (DataLoader): DataLoader for the training dataset.
        val_loader (DataLoader): DataLoader for the validation dataset.
        epochs (int, optional): Number of epochs for fine-tuning (default: 10).
        learning_rate (float, optional): Learning rate for parameter updates (default: 0.001).
        momentum (float, optional): Momentum for parameter updates (default: 0.9).

    Returns:
        tuple: A tuple containing the fine-tuned ExecutorTorchModule and a dictionary
               with training and validation metrics.
    """
    ...
    for epoch in range(epochs):
        ...
        for batch in tqdm(train_loader):
            ...
            for i in range(len(inputs)):
                input_image = inputs[
                    i : i + 1
                ]  # Use list slicing to extract single image as the unqueeze method resulted in errors for some reason
                label = labels[i : i + 1]
                # Forward pass
        ...
        val_correct = 0
        val_total = 0
        val_samples = 100  # Reducing the number of samples to 100 for faster validation
        val_start_time = time.time()

        for i, val_batch in tqdm(enumerate(val_loader)):
            if i == val_samples:
                print(f"Reached {val_samples} samples. Terminating validation loop.")
                break
        ...
        avg_val_loss = val_loss / len(val_loader) / (val_loader.batch_size or 1)

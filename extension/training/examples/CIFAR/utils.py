...
class BalancedCIFARDataset(Dataset):
    """Custom dataset class to load balanced CIFAR-10 data from binary file."""
...
def create_balanced_cifar_dataset(
    ...
    with specified number of images per class, saved in binary format compatible with Android.
    ...
    for class_id in range(10):  # CIFAR-10 has 10 classes (0-9)
        if class_id in class_images:
            ...
            print(f"Class {class_id}: {len(class_images[class_id])} images selected")
...
def get_data_loaders(
    ...
    Args:
        ...
        use_balanced_dataset: Whether to use balanced dataset or standard CIFAR-10
...
def save_json(
    ...
    Args:
        history (Dict[int, Dict[str, float]]): Dictionary with epoch numbers as keys
            and dictionaries of metrics (loss, accuracy, etc.) as values.

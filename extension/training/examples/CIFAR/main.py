...
def export_model(
    ...
    This function takes a PyTorch model and sample input/label tensors, wraps the model
    with a loss function, exports it using torch.export, applies forward-backward pass
    optimization, converts it to edge format, and finally to ExecutorTorch format.
...
def export_model_with_ptd(
    ...
    """
    Export a PyTorch model to an ExecutorTorch module format with external tensor data.

    This function takes a PyTorch model and sample input/label tensors, wraps the model
    with a loss function, exports it using torch.export, applies forward-backward pass
    optimization, converts it to edge format, and finally to ExecutorTorch format with
    external constants and mutable weights.
    ...
    ep = ep.to_executorch(
        config=ExecutorchBackendConfig(
            external_constants=True,  # This is the flag that enables the external
            # constants to be stored in a separate file external to the PTE file.
            external_mutable_weights=True,  # This is the flag that enables all trainable
            # weights will be stored in a separate file external to the PTE file.
        )
    )
    return ep
...
def save_model(ep: ExecuTorchModule, model_path: str) -> None:
    ...
    This function writes the buffer of an ExecutorTorchModule to a file in binary format.
...
def parse_args() -> argparse.Namespace:
    ...
    Returns:
        argparse.Namespace: An object containing all the parsed command line arguments
                           with their respective values (either user-provided or defaults).
    """
    ...
    parser.add_argument(
        ...
        type=str,
        default="./data",
        help="Directory to download and store CIFAR-10 dataset (default: ./data)",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    parser.add_argument(
        ...
        help="Save the et json file (default: cifar10_et_pte_only_model_finetuned_history.json)",
    )

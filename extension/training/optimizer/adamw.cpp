...
void adamw_update_hack(
    ...
    const double eps) {
  auto p_ptr = p.mutable_data_ptr<scalar_t>();
  auto m_ptr = m.const_data_ptr<scalar_t>();
  auto v_ptr = v.const_data_ptr<scalar_t>();
  ...
  for (auto& group : param_groups_) {
    ...
    for (auto param_iter = group.named_parameters().begin();
      ...
      if (p.sizes() != g.sizes() || scalar_type != g.scalar_type() ||
          ...
          !executorch::ET_RUNTIME_NAMESPACE::tensor_is_contiguous(g) ||
          !p.device().is_cpu() || !g.device().is_cpu()) {
        return Error::InvalidArgument;
      }
      ...
      struct {
        [[noreturn]] void fail(Error /* error */) {
          ET_CHECK_MSG(false, "Unsupported dtype in AdamW::step");
        }
      } ctx;

      ET_SWITCH_FLOATH_TYPES(scalar_type, ctx, "AdamW::step", CTYPE, [&] {
        // Decoupled weight decay: p <- p - lr * weight_decay * p. Applied
        // to the parameter directly, BEFORE the moment-based update, and
        // NOT folded into the gradient. This is the defining property of
        // AdamW (Loshchilov & Hutter, 2019).
        if (weight_decay != 0.0) {
          add_out_hack<CTYPE>(p, p, -lr * weight_decay, p);
        }
        ...
        mul_out_hack<CTYPE>(exp_avg, beta1, exp_avg);
        add_out_hack<CTYPE>(exp_avg, g, 1.0 - beta1, exp_avg);

        // Second moment: v <- beta2 * v + (1 - beta2) * g^2
        addcmul_sq_out_hack<CTYPE>(exp_avg_sq, g, beta2, exp_avg_sq);

        // Bias-corrected update.
        const double bias_correction1 = 1.0 - std::pow(beta1, step);
        const double bias_correction2 = 1.0 - std::pow(beta2, step);
        adamw_update_hack<CTYPE>(
            p,
            exp_avg,
            exp_avg_sq,
            lr,

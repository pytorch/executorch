...
struct PyModule final {
  ...
  py::list run_method(
    ...
    allocate_output_storages(method_name);
 ...
 private:
  ...
  void allocate_output_storages(const std::string& method_name) {
    auto method_result = module_->method(method_name);
    THROW_IF_ERROR(
        method_result.error(),
        "Failed to get method %s, error: 0x%" PRIx32,
        method_name.c_str(),
    ...
    // Set up output storage for non-empty buffers

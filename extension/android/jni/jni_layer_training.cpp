...
class ExecuTorchTrainingJni
 ...
 public:
  ...
  namedGradients(facebook::jni::alias_ref<jstring> methodName) {
    // throw std::runtime_error(std::string("Tried running namedGradients on methodName: ") + methodName->toStdString());
    auto method = methodName->toStdString();
...
class SGDHybrid
    : public facebook::jni::HybridClass<SGDHybrid> {
 public:
  constexpr static const char* kJavaDescriptor =
      "Lorg/pytorch/executorch/SGD;";
  ...
  SGDHybrid(
    ...
    while (iterator != end) {
      ...
      cpp_named_parameters.emplace(std::string_view(parameter_names_.back()), *tensor);
    ...
    sgd_optimizer_ = std::make_unique<optimizer::SGD>(cpp_named_parameters, options);
  }
  ...
  step(facebook::jni::alias_ref<
    ...
    while (iterator != end) {
      ...
      cpp_named_gradients.emplace(std::string_view(gradient_names.back()), *tensor);

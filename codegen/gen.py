...
def get_manual_registration_function_name(
    *,
    ...
    if manual_registration_lib_name is None:
    ...
    if manual_registration_lib_name == "all":
        raise ValueError(
            "--manual-registration-lib-name 'all' is reserved for the default API"
        )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        "The library name must be a nonempty C++ identifier other than 'all'. Requires "
        "--manual-registration.",
    )
    parser.add_argument(
        "--generate",

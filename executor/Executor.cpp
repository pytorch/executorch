...
Error ExecutionPlan::resolve_operator(
    int32_t op_index,
    OpFunction* kernels,
    size_t kernel_index,
...
Error ExecutionPlan::init(executorch::ExecutionPlan* s_plan) {
  ...
  ET_CHECK_OR_RETURN_ERROR(
      ...
      InvalidState,
      "ExecutionPlan already initialized, or previously failed to initialize.");
  init_state_ =
...

  {
    ...
    if (err != Error::Ok) {
      // ~ExecutionPlan() will try to clean up entries in this list. Ensure that
      // it doesn't look at any uninitialized entries.
      n_value_ = num_parsed;
      return err;
    }
  }

  {
    ...
    // ~ExecutionPlan() to clean up, and is incremented at the bottom of the
    // loop. This makes it safe for errors to return without updating any state.
    n_delegate_ = 0;
    ...
    for (size_t i = 0; i < n_delegate; ++i) {
      ...
      // ~ExecutionPlan() will try to clean up n_delegate_ entries in the
      // delegates_ array. Only increment this once we know the entry is valid,
      // so that we don't try to clean up an uninitialized entry.
      n_delegate_ = i + 1;
    }
  }
...
ExecutionPlan::set_input(const EValue& input_evalue, size_t input_idx) {
  ET_CHECK_OR_RETURN_ERROR(
      initialized(),
      InvalidState,
      "Input can not be set until execution plan has been initialized.");
  ...
  ET_CHECK_OR_RETURN_ERROR(
      ...
      InvalidArgument,
      "Given input index must be less than the number of inputs in execution plan, but got %zu and %zu",
      input_idx,
      inputs_size());
  ...
  ET_CHECK_OR_RETURN_ERROR(
      ...
      InvalidArgument,
      "The %zu-th input in execution plan is expected Tensor or prim, but received %u",
      input_idx,
      e.tag);
  ...
  ET_CHECK_OR_RETURN_ERROR(
      ...
      InvalidArgument,
      "The %zu-th input of execution plan should have the same type as the input_evalue, but get tag %u and tag %u",
      input_idx,
      e.tag,
      input_evalue.tag);
  ...
  if (e.isTensor()) {
    ...
    ET_CHECK_OR_RETURN_ERROR(
        ...
        t_src.scalar_type(),
        t_dst.scalar_type());
    // Reset the shape for ExecutionPlan's input as the size of forwarded
    // input tensor for shape dynamism. Also is a safety check if need memcpy.
...
__ET_NODISCARD Error
ExecutionPlan::set_inputs(const exec_aten::ArrayRef<EValue>& input_evalues) {
  ET_CHECK_OR_RETURN_ERROR(
      initialized(),
      InvalidState,
      "Inputs can not be set until execution plan has been initialized.");
  ...
  ET_CHECK_OR_RETURN_ERROR(
      ...
      InvalidArgument,
      "The length of given input array (%zu) must be same as the number of inputs in execution plan (%zu).",
      input_evalues.size(),
      input_size);
...
__ET_NODISCARD Error
ExecutionPlan::get_outputs(EValue* output_evalues, size_t length) {
  ET_CHECK_OR_RETURN_ERROR(
      initialized(),
      InvalidState,
      "Outputs can not be retrieved until execution plan has been initialized.");
...
Error ExecutionPlan::execute_instruction() {
...
Error ExecutionPlan::experimental_reset_execution() {
  ET_CHECK_OR_RETURN_ERROR(
...
Error ExecutionPlan::execute() {
  EXECUTORCH_SCOPE_PROF("ExecPlan::execute");
  ET_CHECK_OR_RETURN_ERROR(
      initialized(),
      NotSupported,
      "Cannot execute until plan has been initialized.");
...
size_t ExecutionPlan::values_size() const {
  return n_value_;
}

const EValue& ExecutionPlan::get_value(size_t i) const {
  return values_[i];
}

EValue& ExecutionPlan::mutable_value(size_t i) {
  return values_[i];
}

size_t ExecutionPlan::inputs_size() const {
  return serialization_plan_->inputs()->size();
}

size_t ExecutionPlan::get_input_index(size_t i) const {
  return static_cast<size_t>(serialization_plan_->inputs()->Get(i));
}

const EValue& ExecutionPlan::get_input(size_t i) const {
  return get_value(get_input_index(i));
}

EValue& ExecutionPlan::mutable_input(size_t i) {
  return mutable_value(get_input_index(i));
}

size_t ExecutionPlan::outputs_size() const {
  return serialization_plan_->outputs()->size();
}

size_t ExecutionPlan::get_output_index(size_t i) const {
  return static_cast<size_t>(serialization_plan_->outputs()->Get(i));
}

const EValue& ExecutionPlan::get_output(size_t i) const {
  return get_value(get_output_index(i));
}

EValue& ExecutionPlan::mutable_output(size_t i) {

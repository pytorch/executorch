...
mkdir -p "$LICENSES"
cp "$ET_ROOT/third-party/flatbuffers/LICENSE" "$LICENSES/flatbuffers-LICENSE.txt"
cp "$ET_ROOT/third-party/flatcc/LICENSE" "$LICENSES/flatcc-LICENSE.txt"
if [ -n "$CMSIS_NN" ]; then
...
Regenerate by running extras/tools/build_arduino_library.sh from an
executorch checkout at that commit. Override the op set with ROOT_OPS or
ALL_OPS=1.
PROV

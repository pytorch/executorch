...
# Builds dev_tools_example_runner and prints its path.
...
main() {
  ...
  local runner="${PWD}/${build_dir}/dev_tools_example_runner"
  if [[ ! -f "${runner}" ]]; then
    echo "ERROR: Failed to build ${build_dir}/dev_tools_example_runner" >&2
    exit 1
  else
    echo "Built ${build_dir}/dev_tools_example_runner"
  fi
}

...
class Trie:
    ...
    Creates a Trie out of a list of words. The trie is used to split on `added_tokens` in one pass
    ...
    def add(self, word: str):
        """Passes over every char (utf-8) on word and recursively adds it to the internal `data` trie representation.
    ...
    def split(self, text: str) -> List[str]:
        """Will look for the words added to the trie within `text`.

        Output is the original string splitted along the boundaries of the words found.
...
class PreTrainedTokenizer(PreTrainedTokenizerBase):
    ...
    def added_tokens_encoder(self) -> Dict[str, int]:
        ...
        The added tokens encoder is cached for performance optimisation in `self._added_tokens_encoder`
    ...
    def get_added_vocab(self) -> Dict[str, int]:
        ...
        even if they are already in the vocabulary. This is something we should change.
    ...
    def _update_total_vocab_size(self):
        """Update the size of the full vocabulary with the added tokens.

        Counts the `keys` and not the `values` because
    ...
    def _add_tokens(self, new_tokens: Union[List[str], List[AddedToken]], special_tokens: bool = False) -> int:
        ...
        If the new tokens are not in the vocabulary, they are added to
        ...
        # Note: resize_token_embeddings expects to receive the full size of the new vocabulary, i.e. the length of the
        tokenizer.model.resize_token_embeddings(len(tokenizer))
        ```
        """
        added_tokens = 0
    ...
    def _update_trie(self, unique_no_split_tokens: Optional[str] = []):
        for token in self._added_tokens_decoder.values():
            if token not in self.tokens_trie._tokens:  # noqa: SLF001
            ...
            if token in no_split_token:
                ...
                if isinstance(tok_extended, AddedToken):
                    if tok_extended.rstrip and right:
                        ...
                        tokens[i + 1] = right.lstrip()  # noqa: B909
                    # Strip white spaces on the left
                    if tok_extended.lstrip and left:
                        tokens[i - 1] = left.rstrip()  # Opposite here  # noqa: B909
                    if tok_extended.single_word and left and left[-1] != ' ':
                        tokens[i - 1] += token
                        tokens[i] = ''
                    elif tok_extended.single_word and right and right[0] != ' ':
                        tokens[i + 1] = token + tokens[i + 1]  # noqa: B909
                        tokens[i] = ''
                else:
                    raise ValueError(
    ...
    def _batch_encode_plus(
        ...
        **kwargs,
        ...
        for ids_or_pair_ids in batch_text_or_text_pairs:
            if not isinstance(ids_or_pair_ids, (list, tuple)) or (
                is_split_into_words and not isinstance(ids_or_pair_ids[0], (list, tuple))
            ):

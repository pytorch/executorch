# Copyright 2022 The OpenAI team and The HuggingFace Team. All rights reserved.  # noqa: CPY001
...
class EnglishNumberNormalizer:
    ...
    def process_words(self, words: List[str]) -> Iterator[str]:
        ...
        for i, current in enumerate(words):
            ...
            next_ = words[i + 1] if i != len(words) - 1 else None
            if skip:
                skip = False
                continue
            ...
            if re.match(r"^\d+(\.\d+)?$", current_without_prefix):
                ...
                if next_ not in self.words and not next_is_numeric:
                    # apply special handling only if the next_ word can be numeric
                    if value is not None:
                        yield output(value)
                    yield output(current)
                elif current == "and":
...
class EnglishSpellingNormalizer:
    ...
    def __init__(self, english_spelling_mapping):
        """Initialize the EnglishSpellingNormalizer."""

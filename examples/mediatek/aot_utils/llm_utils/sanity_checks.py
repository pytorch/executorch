...
def check_all_chunks_same_num_layer(num_blocks_per_chunk):
    for i in range(1, len(num_blocks_per_chunk)):
        if num_blocks_per_chunk[i] != num_blocks_per_chunk[0]:
            ...
            raise RuntimeError("This version of the sdk doesn't support different number of "
...
def check_between_exclusive(num, min_, max_, message=None):
    if not (type(num) == type(min_) == type(max_)):
        raise TypeError(f"Got different types for num ({type(num)}), min ({type(min_)}), and max ({type(max_)})")
    if not (min_ < num < max_):
        if message is None:
            raise ValueError(f"Expected number between {min_} and {max_} exclusive, but got: {num}")
        else:
            raise ValueError(f"{message} must be between {min_} and {max_} exclusive, but got: {num}")
...
def check_between_inclusive(num, min_, max_, message=None):
    ...
    if not (min_ <= num <= max_):
        if message is None:
            raise ValueError(f"Expected number between {min_} and {max_} inclusive, but got: {num}")
        else:
            raise ValueError(f"{message} must be between {min_} and {max_} inclusive, but got: {num}")
...
def check_old_arg(path):
    if os.path.isdir(path):
        raise RuntimeError(f"This package's main usage has changed starting from v0.8.0. Please use"
...
def check_shapes(shapes):
    ...
    for shape in shapes:
        if shape.count('t') != 1 or shape.count('c') != 1:
            raise RuntimeError(f"Shape {shape} is in the wrong format. Every shape needs to be of"
...
def check_supported_model(config):
    SUPPORTED_MODELS = ['llama', 'bloom', 'baichuan', 'qwen', 'qwen1.5', 'qwen2', 'milm']
    if not isinstance(config, BaseConfig):
        raise RuntimeError(f"Unsupported config class: {type(config)}. "
            "config needs to be subclassed from BaseConfig")

    if config.model_type not in SUPPORTED_MODELS:
        raise RuntimeError(f"Unsupported model: {config.model_type}. Supported models: "
            f"{SUPPORTED_MODELS}")

def check_supported_tokenizer(config):
    SUPPORTED_TOKENIZERS = ['default', 'bloom', 'baichuan', 'gpt2', 'gpt2_fast', 'qwen', 'qwen2', 'qwen2_fast', 'llama', 'pretrained_fast']
    if not isinstance(config, BaseConfig):
        raise RuntimeError(f"Unsupported config class: {type(config)}. "
            "config needs to be subclassed from BaseConfig")

    if config.tokenizer not in SUPPORTED_TOKENIZERS:
        raise RuntimeError(f"Unsupported tokenizer: {config.tokenizer}. Supported tokenizers: "
...
def check_tokenizer_exist(folder):
    ...
    for f in os.listdir(folder):
        ...
        if f == 'tokenizer_config.json':
            config = True
    if not model:
        raise FileNotFoundError(f"Tokenizer not found in {folder}. Expected tokenizer.model, "
            "tokenizer.json, or tokenizer.tiktoken")
    if not config:
        raise FileNotFoundError(f"Tokenizer config not found in {folder}. Expected "
            "tokenizer_config.json")

def check_weights_exist(weight_dir):
    if len([f for f in os.listdir(weight_dir) if
            ((f.startswith('pytorch_model') and f.endswith('.bin')) or (f.startswith('model') and f.endswith('.safetensors')))
            ]
        ) == 0:
        raise FileNotFoundError(f"No weight files found in {weight_dir}! Weight files should be either .bin or .safetensors file types.")
    ...
    if len(safetensors_l) & len(bin_l):
        raise RuntimeError("Weights should only be in either .bin or .safetensors format, not both.")

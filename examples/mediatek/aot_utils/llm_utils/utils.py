...
#     # Sliding Window mask portion
#     if window_size is not None:
#         sliding_mask_cond = np.arange(input_length + valid_cache)

#         sliding_triangle = sliding_mask_cond <= (
#             sliding_mask_cond - window_size + valid_cache - 1
#             ).reshape(input_length + valid_cache, 1)
        
#         sliding_small_attention_mask = sliding_triangle.astype(np.float32) * mask_value
#         sliding_mask = sliding_small_attention_mask[:valid_input]

#         sliding_mask = np.concatenate(
#             [sliding_mask, np.zeros((input_length - valid_input, valid_cache + input_length), dtype=np.float32)], 
#             axis=-2,
#         )
#         sliding_mask_with_cache = np.concatenate(
#             [np.zeros((input_length, cache_size-valid_cache), dtype=np.float32), sliding_mask],
#             axis=-1,
#         )
#         sliding_mask_final_shape = np.broadcast_to(
#             sliding_mask_with_cache[None, None, :, :],
#             (batch_size, 1, input_length, cache_size + input_length),
#         )


#         combined_mask = combined_mask + sliding_mask_final_shape
...
def get_master_rot_emb(config, dtype, **kwargs):
    ...
    if getattr(config, 'rope_scaling', None) is not None:
        if config.rope_scaling['type'] == 'longrope':
            ...
            t_long = torch.arange(config.max_position_embeddings - length, dtype=torch.float32)
            ...
            assert position_ids is not None, "Must pass position_ids when using Qwen2-VL mrope."
            assert rope_delta is not None, "Must pass rope_delta when using Qwen2-VL mrope."

            mrope_section = config.rope_scaling['mrope_section'] * 2
            cache_position = torch.arange(position_ids.shape[2], config.max_position_embeddings, dtype=torch.float32)
            ...
            additional_softmax_scale = _yarn_get_softmax_scale(config, scaling_factor) * attn_factor
            master_cos = np.cos(emb)[None, None, :, :] * additional_softmax_scale  # (1,1,len,rot_dim)
            master_sin = np.sin(emb)[None, None, :, :] * additional_softmax_scale  # (1,1,len,rot_dim)
            ...
            assert False, (f'Rope scaling only supports longrope, mrope, yarn and linear,'
...
def get_normalized_config(config_filepath):
    ...
    if config_file.get("llm", None) is None:
        if config_file["model_type"] == "llama":
            ...
            from models.llm_models.configuration_gemma import GemmaConfig as config_class
    else:
        if config_file["llm"]["model_type"] == 'whisper_decoder':
            from models.llm_models.configuration_whisper import WhisperConfig as config_class
...
def resolve_model_classes(
    ...
    if config_file.get("llm", None) is None:
        ...
        if config_file["llm"]["model_type"] == "whisper_decoder":
            from models.llm_models.configuration_whisper import WhisperConfig as config_class
            from models.llm_models.modeling_whisper import WhisperEncoderModel as encoder_class # TODO: Change to whisper & add tokenizer
            from models.llm_models.modeling_whisper import WhisperDecoderModelChunk as decoder_class # TODO: Change to whisper & add tokenizer

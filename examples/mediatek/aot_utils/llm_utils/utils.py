...
def _get_embedding_weight(config, weight_dir, state_dict):
    try_last = False
    checkpoint_filename = None
    if state_dict is None:
        checkpoint_files = [
            os.path.join(weight_dir, f) for f in os.listdir(weight_dir) if \
        ...
        if checkpoint_filename is None:
            raise FileNotFoundError(f"Unable to find the first checkpoint file in {weight_dir}! "
    ...
    if embed_key is None:
        if try_last:
            if checkpoint_filename == "pytorch_model.bin" or checkpoint_filename == "model.safetensors":
                ...
                raise KeyError(f"Cannot find embedding layer weight inside {checkpoint_filename}. "
            ...
            print("state_dict keys:", state_dict_keys)
            raise KeyError(f"Cannot find embedding layer weight inside state dict. "
                f"Please ensure embedding layer weight key contains {expected_embedding_subkey}")
    return state_dict[embed_key]

def chunk_and_tokenize_prompt(prompt, tokenizer, sub_responses, max_len, response_handler, preformatter=None, wikitext=False):
    if max_len == 0:
        # No chunking
        ...
        if tokenizer is None:
            ...
            with response_handler:
                if preformatter is not None:
                    ...
                    print("Prompt:")
                    print(prompt)
            prompt_tokens = tokenizer(prompt_formatted, return_tensors='np')["input_ids"].astype(np.int32)
        ...
        if wikitext:
            ...
            for sentence in sentences:
                if preformatter is not None:
                    if len(sub_responses) == 0:
                        curr_chunk_formatted = preformatter.generate_prompt(curr_chunk, None)
                    else:
                        curr_chunk_formatted = preformatter.generate_prompt(curr_chunk, sub_responses[-1])
                else:
                    curr_chunk_formatted = curr_chunk
                if tokenizer is None:
                    curr_chunk_tokens = curr_chunk_formatted
                else:
                    curr_chunk_tokens = tokenizer(curr_chunk_formatted, return_tensors='np')["input_ids"].astype(np.int32)
            ...
            if prev_chunk_tokens is None:
                raise RuntimeError(f"Length of a single line ({curr_chunk_tokens.shape[1]}) is more than maximum length to chunk prompt to ({max_len})")
                ...
                with response_handler:
                    if preformatter is not None:
                        if len(sub_responses) == 0:
                            ...
                            print(f"Prompt (with {preformatter.name} preformatter with input):")
                            print(prev_chunk)
                    else:
                        print("Prompt:")
                        print(prev_chunk)
...
def generate_alibi(
    ...
    pre_pad_tensor = np.zeros((batch_size, num_heads, 1, pre_pad_length), dtype=np.float32)
    post_pad_length = input_length - valid_input
    post_pad_tensor = np.zeros((batch_size, num_heads, 1, post_pad_length), dtype=np.float32)
    alibi = np.concatenate([pre_pad_tensor, alibi, post_pad_tensor], axis=-1).astype(np.float32)
...
def generate_mask(
    ...
    attention_mask = np.pad(small_attention_mask, (0, input_length-valid_input), 'constant', constant_values=mask_value)
    attention_mask_with_cache = np.concatenate([np.zeros((input_length, cache_size), dtype=np.float32), attention_mask], axis=-1)
    attention_mask_final_shape = np.broadcast_to(attention_mask_with_cache[None, None, :, :], (batch_size, 1, input_length, cache_size+input_length))
...
def get_sorted_path_list(folder, ext='.', absolute=False):
    if absolute:
        sorted_list = sorted(os.listdir(folder), key=lambda f: int(f.rsplit('_', 1)[1].split(ext)[0]))
        return [os.path.join(folder, x) for x in sorted_list]
    else:
        return sorted(os.listdir(folder), key=lambda f: int(f.rsplit('_', 1)[1].split(ext)[0]))

def load_checkpoints(weight_dir):
    checkpoint_files = [
        os.path.join(weight_dir, f) for f in os.listdir(weight_dir)
        if (f.startswith('pytorch_model') and f.endswith('.bin')) or (f.startswith('model') and f.endswith('.safetensors'))
    ]
    ...
    for i in range(len(checkpoint_files)):
        if is_safetensors:
            state_dict = {
                **state_dict,
                **load_file(checkpoint_files[i], device="cpu")
            }
        else:
            state_dict = {
                **state_dict,
                **torch.load(checkpoint_files[i], map_location="cpu")
            }

    return state_dict

def resolve_model_classes(config_filepath, bypass_tokenizer=False, response_handler=None):
    config_file = json.load(open(config_filepath, 'r'))
    ...
    if bypass_tokenizer:
        ...
        if config.tokenizer == 'default':
            if config_file['model_type'] == 'llama':
                from aot_utils.llm_utils.tokenizers_.tokenization_llama import LlamaTokenizer as tokenizer_class
        else:
            if config.tokenizer == 'llama':
                from aot_utils.llm_utils.tokenizers_.tokenization_llama import LlamaTokenizer as tokenizer_class
            elif config.tokenizer == 'pretrained_fast':
                from aot_utils.llm_utils.tokenizers_.tokenization_utils_fast import PreTrainedTokenizerFast as tokenizer_class

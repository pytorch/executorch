...
class Gemma3Attention(Attention):
    ...
    def forward(
        ...
        # NOTE: gemma3 applies qk_norm before rot emb
        ...
        # NOTE: gemma3 scales with query pre attn scalar instead of attn scale
        attn_weights = torch.matmul(query_states, key_states.transpose(2, 3))
        attn_weights = attn_weights / self.query_pre_attn_scalar
...
class Gemma2ModelChunk(ModelChunk):
    def __init__(
        ...
        if self.include_tail:
            self.norm = GemmaRMSNorm(config.hidden_size, eps=config.norm_eps).float()
            ...
            # Not Fake Weights
                ...
                temp_state_dict = {
                    ...
                    **{
                        ...
                        f"layers.{inner_layer_idx}.pre_feedforward_layernorm.weight": state_dict.pop(
                            f"{prefix}layers.{outer_layer_idx}.pre_feedforward_layernorm.weight"
                        ).to(
                            torch.float32
                        ),
                        f"layers.{inner_layer_idx}.post_feedforward_layernorm.weight": state_dict.pop(
                            f"{prefix}layers.{outer_layer_idx}.post_feedforward_layernorm.weight"
                        ).to(
                            torch.float32
                        ),
            ...
            if fake_weights:
                ...
                if self.config.use_qk_norm:
                    temp_state_dict = {
                        **temp_state_dict,
                        **{
                            f"layers.{inner_layer_idx}.self_attn.q_norm.weight": torch.rand(
                                self.config.head_dim, dtype=self.dtype
                            ),
                ...
                if self.config.use_qk_norm:
                    temp_state_dict = {
                        **temp_state_dict,
                        **{
                            f"layers.{inner_layer_idx}.self_attn.q_norm.weight": state_dict.pop(
                                f"{prefix}layers.{outer_layer_idx}.self_attn.q_norm.weight"
                            ),

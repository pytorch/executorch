...
int main(int argc, char** argv) {
  constexpr size_t kMethodAllocatorPoolSize = 4 * 1024U * 1024U; // 4 MB
  constexpr size_t kTempAllocatorPoolSize = 1024U * 1024U; // 1 MB
  ...
  // Keep allocator pools local to avoid mutable globally accessible state.
  auto method_allocator_pool = std::make_unique<uint8_t[]>(kMethodAllocatorPoolSize);
  auto temp_allocator_pool = std::make_unique<uint8_t[]>(kTempAllocatorPoolSize);

  // Set up memory allocators
  MemoryAllocator method_allocator{
      MemoryAllocator(kMethodAllocatorPoolSize, method_allocator_pool.get())};
  MemoryAllocator temp_allocator{
      MemoryAllocator(kTempAllocatorPoolSize, temp_allocator_pool.get())};
  ...
  for (size_t id = 0; id < num_buffers; ++id) {
    ...
    planned_spans.emplace_back(planned_buffers.back().get(), buffer_size);
  }

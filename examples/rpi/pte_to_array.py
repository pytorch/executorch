...
def update_model_file(c_file_path, hex_array_contents):
    ...
    # Try to find and replace using any of the defined patterns
    ...
    if new_content is None:
        ...
        raise ValueError(f"Could not find any model array pattern in {c_file_path}.\n"
                        f"Available patterns: {available_patterns}\n"
                        f"Add a new pattern to ARRAY_PATTERNS if needed.")
    ...
    byte_count = len(hex_array_contents.split(','))
    print(f"Successfully updated {c_file_path} with {byte_count} bytes using pattern '{matched_pattern}'")
...
Examples:
  ...
  python3 pte_to_array.py --model model.pte --file model.c --add-pattern "my_pattern" "regex_here"
    ...
    if not os.path.exists(args.model):
        print(f"Error: PTE file '{args.model}' does not exist")
        return 1
    ...
    try:
        ...
        if file_ext == '.c':
            ...
            if not os.path.exists(args.file):
                print(f"Error: C file '{args.file}' does not exist")
                return 1

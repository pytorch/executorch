...
"""Unit tests for cuda_packers.py.
...
from executorch.extension.llm.export.load import (
    assign_one,
    assign_state_dict,
    load_checkpoint,
)
from executorch.extension.llm.export.quant.convert import to_exportable
...
class TestPackLinearInt4(unittest.TestCase):
    ...
    def _pack(self, symmetric=False, group_size=32):
        ...
        q = to_exportable("weight", quantize_weight(self.weight, config))
...
class TestPackModel(unittest.TestCase):
    """assign_state_dict + convert_quantized_tensors_for_cuda over a whole model."""
    ...
    def test_mixed_precision(self):
        ...
        assign_state_dict(model, {"q_proj.weight": q4, "v_proj.weight": q8})
    ...
    def test_load_checkpoint_from_disk(self):
        torch.manual_seed(0)
...
class TestAssignAndPass(unittest.TestCase):
    """assign_one / the CUDA pass on CPU (no matmul; no CUDA required)."""

    def test_assign_one_quantized_then_pass(self):
        ...
        assign_one(model, "proj.weight", to_exportable("proj.weight", q))
        self.assertNotEqual(model.proj.weight.device.type, "meta")
        convert_quantized_tensors_for_cuda(model)
        self.assertIsInstance(model.proj.weight.data, CudaCoalescedInt4Tensor)

    def test_assign_one_plain_tensor(self):
        with torch.device("meta"):
            model = nn.ModuleDict({"norm": nn.LayerNorm(64, bias=False)})
        assign_one(model, "norm.weight", torch.randn(64, dtype=torch.bfloat16))
        self.assertEqual(model.norm.weight.dtype, torch.bfloat16)


class TestPackErrorPaths(unittest.TestCase):

    def test_unsupported_module_type(self):
        ...
        assign_state_dict(model, {"custom.weight": q})
    ...
    def test_missing_weight_detected(self):
        ...
        with self.assertRaises(RuntimeError) as ctx:
            assign_state_dict(model, {"a.weight": q})

...
// Image preprocessing helpers for the Gemma 4 31B vision tower path.
//
// Ported from examples/models/gemma4/image_utils.h (the E2B/E4B variant).
// Both models share the same vision-tower input layout:
//   pixel_values        : (1, P, 3*16*16)  float32 in [0, 1]
//   pixel_position_ids  : (1, P, 2)        int64, padding rows = -1
// where P = max_soft_tokens * 9 (3x3 average-pool kernel over patch grid).
...
namespace executorch::examples::gemma4_31b {

using ::executorch::extension::TensorPtr;

// Vision encoder constants (must match Gemma4VisionConfig defaults)
...
struct ImageData {
  TensorPtr pixel_values; // (1, max_patches, 768) float32
  TensorPtr pixel_position_ids; // (1, max_patches, 2) int64
  int64_t num_valid_patches;
  int32_t original_width;
  int32_t original_height;
};

/// Compute target dimensions preserving aspect ratio. Both dimensions will be
/// divisible by kCellSize (48) so the patch grid divides evenly into the
/// 3x3 pooling kernel.
...
/// Patchify an RGB image (HWC uint8) into pixel_values and pixel_position_ids
/// tensors padded out to ``max_soft_tokens * 9`` patches.
///

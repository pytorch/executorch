...
# Reuse the gemma4 RMSNorm: numerically identical to HF's Gemma4RMSNorm
# (same float32 upcast + pow(mean_squared, -0.5) recipe).
from executorch.examples.models.gemma4.text_decoder import RMSNorm, RMSNormNoWeight
...
class Gemma4VisionMLP(nn.Module):
    """HF ``Gemma4VisionMLP``: SwiGLU-tanh with gate/up/down. No clipping at 31B."""
    ...
    def __init__(self, config: Gemma4VisionConfig):
        ...
        # Match HF's ACT2FN["gelu_pytorch_tanh"] (tanh approximation).
        assert (
            config.hidden_activation == "gelu_pytorch_tanh"
        ), f"Only gelu_pytorch_tanh is supported, got {config.hidden_activation}"
...
def _rotate_half(x: torch.Tensor) -> torch.Tensor:
    """Match HF's ``rotate_half``: concat(-x[half:], x[:half], dim=-1)."""
    half = x.shape[-1] // 2
    x1 = x[..., :half]
    x2 = x[..., half:]

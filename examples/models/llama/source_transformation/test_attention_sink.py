...
class CachePositionsManagerWithSinkTest(unittest.TestCase):
    ...
    def test_sink_never_overwritten(self):
        ...
        self.assertEqual(
            manager.cache_positions[:4].tolist(), [0, 1, 2, 3]
        )
    ...
    def test_cache_positions_updated(self):
        ...
        self.assertEqual(
            manager.cache_positions.tolist(), [0, 1, 2, 3, 4, 5, 6, 7]
        )
        ...
        self.assertEqual(
            manager.cache_positions.tolist(), [0, 1, 8, 3, 4, 5, 6, 7]
        )
...
class KVCacheWithAttentionSinkTest(unittest.TestCase):
    ...
    def test_causal_mask_attends_to_sink(self):
        ...
        mask = self.kv_cache.create_causal_mask_for_ring_buffer(
            start_pos=15, seq_len=1
        )
...
class AttentionSinkE2ETest(unittest.TestCase):
    ...
    def _build_model(self, args, sink_size, window_size, use_custom_sdpa=False):
        ...
        model = enable_attention_sink(model, params=args, sink_size=sink_size, window_size=window_size)
    ...
    def test_beyond_context_window_custom_sdpa(self):
        ...
        for m in model.modules():
            if isinstance(m, CustomKVCacheWithAttentionSink):
                ...
                break
        self.assertTrue(found_custom_cache, "Expected CustomKVCacheWithAttentionSink in model")

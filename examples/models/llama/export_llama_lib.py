...
def _prepare_for_llama_export(config: DictConfig) -> LLMEdgeManager:
    ...
    elif config.quantization.mode in ["8da4w", "8da4w-gptq"]:
    ...
    return (
        ...
        .source_transform(
            _get_source_transforms(config.model.name, dtype_override, config)
        )
    )
...
def _get_source_transforms(
    modelname: str, dtype_override: Optional[DType], config: DictConfig
    ...
    if config.quantization.mode:
        ...
        transforms.append(
            get_quant_weight_transform(config, dtype_override, verbose_export())
        )
    ...
    if config.quantization.embedding_quantize:
        ...
        transforms.append(get_quant_embedding_transform(config))

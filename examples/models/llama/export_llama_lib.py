...
def _calibrate_static_kv_cache(
    ...
    if not tasks:
        logging.warning(
            ...
            "fixed scale %s",
            llm_config.model.static_kv_cache_scale,
        )
        return
...
def _prepare_for_llama_export(llm_config: LlmConfig) -> LLMEdgeManager:
    ...
    edge_manager = edge_manager.set_output_dir(output_dir_path).source_transform(
        _get_source_transforms(
            ...
            static_kv_cache_scale=llm_config.model.static_kv_cache_scale,
...
def _validate_static_kv_cache_args(llm_config):
    ...
    if (
        not math.isfinite(llm_config.model.static_kv_cache_scale)
        or llm_config.model.static_kv_cache_scale <= 0
    ):
        raise ValueError("model.static_kv_cache_scale must be finite and positive")

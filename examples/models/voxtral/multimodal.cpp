...
#define ET_VOXTRAL_HAS_CUDA_RUNTIME 1
#include <cuda_runtime_api.h>
#else
#define ET_VOXTRAL_HAS_CUDA_RUNTIME 0
#endif
#else
#define ET_VOXTRAL_HAS_CUDA_RUNTIME 0
#endif
...
#if ET_VOXTRAL_HAS_CUDA_RUNTIME
class CudaMemoryTracker {
 public:
  ...
  ~CudaMemoryTracker() {
    ...
    std::cout << "CUDA peak memory usage observed: " << peak_mb << " MB of " << total_mb << " MB total" << std::endl;
  }
 ...
 private:
  bool query(size_t* free_bytes, size_t* total_bytes) {
    ...
    if (err != cudaSuccess) {
      if (!error_logged_) {
        error_logged_ = true;
        std::cerr << "Warning: cudaMemGetInfo failed with error: " << cudaGetErrorString(err) << std::endl;
      }
      available_ = false;
      return false;
    }
  ...
  void log_state(
    ...
    std::cout << "CUDA memory (" << tag << "): used " << used_mb << " MB, free " << free_mb << " MB, total " << total_mb << " MB" << std::endl;
  }

...
class TestLlava(unittest.TestCase):
    def setUp(self):
        ...
        self.prompt_before_image, self.resized, self.prompt_after_image = self.llava_model.get_inputs_for_prefill()

    def test_prefill_logits(self):
        prefill_logits = self.llava.prefill(self.prompt_before_image, self.resized, self.prompt_after_image)
        prefill_logits_ref = self.llava.prefill_ref(self.prompt_before_image, self.resized, self.prompt_after_image)[0]
    ...
    def test_generated_output(self):
        ...
        ref_outputs = self.llava_model.tokenizer.batch_decode(output_ids, skip_special_tokens=True)[0].strip()
        ...
        outputs = self.llava_model.tokenizer.batch_decode(torch.tensor([new_tokens]), skip_special_tokens=True)[0].strip()

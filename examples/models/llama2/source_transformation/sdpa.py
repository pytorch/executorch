...
def replace_sdpa_with_qnn_sdpa(module: torch.nn.Module):
    for name, child in module.named_children():
        if isinstance(child, SDPA):
            setattr(
                module,
                name,
                SDPAQNN(child.kv_cache, child.dim, child.head_dim, child.n_rep),
            )
        else:
            replace_sdpa_with_qnn_sdpa(child)
    return module

class KVCacheQNN(torch.nn.Module):
    def __init__(
        self,
        max_batch_size: int,
        max_seq_length: int,
        ...
        self.register_buffer(
            "past_k_caches", torch.zeros(cache_shape, dtype=dtype, device="cpu"), persistent=False
        )
        self.register_buffer(
            "past_v_caches", torch.zeros(cache_shape, dtype=dtype, device="cpu"), persistent=False
        )
...
def replace_kv_cache_with_qnn_kv_cache(module: torch.nn.Module):
    for name, child in module.named_children():
        if isinstance(child, KVCache):
            ...
            replace_kv_cache_with_qnn_kv_cache(child)
    return module

...
def from_float(
    ...
    scale = input[torch.arange(input.size(0)), abs_max_id] / -8
    inv_scale = torch.div(1.0, scale.to(torch.float32))

    clamped = torch.clamp(
        input=torch.floor(inv_scale.unsqueeze(1) * input + zero_point),
        min=0,
        max=15,
    ).to(torch.uint8)
    alternate = _interleave(clamped, group_size)
    return torch.cat([_pack_to_two_uint8(scale), pack_uint4(alternate)], dim=1)

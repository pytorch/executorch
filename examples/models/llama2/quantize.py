...
def embedding_quant(weight, group_size: Optional[int] = None) -> Tuple[torch.Tensor, torch.Tensor]:
    """
...
def replace_embedding_weight_only_grouped_int8_per_channel(module, group_size: Optional[int] = None):
    for name, child in module.named_children():
        print(f"name: {name}")
        if name == "XXXXoutputXXXXXXX":
            print("skipping quantizing output")
            ...
            setattr(
                module,
                name,
                QuantizedGroupEmbedding(weight_int8, scales_fp16, group_size=group_size),
            )
        else:
...
class EmbeddingOnlyInt8QuantHandler:
    ...
    def convert_for_runtime(self) -> nn.Module:
        replace_embedding_weight_only_grouped_int8_per_channel(self.mod, self.group_size))
        return self.mod

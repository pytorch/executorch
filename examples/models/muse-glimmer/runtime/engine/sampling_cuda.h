...
// lowest token index, matching muse_glimmer::argmax_index. The launch is
// asynchronous with respect to `stream`.
...
// CUDA counterpart of muse_glimmer::sample_excluding_token_in_place for batched
// rows. Mutates each probability row by excluding and renormalizing before
// sampling.
...
// CUDA counterpart of muse_glimmer::sample_from_residual_in_place for batched
// rows. Mutates target probabilities to normalized max(p-q, 0) when residual
// mass exists, otherwise preserves p, then samples one token per row.

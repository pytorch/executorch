...
class DFlashSession final : public LLMSession, public DFlashMultimodalSession {
 public:
  ...
  Error prefill_tokens(
    ...
    int64_t offset = 0;
    const int64_t retained_hidden_start = prepare_backend_prefill(token_count);
    while (offset < token_count) {
      const PrefillChunkPlan plan = plan_backend_prefill_chunk(
          offset, token_count, retained_hidden_start);
      const bool is_final_chunk = offset + plan.chunk == token_count;
      auto result = run_target_from_embeddings(
          ...
          /*timing=*/nullptr,
          plan.use_target_prefill,
          plan.retain_from_row);
      ...
      target_pos_ += plan.chunk;
      logical_pos_ += plan.chunk;
      offset += plan.chunk;
    }
 ...
 private:
  ...
  Error prime_draft_cache() {
    auto prime_chunk_result = prepare_backend_draft_cache();
    ET_CHECK_OK_OR_RETURN_ERROR(prime_chunk_result.error());
    const int64_t prime_chunk = prime_chunk_result.get();
    ET_CHECK_OR_RETURN_ERROR(
  ...
  Error run_decode_cycle() {
    ...
    return run_backend_decode_cycle(room);
  }

#ifdef EXECUTORCH_BUILD_CUDA
  void shutdown_backend() {}

  void initialize_backend_sampling() {}
  ...
  int64_t prepare_backend_prefill(int64_t token_count) {
    ...
    return retained_hidden_start;
  }

  PrefillChunkPlan plan_backend_prefill_chunk(
      int64_t offset,
      int64_t token_count,
      int64_t retained_hidden_start) const {
    ...
    return PrefillChunkPlan{chunk, use_target_prefill, retain_from_row};
  }

  Result<int64_t> prepare_backend_draft_cache() {
  ...
  Error ensure_cuda_device_buffers() {
    ...
    ET_CHECK_OR_RETURN_ERROR(
        context_length > 0,
        InvalidProgram,
        "CUDA DFlash requires a positive context size");
    ...
    cuda_position_table_ = clone_tensor_ptr_to(
        from_blob(
            ...
            executorch::aten::ScalarType::Long),
        cuda_device);
    // Prefill feeds far wider token spans than a decode cycle, so this staging
    // buffer is sized for the widest target call the session can make.
  ...
  Error ensure_cuda_vocab_buffers(int64_t vocab_size) {
    ET_CHECK_OR_RETURN_ERROR(
        vocab_size > 0,
        InvalidProgram,
        "CUDA DFlash requires a positive logits vocabulary size");
    if (cuda_vocab_size_ != 0) {
      ET_CHECK_OR_RETURN_ERROR(
          cuda_vocab_size_ == vocab_size,
          InvalidProgram,
          "CUDA DFlash logits vocabulary size changed");
      return Error::Ok;
    }
    ...
    const auto cuda_device =
        executorch::aten::Device(executorch::aten::DeviceType::CUDA, 0);
    std::vector<float> storage(block_length_ * cuda_vocab_size_);
    auto storage_tensor = from_blob(
        storage.data(),
        {static_cast<SizesType>(block_length_),
         static_cast<SizesType>(cuda_vocab_size_)},
        executorch::aten::ScalarType::Float);
    cuda_target_probabilities_ =
        clone_tensor_ptr_to(storage_tensor, cuda_device);
    cuda_draft_probabilities_ =
        clone_tensor_ptr_to(storage_tensor, cuda_device);
    cuda_target_logits_ = clone_tensor_ptr_to(storage_tensor, cuda_device);
    cuda_draft_logits_ = clone_tensor_ptr_to(storage_tensor, cuda_device);
    ET_CHECK_OR_RETURN_ERROR(
  ...
  Result<TensorPtr> stage_cuda_tokens(const uint64_t* tokens, int64_t count) {
    ...
    // Before the first delegated execution, the backend's shared stream has not
    // necessarily been installed yet. Complete host staging before handing the
    // buffer to the method; later decode-only inputs are already on device.
    ET_CUDA_CHECK_OR_RETURN_ERROR(
        cudaStreamSynchronize(*cuda_execution_stream_));
    return device_view(
  ...
  Result<TensorPtr> stage_cuda_embeddings(
    ...
    ET_CHECK_OR_RETURN_ERROR(
        count > 0 && embed_dim > 0,
        InvalidArgument,
        "CUDA spliced embedding dimensions must be positive");
  ...
  Error prepare_cuda_sampling_workspaces(cudaStream_t stream) {
    ...
    ET_CUDA_CHECK_OR_RETURN_ERROR(error);
  ...
  Error refresh_cuda_execution_stream() {
    ...
    return cuda_vocab_size_ > 0
        ? prepare_cuda_sampling_workspaces(*cuda_execution_stream_)
        : Error::Ok;
  }

  Result<TensorPtr> copy_cuda_logits(
      const executorch::aten::Tensor& source,
      TensorPtr& destination) {
    // Module execution installs the backend stream in the shared stream
    // registry. Refresh here, while the execution lock is still held, before
    // copying out of the method's planned output buffer.
    ET_CHECK_OK_OR_RETURN_ERROR(refresh_cuda_execution_stream());
    ET_CHECK_OR_RETURN_ERROR(
        source.dim() >= 1 &&
            source.scalar_type() == executorch::aten::ScalarType::Float,
        InvalidProgram,
        "DFlash logits must be a non-scalar float32 tensor");
    const int64_t vocab_size = source.size(source.dim() - 1);
    ET_CHECK_OK_OR_RETURN_ERROR(ensure_cuda_vocab_buffers(vocab_size));
    ET_CHECK_OR_RETURN_ERROR(
        destination != nullptr && source.numel() <= destination->numel(),
        InvalidProgram,
        "DFlash logits exceed the session-owned CUDA buffer");
    ET_CUDA_CHECK_OR_RETURN_ERROR(cudaMemcpyAsync(
  ...
  Result<TargetResult> run_cuda_target_device(
    ...
    // Candidate tokens and positions are already device-resident. The target
    // output is copied into session-owned device memory before releasing the
    // execution lock; hidden state and cycle results are copied to the host.
    std::lock_guard<std::mutex> guard(*config_.exec_mutex);
  ...
  Error run_cuda_speculative_cycle() {
    ...
    ET_CUDA_CHECK_OR_RETURN_ERROR(copy_error);
    ET_CUDA_CHECK_OR_RETURN_ERROR(sync_error);
    ET_CHECK_OR_RETURN_ERROR(
  ...
  Error run_backend_decode_cycle(int64_t room) {
    if (room < n_draft_ + 1) {
  ...
  int64_t prepare_backend_prefill(int64_t /*token_count*/) const {
    return logical_pos_;
  }
  ...
  PrefillChunkPlan plan_backend_prefill_chunk(
    ...
    return PrefillChunkPlan{
        chunk, /*use_target_prefill=*/false, /*retain_from_row=*/0};
  }

  Result<int64_t> prepare_backend_draft_cache() const {
    return config_.max_prefill_chunk;
  }
  ...
  Result<TargetResult> process_target_outputs(
    ...
    ET_CHECK_OR_RETURN_ERROR(
        ...
        InvalidProgram,
        "DFlash logits must be float32");
    auto logits = copy_cuda_logits(output_logits, cuda_target_logits_);
    ...
    if (sample_last) {
      ...
      const auto error = muse_glimmer::cuda::sample_token(
          static_cast<const float*>(last_logits_tensor_->const_data_ptr()) +
...
#ifdef EXECUTORCH_BUILD_CUDA
    auto logits =
        copy_cuda_logits((*outputs)[0].toTensor(), cuda_draft_logits_);
    ET_CHECK_OK_OR_RETURN_ERROR(logits.error());
    draft_logits_tensor_ = std::move(logits.get());
#else
  ...
  TensorPtr cuda_target_logits_;
  TensorPtr cuda_draft_logits_;

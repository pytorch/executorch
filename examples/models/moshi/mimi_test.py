...
#     apply_output=False,
# )
# et_transformer = Transformer(model_args)
# print(et_transformer)
...
def mimi_streaming_test(mimi, max_duration_sec=10.0):
    ...
    exported_decode = export_for_training(mimi_decode, (all_codes_th,), strict=False).module()


    emb = mimi.decode_latent(all_codes_th)
    emb = mimi._to_encoder_framerate(emb)
    (emb,) = mimi.decoder_transformer(emb)
    with mimi._context_for_encoder_decoder:
        pcm = mimi.decoder(emb)

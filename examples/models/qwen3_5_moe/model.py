...
Reference implementations:
  - nano_qwen35_moe/export_model.py (export patterns)
  - llama/attention.py (GatedDeltaNet + full attention with q_gate)
  - vllm/model_executor/models/qwen3_5.py (architecture details)
"""
    ...
    def from_hf_config(config_path):
        ...
        return Qwen35MoEConfig(
            ...
            rope_theta=cfg.get("rope_theta",
                               rope_params.get("rope_theta", 10_000_000.0)),
...
class KVCache(nn.Module):
    ...
    def __init__(self, n_kv_heads, head_dim, max_seq_len):
        super().__init__()
        self.register_buffer("k_cache", torch.zeros(1, n_kv_heads, max_seq_len, head_dim))
        self.register_buffer("v_cache", torch.zeros(1, n_kv_heads, max_seq_len, head_dim))
...
class FullAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.q_proj = nn.Linear(config.hidden_size, self.n_heads * self.head_dim * 2, bias=False)
        ...
        self.rotary_emb = RotaryEmbedding(self.head_dim, config.partial_rotary_factor, config.rope_theta)
...
class GatedDeltaNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.register_buffer(
            "recurrent_state", torch.zeros(1, self.num_v_heads, self.head_k_dim, self.head_v_dim)
        )
...
class Qwen35MoE(nn.Module):
    ...
    def from_hf_checkpoint(model_dir, max_seq_len=4096):
        ...
        # Validate
        runtime_prefixes = ("lm_head.weight", ".kv_cache.", ".conv_state",
                            ".recurrent_state", ".mask", ".inv_freq")
        ...
        with safe_open(shard_path, framework="pt", device="cpu") as f:
            for ckpt_key in f.keys():
                _process_checkpoint_key(
                    ckpt_key, f.get_tensor(ckpt_key),
        ...
        for layer_idx in range(config.num_hidden_layers):
            ...
            if gate_list[0] is not None:
                ...
                for g in range(num_groups):
                    ...
                    state_dict[f"layers.{layer_idx}.mlp.cond_ffn.gate_up_projs.{g}.weight"] = (
...
def _get_mapped_key(hf_key):
    ...
    # Handle multimodal prefix
    if hf_key.startswith("model.language_model."):
        hf_key = hf_key.replace("model.language_model.", "model.", 1)
...
def _remap_hf_keys(state_dict, config):
    """Remap HF checkpoint keys and stack expert weights."""
    new_sd = {}
    expert_weights = {}  # (layer_idx, proj, expert_idx) → tensor

    for key, value in state_dict.items():
        # Normalize multimodal prefix
        norm_key = key
        if norm_key.startswith("model.language_model."):
            norm_key = norm_key.replace("model.language_model.", "model.", 1)

        # Skip non-text keys (visual, mtp)
        if not norm_key.startswith(("model.", "lm_head.")):
            continue

        # Fused expert weights: gate_up_proj [E, 2*H, D] and down_proj [E, D, H]
        m = _FUSED_EXPERT_RE.match(norm_key)
        if m:
            layer_idx = int(m.group(1))
            proj_name = m.group(2)
            if proj_name == "gate_up_proj":
                # Split fused gate+up along dim 1: [E, 2*H, D] -> [E, H, D] each
                w_gate, w_up = value.chunk(2, dim=1)
                new_sd[f"layers.{layer_idx}.mlp.cond_ffn.w_gate"] = w_gate.contiguous()
                new_sd[f"layers.{layer_idx}.mlp.cond_ffn.w_up"] = w_up.contiguous()
            else:  # down_proj
                new_sd[f"layers.{layer_idx}.mlp.cond_ffn.w_down"] = value
            continue

        # Per-expert weights (alternative format): experts.{e}.{gate,up,down}_proj.weight
        m = _EXPERT_RE.match(norm_key)
        if m:
            layer_idx, expert_idx, proj = int(m.group(1)), int(m.group(2)), m.group(3)
            expert_weights[(layer_idx, proj, expert_idx)] = value
            continue
    ...
    # Stack per-expert weights (only if using non-fused format)
    if expert_weights:
        proj_map = {"gate": "w_gate", "up": "w_up", "down": "w_down"}
        for layer_idx in range(config.num_hidden_layers):
            for proj, param_name in proj_map.items():
                experts_for_layer = [
                    expert_weights.get((layer_idx, proj, e))
                    for e in range(config.num_experts)
                ]
                if experts_for_layer[0] is not None:
                    stacked = torch.stack(experts_for_layer, dim=0)
                    new_sd[f"layers.{layer_idx}.mlp.cond_ffn.{param_name}"] = stacked

    # Handle tied embeddings
    if "lm_head.weight" not in new_sd:
        new_sd["lm_head.weight"] = new_sd["embed_tokens.weight"].clone()

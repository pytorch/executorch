...
def _export_cuda(model, config, args):
    ...
    # Limit autotuning to GEMMs. Global max autotuning also expands generated
    # pointwise/reduction kernels, which is unsafe for released KV storage in
    # PyTorch 2.14. GEMM autotuning keeps the prefill path Triton-only.
    et_prog = to_edge_transform_and_lower(
        {"decode": decode_ep, "prefill": prefill_ep},
        partitioner={
            "decode": [
                CudaPartitioner(
                    [
                        ...
                        CompileSpec("max_autotune_gemm", b"ON"),

...
class UnifiedRunnerContractTest(unittest.TestCase):
    ...
    def test_main_cli_validates_text_mode_requirements(self):
        ...
        self.assertIn('Provide either --codes_path or --text, not both.', self.main)
        self.assertIn('"--text requires --tokenizer_path."', self.main)
        self.assertIn("config.top_p = static_cast<float>(FLAGS_top_p);", self.main)

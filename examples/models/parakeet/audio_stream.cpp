...
class PortAudioStream : public AudioStream {
 ...
 private:
  static int pa_callback(
    ...
    // Debug: track callback invocations and detect silence
    stream->callback_count_++;
    if (stream->callback_count_ <= 3) {
      std::cerr << "[AudioDebug] callback #" << stream->callback_count_
                << " frames=" << frames_per_buffer
                << " input_buffer=" << (input_buffer ? "valid" : "null")
                << " status_flags=" << status_flags << std::endl;
      if (input_buffer != nullptr) {
        ...
        std::cerr << "[AudioDebug] first few samples: "

...
from executorch.examples.models.llama.norm import RMSNorm  # noqa F401
...
class TransformerBlockSequence(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        for layer_id in range(args.n_layers):
            self.layers.append(TransformerBlock(layer_id, args, self.rope))
    ...
    def forward(
        ...
        freqs_cos,
        freqs_sin,
        *args,  # k_caches (n_layers tensors) + v_caches (n_layers tensors) + attn_mask
    ):
        # After torch.export, list arguments get flattened into individual args
        # Reconstruct them: args = [k_cache_0, ..., k_cache_n, v_cache_0, ..., v_cache_n, attn_mask]
        n_layers = len(self.layers)
        k_caches = list(args[:n_layers])
        v_caches = list(args[n_layers : 2 * n_layers])
        attn_mask = args[2 * n_layers]

        new_k_caches = []
        new_v_caches = []
        for i, layer in enumerate(self.layers):
            x, new_k, new_v = layer(
                x, freqs_cos, freqs_sin, k_caches[i], v_caches[i], attn_mask
            )
...
def load_model_in_pieces_ITO(
    ...
    """
    Loads model in 3 pieces: input, single transformer block (all layers), output
    """
    import json
    ...
    with torch.device("meta"):
        ...
        transformer_blocks = TransformerBlockSequence(args)
    ...
    missing, unexpected = transformer_blocks.load_state_dict(
        checkpoint,
        strict=False,
        assign=True,
    )
    ...
    # Create example inputs for TransformerBlockSequence
    # The forward method uses *args to receive k_caches and v_caches, so we need to unpack them
    ...
    # Add unpacked k_caches
    for _ in range(args.n_layers):
        transformer_example_inputs += (torch.zeros(cache_shape, dtype=float_dtype),)
    # Add unpacked v_caches
    for _ in range(args.n_layers):
        transformer_example_inputs += (torch.zeros(cache_shape, dtype=float_dtype),)
    # Add attn_mask
    transformer_example_inputs += (torch.zeros(attn_mask_shape, dtype=float_dtype),)

    example_inputs = [
        (
            torch.zeros(1, seq_length, dtype=torch.int64),
            torch.tensor([0], dtype=torch.long),
        ),  # InputBlock
        transformer_example_inputs,  # TransformerBlockSequence
        (  # OutputBlock
            torch.zeros(h_shape, dtype=float_dtype),  # h
...
def load_model_in_pieces_IAFO(
    ...
    example_inputs = [
        ...
        (torch.zeros(h_shape, dtype=float_dtype),),  # FeedForwardBlock  # h
        (  # OutputBlock
            torch.zeros(h_shape, dtype=float_dtype),  # h
            torch.tensor([0], dtype=torch.long),  # input_length
        ),

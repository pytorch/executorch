...
def compile(args):
    ...
    if args.ptq != None:
        ...
        single_llama.quantize(
            quant_dtype=quant_dtype,
            args=args,
            custom_annotations=(matmul_annotate_func,),
        )
    ...
    return single_llama.get_quant_attrs()


def inference(args, quant_attrs, pre_gen_pte=""):
...
def main():
    ...
    if args.pre_gen_pte:
        ...
        inference(args, quant_attrs, args.pre_gen_pte)
        exit(f"Finish the running pre_gen_pte from {args.pre_gen_pte}")

    if args.compile_only:
        quant_attrs = compile(args)
        json.dump(
            {"scale": quant_attrs["scale"], "zero_point": quant_attrs["zero_point"]},

 ...
 */
DECLARE_PKG_OPS_OPTS_LIST(PKG_Embedding)
...
static std::array<const char*, 1> sg_opNames{{"Embedding"}};
 ...
 * can be set by Qnn_ErrorHandle_t EmbeddingOpPackageLogSetLevel(QnnLog_Level_t maxLogLevel)
 */
/*
...
Qnn_ErrorHandle_t EmbeddingOpPackageInit(QnnOpPackage_GlobalInfrastructure_t infrastructure) {
...
Qnn_ErrorHandle_t EmbeddingOpPackageGetInfo(const QnnOpPackage_Info_t** info) {
...
Qnn_ErrorHandle_t EmbeddingOpPackageLogInitialize(QnnLog_Callback_t callback, QnnLog_Level_t maxLogLevel) {
...
Qnn_ErrorHandle_t EmbeddingOpPackageLogSetLevel(QnnLog_Level_t maxLogLevel) {
    if (maxLogLevel < QNN_LOG_LEVEL_ERROR) return QNN_LOG_ERROR_INVALID_ARGUMENT;
    sg_maxLogLevel = maxLogLevel;
    return QNN_SUCCESS;
}

Qnn_ErrorHandle_t EmbeddingOpPackageLogTerminate() {
...
Qnn_ErrorHandle_t EmbeddingOpPackageValidateOpConfig (Qnn_OpConfig_t opConfig){
     ...
     */
    if (std::string(opConfig.v1.typeName) == "Embedding"){
        // check function signature
        if (opConfig.v1.numOfParams != 0 || opConfig.v1.numOfInputs != 2 || opConfig.v1.numOfOutputs != 1){
            return QNN_OP_PACKAGE_ERROR_VALIDATION_FAILURE;
        }
        // check implmentation constraints
        if (opConfig.v1.inputTensors[0].v1.rank != 2 ||
            opConfig.v1.inputTensors[1].v1.rank > 3) {
            return QNN_OP_PACKAGE_ERROR_VALIDATION_FAILURE;
        }
        // check IO tensor information
        if (opConfig.v1.inputTensors[0].v1.type != QNN_TENSOR_TYPE_STATIC ||
            opConfig.v1.inputTensors[0].v1.dataType != QNN_DATATYPE_SFIXED_POINT_8 ||
            opConfig.v1.inputTensors[1].v1.dataType != QNN_DATATYPE_INT_32 ||
            opConfig.v1.inputTensors[0].v1.dataType != opConfig.v1.outputTensors[0].v1.dataType) {
            return QNN_OP_PACKAGE_ERROR_VALIDATION_FAILURE;
        }
    }
    else{
 ...
 * (version 1.3.0) Qnn_ErrorHandle_t EmbeddingOpPackageCreateKernels (QnnOpPackage_GraphInfrastructure_t
 * graphInfrastructure, QnnOpPackage_Node_t node, QnnOpPackage_Kernel_t** kernels, uint32_t*
 * numKernels)
 * (version 1.3.0) Qnn_ErrorHandle_t EmbeddingOpPackageFreeKernels (QnnOpPackage_Kernel_t* kernels)
 *
 * (version 1.4.0) Qnn_ErrorHandle_t EmbeddingOpPackageCreateOpImpl (QnnOpPackage_GraphInfrastructure_t
 * graphInfrastructure, QnnOpPackage_Node_t node, QnnOpPackage_OpImpl_t* opImpl)
 *(version 1.4.0) Qnn_ErrorHandle_t EmbeddingOpPackageFreeOpImpl (QnnOpPackage_OpImpl_t opImpl)
 */

Qnn_ErrorHandle_t EmbeddingOpPackageTerminate() {
if (!sg_packageInitialized) return QNN_OP_PACKAGE_ERROR_LIBRARY_NOT_INITIALIZED;
...
/* latest version */
Qnn_ErrorHandle_t EmbeddingOpPackageInterfaceProvider(QnnOpPackage_Interface_t* interface) {
  if (!interface) return QNN_OP_PACKAGE_ERROR_INVALID_ARGUMENT;
  interface->interfaceVersion      = {1, 4, 0};
  interface->v1_4.init             = EmbeddingOpPackageInit;
  interface->v1_4.terminate        = EmbeddingOpPackageTerminate;
  interface->v1_4.getInfo          = EmbeddingOpPackageGetInfo;
  interface->v1_4.validateOpConfig = EmbeddingOpPackageValidateOpConfig;
  interface->v1_4.createOpImpl     = nullptr;
  interface->v1_4.freeOpImpl       = nullptr;
  interface->v1_4.logInitialize    = EmbeddingOpPackageLogInitialize;
  interface->v1_4.logSetLevel      = EmbeddingOpPackageLogSetLevel;
  interface->v1_4.logTerminate     = EmbeddingOpPackageLogTerminate;
  return QNN_SUCCESS;
}

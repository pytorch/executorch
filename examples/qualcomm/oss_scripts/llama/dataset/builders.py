...
class DatasetBuilder:
    ...
    def build_qat_dataloaders(self) -> Tuple[
        ...
        --calib_tasks provides the calibration set and --train_tasks /
        --train_hf_dataset provide the training set. An optional val split is
        cut from the training pool via --train_val_ratio.
    ...
    def _split_train_val(
        ...
        # With drop_last=True a train subset smaller than batch_size yields zero
        # batches.
        if len(train_loader) == 0:
            raise ValueError(
                f"QAT train split has {n_train} samples but batch_size={batch_size} "
                f"with drop_last=True yields 0 batches. Increase the training data "
                f"(e.g. --train_limit) or reduce --batch_size."
            )

...
def reverse_quantize_module_swap(model: nn.Module) -> nn.Module:
    ...
    model = reverse_replace_all_embedding_with_quantized(model) # if embedding_quantize was false, does nothing
    return model
...
def compute_scales(model, data, num_points=100):
    print("Computing scales!")
    recipe = QuantizationRecipe(
        weight_bits=4, # TODO: should be based on dtype!
        weight_quantization=True,
        dynamic_weights=False,
        weight_group_size="per_channel",
        activation_bits=16, # same as above
    ...
    set_weight_range_activation_loss(quantized_model, data, 1, num_points) # batch_size = 1 for us
...
def make_custom_quantizer(quant_dtype, range_setting=None, custom_annotations=(), linear_only=False):
    ...
    if linear_only:
        ...
        conv_keys = {op for op in all_keys if op.__name__ in ('conv1d.default', 'conv2d.default', 'conv_transpose2d.input', 'linear.default')}
...
def set_scales(prepared_model, scales_state_dict):
    print("Setting the scales of the manual observers!")
    num_heads = 32 # should set by argument
    head_dim = 2048 // num_heads
    for node in prepared_model.graph.nodes:
        if type(node.target) == str:
            l = node.target.split(".")
            if len(l) > 3 and l[-3] in ("wq_sha", "wk_sha", "wv_sha"):
                shorter = l[-3][:2]
                key = ".".join(["model"] + l[:-3] + [shorter])
                ...
                try:
                    ...
                    print("Failed to set scale for ", key)
            elif len(l) > 1 and l[-2] in ("wo_sha", "w1_conv", "w2_conv", "w3_conv"):
                shorter = l[-2][:2]
                key = ".".join(["model"] + l[:-2] + [shorter])
                observer_name = str(list(node.users.keys())[0])
                observer = getattr(prepared_model, observer_name)
                try:
            ...
            #     observer_name = str(list(node.users.keys())[0])
            #     observer = getattr(prepared_model, observer_name)
            #     #if type(observer.target) == str and observer.target[:10] == "activation":
            #     try:
            #         observer.set_scale(scales_state_dict[key])
            #         print("Set scale for", key)
            #     except:

...
from executorch.examples.qualcomm.oss_scripts.qwen3.model.static_qwen3 import (
    Qwen3ForCausalLM,
    Qwen3Config,
    Qwen3DecoderLayer,
)
...
def compile(args, pte_filename, tokenizer):
    ...
    qmodel = GPTQModel.from_quantized(args.model_dir)
    state_dict = qmodel.model.state_dict()
    qcfg = qmodel.quantize_config
    if qcfg.desc_act:
        raise RuntimeError(
    ...
    for llama_instance in llama_instance_list:
        for layer in llama_instance.model.layers:
            ...
            convert_linear_to_qlinear(layer.self_attn, qlinear_cls)
            convert_linear_to_qlinear(layer.mlp, qlinear_cls)
    ...
    for llama_instance in llama_instance_list:
        for layer in llama_instance.model.layers:
            if args.use_tman:
                convert_qlinear_to_tman_linear(layer.self_attn)
                layer.self_attn.prepare_tman()
                convert_qlinear_to_tman_linear(layer.mlp)
            else:
                convert_qlinear_to_linear(layer.self_attn)
                layer.self_attn.prepare_sha()
                convert_qlinear_to_linear(layer.mlp)
...
def _build_parser():
    ...
    parser.add_argument(
        "--llama_model",
        choices=["stories110m", "llama3_2", "qwen3"],
        help="The Llama model to export. Current available options are: [stories110m, llama3_2, qwen3]",
        required=True,
    )
...
def export_llama(args) -> None:
    ...
    elif args.llama_model == "qwen3":
        assert isinstance(
            tokenizer, HuggingFaceTokenizer
        ), f"Wrong tokenizer provided for qwen3."
        runtime_tokenizer_path = args.tokenizer_model
    else:

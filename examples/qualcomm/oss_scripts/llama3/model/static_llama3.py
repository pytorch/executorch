...
class Llama3Attention(nn.Module):
    ...
    def __init__(self, config: Llama3Config, layer_idx: int):
        super().__init__()
        self.config = config
...
class Llama3DecoderLayer(nn.Module):
    def __init__(self, config: Llama3Config, layer_idx: int):
        ...
        self.self_attn = Llama3Attention(config=config, layer_idx=layer_idx)
...
class Llama3Model(nn.Module):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`Llama3DecoderLayer`]

    Args:
        config: Llama3Config
    """

    def __init__(self, config: Llama3Config, max_seq_len: int):
        super().__init__()
        ...
        self.layers = nn.ModuleList(
            [Llama3DecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class Llama3ForCausalLM(PreTrainedModel):
    ...
    def __init__(
        ...
        self.model = Llama3Model(config, max_seq_len)
        self.vocab_size = config.vocab_size

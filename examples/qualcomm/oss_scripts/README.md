...
1. `albert`,`bert`,`distilbert`, `eurobert`, `roberta`:
   - Required Dataset : wikisent2
      ...
      ```bash
      python albert.py --soc_model ${SOC_MODEL} --build_folder path/to/build-android/ --device ${DEVICE_SERIAL} -d path/to/wikisent2
...
2. `conv_former`, `convnext_small`, `cvt`, `deit`, `dino_v2`, `efficientnet`, `fbnet`, `focalnet`, `gMLP_image_classification`, `maxvit_t`, `mobilevit1`, `mobilevit_v2`, `pvt`, `squeezenet`, `swin_transformer`, `swin_v2_t`, `vit_b_16` :
   - Required Dataset : ImageNet
      ...
      ```bash
      python SCRIPT_NAME.py --soc_model ${SOC_MODEL} --build_folder path/to/build-android/ --device ${DEVICE_SERIAL} -d path/to/ImageNet

3. `dit`:

      ```bash
        python dit.py --soc_model ${SOC_MODEL} --build_folder path/to/build-android/ --device ${DEVICE_SERIAL}
4. `esrgan`:
    ...
    - Required OSS Repo: Real-ESRGAN
      ...
      ```bash
      python esrgan.py --soc_model ${SOC_MODEL} --build_folder path/to/build-android/ --device ${DEVICE_SERIAL} --oss_repo path/to/Real-ESRGAN
...
5. `fastvit`:
    ...
    - Pretrained weight:
      ...
      ```bash
      python fastvit.py --soc_model ${SOC_MODEL} --build_folder path/to/build-android/ --device ${DEVICE_SERIAL} --oss_repo path/to/ml-fastvit -p path/to/pretrained_weight -d path/to/ImageNet
...
6. `regnet`:
    ...
    ```bash
      python regnet.py --soc_model ${SOC_MODEL} --build_folder path/to/build-android/ --device ${DEVICE_SERIAL} -d path/to/ImageNet --weights <WEIGHTS>
...
7. `retinanet`:
    - Required Dataset: COCO
      ...
      ```bash
      python retinanet.py --soc_model ${SOC_MODEL} --build_folder path/to/build-android/ --device ${DEVICE_SERIAL} -d path/to/PATH/TO/COCO #(which contains 'val_2017' & 'annotations')
...
8. `ssd300_vgg16`:
   ...
   - Required Dataset: VOCSegmentation
      ...
      ```bash
      python ssd300_vgg16.py --soc_model ${SOC_MODEL} --build_folder path/to/build-android/ --device ${DEVICE_SERIAL} --oss_repo path/to/a-PyTorch-Tutorial-to-Object-Detection -p path/to/pretrained_weight
...
10. `efficientSAM`:
    ...
    - Required OSS Repo:
      ...
      ```bash
      python efficientSAM.py --soc_model ${SOC_MODEL} --build_folder path/to/build-android/ --device ${DEVICE_SERIAL} --oss_repo path/to/EfficientSAM -p path/to/pretrained_weight -d path/to/ImageNet

...
from executorch.examples.qualcomm.oss_scripts.qwen.decoder_model_wrapper import QnnCausalLMExportableModule
...
class QnnLLMEdgeManager:
    ...
    def pt2e_calibrate(
        ...
        try:
            from executorch.examples.qualcomm.oss_scripts.qwen.eval_qwen_qnn import (
        ...
        def calibrate_template(
            ...
            with torch.no_grad():
                while token_list[-1] != tokenizer.eos_id and pos < max_len:
                    ...
                    logits = module(
                        torch.full((1, 1), token_list[pos]), cur_pos
                    )
                    pos += 1
    ...
    def pt2e_quantize(self, quant_dtype, fixed_point_type, calibration_tasks, calibration_limit, calibration_data, tokenizer_path):
        self.export()
        ...
        if quant_dtype == QuantDtype.use_16a4w_block:
            conv_nodes = [
                n for n in self.graph_module.graph.nodes if "conv" in n.name
            ]
            block_size_map = {n.name: (1, 16, 1, 1) for n in conv_nodes}
            quantizer.set_block_size_map(block_size_map)
        self.graph_module = prepare_pt2e(self.graph_module, quantizer)
        self.pt2e_calibrate(calibration_tasks, calibration_limit, self.config.max_seq_len, calibration_data, tokenizer_path)
        self.graph_module = convert_pt2e(self.graph_module)
        logging.info("After QDQ, validate convert_pt2e")
        self.pt2e_calibrate(calibration_tasks, calibration_limit, self.config.max_seq_len, calibration_data, tokenizer_path)
        ...
        ] = partial(self._tag_ios, fixed_point_type=fixed_point_type, config=self.config)
        self.use_fp16 = False
    
    def to_edge_transform_and_lower_to_qnn(self, soc_model, skip_node_id_set, skip_node_op_set):

...
from executorch.examples.qualcomm.oss_scripts.qwen.qnn_decoder_model_manager import get_qnn_llm_edge_manager
import torch
...
def compile(args):  # noqa: C901
    ...
    source_transforms = []
    source_transforms.append(convert_linear_to_conv2d)

    manager = get_qnn_llm_edge_manager(args.decoder_model, args.max_seq_len, args.enable_spinquant_r3).source_transform(source_transforms)
    ...
    if args.ptq:
        ...
        manager.pt2e_quantize(quant_dtype, fixed_point_type, args.calibration_tasks, args.calibration_limit, args.prompt, tokenizer_json_path)
    
    manager.to_edge_transform_and_lower_to_qnn(args.model, skip_node_id_set, skip_node_op_set)
    if args.ptq:

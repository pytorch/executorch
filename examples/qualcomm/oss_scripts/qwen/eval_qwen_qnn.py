...
from executorch.examples.qualcomm.utils import make_output_dir, setup_common_args_and_variables
import os
import numpy as np
...
def create_device_inputs(example_inputs):
    ...
    for index, data in enumerate(example_inputs):
        inputs.append(data)
        input_list += " ".join(
            [f"input_{index}_{i}.raw" for i in range(len(data))]
        )
        input_list += "\n"
    ...
    def _model_call(self, inps):
        ...
        def post_process():
            for f in sorted(
                ...
                output_tensor = None
                if self.quant_attrs:
                    output_tensor = torch.from_numpy(np.fromfile(
                            os.path.join(output_data_folder, f), dtype=np.uint16
                        ).reshape(1, 1, -1))
                    output_tensor = (output_tensor.to(torch.float32) - self.quant_attrs["zero_point"]) * self.quant_attrs["scale"]
                else:
                    output_tensor = torch.from_numpy(np.fromfile(
    ...
    if (model := args.pte) is not None:  # pyre-ignore
        ...
        if args.logits_quant_attr_path is not None:
            quant_attrs = json.load(
                open(f"{args.logits_quant_attr_path}")
            )
        ...
        raise RuntimeError(
            "Currently only support evaluate pte on device"
        )

...
class Attention(nn.Module):
    ...
    def prepare_sha(self):
        self.wq_sha = nn.ModuleList(
            [
                nn.Conv2d(
                    ...
                    self.head_dim,
                    1,
                    bias=getattr(self.config, "attention_bias", True),
                )
        ...
        self.wo_sha = nn.Conv2d(
            self.n_heads * self.head_dim,
            self.dim,
            1,
            bias=getattr(self.config, "attention_bias", False),
        )
...
class DecoderModel(nn.Module):
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [
                DecoderLayer(config, self.output_new_cache_only)
                for _ in range(self.n_layers)
            ]
        )
        self.norm = RMSNorm(self.dim, eps=config.rms_norm_eps)
...
class DecoderModelForCausalLM(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.model = DecoderModel(config, ar_len, output_new_cache_only, use_i64_token)
        self.vocab_size = config.vocab_size
    ...
    def get_example_inputs(self, use_kv_cache=True):
        ...
        if self.max_seq_len != model.ar_len:
            atten_mask = torch.cat(
                [
                    torch.ones(model.ar_len, self.max_seq_len - model.ar_len) * -255.0,
                    atten_mask,
                ],
                dim=-1,
            )
        atten_mask = atten_mask[None, :, :].expand(
            model.max_batch_size, model.ar_len, self.max_seq_len
        )
        if use_kv_cache:
            pos_ids = torch.zeros(
                (model.max_batch_size, model.ar_len), dtype=torch.int32
            )
            k_cache, v_cache = [], []

...
def _qnn_attention(
    ...
    **kwargs,
    ...
    if getattr(module.config, "enable_spinquant_r3", False):
        r3_weight = torch.tensor(scipy.linalg.hadamard(module.head_dim, dtype=float) / math.sqrt(module.head_dim), dtype=torch.float32)
        query = torch.matmul(query, r3_weight.T)
        key = torch.matmul(key, r3_weight.T)
...
class QnnCausalLMExportableModule(torch.nn.Module):
    def __init__(self, model):
        ...
        self._metadata = save_config_to_constant_methods(model.config, model.generation_config)

...
Result<torch::executor::Tensor> Runner::run_model_step(
  ...
  // inputs:[tokens, start_pos, atten_mask, k_cache, v_cache, kv_mask]
  std::vector<EValue> inputs = {
      token, start_pos, atten_mask, k_cache, v_cache, kv_mask};
  ...
  /* ET_CHECK_MSG(
      outputs_res->size() == 4,
      "Expected outputs returned from executing LLM to be 4, got %u.",
     outputs_res->size()); */

  // Bump start_pos by 1
  start_pos.mutable_data_ptr<int32_t>()[0]++;
  // Update kv_cache
  k_cache = outputs_res.get()[2].toTensor();
  v_cache = outputs_res.get()[3].toTensor();
  // Update kv_mask
  kv_mask = outputs_res.get()[4].toTensor();
...
Error Runner::generate(
  ...
  tokenizer_->encode(
      prompt.c_str(), n_bos_, 0, prompt_tokens, &num_prompt_tokens);
  ...
  Tensor k_cache = managed_k_cache.get_aliasing_tensor();
  Tensor v_cache = managed_v_cache.get_aliasing_tensor();
  Tensor kv_mask = managed_kv_mask.get_aliasing_tensor();
  ...
  while (pos < seq_len - 1) {
    ...
    Result<torch::executor::Tensor> logits_res = run_model_step(
        cur_token, token, start_pos, atten_mask, k_cache, v_cache, kv_mask);

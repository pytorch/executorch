...
def create_device_inputs(example_inputs):
    ...
    for i, data in enumerate(example_inputs):
        data.numpy().tofile(f"{args.artifact}/input_0_{i}.raw")
...
def calibrate(example_inputs, module: torch.fx.GraphModule):
    ...
    _, _, atten_mask, k_cache, v_cache, kv_mask = example_inputs
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-P",
        "--ptq",
        help="If specified, will do PTQ quantization. default is 16bits activation and 4bits weight. Support 16a16w and 16a4w.",
        default="16a4w",
    )
    ...
    if args.ptq == "16a16w":
        ...
        raise AssertionError(
            f"No support for quant type {args.ptq}. Support 16a16w and 16a4w."
        )
    ...
    def post_process():
        for f in sorted(
            ...
            with open(os.path.join(output_data_folder, f)) as text:
                outputs.append(text.read())

...
namespace executorch_jni {
    ...
    public:
        ...
        facebook::jni::local_ref<JEValue> execute_method(
            ...
            std::vector<std::unique_ptr<TensorImpl>> tensor_impl_buffer;
            std::vector<std::unique_ptr<std::vector<exec_aten::SizesType>>> shape_vec_buffer;
            std::vector<std::unique_ptr<std::vector<uint8_t>>> temp_dim_order_storage_buffer;
            std::vector<std::unique_ptr<std::vector<int32_t>>> temp_strides_storage_buffer;
            ...
            for (int i = 0; i < jinputs->size(); i++) {
                ...
                if (typeCode == JEValue::kTypeCodeTensor) {
                    std::unique_ptr<std::vector<exec_aten::SizesType>> shape_vec =
                            std::make_unique<std::vector<exec_aten::SizesType>>(std::vector<exec_aten::SizesType>());
                    shape_vec_buffer.push_back(std::move(shape_vec));
                    std::unique_ptr<std::vector<uint8_t>> temp_dim_order_storage = std::make_unique<std::vector<uint8_t>>(std::vector<uint8_t>());
                    temp_dim_order_storage_buffer.push_back(std::move(temp_dim_order_storage));
                    std::unique_ptr<std::vector<int32_t>> temp_strides_storage = std::make_unique<std::vector<int32_t>>(std::vector<int32_t>());
                    temp_strides_storage_buffer.push_back(std::move(temp_strides_storage));
                    std::unique_ptr<TensorImpl> tensor_impl = std::make_unique<TensorImpl>(TensorImpl(JEValue::JEValueToTensorImpl(
                            jevalue,
                            *shape_vec_buffer.back(),
                            *temp_dim_order_storage_buffer.back(),
                            *temp_strides_storage_buffer.back())));
                    evalues.emplace_back(EValue(exec_aten::Tensor(tensor_impl.get())));
                    tensor_impl_buffer.push_back(std::move(tensor_impl));

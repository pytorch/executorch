...
from executorch.backends.nxp.tests.nsys_testing import get_test_name, OUTPUTS_DIR
...
class MLPerfTinyAnomalyDetection(MLPerfTinyModel):
    ...
    def get_class_from_reconstruction_error(
        ...
        threshold = 17.0  # Empirically chosen

        # high error -> anomaly (above threshold)
        return np.mean(np.square(inps - preds), axis=1) > threshold

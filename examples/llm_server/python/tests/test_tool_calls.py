...
def test_reasoning_returned_by_default():
    serving = _reasoning_serving()
    calls, reasoning, content = serving._extract_response(
        _reasoning_req(), "my plan ANSWER: visible text"
    )
    assert calls is None
    assert reasoning == "my plan"
    assert content == "visible text"


def test_reasoning_explicit_opt_out():
    serving = _reasoning_serving()
    calls, reasoning, content = serving._extract_response(
        _reasoning_req(chat_template_kwargs={"return_reasoning": False}),
        "my plan ANSWER: visible text",
    )
    assert calls is None

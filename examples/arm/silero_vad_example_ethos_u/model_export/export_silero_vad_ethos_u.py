...
def collect_inputs(
    ...
    for audio_path in audio_paths:
        ...
        for frame_index in range(count_frames(audio)):
            ...
            examples.append((model_input, state.clone()))
            with torch.no_grad():
                _, state = model(model_input, state)
            state = state.detach()
            if len(examples) >= max_frames:
                return examples
...
def run_stream(
    ...
    with torch.no_grad():
        for frame_index in range(total_frames):
            model_input, context = make_model_input(audio, frame_index, context)
            prob, state = model(model_input, state)
...
def quantize_model(
    ...
    with torch.no_grad():
        for model_input, state in calibration_examples:
            prepared_model(model_input, state)
...
if __name__ == "__main__":
    ...
    calibration_examples = collect_inputs(
        model,
    ...
    with torch.no_grad():
        quantized_program = quantize_model(model, quantizer, calibration_examples)
